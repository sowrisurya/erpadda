...
def boot_session(bootinfo):
	...
	if vmraid.session['user']!='Guest':
		...
		bootinfo.sysdefaults.territory = vmraid.db.get_single_value('Selling Settings',
			'territory')
		bootinfo.sysdefaults.customer_group = vmraid.db.get_single_value('Selling Settings',
			'customer_group')
		bootinfo.sysdefaults.allow_stale = cint(vmraid.db.get_single_value('Accounts Settings',
			'allow_stale'))
		bootinfo.sysdefaults.quotation_valid_till = cint(vmraid.db.get_single_value('Selling Settings',
			'default_valid_till'))
		...
		if not bootinfo.customer_count:
			bootinfo.setup_complete = vmraid.db.sql("""SELECT `name`
				FROM `tabCompany`
				LIMIT 1""") and 'Yes' or 'No'

		bootinfo.docs += vmraid.db.sql("""select name, default_currency, cost_center, default_selling_terms, default_buying_terms,
			default_letter_head, default_bank_account, enable_perpetual_inventory, country from `tabCompany`""",
			as_dict=1, update={"doctype":":Company"})

		party_account_types = vmraid.db.sql(""" select name, ifnull(account_type, '') from `tabParty Type`""")
...
def load_country_and_currency(bootinfo):
	...
	bootinfo.docs += vmraid.db.sql("""select name, fraction, fraction_units,
