...
from vmraid.utils import get_datetime, flt, getdate
import json
from six import iteritems
from erpadda.loan_management.doctype.loan_security_price.loan_security_price import get_loan_security_price
...
class LoanSecurityUnpledge(Document):
	...
	def validate_duplicate_securities(self):
		...
		for d in self.securities:
			if d.loan_security not in security_list:
				...
				vmraid.throw(_("Row {0}: Loan Security {1} added multiple times").format(
					d.idx, vmraid.bold(d.loan_security)))

	def validate_unpledge_qty(self):
		from erpadda.loan_management.doctype.loan_security_shortfall.loan_security_shortfall import get_ltv_ratio

		pledge_qty_map = get_pledged_security_qty(self.loan)

		ltv_ratio_map = vmraid._dict(vmraid.get_all("Loan Security Type",
		...
		loan_details = vmraid.get_value("Loan", self.loan, ['total_payment', 'total_principal_paid',
		...
		for security in self.securities:
			...
			if security.qty > pledged_qty:
				msg = _("Row {0}: {1} {2} of {3} is pledged against Loan {4}.").format(security.idx, pledged_qty, security.uom,
					vmraid.bold(security.loan_security), vmraid.bold(self.loan))
				msg += "<br>"
	...
	def update_loan_status(self, cancel=0):
		if cancel:
			...
			if not pledged_qty:
				vmraid.db.set_value('Loan', self.loan,
					{
						'status': 'Closed',
...
def get_pledged_security_qty(loan):
	...
	unpledges = vmraid._dict(vmraid.db.sql("""
	...
	pledges = vmraid._dict(vmraid.db.sql("""
