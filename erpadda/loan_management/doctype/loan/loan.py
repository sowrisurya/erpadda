...
import vmraid, math, json
import erpadda
from vmraid import _
from six import string_types
from vmraid.utils import flt, rounded, add_months, nowdate, getdate, now_datetime
...
class Loan(AccountsController):
	...
	def validate_accounts(self):
		for fieldname in ['payment_account', 'loan_account', 'interest_income_account', 'penalty_income_account']:
			...
			if company != self.company:
				vmraid.throw(_("Account {0} does not belongs to company {1}").format(vmraid.bold(self.get(fieldname)),
	...
	def check_sanctioned_amount_limit(self):
		...
		if sanctioned_amount_limit and flt(self.loan_amount) + flt(total_loan_amount) > flt(sanctioned_amount_limit):
			vmraid.throw(_("Sanctioned Amount limit crossed for {0} {1}").format(self.applicant_type, vmraid.bold(self.applicant)))
	...
	def link_loan_security_pledge(self):
		if self.is_secured_loan:
			loan_security_pledge = vmraid.db.get_value('Loan Security Pledge', {'loan_application': self.loan_application},
				'name')

			if loan_security_pledge:
				vmraid.db.set_value('Loan Security Pledge', loan_security_pledge, {
					'loan': self.name,
					'status': 'Pledged',
	...
	def unlink_loan_security_pledge(self):
		...
		if pledge_list:
			vmraid.db.sql("""UPDATE `tabLoan Security Pledge` SET
...
def get_total_loan_amount(applicant_type, applicant, company):
	return vmraid.db.get_value('Loan',
...
def get_sanctioned_amount_limit(applicant_type, applicant, company):
	return vmraid.db.get_value('Sanctioned Loan Amount',
...
def make_loan_write_off(loan, company=None, posting_date=None, amount=0, as_dict=0):
	...
	if amount and (amount > pending_amount):
		vmraid.throw('Write Off amount cannot be greater than pending loan amount')
...
def validate_employee_currency_with_company_currency(applicant, company):
	from erpadda.payroll.doctype.salary_structure_assignment.salary_structure_assignment import get_employee_currency
	if not applicant:
	...
	if employee_currency != company_currency:
		vmraid.throw(_("Loan cannot be repayed from salary for Employee {0} because salary is processed in currency {1}")
