...
import erpadda
import unittest
from vmraid.utils import (nowdate, add_days, getdate, now_datetime, add_to_date, get_datetime,
...
from erpadda.loan_management.doctype.process_loan_interest_accrual.process_loan_interest_accrual import (process_loan_interest_accrual_for_demand_loans,
	process_loan_interest_accrual_for_term_loans)
from erpadda.loan_management.doctype.loan_interest_accrual.loan_interest_accrual import days_in_year
from erpadda.loan_management.doctype.process_loan_security_shortfall.process_loan_security_shortfall import create_process_loan_security_shortfall
from erpadda.loan_management.doctype.loan.loan import unpledge_security, request_loan_closure, make_loan_write_off
from erpadda.loan_management.doctype.loan_security_unpledge.loan_security_unpledge import get_pledged_security_qty
from erpadda.loan_management.doctype.loan_application.loan_application import create_pledge
from erpadda.loan_management.doctype.loan_disbursement.loan_disbursement import get_disbursal_amount
...
class TestLoan(unittest.TestCase):
	...
	def test_regular_loan_repayment(self):
		...
		amounts = vmraid.db.get_all('Loan Interest Accrual', {'loan': loan.name}, ['paid_interest_amount'])
	...
	def test_loan_closure(self):
		...
		amount = vmraid.db.get_value('Loan Interest Accrual', {'loan': loan.name}, ['sum(paid_interest_amount)'])
	...
	def test_loan_repayment_for_term_loan(self):
		...
		amounts = vmraid.db.get_value('Loan Interest Accrual', {'loan': loan.name}, ['paid_interest_amount',
	...
	def test_security_shortfall(self):
		...
		vmraid.db.sql("""UPDATE `tabLoan Security Price` SET loan_security_price = 100
		...
		vmraid.db.sql(""" UPDATE `tabLoan Security Price` SET loan_security_price = 250
			where loan_security='Test Security 2'""")
	...
	def test_penalty(self):
		...
		calculated_penalty_amount = vmraid.db.get_value('Loan Interest Accrual',
...
def create_loan_security_price(loan_security, loan_security_price, uom, from_date, to_date):
	...
	if not vmraid.db.get_value("Loan Security Price",{"loan_security": loan_security,
