...
from vmraid.utils import (nowdate, getdate, now_datetime, get_datetime, flt, date_diff, get_last_day, cint,
...
def make_accrual_interest_entry_for_demand_loans(posting_date, process_loan_interest, open_loans=None, loan_type=None, accrual_type="Regular"):
	...
	if not open_loans:
		open_loans = vmraid.get_all("Loan",
...
def make_accrual_interest_entry_for_term_loans(posting_date, process_loan_interest, term_loan=None, loan_type=None, accrual_type="Regular"):
	...
	if accrued_entries:
		vmraid.db.sql("""UPDATE `tabRepayment Schedule`
...
def get_term_loans(date, term_loan=None, loan_type=None):
	...
	term_loans = vmraid.db.sql("""SELECT l.name, l.total_payment, l.total_amount_paid, l.loan_account,
...
def get_last_accrual_date(loan):
	last_posting_date = vmraid.db.sql(""" SELECT MAX(posting_date) from `tabLoan Interest Accrual`
