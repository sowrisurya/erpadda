...
from vmraid.utils import flt, rounded, cint
from vmraid.model.mapper import get_mapped_doc
from vmraid.model.document import Document
from erpadda.loan_management.doctype.loan.loan import (get_monthly_repayment_amount, validate_repayment_method,
...
class LoanApplication(Document):
	...
	def validate_loan_amount(self):
		...
		if maximum_loan_limit and self.loan_amount > maximum_loan_limit:
			vmraid.throw(_("Loan Amount cannot exceed Maximum Loan Amount of {0}").format(maximum_loan_limit))

		if self.maximum_loan_amount and self.loan_amount > self.maximum_loan_amount:
			vmraid.throw(_("Loan Amount exceeds maximum loan amount of {0} as per proposed securities").format(self.maximum_loan_amount))
			...
			if self.repayment_method == "Repay Fixed Amount per Period":
				...
				if monthly_interest_rate:
					...
					if self.repayment_amount - min_repayment_amount <= 0:
						vmraid.throw(_("Repayment Amount must be greater than " \
...
def create_loan(source_name, target_doc=None, submit=0):
	def update_accounts(source_doc, target_doc, source_parent):
		account_details = vmraid.get_all("Loan Type",
