...
from vmraid.utils import now_datetime, cint
...
class LoanSecurityPledge(Document):
	...
	def validate_duplicate_securities(self):
		...
		for security in self.securities:
			if security.loan_security not in security_list:
				...
				vmraid.throw(_('Loan Security {0} added multiple times').format(vmraid.bold(
	...
	def validate_loan_security_type(self):
		...
		if self.loan:
			existing_pledge = vmraid.db.get_value('Loan Security Pledge', {'loan': self.loan}, ['name'])

		if existing_pledge:
			loan_security_type = vmraid.db.get_value('Pledge', {'parent': existing_pledge}, ['loan_security_type'])
		else:
...
def update_loan(loan, maximum_value_against_pledge):
	...
	vmraid.db.sql(""" UPDATE `tabLoan` SET maximum_loan_amount=%s, is_secured_loan=1
