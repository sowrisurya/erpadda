...
from vmraid.utils import flt, getdate, cint
from six import iteritems
from vmraid.model.document import Document
from vmraid.utils import date_diff, add_days, getdate, add_months, get_first_day, get_datetime
...
from erpadda.loan_management.doctype.loan_security_shortfall.loan_security_shortfall import update_shortfall_status
...
from erpadda.loan_management.doctype.loan_interest_accrual.loan_interest_accrual import get_per_day_interest, get_last_accrual_date
...
class LoanRepayment(AccountsController):
	...
	def set_missing_values(self, amounts):
		...
		shortfall_amount = flt(vmraid.db.get_value('Loan Security Shortfall', {'loan': self.against_loan, 'status': 'Pending'},
			'shortfall_amount'))
	...
	def check_future_entries(self):
		future_repayment_date = vmraid.db.get_value("Loan Repayment", {"posting_date": (">", self.posting_date),
	...
	def book_unaccrued_interest(self):
		...
		if self.total_interest_paid > self.interest_payable:
			if not self.is_term_loan:
				...
				lia = vmraid.db.get_value('Loan Interest Accrual', {'process_loan_interest_accrual':
	...
	def update_paid_amount(self):
		loan = vmraid.get_doc("Loan", self.against_loan)

		for payment in self.repayment_details:
			vmraid.db.sql(""" UPDATE `tabLoan Interest Accrual`
				SET paid_principal_amount = `paid_principal_amount` + %s,
					paid_interest_amount = `paid_interest_amount` + %s
				WHERE name = %s""",
				(flt(payment.paid_principal_amount), flt(payment.paid_interest_amount), payment.loan_interest_accrual))

		vmraid.db.sql(""" UPDATE `tabLoan` SET total_amount_paid = %s, total_principal_paid = %s
	...
	def mark_as_unpaid(self):
		...
		if loan.status == "Loan Closure Requested":
			vmraid.db.set_value("Loan", self.against_loan, "status", "Disbursed")
	...
	def make_gl_entries(self, cancel=0, adv_adj=0):
		gle_map = []
		loan_details = vmraid.get_doc("Loan", self.against_loan)
