...
def update_shortfall_status(loan, security_value, on_cancel=0):
	loan_security_shortfall = vmraid.db.get_value("Loan Security Shortfall",
	...
	if security_value >= loan_security_shortfall.shortfall_amount:
		vmraid.db.set_value("Loan Security Shortfall", loan_security_shortfall.name, {
			"status": "Completed",
...
def add_security(loan):
	loan_details = vmraid.db.get_value("Loan", loan, ['applicant', 'company', 'applicant_type'], as_dict=1)
...
def check_for_ltv_shortfall(process_loan_security_shortfall):
	...
	loans = vmraid.get_all('Loan', fields=['name', 'loan_amount', 'total_principal_paid', 'total_payment',
		'total_interest_payable', 'disbursed_amount', 'status'],
		filters={'status': ('in',['Disbursed','Partially Disbursed']), 'is_secured_loan': 1})

	loan_shortfall_map = vmraid._dict(vmraid.get_all("Loan Security Shortfall",
...
def create_loan_security_shortfall(loan, loan_amount, security_value, shortfall_amount, shortfall_ratio,
	...
	existing_shortfall = vmraid.db.get_value("Loan Security Shortfall", {"loan": loan, "status": "Pending"}, "name")
...
def update_pending_shortfall(shortfall):
	...
	vmraid.db.set_value("Loan Security Shortfall", shortfall,
		{
			"status": "Completed",
			"shortfall_amount": 0,
			"shortfall_percentage": 0
