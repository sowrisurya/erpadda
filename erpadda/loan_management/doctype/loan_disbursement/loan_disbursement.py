...
from vmraid.utils import nowdate, getdate, add_days, flt
...
class LoanDisbursement(AccountsController):
	...
	def set_missing_values(self):
		...
		if not self.bank_account and self.applicant_type == "Customer":
			self.bank_account = vmraid.db.get_value("Customer", self.applicant, "default_bank_account")
	...
	def set_status_and_amounts(self, cancel=0):
		loan_details = vmraid.get_all("Loan",
		...
		vmraid.db.set_value("Loan", self.against_loan, {
...
def get_total_pledged_security_value(loan):
	...
	loan_security_price_map = vmraid._dict(vmraid.get_all("Loan Security Price",
	...
	hair_cut_map = vmraid._dict(vmraid.get_all('Loan Security',
...
def get_disbursal_amount(loan, on_current_security_price=0):
	loan_details = vmraid.get_value("Loan", loan, ["loan_amount", "disbursed_amount", "total_payment",
		"total_principal_paid", "total_interest_payable", "status", "is_term_loan", "is_secured_loan",
		"maximum_loan_amount"], as_dict=1)

	if loan_details.is_secured_loan and vmraid.get_all('Loan Security Shortfall', filters={'loan': loan,
		'status': 'Pending'}):
		return 0
