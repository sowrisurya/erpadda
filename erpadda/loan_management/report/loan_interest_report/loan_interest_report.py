...
from vmraid.utils import flt, getdate, add_days
from erpadda.loan_management.report.applicant_wise_loan_security_exposure.applicant_wise_loan_security_exposure \
...
def get_sanctioned_amount_map():
	return vmraid._dict(vmraid.get_all("Sanctioned Loan Amount", fields=["applicant", "sanctioned_amount_limit"],
		as_list=1))

def get_payments(loans):
	return vmraid._dict(vmraid.get_all("Loan Repayment", fields=["against_loan", "sum(amount_paid)"],
		filters={"against_loan": ("in", loans)}, group_by="against_loan", as_list=1))

def get_interest_accruals(loans):
	accrual_map = {}

	interest_accruals = vmraid.get_all("Loan Interest Accrual",
...
def get_penal_interest_rate_map():
	return vmraid._dict(vmraid.get_all("Loan Type", fields=["name", "penalty_interest_rate"], as_list=1))
