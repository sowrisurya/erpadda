...
import vmraid
from vmraid.utils import datetime
...
class TestCropCycle(unittest.TestCase):
	def test_crop_cycle_creation(self):
		cycle = vmraid.get_doc('Crop Cycle', 'Basil from seed 2017')
		self.assertTrue(vmraid.db.exists('Crop Cycle', 'Basil from seed 2017'))
...
def check_task_creation():
	...
	all_tasks = vmraid.get_all('Task')

	for task in all_tasks:
		sample_task = vmraid.get_doc('Task', task.name)
...
def check_project_creation():
	return True if vmraid.db.exists('Project', {'project_name': 'Basil from seed 2017'}) else False
