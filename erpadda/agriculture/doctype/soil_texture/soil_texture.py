...
class SoilTexture(Document):
	...
	def load_contents(self):
		docs = vmraid.get_all("Agriculture Analysis Criteria", filters={'linked_doctype':'Soil Texture'})
		for doc in docs:
	...
	def validate(self):
		...
		for soil_type in self.soil_types:
			if self.get(soil_type) > 100 or self.get(soil_type) < 0:
				vmraid.throw(_("{0} should be a value between 0 and 100").format(soil_type))
		if sum(self.get(soil_type) for soil_type in self.soil_types) != 100:
			vmraid.throw(_('Soil compositions do not add up to 100'))
