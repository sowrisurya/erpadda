...
def get_employees_with_number(number):
	...
	employee_emails = vmraid.cache().hget('employees_with_number', number)
	if employee_emails: return employee_emails

	employees = vmraid.get_all('Employee', filters={
		'cell_number': ['like', '%{}%'.format(number)],
		'user_id': ['!=', '']
	}, fields=['user_id'])

	employee_emails = [employee.user_id for employee in employees]
	vmraid.cache().hset('employees_with_number', number, employee_emails)
...
def get_linked_call_logs(doctype, docname):
	...
	logs = vmraid.get_all('Dynamic Link', fields=['parent'], filters={
		'parenttype': 'Call Log',
	...
	logs = vmraid.get_all('Call Log', fields=['*'], filters={
		'name': ['in', logs]
	})
