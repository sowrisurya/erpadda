...
def calculate_work_experience(employee, gratuity_rule):
	...
	total_working_days_per_year, minimum_year_for_gratuity = vmraid.db.get_value("Gratuity Rule", gratuity_rule, ["total_working_days_per_year", "minimum_year_for_gratuity"])

	date_of_joining, relieving_date = vmraid.db.get_value('Employee', employee, ['date_of_joining', 'relieving_date'])
	if not relieving_date:
		vmraid.throw(_("Please set Relieving Date for employee: {0}").format(bold(get_link_to_form("Employee", employee))))

	method = vmraid.db.get_value("Gratuity Rule", gratuity_rule, "work_experience_calculation_function")
...
def get_work_experience_using_method(method, current_work_experience, minimum_year_for_gratuity, employee):
	...
	if current_work_experience < minimum_year_for_gratuity:
		vmraid.throw(_("Employee: {0} have to complete minimum {1} years for gratuity").format(bold(employee), minimum_year_for_gratuity))
	return current_work_experience
...
def calculate_gratuity_amount(employee, gratuity_rule, experience):
	...
	calculate_gratuity_amount_based_on = vmraid.db.get_value("Gratuity Rule", gratuity_rule, "calculate_gratuity_amount_based_on")
	gratuity_amount = 0
	...
	if not slab_found:
		vmraid.throw(_("No Suitable Slab found for Calculation of gratuity amount in Gratuity Rule: {0}").format(bold(gratuity_rule)))
	return gratuity_amount

def get_applicable_components(gratuity_rule):
	applicable_earnings_component = vmraid.get_all("Gratuity Applicable Component", filters= {'parent': gratuity_rule}, fields=["salary_component"])
	if len(applicable_earnings_component) == 0:
		vmraid.throw(_("No Applicable Earnings Component found for Gratuity Rule: {0}").format(bold(get_link_to_form("Gratuity Rule",gratuity_rule))))
...
def get_total_applicable_component_amount(employee, applicable_earnings_component, gratuity_rule):
	...
	component_and_amounts = vmraid.get_list("Salary Detail",
		filters={
			"docstatus": 1,
			'parent': sal_slip,
...
def get_gratuity_rule_slabs(gratuity_rule):
	return vmraid.get_all("Gratuity Rule Slab", filters= {'parent': gratuity_rule}, fields = ["*"], order_by="idx")

def get_salary_structure(employee):
	return vmraid.get_list("Salary Structure Assignment", filters = {
...
def get_last_salary_slip(employee):
	return vmraid.get_list("Salary Slip", filters = {
			"employee": employee, 'docstatus': 1
		},
