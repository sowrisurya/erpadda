...
from erpadda.payroll.doctype.salary_slip.test_salary_slip import make_employee_salary_slip, make_earning_salary_component, \
...
from vmraid.utils import getdate, add_days, get_datetime, flt
...
class TestGratuity(unittest.TestCase):
	...
	def setUp(self):
		vmraid.db.sql("DELETE FROM `tabGratuity`")
		vmraid.db.sql("DELETE FROM `tabAdditional Salary` WHERE ref_doctype = 'Gratuity'")
	...
	def test_check_gratuity_amount_based_on_current_slab_and_additional_salary_creation(self):
		...
		#amount Calculation
		component_amount = vmraid.get_list("Salary Detail",
		filters={
			"docstatus": 1,
			'parent': sal_slip,
...
def create_employee_and_get_last_salary_slip():
	...
	if not vmraid.db.get_value("Employee", "test_employee@salary.com", "holiday_list"):
		...
		make_holiday_list()
		vmraid.db.set_value("Company", '_Test Company', "default_holiday_list", "Salary Slip Test Holiday List")
