...
from vmraid.utils import getdate, date_diff, comma_and, formatdate

class AdditionalSalary(Document):
	def on_submit(self):
		if self.ref_doctype == "Employee Advance" and self.ref_docname:
			vmraid.db.set_value("Employee Advance", self.ref_docname, "return_amount", self.amount)
	...
	def validate_recurring_additional_salary_overlap(self):
		if self.is_recurring:
			...
			if additional_salaries and len(additional_salaries):
				vmraid.throw(_("Additional Salary: {0} already exist for Salary Component: {1} for period {2} and {3}").format(
	...
	def validate_dates(self):
		date_of_joining, relieving_date = vmraid.db.get_value("Employee", self.employee,
	...
	def validate_employee_referral(self):
		if self.ref_doctype == "Employee Referral":
			referral_details = vmraid.db.get_value("Employee Referral", self.ref_docname,
				["is_applicable_for_referral_bonus", "status"], as_dict=1)

			if not referral_details.is_applicable_for_referral_bonus:
				vmraid.throw(_("Employee Referral {0} is not applicable for referral bonus.").format(
					self.ref_docname))
			...
			if referral_details.status != "Accepted":
				vmraid.throw(_("Additional Salary for referral bonus can only be created against Employee Referral with status {0}").format(
					vmraid.bold("Accepted")))
...
def get_additional_salaries(employee, start_date, end_date, component_type):
	additional_salary_list = vmraid.db.sql("""
	...
	for d in additional_salary_list:
		if d.overwrite:
			if d.component in components_to_overwrite:
				vmraid.throw(_("Multiple Additional Salaries with overwrite property exist for Salary Component {0} between {1} and {2}.").format(
					vmraid.bold(d.component), start_date, end_date), title=_("Error"))
