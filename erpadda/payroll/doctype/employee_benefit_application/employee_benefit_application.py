...
from vmraid.utils import date_diff, getdate, rounded, add_days, cstr, cint, flt
from vmraid.model.document import Document
from erpadda.payroll.doctype.payroll_period.payroll_period import get_payroll_period_days, get_period_factor
from erpadda.payroll.doctype.salary_structure_assignment.salary_structure_assignment import get_assigned_salary_structure
from erpadda.hr.utils import get_sal_slip_total_benefit_given, get_holidays_for_employee, get_previous_claimed_amount
...
class EmployeeBenefitApplication(Document):
	def validate(self):
		...
		if self.max_benefits and self.max_benefits > 0:
			...
			vmraid.throw(_("As per your assigned Salary Structure you cannot apply for benefits").format(self.employee))
			...
			for benefit in self.employee_benefits:
				if benefit.pay_against_benefit_claim == 1:
					...
					if benefit_claimed > 0 and benefit_claim_remining > benefit.amount:
						vmraid.throw(_("An amount of {0} already claimed for the component {1}, set the amount equal or greater than {2}").format(
	...
	def validate_remaining_benefit_amount(self):
		...
		if salary_struct_name:
			...
			if salary_structure.earnings:
				for earnings in salary_structure.earnings:
					if earnings.is_flexible_benefit == 1 and earnings.salary_component not in benefit_components:
						pay_against_benefit_claim, max_benefit_amount = vmraid.db.get_value("Salary Component", earnings.salary_component, ["pay_against_benefit_claim", "max_benefit_amount"])
			...
			if pro_rata_amount == 0  and non_pro_rata_amount == 0:
				vmraid.throw(_("Please add the remaining benefits {0} to any of the existing component").format(self.remaining_benefit))
			elif non_pro_rata_amount > 0 and non_pro_rata_amount < rounded(self.remaining_benefit):
				vmraid.throw(_("You can claim only an amount of {0}, the rest amount {1} should be in the application as pro-rata component").format(
					non_pro_rata_amount, self.remaining_benefit - non_pro_rata_amount))
			elif non_pro_rata_amount == 0:
				vmraid.throw(_("Please add the remaining benefits {0} to the application as pro-rata component").format(
	...
	def validate_max_benefit_for_component(self):
		if self.employee_benefits:
			...
			if max_benefit_amount > self.max_benefits:
				vmraid.throw(_("Maximum benefit amount of employee {0} exceeds {1}").format(self.employee, self.max_benefits))

	def validate_max_benefit(self, earning_component_name):
		max_benefit_amount = vmraid.db.get_value("Salary Component", earning_component_name, "max_benefit_amount")
		benefit_amount = 0
		for employee_benefit in self.employee_benefits:
			if employee_benefit.earning_component == earning_component_name:
				benefit_amount += employee_benefit.amount
		prev_sal_slip_flexi_amount = get_sal_slip_total_benefit_given(self.employee, vmraid.get_doc("Payroll Period", self.payroll_period), earning_component_name)
		benefit_amount += prev_sal_slip_flexi_amount
		if rounded(benefit_amount, 2) > max_benefit_amount:
			vmraid.throw(_("Maximum benefit amount of component {0} exceeds {1}").format(earning_component_name, max_benefit_amount))
	...
	def validate_duplicate_on_payroll_period(self):
		...
		if application:
			vmraid.throw(_("Employee {0} already submited an apllication {1} for the payroll period {2}").format(self.employee, application, self.payroll_period))
...
def get_benefit_component_amount(employee, start_date, end_date, salary_component, sal_struct, payroll_frequency, payroll_period):
	...
	component_max_benefit, depends_on_payment_days = vmraid.db.get_value("Salary Component",
		salary_component, ["max_benefit_amount", "depends_on_payment_days"])

	benefit_amount = 0
	if benefit_application:
		benefit_amount = vmraid.db.get_value("Employee Benefit Application Detail",
...
def get_benefit_amount_based_on_pro_rata(sal_struct, component_max_benefit):
	...
	for d in sal_struct.get("earnings"):
		if d.is_flexible_benefit == 1:
			component = vmraid.db.get_value("Salary Component", d.salary_component, ["max_benefit_amount", "pay_against_benefit_claim"], as_dict=1)
...
def get_earning_components(doctype, txt, searchfield, start, page_len, filters):
	...
	if salary_structure:
		...
		vmraid.throw(_("Salary Structure not found for employee {0} and date {1}")
