...
class RetentionBonus(Document):
	def validate(self):
		if vmraid.get_value('Employee', self.employee, 'status') == 'Left':
			vmraid.throw(_('Cannot create Retention Bonus for left Employees'))
	...
	def on_submit(self):
		...
		if not additional_salary:
			...
			bonus_added = vmraid.db.get_value('Additional Salary', additional_salary, 'amount') + self.bonus_amount
	...
	def on_cancel(self):
		...
		if self.additional_salary:
			bonus_removed = vmraid.db.get_value('Additional Salary', self.additional_salary, 'amount') - self.bonus_amount
			if bonus_removed == 0:
	...
	def get_additional_salary(self):
		return vmraid.db.exists('Additional Salary', {
