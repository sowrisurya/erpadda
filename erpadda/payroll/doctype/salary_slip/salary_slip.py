...
from vmraid.utils import add_days, cint, cstr, flt, getdate, rounded, date_diff, money_in_words, formatdate, get_first_day
...
from erpadda.payroll.doctype.payroll_period.payroll_period import get_period_factor, get_payroll_period
from erpadda.payroll.doctype.employee_benefit_application.employee_benefit_application import get_benefit_component_amount
from erpadda.payroll.doctype.employee_benefit_claim.employee_benefit_claim import get_benefit_claim_amount, get_last_payroll_period_benefits
from erpadda.loan_management.doctype.loan_repayment.loan_repayment import calculate_amounts, create_repayment_entry
...
class SalarySlip(TransactionBase):
	...
	def validate(self):
		...
		if vmraid.db.get_single_value("Payroll Settings", "max_working_hours_against_timesheet"):
			max_working_hours = vmraid.db.get_single_value("Payroll Settings", "max_working_hours_against_timesheet")
			if self.salary_slip_based_on_timesheet and (self.total_working_hours > int(max_working_hours)):
				vmraid.msgprint(_("Total working hours should not be greater than max working hours {0}").
	...
	def on_submit(self):
		if self.net_pay < 0:
			...
			if (vmraid.db.get_single_value("Payroll Settings", "email_salary_slip_to_employee")) and not vmraid.flags.via_payroll_entry:
				self.email_salary_slip()

		self.update_payment_status_for_gratuity()

	def update_payment_status_for_gratuity(self):
		add_salary = vmraid.db.get_all("Additional Salary",
			filters = {
	...
	def check_existing(self):
		if not self.salary_slip_based_on_timesheet:
			...
			ret_exist = vmraid.db.sql("""select name from `tabSalary Slip`
						where start_date = %s and end_date = %s and docstatus != 2
						and employee = %s and name != %s {0}""".format(cond),
						(self.start_date, self.end_date, self.employee, self.name))
			if ret_exist:
				self.employee = ''
				vmraid.throw(_("Salary Slip of employee {0} already created for this period").format(self.employee))
		else:
			for data in self.timesheets:
				if vmraid.db.get_value('Timesheet', data.time_sheet, 'status') == 'Payrolled':
					vmraid.throw(_("Salary Slip of employee {0} already created for time sheet {1}").format(self.employee, data.time_sheet))
	...
	def get_emp_and_working_day_details(self):
		...
		if self.employee:
			...
			if not self.salary_slip_based_on_timesheet:
				self.get_date_details()
			self.validate_dates()
			joining_date, relieving_date = vmraid.get_cached_value("Employee", self.employee,
			...
			if struct:
				...
				ps = vmraid.db.get_value("Payroll Settings", None, ["payroll_based_on","consider_unmarked_attendance_as"], as_dict=1)
				return [ps.payroll_based_on, ps.consider_unmarked_attendance_as]

	def set_time_sheet(self):
		if self.salary_slip_based_on_timesheet:
			self.set("timesheets", [])
			timesheets = vmraid.db.sql(""" select * from `tabTimesheet` where employee = %(employee)s and start_date BETWEEN %(start_date)s AND %(end_date)s and (status = 'Submitted' or
	...
	def check_sal_struct(self, joining_date, relieving_date):
		...
		if st_name:
			...
			vmraid.msgprint(_("No active or default Salary Structure found for employee {0} for the given dates")
	...
	def get_working_days_details(self, joining_date=None, relieving_date=None, lwp=None, for_preview=0):
		...
		include_holidays_in_total_working_days = vmraid.db.get_single_value("Payroll Settings", "include_holidays_in_total_working_days")
		...
		if not lwp:
			...
			vmraid.msgprint(_("Leave Without Pay does not match with approved {} records")
		...
		if flt(payment_days) > flt(lwp):
			...
			consider_unmarked_attendance_as = vmraid.db.get_value("Payroll Settings", None, "consider_unmarked_attendance_as") or "Present"
			...
			if payroll_based_on == "Attendance" and consider_unmarked_attendance_as =="Absent":
				...
				if include_holidays_in_total_working_days:
					for holiday in holidays:
						if not vmraid.db.exists("Attendance", {"employee": self.employee, "attendance_date": holiday, "docstatus": 1 }):
							self.payment_days += 1
		else:
			self.payment_days = 0

	def get_unmarked_days(self):
		marked_days = vmraid.get_all("Attendance", filters = {
	...
	def get_payment_days(self, joining_date, relieving_date, include_holidays_in_total_working_days):
		...
		if relieving_date:
			if getdate(self.start_date) <= relieving_date <= getdate(self.end_date):
				...
				vmraid.throw(_("Employee relieved on {0} must be set as 'Left'")
	...
	def calculate_lwp_ppl_and_absent_days_based_on_attendance(self, holidays):
		...
		leave_types = vmraid.get_all("Leave Type",
	...
	def get_data_for_eval(self):
		...
		data.update(vmraid.get_doc("Salary Structure Assignment",
			{"employee": self.employee, "salary_structure": self.salary_structure}).as_dict())

		data.update(vmraid.get_doc("Employee", self.employee).as_dict())
	...
	def eval_condition_and_formula(self, d, data):
		try:
			...
			vmraid.throw(_("{0} <br> This error can be due to missing or deleted field.").format(err),
				title=_("Name error"))
	...
	def add_employee_benefits(self, payroll_period):
		for struct_row in self._salary_structure_doc.get("earnings"):
			if struct_row.is_flexible_benefit == 1:
				if vmraid.db.get_value("Salary Component", struct_row.salary_component, "pay_against_benefit_claim") != 1:
	...
	def add_tax_components(self, payroll_period):
		...
		if not tax_components:
			tax_components = [d.name for d in vmraid.get_all("Salary Component", filters={"variable_based_on_taxable_salary": 1})
	...
	def calculate_variable_based_on_taxable_salary(self, tax_component, payroll_period):
		if not payroll_period:
			vmraid.msgprint(_("Start and end dates not in a valid Payroll Period, cannot calculate {0}.")
	...
	def get_income_tax_slabs(self, payroll_period):
		income_tax_slab, ss_assignment_name = vmraid.db.get_value("Salary Structure Assignment",
			{"employee": self.employee, "salary_structure": self.salary_structure, "docstatus": 1}, ["income_tax_slab", 'name'])

		if not income_tax_slab:
			vmraid.throw(_("Income Tax Slab not set in Salary Structure Assignment: {0}").format(ss_assignment_name))
		...
		if getdate(income_tax_slab_doc.effective_from) > getdate(payroll_period.start_date):
			vmraid.throw(_("Income Tax Slab must be effective on or before Payroll Period Start Date: {0}")
	...
	def get_tax_paid_in_period(self, start_date, end_date, tax_component):
		...
		total_tax_paid = flt(vmraid.db.sql("""
			select
				sum(sd.amount)
			from
				`tabSalary Detail` sd join `tabSalary Slip` ss on sd.parent=ss.name
	...
	def get_taxable_earnings(self, allow_tax_exemption=False, based_on_payment_days=0):
		...
		if not joining_date:
			vmraid.throw(_("Please set the Date Of Joining for employee {0}").format(vmraid.bold(self.employee_name)))
	...
	def get_amount_based_on_payment_days(self, row, joining_date, relieving_date):
		...
		# apply rounding
		if vmraid.get_cached_value("Salary Component", row.salary_component, "round_to_the_nearest_integer"):
	...
	def calculate_unclaimed_taxable_benefits(self, payroll_period):
		...
		total_benefits_paid = flt(vmraid.db.sql("""
			select sum(sd.amount)
		...
		total_benefits_claimed = flt(vmraid.db.sql("""
	...
	def get_total_exemption_amount(self, payroll_period, tax_slab):
		...
		if tax_slab.allow_tax_exemption:
			if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
				exemption_proof = vmraid.db.get_value("Employee Tax Exemption Proof Submission",
					{"employee": self.employee, "payroll_period": payroll_period.name, "docstatus": 1},
					["exemption_amount"])
				if exemption_proof:
					total_exemption_amount = exemption_proof
			else:
				declaration = vmraid.db.get_value("Employee Tax Exemption Declaration",
					{"employee": self.employee, "payroll_period": payroll_period.name, "docstatus": 1},
	...
	def get_income_form_other_sources(self, payroll_period):
		return vmraid.get_all("Employee Other Income",
			filters={
				"employee": self.employee,
	...
	def eval_tax_slab_condition(self, condition, data):
		try:
			...
			if condition:
				return vmraid.safe_eval(condition, self.whitelisted_globals, data)
		except NameError as err:
			vmraid.throw(_("{0} <br> This error can be due to missing or deleted field.").format(err),
				title=_("Name error"))
		except SyntaxError as err:
			vmraid.throw(_("Syntax error in condition: {0}").format(err))
		except Exception as e:
			vmraid.throw(_("Error in formula or condition: {0}").format(e))
			raise
	...
	def set_loan_repayment(self):
		...
		for payment in self.get('loans'):
			...
			if payment.total_payment > total_amount:
				vmraid.throw(_("""Row {0}: Paid amount {1} is greater than pending accrued amount {2} against loan {3}""")
					.format(payment.idx, vmraid.bold(payment.total_payment),
						vmraid.bold(total_amount), vmraid.bold(payment.loan)))
	...
	def make_loan_repayment_entry(self):
		for loan in self.loans:
			...
			vmraid.db.set_value("Salary Slip Loan", loan.name, "loan_repayment_entry", repayment_entry.name)
	...
	def email_salary_slip(self):
		...
		if receiver:
			email_args = {
				...
				"attachments": [vmraid.attach_print(self.doctype, self.name, file_name=self.name, password=password)],
	...
	def pull_emp_details(self):
		emp = vmraid.db.get_value("Employee", self.employee, ["bank_name", "bank_ac_no", "salary_mode"], as_dict=1)
		if emp:
	...
	def calculate_total_for_salary_slip_based_on_timesheet(self):
		...
		self.base_hour_rate = flt(self.hour_rate) * flt(self.exchange_rate)
		salary_component = vmraid.db.get_value('Salary Structure', {'name': self.salary_structure}, 'salary_component')
		if self.earnings:
	...
	def compute_year_to_date(self):
		...
		salary_slip_sum = vmraid.get_list('Salary Slip',
...
def unlink_ref_doc_from_salary_slip(ref_no):
	linked_ss = vmraid.db.sql_list("""select name from `tabSalary Slip`
