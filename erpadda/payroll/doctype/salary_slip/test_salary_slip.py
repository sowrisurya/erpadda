...
from vmraid.utils import getdate, nowdate, add_days, add_months, flt, get_first_day, get_last_day, cstr
...
from erpadda.payroll.doctype.employee_tax_exemption_declaration.test_employee_tax_exemption_declaration \
...
class TestSalarySlip(unittest.TestCase):
	...
	def test_payment_days_based_on_attendance(self):
		...
		vmraid.db.set_value("Payroll Settings", None, "payroll_based_on", "Attendance")
		vmraid.db.set_value("Payroll Settings", None, "daily_wages_fraction_for_half_day", 0.75)
		...
		vmraid.db.set_value("Payroll Settings", None, "payroll_based_on", "Leave")
	...
	def test_salary_slip_with_holidays_included(self):
		...
		vmraid.db.set_value("Employee", vmraid.get_value("Employee",
	...
	def test_email_salary_slip(self):
		...
		vmraid.db.set_value("Payroll Settings", None, "email_salary_slip_to_employee", 1)
		...
		email_queue = vmraid.db.sql("""select name from `tabEmail Queue`""")
		self.assertTrue(email_queue)

	def test_loan_repayment_salary_slip(self):
		from erpadda.loan_management.doctype.loan.test_loan import create_loan_type, create_loan, make_loan_disbursement_entry, create_loan_accounts
		from erpadda.loan_management.doctype.process_loan_interest_accrual.process_loan_interest_accrual import process_loan_interest_accrual_for_term_loans
		...
		vmraid.db.sql("delete from tabLoan")
	...
	def test_multi_currency_salary_slip(self):
		...
		vmraid.db.sql("""delete from `tabSalary Structure` where name='Test Multi Currency Salary Slip'""")
	...
	def test_year_to_date_computation(self):
		...
		salary_slips = vmraid.get_all('Salary Slip', fields=['year_to_date', 'net_pay'], filters={'employee_name':
	...
	def test_component_wise_year_to_date_computation(self):
		...
		salary_slips = vmraid.get_all("Salary Slip", fields=["name"], filters={"employee_name":
			"test_ytd@salary.com"}, order_by = "posting_date")
...
def make_employee_salary_slip(user, payroll_frequency, salary_structure=None):
	...
	employee = vmraid.db.get_value("Employee", {"user_id": user})
	salary_structure_doc = make_salary_structure(salary_structure, payroll_frequency, employee=employee)
	salary_slip_name = vmraid.db.get_value("Salary Slip", {"employee": vmraid.db.get_value("Employee", {"user_id": user})})

	if not salary_slip_name:
		salary_slip = make_salary_slip(salary_structure_doc.name, employee = employee)
		salary_slip.employee_name = vmraid.get_value("Employee",
			{"name":vmraid.db.get_value("Employee", {"user_id": user})}, "employee_name")
...
def make_salary_component(salary_components, test_tax, company_list=None):
	for salary_component in salary_components:
		if not vmraid.db.exists('Salary Component', salary_component["salary_component"]):
			if test_tax:
			...
			vmraid.get_doc(salary_component).insert()
...
def get_tax_paid_in_period(employee):
	tax_paid_amount = vmraid.db.sql("""select sum(sd.amount) from `tabSalary Detail`
...
def create_salary_slips_for_payroll_period(employee, salary_structure, payroll_period, deduct_random=True, num=12):
	...
	i = 0
	while i < num:
		slip = vmraid.get_doc({"doctype": "Salary Slip", "employee": employee,
...
def make_leave_application(employee, from_date, to_date, leave_type, company=None):
	leave_application = vmraid.get_doc(dict(
...
def setup_test():
	...
	vmraid.db.set_value("Company", erpadda.get_default_company(), "default_holiday_list", "Salary Slip Test Holiday List")
...
def make_holiday_list():
	...
	if not vmraid.db.get_value("Holiday List", "Salary Slip Test Holiday List"):
