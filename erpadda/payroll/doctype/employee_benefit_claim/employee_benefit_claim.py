...
from erpadda.payroll.doctype.employee_benefit_application.employee_benefit_application import get_max_benefits
from erpadda.hr.utils import get_previous_claimed_amount
...
class EmployeeBenefitClaim(Document):
	def validate(self):
		...
		payroll_period = get_payroll_period(self.claim_date, self.claim_date, vmraid.db.get_value("Employee", self.employee, "company"))
		if not payroll_period:
			vmraid.throw(_("{0} is not in a valid Payroll Period").format(vmraid.format(self.claim_date, dict(fieldtype='Date'))))
	...
	def validate_benefit_claim_amount(self, max_benefits, payroll_period):
		...
		if max_benefits < claimed_amount:
			vmraid.throw(_("Maximum benefit of employee {0} exceeds {1} by the sum {2} of previous claimed\
	...
	def validate_max_benefit_for_sal_struct(self, max_benefits):
		if self.claimed_amount > max_benefits:
			vmraid.throw(_("Maximum benefit amount of employee {0} exceeds {1}").format(self.employee, max_benefits))
	...
	def validate_max_benefit_for_component(self, payroll_period):
		if self.max_amount_eligible:
			...
			if claimed_amount > self.max_amount_eligible:
				vmraid.throw(_("Maximum amount eligible for the component {0} exceeds {1}")
	...
	def validate_non_pro_rata_benefit_claim(self, max_benefits, payroll_period):
		...
		if max_benefits < pro_rata_amount + claimed_amount:
			vmraid.throw(_("Maximum benefit of employee {0} exceeds {1} by the sum {2} of benefit application pro-rata component\
	...
	def get_pro_rata_amount_in_application(self, payroll_period):
		...
		if application:
			return vmraid.db.get_value("Employee Benefit Application", application, "pro_rata_dispensed_amount")
		return False
...
def get_benefit_pro_rata_ratio_amount(employee, on_date, sal_struct):
	...
	for sal_struct_row in sal_struct.get("earnings"):
		try:
			pay_against_benefit_claim, max_benefit_amount = vmraid.db.get_value("Salary Component", sal_struct_row.salary_component, ["pay_against_benefit_claim", "max_benefit_amount"])
		except TypeError:
...
def get_benefit_claim_amount(employee, start_date, end_date, salary_component=None):
	...
	claimed_amount = flt(vmraid.db.sql(query, {
		'employee': employee,
