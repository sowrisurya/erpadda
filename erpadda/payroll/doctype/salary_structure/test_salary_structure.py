...
from vmraid.utils import nowdate, add_days, add_years, getdate, add_months
from erpadda.payroll.doctype.salary_structure.salary_structure import make_salary_slip
from erpadda.payroll.doctype.salary_slip.test_salary_slip import make_earning_salary_component,\
...
from erpadda.payroll.doctype.employee_tax_exemption_declaration.test_employee_tax_exemption_declaration import create_payroll_period
...
class TestSalaryStructure(unittest.TestCase):
	def setUp(self):
		...
		vmraid.db.set_value("Company", erpadda.get_default_company(), "default_holiday_list", "Salary Structure Test Holiday List")
	...
	def test_salary_structures_assignment(self):
		...
		vmraid.db.sql('''delete from `tabSalary Structure Assignment` where employee=%s and salary_structure=%s ''',
		...
		salary_structure_assignment = vmraid.get_doc("Salary Structure Assignment",{'employee':employee_doc_name, 'from_date':'2013-01-01'})
...
def make_salary_structure(salary_structure, payroll_frequency, employee=None, dont_submit=False, other_details=None,
	test_tax=False, company=None, currency=erpadda.get_default_currency(), payroll_period=None):
	if test_tax:
		vmraid.db.sql("""delete from `tabSalary Structure` where name=%s""",(salary_structure))

	if not vmraid.db.exists('Salary Structure', salary_structure):
		details = {
		...
		salary_structure_doc = vmraid.get_doc("Salary Structure", salary_structure)

	if employee and not vmraid.db.get_value("Salary Structure Assignment",
...
def create_salary_structure_assignment(employee, salary_structure, from_date=None, company=None, currency=erpadda.get_default_currency(),
	payroll_period=None):
