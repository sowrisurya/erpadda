...
from vmraid.utils import flt, cint, cstr
...
class SalaryStructure(Document):
	...
	def set_missing_values(self):
		...
		for table in ["earnings", "deductions"]:
			for d in self.get(table):
				component_default_value = vmraid.db.get_value("Salary Component", cstr(d.salary_component),
	...
	def validate_component_based_on_tax_slab(self):
		for row in self.deductions:
			if row.variable_based_on_taxable_salary and (row.amount or row.formula):
				vmraid.throw(_("Row #{0}: Cannot set amount or formula for Salary Component {1} with Variable Based On Taxable Salary")
		...
		if self.earnings:
			...
			for earning_component in self.earnings:
				if earning_component.is_flexible_benefit == 1:
					have_a_flexi = True
					max_of_component = vmraid.db.get_value("Salary Component", earning_component.salary_component, "max_benefit_amount")
			...
			if have_a_flexi and flexi_amount and flt(self.max_benefits) > flexi_amount:
				vmraid.throw(_("Total flexible benefit component amount {0} should not be less than max benefits {1}")
					.format(flexi_amount, self.max_benefits))
		if not have_a_flexi and flt(self.max_benefits) > 0:
			vmraid.throw(_("Salary Structure should have flexible benefit component(s) to dispense benefit amount"))
	...
	def get_employees(self, **kwargs):
		...
		employees = vmraid.db.sql_list("select name from tabEmployee where status='Active' {condition}"
	...
	def assign_salary_structure(self, grade=None, department=None, designation=None, employee=None,
		...
		if employees:
			if len(employees) > 20:
				vmraid.enqueue(assign_salary_structure_for_employees, timeout=600,
...
def assign_salary_structure_for_employees(employees, salary_structure, payroll_payable_account=None, from_date=None, base=None, variable=None, income_tax_slab=None):
	...
	for employee in employees:
		...
		vmraid.publish_progress(count*100/len(set(employees) - set(existing_assignments_for)), title = _("Assigning Structures..."))
...
def create_salary_structures_assignment(employee, salary_structure, payroll_payable_account, from_date, base, variable, income_tax_slab=None):
	if not payroll_payable_account:
		payroll_payable_account = vmraid.db.get_value('Company', salary_structure.company, 'default_payroll_payable_account')
		if not payroll_payable_account:
			vmraid.throw(_('Please set "Default Payroll Payable Account" in Company Defaults'))
	payroll_payable_account_currency = vmraid.db.get_value('Account',  payroll_payable_account, 'account_currency')
	...
	if payroll_payable_account_currency != salary_structure.currency and payroll_payable_account_currency != company_curency:
		vmraid.throw(_("Invalid Payroll Payable Account. The account currency must be {0} or {1}").format(salary_structure.currency, company_curency))
...
def get_existing_assignments(employees, salary_structure, from_date):
	...
	if salary_structures_assignments:
		vmraid.msgprint(_("Skipping Salary Structure Assignment for the following employees, as Salary Structure Assignment records already exists against them. {0}")
...
def make_salary_slip(source_name, target_doc = None, employee = None, as_print = False, print_format = None, for_preview=0, ignore_permissions=False):
	def postprocess(source, target):
		if employee:
			employee_details = vmraid.db.get_value("Employee", employee,
			...
			if not target.payroll_cost_center and target.department:
				target.payroll_cost_center = vmraid.db.get_value("Department", target.department, "payroll_cost_center")
...
def get_employees(salary_structure):
	employees = vmraid.get_list('Salary Structure Assignment',
		filters={'salary_structure': salary_structure, 'docstatus': 1}, fields=['employee'])

	if not employees:
		vmraid.throw(_("There's no Employee with Salary Structure: {0}. Assign {1} to an Employee to preview Salary Slip").format(
