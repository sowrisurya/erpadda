...
from vmraid.utils import cint, flt, add_days, getdate, add_to_date, DATE_FORMAT, date_diff, comma_and
...
class PayrollEntry(Document):
	...
	def validate_employee_details(self):
		...
		for employee_details in self.employees:
			if vmraid.db.exists("Salary Slip", {"employee": employee_details.employee, "start_date": self.start_date, "end_date": self.end_date, "docstatus": 1}):
	...
	def on_cancel(self):
		vmraid.delete_doc("Salary Slip", vmraid.db.sql_list("""select name from `tabSalary Slip`
	...
	def fill_employee_details(self):
		...
		if not employees:
			error_msg = _("No employees found for the mentioned criteria:<br>Company: {0}<br> Currency: {1}<br>Payroll Payable Account: {2}").format(
				vmraid.bold(self.company), vmraid.bold(self.currency), vmraid.bold(self.payroll_payable_account))
			if self.branch:
	...
	def submit_salary_slips(self):
		...
		if len(ss_list) > 30:
			vmraid.enqueue(submit_salary_slips_for_employees, timeout=600, payroll_entry=self, salary_slips=ss_list)
		else:
	...
	def get_salary_component_account(self, salary_component):
		account = vmraid.db.get_value("Salary Component Account",
			{"parent": salary_component, "company": self.company}, "account")

		if not account:
			vmraid.throw(_("Please set account in Salary Component {0}")
	...
	def get_salary_components(self, component_type):
		...
		if salary_slips:
			salary_components = vmraid.db.sql("""
		...
		if salary_components:
			...
			for item in salary_components:
				...
				if component_type == "earnings":
					is_flexible_benefit, only_tax_impact = vmraid.db.get_value("Salary Component", item['salary_component'], ['is_flexible_benefit', 'only_tax_impact'])
	...
	def make_payment_entry(self):
		...
		salary_slip_name_list = vmraid.db.sql(""" select t1.name from `tabSalary Slip` t1
		...
		if salary_slip_name_list and len(salary_slip_name_list) > 0:
			...
			for salary_slip_name in salary_slip_name_list:
				...
				for sal_detail in salary_slip.earnings:
					is_flexible_benefit, only_tax_impact, creat_separate_je, statistical_component = vmraid.db.get_value("Salary Component", sal_detail.salary_component,
				...
				for sal_detail in salary_slip.deductions:
					statistical_component = vmraid.db.get_value("Salary Component", sal_detail.salary_component, 'statistical_component')
	...
	def validate_employee_attendance(self):
		...
		for employee_detail in self.employees:
			employee_joining_date = vmraid.db.get_value("Employee", employee_detail.employee, 'date_of_joining')
			start_date = self.start_date
	...
	def get_count_holidays_of_employee(self, employee, start_date):
		...
		holidays = 0
		if holiday_list:
			days = vmraid.db.sql("""select count(*) from tabHoliday where
	...
	def get_count_employee_attendance(self, employee, start_date):
		marked_days = 0
		attendances = vmraid.get_all("Attendance",
			fields = ["count(*)"],
			filters = {
				"employee": employee,
...
def remove_payrolled_employees(emp_list, start_date, end_date):
	for employee_details in emp_list:
		if vmraid.db.exists("Salary Slip", {"employee": employee_details.employee, "start_date": start_date, "end_date": end_date, "docstatus": 1}):
...
def create_salary_slips_for_employees(employees, args, publish_progress=True):
	...
	for emp in employees:
		if emp not in salary_slips_exists_for:
			...
			count+=1
			if publish_progress:
				vmraid.publish_progress(count*100/len(set(employees) - set(salary_slips_exists_for)),
	...
	if salary_slips_not_created:
		vmraid.msgprint(_("Salary Slips already exists for employees {}, and will not be processed by this payroll.")
			.format(vmraid.bold(", ".join([emp for emp in salary_slips_not_created]))) , title=_("Message"), indicator="orange")
...
def submit_salary_slips_for_employees(payroll_entry, salary_slips, publish_progress=True):
	...
	if submitted_ss:
		...
		vmraid.msgprint(_("Salary Slip submitted for period from {0} to {1}")
	...
	if not submitted_ss and not not_submitted_ss:
		vmraid.msgprint(_("No salary slip found to submit for the above selected criteria OR salary slip already submitted"))
...
def employee_query(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.db.sql("""select name, employee_name from `tabEmployee`
		where status = 'Active'
			and docstatus < 2
			and ({key} like %(txt)s
