...
from erpadda.payroll.doctype.payroll_entry.payroll_entry import get_start_end_dates, get_end_date
from erpadda.hr.doctype.employee.test_employee import make_employee
from erpadda.payroll.doctype.salary_slip.test_salary_slip import get_salary_component_account, \
		make_earning_salary_component, make_deduction_salary_component, create_account, make_employee_salary_slip
from erpadda.payroll.doctype.salary_structure.test_salary_structure import make_salary_structure, create_salary_structure_assignment
from erpadda.loan_management.doctype.loan.test_loan import create_loan, make_loan_disbursement_entry, create_loan_type, create_loan_accounts
...
class TestPayrollEntry(unittest.TestCase):
	@classmethod
	def setUpClass(cls):
		vmraid.db.set_value("Company", erpadda.get_default_company(), "default_holiday_list", '_Test Holiday List')
	...
	def test_payroll_entry(self): # pylint: disable=no-self-use
		...
		for data in vmraid.get_all('Salary Component', fields = ["name"]):
			if not vmraid.db.get_value('Salary Component Account',
		...
		if not vmraid.db.get_value("Salary Slip", {"start_date": dates.start_date, "end_date": dates.end_date}):
	...
	def test_multi_currency_payroll_entry(self): # pylint: disable=no-self-use
		...
		vmraid.db.sql("""delete from `tabSalary Slip` where employee=%s""",(vmraid.db.get_value("Employee", {"user_id": "test_muti_currency_employee@payroll.com"})))
	...
	def test_payroll_entry_with_employee_cost_center(self): # pylint: disable=no-self-use
		...
		if not vmraid.db.get_value("Company", "_Test Company", "default_payroll_payable_account") or \
			vmraid.db.get_value("Company", "_Test Company", "default_payroll_payable_account") != "_Test Payroll Payable - _TC":
				vmraid.db.set_value("Company", "_Test Company", "default_payroll_payable_account",
		...
		if not vmraid.db.get_value("Salary Slip", {"start_date": dates.start_date, "end_date": dates.end_date}):
			pe = make_payroll_entry(start_date=dates.start_date, end_date=dates.end_date, payable_account="_Test Payroll Payable - _TC",
				currency=vmraid.db.get_value("Company", "_Test Company", "default_currency"), department="cc - _TC", company="_Test Company", payment_account="Cash - _TC", cost_center="Main - _TC")
	...
	def test_loan(self):
		...
		if not company_doc.default_payroll_payable_account:
			company_doc.default_payroll_payable_account = vmraid.db.get_value('Account',
		...
		name = vmraid.db.get_value('Salary Slip',
...
def get_payment_account():
	return vmraid.get_value('Account',
