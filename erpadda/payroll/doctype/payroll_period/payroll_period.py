...
from vmraid.utils import date_diff, getdate, formatdate, cint, month_diff, flt, add_months
...
class PayrollPeriod(Document):
	...
	def validate_overlap(self):
		...
		overlap_doc = vmraid.db.sql(query.format(self.doctype),{
...
def get_payroll_period_days(start_date, end_date, employee, company=None):
	...
	if len(payroll_period) > 0:
		...
		if not cint(vmraid.db.get_value("Payroll Settings", None, "include_holidays_in_total_working_days")):
...
def get_period_factor(employee, start_date, end_date, payroll_frequency, payroll_period, depends_on_payment_days=0):
	...
	joining_date, relieving_date = vmraid.db.get_value("Employee", employee, ["date_of_joining", "relieving_date"])
