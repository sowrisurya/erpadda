...
def get_columns(salary_slips):
	...
	for component in vmraid.db.sql("""select distinct sd.salary_component, sc.type
...
def get_salary_slips(filters, company_currency):
	...
	salary_slips = vmraid.db.sql("""select * from `tabSalary Slip` where %s
...
def get_ss_earning_map(salary_slips, currency, company_currency):
	ss_earnings = vmraid.db.sql("""select sd.parent, sd.salary_component, sd.amount, ss.exchange_rate, ss.name
		from `tabSalary Detail` sd, `tabSalary Slip` ss where sd.parent=ss.name and sd.parent in (%s)""" %
		(', '.join(['%s']*len(salary_slips))), tuple([d.name for d in salary_slips]), as_dict=1)

	ss_earning_map = {}
	for d in ss_earnings:
		ss_earning_map.setdefault(d.parent, vmraid._dict()).setdefault(d.salary_component, [])
		if currency == company_currency:
...
def get_ss_ded_map(salary_slips, currency, company_currency):
	ss_deductions = vmraid.db.sql("""select sd.parent, sd.salary_component, sd.amount, ss.exchange_rate, ss.name
		from `tabSalary Detail` sd, `tabSalary Slip` ss where sd.parent=ss.name and sd.parent in (%s)""" %
		(', '.join(['%s']*len(salary_slips))), tuple([d.name for d in salary_slips]), as_dict=1)

	ss_ded_map = {}
	for d in ss_deductions:
		ss_ded_map.setdefault(d.parent, vmraid._dict()).setdefault(d.salary_component, [])
		if currency == company_currency:
