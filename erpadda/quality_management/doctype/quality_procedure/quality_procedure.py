...
import vmraid
from vmraid.utils.nestedset import NestedSet, rebuild_tree
...
class QualityProcedure(NestedSet):
	...
	def on_trash(self):
		...
		vmraid.db.sql('''update `tabQuality Procedure Process`
	...
	def set_parent(self):
		for process in self.processes:
			...
			has_parent = vmraid.db.get_value("Quality Procedure", process.procedure, "parent_quality_procedure")
	...
	def check_for_incorrect_child(self):
		for process in self.processes:
			if process.procedure:
				...
				parent_quality_procedure = vmraid.db.get_value("Quality Procedure", process.procedure, "parent_quality_procedure")
				if parent_quality_procedure and parent_quality_procedure != self.name:
					vmraid.throw(_("{0} already has a Parent Procedure {1}.").format(vmraid.bold(process.procedure), vmraid.bold(parent_quality_procedure)),
...
def get_children(doctype, parent=None, parent_quality_procedure=None, is_root=False):
	...
	if parent:
		...
		return [dict(
				value=d.procedure,
				expandable=vmraid.db.get_value('Quality Procedure', d.procedure, 'is_group'))
				for d in parent_procedure.processes if d.procedure
			]
	else:
		return vmraid.get_all(doctype, fields=['name as value', 'is_group as expandable'],
