...
def execute(filters=None):
	...
	data = vmraid.db.get_all("Project", filters=filters, fields=["name", 'status', "percent_complete", "expected_start_date", "expected_end_date", "project_type"], order_by="expected_end_date")

	for project in data:
		project["total_tasks"] = vmraid.db.count("Task", filters={"project": project.name})
		project["completed_tasks"] = vmraid.db.count("Task", filters={"project": project.name, "status": "Completed"})
		project["overdue_tasks"] = vmraid.db.count("Task", filters={"project": project.name, "status": "Overdue"})
