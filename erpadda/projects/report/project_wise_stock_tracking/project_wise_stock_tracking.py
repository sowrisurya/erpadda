...
def get_project_details():
	return vmraid.db.sql(""" select name, project_name, status, company, customer, estimated_costing,
		expected_start_date, expected_end_date from tabProject where docstatus < 2""", as_dict=1)

def get_purchased_items_cost():
	pr_items = vmraid.db.sql("""select project, sum(base_net_amount) as amount
...
def get_issued_items_cost():
	se_items = vmraid.db.sql("""select se.project, sum(se_item.amount) as amount
...
def get_delivered_items_cost():
	dn_items = vmraid.db.sql("""select dn.project, sum(dn_item.base_net_amount) as amount
		from `tabDelivery Note` dn, `tabDelivery Note Item` dn_item
		where dn.name = dn_item.parent and dn.docstatus = 1 and ifnull(dn.project, '') != ''
		group by dn.project""", as_dict=1)

	si_items = vmraid.db.sql("""select si.project, sum(si_item.base_net_amount) as amount
