...
from vmraid.utils import flt, time_diff_in_hours, get_datetime, getdate, cint, date_diff, add_to_date
from vmraid.model.document import Document
from erpadda.manufacturing.doctype.workstation.workstation import (check_if_within_operating_hours,
...
class OverWorkLoggedError(vmraid.ValidationError): pass
...
class Timesheet(Document):
	...
	def validate_project(self, data):
		if self.parent_project and self.parent_project != data.project:
			vmraid.throw(_("Row {0}: Project must be same as the one set in the Timesheet: {1}.").format(data.idx, self.parent_project))
	...
	def get_overlap_for(self, fieldname, args, value):
		...
		existing = vmraid.db.sql("""select ts.name as name, tsd.from_time as from_time, tsd.to_time as to_time from
...
def get_projectwise_timesheet_data(project=None, parent=None, from_time=None, to_time=None):
	...
	return vmraid.db.sql("""SELECT tsd.name as name,
...
def get_timesheet(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.db.sql("""select distinct tsd.parent from `tabTimesheet Detail` tsd,
...
def get_activity_cost(employee=None, activity_type=None, currency=None):
	...
	rate = vmraid.db.get_values("Activity Cost", {"employee": employee,
		"activity_type": activity_type}, ["costing_rate", "billing_rate"], as_dict=True)
	if not rate:
		rate = vmraid.db.get_values("Activity Type", {"activity_type": activity_type},
...
def get_events(start, end, filters=None):
	...
	return vmraid.db.sql("""select `tabTimesheet Detail`.name as name,
...
def get_timesheets_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	...
	if customer:
		sales_invoices = [d.name for d in vmraid.get_all('Sales Invoice', filters={'customer': customer})] or [None]
