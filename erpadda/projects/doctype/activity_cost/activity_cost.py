...
class DuplicationError(vmraid.ValidationError): pass
...
class ActivityCost(Document):
	...
	def check_unique(self):
		if self.employee:
			if vmraid.db.sql("""select name from `tabActivity Cost` where employee_name= %s and activity_type= %s and name != %s""",
				(self.employee_name, self.activity_type, self.name)):
					vmraid.throw(_("Activity Cost exists for Employee {0} against Activity Type - {1}")
						.format(self.employee, self.activity_type), DuplicationError)
		else:
			if vmraid.db.sql("""select name from `tabActivity Cost` where ifnull(employee, '')='' and activity_type= %s and name != %s""",
				(self.activity_type, self.name)):
					vmraid.throw(_("Default Activity Cost exists for Activity Type - {0}")
