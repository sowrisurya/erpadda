...
import vmraid
import unittest
from vmraid.utils import getdate, nowdate, add_days
...
class TestTask(unittest.TestCase):
	...
	def test_reschedule_dependent_task(self):
		...
		self.assertEqual(vmraid.db.get_value("Task", task2.name, "exp_start_date"),
			getdate(add_days(nowdate(), 21)))
		self.assertEqual(vmraid.db.get_value("Task", task2.name, "exp_end_date"),
			getdate(add_days(nowdate(), 25)))

		self.assertEqual(vmraid.db.get_value("Task", task3.name, "exp_start_date"),
			getdate(add_days(nowdate(), 26)))
		self.assertEqual(vmraid.db.get_value("Task", task3.name, "exp_end_date"),
	...
	def test_close_assignment(self):
		...
		def get_owner_and_status():
			return vmraid.db.get_value("ToDo",
...
def create_task(subject, start=None, end=None, depends_on=None, project=None, parent_task=None, is_group=0, is_template=0, begin=0, duration=0, save=True):
	if not vmraid.db.exists("Task", subject):
		...
		task.project = project or None if is_template else vmraid.get_value("Project", {"project_name": "_Test Project"})
