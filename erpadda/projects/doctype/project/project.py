...
from vmraid.utils import (flt, getdate, get_url, now,
...
class Project(Document):
	...
	def onload(self):
		self.set_onload('activity_summary', vmraid.db.sql('''select activity_type,
	...
	def update_percent_complete(self):
		...
		if not total:
			...
			if (self.percent_complete_method == "Task Completion" and total > 0) or (
				...
				completed = vmraid.db.sql("""select count(name) from tabTask where
					project=%s and status in ('Cancelled', 'Completed')""", self.name)[0][0]
				self.percent_complete = flt(flt(completed) / total * 100, 2)

			if (self.percent_complete_method == "Task Progress" and total > 0):
				progress = vmraid.db.sql("""select sum(progress) from tabTask where
					project=%s""", self.name)[0][0]
				self.percent_complete = flt(flt(progress) / total, 2)

			if (self.percent_complete_method == "Task Weight" and total > 0):
				weight_sum = vmraid.db.sql("""select sum(task_weight) from tabTask where
					project=%s""", self.name)[0][0]
				weighted_progress = vmraid.db.sql("""select progress, task_weight from tabTask where
	...
	def update_costing(self):
		from_time_sheet = vmraid.db.sql("""select
		...
		from_expense_claim = vmraid.db.sql("""select
	...
	def update_purchase_costing(self):
		total_purchase_cost = vmraid.db.sql("""select sum(base_net_amount)
			from `tabPurchase Invoice Item` where project = %s and docstatus=1""", self.name)

		self.total_purchase_cost = total_purchase_cost and total_purchase_cost[0][0] or 0

	def update_sales_amount(self):
		total_sales_amount = vmraid.db.sql("""select sum(base_net_total)
			from `tabSales Order` where project = %s and docstatus=1""", self.name)

		self.total_sales_amount = total_sales_amount and total_sales_amount[0][0] or 0

	def update_billed_amount(self):
		total_billed_amount = vmraid.db.sql("""select sum(base_net_total)
	...
	def send_welcome_email(self):
		...
		for user in self.users:
			if user.welcome_email_sent == 0:
				vmraid.sendmail(user.user, subject=_("Project Collaboration Invitation"),
								content=content.format(*messages))
				user.welcome_email_sent = 1

def get_timeline_data(doctype, name):
	'''Return timeline for attendance'''
	return dict(vmraid.db.sql('''select unix_timestamp(from_time), count(*)
...
def get_project_list(doctype, txt, filters, limit_start, limit_page_length=20, order_by="modified"):
	return vmraid.db.sql('''select distinct project.*
...
def allow_to_make_project_update(project, time, frequency):
	data = vmraid.db.sql(""" SELECT name from `tabProject Update`
...
def create_duplicate_project(prev_doc, project_name):
	...
	task_list = vmraid.get_all("Task", filters={
...
def get_projects_for_collect_progress(frequency, fields):
	...
	return vmraid.get_all("Project", fields = fields,
...
def send_project_update_email_to_users(project):
	...
	incoming_email_account = vmraid.db.get_value('Email Account',
		dict(enable_incoming=1, default_incoming=1), 'email_id')

	vmraid.sendmail(recipients=get_users_email(doc),
		message=doc.message,
		subject=_(subject),
...
def collect_project_status():
	for data in vmraid.get_all("Project Update",
		{'date': today(), 'sent': 0}):
		replies = vmraid.get_all('Communication',
		...
		for d in replies:
			...
			user_data = vmraid.db.get_values("User", {"email": d.sender},
...
def send_project_status_email_to_users():
	...
	for d in vmraid.get_all("Project Update",
		...
		vmraid.sendmail(recipients=get_users_email(project_doc),
...
def update_project_sales_billing():
	...
	for project_details in vmraid.db.sql('''
		...
		project = project_map.setdefault(project_details.name, vmraid.get_doc('Project', project_details.name))
...
def create_kanban_board_if_not_exists(project):
	...
	if not vmraid.db.exists('Kanban Board', project):
