...
@vmraid.whitelist()
def daily_reminder():
    project = vmraid.db.sql("""SELECT `tabProject`.project_name,`tabProject`.frequency,`tabProject`.expected_start_date,`tabProject`.expected_end_date,`tabProject`.percent_complete FROM `tabProject`;""")
    for projects in project:
        ...
        date_end = projects [3]
        progress = projects [4]
        draft = vmraid.db.sql("""SELECT count(docstatus) from `tabProject Update` WHERE `tabProject Update`.project = %s AND `tabProject Update`.docstatus = 0;""",project_name)
        for drafts in draft:
            number_of_drafts = drafts[0]
        update = vmraid.db.sql("""SELECT name,date,time,progress,progress_details FROM `tabProject Update` WHERE `tabProject Update`.project = %s AND date = DATE_ADD(CURDATE(), INTERVAL -1 DAY);""",project_name)
...
def email_sending(project_name,frequency,date_start,date_end,progress,number_of_drafts,update):
    ...
    holiday = vmraid.db.sql("""SELECT holiday_date FROM `tabHoliday` where holiday_date = CURDATE();""")
    ...
    if len(holiday) == 0:
        ...
        for emails in email:
            vmraid.sendmail(recipients=emails,subject=vmraid._(project_name + ' ' + 'Summary'),message = msg)
    else:
    	pass