...
def register_marketplace(company, company_description):
	...
	settings = vmraid.get_single('Marketplace Settings')
	...
	if message.get('hub_seller_name'):
		...
		settings.add_hub_user(vmraid.session.user)
...
def validate_registerer():
	if current_user == 'Administrator':
		vmraid.throw(_('Please login as another user to register on Marketplace'))

	valid_roles = ['System Manager', 'Item Manager']

	if not vmraid.utils.is_subset(valid_roles, vmraid.get_roles()):
		vmraid.throw(_('Only users with {0} role can register on Marketplace').format(', '.join(valid_roles)),
...
def map_fields(items):
	...
	table_fields = [d.fieldname for d in vmraid.get_meta('Item').get_table_fields()]

	hub_seller_name = vmraid.db.get_value('Marketplace Settings', 'Marketplace Settings', 'hub_seller_name')
...
def update_item(ref_doc, data):
	...
	try:
		...
		vmraid.log_error(message=e, title='Hub Sync Error')
...
def publish_selected_items(items_to_publish):
	...
	if not len(items_to_publish):
		vmraid.throw(_('No items to publish'))

	for item in items_to_publish:
		item_code = item.get('item_code')
		vmraid.db.set_value('Item', item_code, 'publish_in_hub', 1)
		...
		vmraid.get_doc(hub_dict).insert(ignore_if_duplicate=True)
...
def unpublish_item(item_code, hub_item_name):
	...
	if response:
		vmraid.db.set_value('Item', item_code, 'publish_in_hub', 0)
		vmraid.delete_doc('Hub Tracked Item', item_code)
	else:
		vmraid.throw(_('Unable to update remote activity'))

@vmraid.whitelist()
def get_unregistered_users():
	settings = vmraid.get_single('Marketplace Settings')
	registered_users = [user.user for user in settings.users] + ['Administrator', 'Guest']
	all_users = [user.name for user in vmraid.db.get_all('User', filters={'enabled': 1})]
...
def item_sync_preprocess(intended_item_publish_count):
	...
	if response:
		vmraid.db.set_value("Marketplace Settings", "Marketplace Settings", "sync_in_progress", 1)
		return response
	else:
		vmraid.throw(_('Unable to update remote activity'))
...
def item_sync_postprocess():
	...
	if response:
		vmraid.db.set_value('Marketplace Settings', 'Marketplace Settings', 'last_sync_datetime', vmraid.utils.now())
	else:
		vmraid.throw(_('Unable to update remote activity'))

	vmraid.db.set_value('Marketplace Settings', 'Marketplace Settings', 'sync_in_progress', 0)
...
def convert_relative_image_urls_to_absolute(items):
	...
	for item in items:
		...
		if file_path.startswith('/files/'):
			item['image'] = urljoin(vmraid.utils.get_url(), file_path)


def get_hub_connection():
	settings = vmraid.get_single('Marketplace Settings')
	marketplace_url = settings.marketplace_url
	hub_user = settings.get_hub_user(vmraid.session.user)

	if hub_user:
		password = hub_user.get_password()
		hub_connection = VMRaidClient(marketplace_url, hub_user.user, password)
		return hub_connection
	else:
		read_only_hub_connection = VMRaidClient(marketplace_url)
