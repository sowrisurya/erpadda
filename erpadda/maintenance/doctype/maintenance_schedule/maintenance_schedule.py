...
from vmraid.utils import add_days, getdate, cint, cstr
...
class MaintenanceSchedule(TransactionBase):
	...
	def generate_schedule(self):
		self.set('schedules', [])
		vmraid.db.sql("""delete from `tabMaintenance Schedule Detail`
	...
	def on_submit(self):
		...
		for d in self.get('items'):
			...
			if no_email_sp:
				vmraid.msgprint(
					vmraid._("Setting Events to {0}, since the Employee attached to the below Sales Persons does not have a User ID{1}").format(
						self.owner, "<br>" + "<br>".join(no_email_sp)
				))

			scheduled_date = vmraid.db.sql("""select scheduled_date from
				`tabMaintenance Schedule Detail` where sales_person=%s and item_code=%s and
				parent=%s""", (d.sales_person, d.item_code, self.name), as_dict=1)

			for key in scheduled_date:
				description =vmraid._("Reference: {0}, Item Code: {1} and Customer: {2}").format(self.name, d.item_code, self.customer)
	...
	def validate_schedule_date_for_holiday_list(self, schedule_date, sales_person):
		...
		holidays = vmraid.db.sql_list('''select holiday_date from `tabHoliday` where parent=%s''', holiday_list)
	...
	def validate_sales_order(self):
		for d in self.get('items'):
			if d.sales_order:
				chk = vmraid.db.sql("""select ms.name from `tabMaintenance Schedule` ms,
	...
	def validate_serial_no(self, item_code, serial_nos, amc_start_date):
		for serial_no in serial_nos:
			sr_details = vmraid.db.get_value("Serial No", serial_no,
			...
			if sr_details.get("item_code") != item_code:
				vmraid.throw(_("Serial No {0} does not belong to Item {1}")
					.format(vmraid.bold(serial_no), vmraid.bold(item_code)), title="Invalid")
