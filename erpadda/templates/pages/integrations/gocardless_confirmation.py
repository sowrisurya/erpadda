...
from erpadda.erpadda_integrations.doctype.gocardless_settings.gocardless_settings import gocardless_initialization, get_gateway_controller
...
def confirm_payment(redirect_flow_id, reference_doctype, reference_docname):
	...
	try:
		...
		if gocardless_success_page:
			confirmation_url = vmraid.get_attr(gocardless_success_page[-1])(reference_doctype, reference_docname)
...
def create_mandate(data):
	...
	if vmraid.db.exists("GoCardless Mandate", mandate):
		...
		reference_doc = vmraid.db.get_value(data.get('reference_doctype'), data.get('reference_docname'), ["reference_doctype", "reference_name"], as_dict=1)
		erpadda_customer = vmraid.db.get_value(reference_doc.reference_doctype, reference_doc.reference_name, ["customer_name"], as_dict=1)
