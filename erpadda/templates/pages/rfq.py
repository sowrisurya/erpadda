...
def check_supplier_has_docname_access(supplier):
	status = True
	if vmraid.form_dict.name not in vmraid.db.sql_list("""select parent from `tabRequest for Quotation Supplier`
...
def update_supplier_details(context):
	...
	context.doc.currency = supplier_doc.default_currency or vmraid.get_cached_value('Company',  context.doc.company,  "default_currency")
	context.doc.currency_symbol = vmraid.db.get_value("Currency", context.doc.currency, "symbol", cache=True)
	context.doc.number_format = vmraid.db.get_value("Currency", context.doc.currency, "number_format", cache=True)
	context.doc.buying_price_list = supplier_doc.default_price_list or ''

def get_link_quotation(supplier, rfq):
	quotation = vmraid.db.sql(""" select distinct `tabSupplier Quotation Item`.parent as name,
