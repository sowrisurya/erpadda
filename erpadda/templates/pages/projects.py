...
def get_context(context):
	project_user = vmraid.db.get_value("Project User", {"parent": vmraid.form_dict.project, "user": vmraid.session.user} , ["user", "view_attachments"], as_dict= True)
	if vmraid.session.user != 'Administrator' and (not project_user or vmraid.session.user == 'Guest'):
	...
	project.tasks = get_tasks(project.name, start=0, item_status='open',
		search=vmraid.form_dict.get("search"))
...
# 		filters["status"] = item_status
	tasks = vmraid.get_all("Task", filters=filters,
		fields=["name", "subject", "status", "_seen", "_comments", "modified", "description"],
		limit_start=start, limit_page_length=10)

	for task in tasks:
		task.todo = vmraid.get_all('ToDo',filters={'reference_name':task.name, 'reference_type':'Task'},
		fields=["assigned_by", "owner", "modified", "modified_by"])

		if task.todo:
			task.todo=task.todo[0]
			task.todo.user_image = vmraid.db.get_value('User', task.todo.owner, 'user_image')
		...
		task.css_seen = ''
		if task._seen:
			if vmraid.session.user in json.loads(task._seen):
				task.css_seen = 'seen'
...
def get_task_html(project, start=0, item_status=None):
	return vmraid.render_template("erpadda/templates/includes/projects/project_tasks.html",
		{"doc": {
			"name": project,
...
def get_timesheets(project, start=0, search=None):
	...
	timesheets = vmraid.get_all('Timesheet Detail', filters=filters,
	fields=['project','activity_type','from_time','to_time','parent'],
	limit_start=start, limit_page_length=10)
	for timesheet in timesheets:
		timesheet.infos = vmraid.get_all('Timesheet', filters={"name": timesheet.parent},
			fields=['name','_comments','_seen','status','modified','modified_by'],
			limit_start=start, limit_page_length=10)

		for timesheet.info in timesheet.infos:
			timesheet.info.user_image = vmraid.db.get_value('User', timesheet.info.modified_by, 'user_image')

			timesheet.info.comment_count = len(json.loads(timesheet.info._comments or "[]"))

			timesheet.info.css_seen = ''
			if timesheet.info._seen:
				if vmraid.session.user in json.loads(timesheet.info._seen):
					timesheet.info.css_seen = 'seen'
	return timesheets

@vmraid.whitelist()
def get_timesheet_html(project, start=0):
	return vmraid.render_template("erpadda/templates/includes/projects/project_timesheets.html",
		{"doc": {"timesheets": get_timesheets(project, start)}}, is_path=True)

def get_attachments(project):
	return vmraid.get_all('File', filters= {"attached_to_name": project, "attached_to_doctype": 'Project', "is_private":0},
