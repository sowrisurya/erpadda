...
def get_context(context):
	...
	for item in homepage.products:
		route = vmraid.db.get_value('Item', item.item_code, 'route')
		if route:
			item.route = '/' + route
	...
	if homepage.hero_section_based_on == 'Homepage Section' and homepage.hero_section:
		homepage.hero_section_doc = vmraid.get_doc('Homepage Section', homepage.hero_section)

	if homepage.slideshow:
		doc = vmraid.get_doc('Website Slideshow', homepage.slideshow)
		context.slideshow = homepage.slideshow
		context.slideshow_header = doc.header
		context.slides = doc.slideshow_items

	context.blogs = vmraid.get_all('Blog Post',
	...
	homepage_sections = vmraid.get_all('Homepage Section',
		filters=[['name', '!=', homepage_hero_section]] if homepage_hero_section else None,
		order_by='section_order asc'
	)
	context.homepage_sections = [vmraid.get_doc('Homepage Section', name) for name in homepage_sections]
