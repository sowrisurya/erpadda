...
import vmraid, requests, json
from vmraid import _
from vmraid.utils import get_site_url, get_url_to_form, get_link_to_form
from vmraid.model.document import Document
from vmraid.utils.file_manager import get_file, get_file_path
...
class LinkedInSettings(Document):
	...
	def get_authorization_url(self):
		params = urlencode({
			...
			"redirect_uri": "{0}/api/method/erpadda.crm.doctype.linkedin_settings.linkedin_settings.callback?".format(vmraid.utils.get_url()),
	...
	def get_member_profile(self):
		...
		vmraid.db.set_value(self.doctype, self.name, {
	...
	def upload_image(self, media):
		...
		if response.status_code == 200:
			...
			if response.status_code < 200 and response.status_code > 299:
				vmraid.throw(_("Error While Uploading Image"), title="{0} {1}".format(response.status_code, response.reason))
				return None
			return asset
	...
	def http_post(self, url, headers=None, body=None, data=None):
		try:
			...
			if response.status_code == 401:
				...
				vmraid.db.commit()
				vmraid.throw(content["message"], title="LinkedIn Error - Unauthorized")
			elif response.status_code == 403:
				vmraid.msgprint(_("You Didn't have permission to access this API"))
				vmraid.throw(content["message"], title="LinkedIn Error - Access Denied")
			else:
