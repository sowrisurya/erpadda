...
def create_test_lead():
    test_lead = vmraid.db.exists({'doctype': 'Lead', 'lead_name': 'Test Lead'})
    if test_lead:
        return vmraid.get_doc('Lead', test_lead[0][0])
...
def create_test_appointments():
    test_appointment = vmraid.db.exists(
        {'doctype': 'Appointment', 'scheduled_time':datetime.datetime.now(),'email':'test@example.com'})
    if test_appointment:
        return vmraid.get_doc('Appointment', test_appointment[0][0])
