...
from vmraid.utils.verified_command import verify_request, get_signed_params
...
class Appointment(Document):
	...
	def before_insert(self):
		...
		if not number_of_agents == 0:
			if (number_of_appointments_in_same_slot >= number_of_agents):
				vmraid.throw('Time slot is not available')
		# Link lead
		if not self.party:
	...
	def send_confirmation_email(self):
		...
		vmraid.sendmail(recipients=[self.customer_email],
						template=template,
						args=args,
	...
	def set_verified(self, email):
		if not email == self.customer_email:
			vmraid.throw('Email verification failed.')
		# Create new lead
...
def _get_employee_from_user(user):
	employee_docname = vmraid.db.exists(
		{'doctype': 'Employee', 'user_id': user})
	if employee_docname:
		# vmraid.db.exists returns a tuple of a tuple
		return vmraid.get_doc('Employee', employee_docname[0][0])
	return None
