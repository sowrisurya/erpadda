...
vmraid.ui.form.on('Social Media Post', {
    validate: function(frm){
        if (frm.doc.twitter === 0 && frm.doc.linkedin === 0){
            vmraid.throw(__("Select atleast one Social Media from Share on."))
        }
        if (frm.doc.scheduled_time) {
            ...
            if (scheduled_time.getTime() < date_time.getTime()){
                vmraid.throw(__("Invalid Scheduled Time"));
            }
        }
        if (frm.doc.text?.length > 280){
            vmraid.throw(__("Length Must be less than 280."))
        }
    },
...
var post = function(frm){
    vmraid.dom.freeze();
    vmraid.call({
        method: "erpadda.crm.doctype.social_media_post.social_media_post.publish",
        args: {
