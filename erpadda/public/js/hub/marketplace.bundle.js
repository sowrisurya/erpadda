...
vmraid.provide('vmraid.route');

vmraid.utils.make_event_emitter(vmraid.route);
vmraid.utils.make_event_emitter(erpadda.hub);

erpadda.hub.Marketplace = class Marketplace {
	constructor({ parent }) {
		this.$parent = $(parent);
	...
	make_body() {
		...
		if (!hub.is_server) {
			erpadda.hub.on('seller-registered', () => {
	...
	show_register_dialog() {
		if(vmraid.session.user === 'Administrator') {
			vmraid.msgprint(__('You need to be a user other than Administrator with System Manager and Item Manager roles to register on Marketplace.'));
			return;
		}

		if (!is_subset(['System Manager', 'Item Manager'], vmraid.user_roles)) {
			vmraid.msgprint(__('You need to be a user with System Manager and Item Manager roles to register on Marketplace.'));
			return;
		}
	...
	register_marketplace({company, company_description}) {
		vmraid.call({
		    method: 'erpadda.hub_node.api.register_marketplace',
		    args: {
				company,
				company_description
			}
			...
			if (r.message && r.message.ok) {
				...
				this.update_hub_settings()
					.then(() => {
						vmraid.set_route('marketplace', 'publish');
						erpadda.hub.trigger('seller-registered');
					});
			}
		});
	}
	...
	show_add_user_dialog() {
		if (!is_subset(['System Manager', 'Item Manager'], vmraid.user_roles)) {
			vmraid.msgprint(__('You need to be a user with System Manager and Item Manager roles to add users to Marketplace.'));
			return;
		}
			...
			.then(r => {
				...
				const d = new vmraid.ui.Dialog({
					...
					primary_action({ users }) {
						...
						vmraid.call('erpadda.hub_node.api.register_users', {
						...
						.then(r => {
							...
							if (r.message && r.message.length) {
								vmraid.show_alert(__('Added {0} users', [r.message.length]));
							}
						});
					}
				});
	...
	get_unregistered_users() {
		return vmraid.call('erpadda.hub_node.api.get_unregistered_users')
	}
...
Object.assign(hub, {
	...
	is_user_registered() {
		return this.is_seller_registered() && hub.settings.users
			.filter(hub_user => hub_user.user === vmraid.session.user)
			.length === 1;
	},
	...
	get_settings() {
		...
		return vmraid.db.get_doc('Marketplace Settings');
	}
});
