...
export default {
	...
	mounted() {
		vmraid.route.on('change', () => {
			if (vmraid.get_route_str() === 'marketplace/home') {
				this.get_items();
			}
		})
	},
	methods: {
		get_items() {
			hub.call('get_data_for_homepage', vmraid.defaults ? {
				country: vmraid.defaults.get_user_default('country')
			} : null)
			.then((data) => {
		...
		go_to_item_details_page(hub_item_name) {
			vmraid.set_route(`marketplace/item/${hub_item_name}`);
		},

		set_search_route() {
			vmraid.set_route('marketplace', 'search', 'All', this.search_value);
		},
	}
}
</script>
