...
export default {
	...
	data() {
		return {
			page_name: vmraid.get_route()[1],
			hub_item_name: vmraid.get_route()[2],
	...
	mounted() {
		...
		// erpadda.hub.item_view_cache = erpadda.hub.item_view_cache || [];
		// if (erpadda.hub.item_view_cache.includes(this.hub_item_name)) {
		// 	return;
		// }
		...
		this.item_received.then(() => {
			setTimeout(() => {
				...
				// .then(() => {
				// 	erpadda.hub.item_view_cache.push(this.hub_item_name);
				// });
			}, 5000);
		});
	},
	methods: {
		...
		go_to_seller_profile_page(seller_name) {
			vmraid.set_route(`marketplace/seller/${seller_name}`);
		},
		build_data() {
		...
		add_to_saved_items() {
			hub.call('add_item_to_user_saved_items', {
					hub_item_name: this.hub_item_name,
					hub_user: vmraid.session.user
				})
				.then(() => {
					const saved_items_link = `<b><a href="#marketplace/saved-items">${__('Saved')}</a></b>`;
					vmraid.show_alert(saved_items_link);
					erpadda.hub.trigger('action:item_save');
				})
				.catch(e => {
					console.error(e);
				});
		...
		add_to_featured_items() {
			hub.call('add_item_to_seller_featured_items', {
				...
				.then(() => {
					...
					vmraid.show_alert(featured_items_link);
					erpadda.hub.trigger('action:item_feature');
				})
				.catch(e => {
					console.error(e);
				});
		...
		make_contact_seller_dialog() {
			this.contact_seller_dialog = new vmraid.ui.Dialog({
				...
				primary_action: ({ message }) => {
					...
					hub.call('send_message', {
						...
						.then(() => {
							...
							vmraid.set_route('marketplace', 'buying', this.item.name);
							erpadda.hub.trigger('action:send_message');
						});
				}
			});
		},
		...
		make_report_item_dialog() {
			this.report_item_dialog = new vmraid.ui.Dialog({
				...
				primary_action: ({ message }) => {
					hub.call('add_reported_item', {
						...
						.then(() => {
							d.hide();
							vmraid.show_alert(__('Item Reported'));
						});
				}
			});
		},
		...
		update_details(values) {
			vmraid.call('erpadda.hub_node.api.update_item', {
				...
				.then(() => {
					vmraid.show_alert(__('{0} Updated', [this.item.item_name]));
				});
		},
		...
		unpublish_item() {
			vmraid.confirm(__('Unpublish {0}?', [this.item.item_name]), () => {
				vmraid
					.call('erpadda.hub_node.api.unpublish_item', {
						item_code: this.item.item_code,
						hub_item_name: this.hub_item_name
					})
					.then(r => {
						vmraid.set_route(`marketplace/home`);
						vmraid.show_alert(__('Item listing removed'));
					});
			});
		}
	}
