					...
					<div class="message-list">
						<div class="level margin-bottom" v-for="message in messages" :key="message.name">
							<div class="level-left ellipsis" style="width: 80%;">
								<div v-html="vmraid.avatar(message.sender)" />
								<div style="white-space: normal;" v-html="message.message" />
							</div>
							<div class="level-right text-muted" v-html="vmraid.datetime.comment_when(message.creation, true)" />
						</div>
					</div>
...
export default {
	...
	data() {
		return {
			message_type: vmraid.get_route()[1],
			item_details: null,
			messages: []
		}
	},
	...
	methods: {
		send_message(message) {
			this.messages.push({
				...
				message: message,
				creation: Date.now(),
				name: vmraid.utils.get_random(6)
			});
