...
erpadda.TransactionController = class TransactionController extends erpadda.taxes_and_totals {
		...
		if (scan_barcode_field && scan_barcode_field.get_value()) {
			...
			if (vmraid.is_mobile()) {
				...
				$(`<span class="input-group-btn" style="vertical-align: top">
					...
					.on('click', '.btn', () => {
						vmraid.barcode.scan_barcode().then(barcode => {
	...
	scan_barcode() {
		...
		if(this.frm.doc.scan_barcode) {
			vmraid.call({
				method: "erpadda.selling.page.point_of_sale.point_of_sale.search_serial_or_batch_or_barcode_number",
				...
				if (!row_to_modify) {
					// add new row
					row_to_modify = vmraid.model.add_child(this.frm.doc, cur_grid.doctype, 'items');
				}

				show_description(row_to_modify.idx, row_to_modify.item_code);

				this.frm.from_barcode = this.frm.from_barcode ? this.frm.from_barcode + 1 : 1;
				vmraid.model.set_value(row_to_modify.doctype, row_to_modify.name, {
					item_code: data.item_code,
					qty: (row_to_modify.qty || 0) + 1
				});

				['serial_no', 'batch_no', 'barcode'].forEach(field => {
					if (data[field] && vmraid.meta.has_field(row_to_modify.doctype, field)) {

						let value = (row_to_modify[field] && field === "serial_no")
							? row_to_modify[field] + '\n' + data[field] : data[field];

						vmraid.model.set_value(row_to_modify.doctype,
		...
		for(var i=0, l=children.length; i<l; i++) {
			...
			for(var k in d) {
				...
				if (["doctype", "name"].indexOf(k)===-1) {
					...
					if (k !== 'free_item_data') {
						vmraid.model.set_value(d.doctype, d.name, k, v);
					}
				}
			}
