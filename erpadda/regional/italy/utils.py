...
import json
import vmraid
from vmraid.utils import flt, cstr
from erpadda.controllers.taxes_and_totals import get_itemised_tax
from vmraid import _
from vmraid.core.doctype.file.file import remove_file
...
def export_invoices(filters=None):
	...
	zip_filename = "{0}-einvoices.zip".format(
		vmraid.utils.get_datetime().strftime("%Y%m%d_%H%M%S"))
...
def prepare_invoice(invoice, progressive_number):
	...
	if not invoice.type_of_document:
		if invoice.is_return and invoice.return_against:
			...
			invoice.return_against_unamended =  get_unamended_name(vmraid.get_doc("Sales Invoice", invoice.return_against))
		else:
...
def sales_invoice_validate(doc):
	...
	if not doc.company_address:
		vmraid.throw(_("Please set an Address on the Company '%s'" % doc.company), title=_("E-Invoicing Information Missing"))
	else:
	...
	if not company_fiscal_regime:
		vmraid.throw(_("Fiscal Regime is mandatory, kindly set the fiscal regime in the company {0}")
			.format(doc.company))
	else:
	...
	if not doc.company_tax_id and not doc.company_fiscal_code:
		vmraid.throw(_("Please set either the Tax ID or Fiscal Code on Company '%s'" % doc.company), title=_("E-Invoicing Information Missing"))
	...
	if customer.customer_type == "Individual":
		...
		if not doc.customer_fiscal_code:
			vmraid.throw(_("Please set Fiscal Code for the customer '%s'" % doc.customer), title=_("E-Invoicing Information Missing"))
	else:
		if customer.is_public_administration:
			doc.customer_fiscal_code = customer.fiscal_code
			if not doc.customer_fiscal_code:
				vmraid.throw(_("Please set Fiscal Code for the public administration '%s'" % doc.customer), title=_("E-Invoicing Information Missing"))
		else:
			doc.tax_id = customer.tax_id
			if not doc.tax_id:
				vmraid.throw(_("Please set Tax ID for the customer '%s'" % doc.customer), title=_("E-Invoicing Information Missing"))
	...
	if not len(doc.taxes):
		vmraid.throw(_("Please set at least one row in the Taxes and Charges Table"), title=_("E-Invoicing Information Missing"))
	else:
		for row in doc.taxes:
			if row.rate == 0 and row.tax_amount == 0 and not row.tax_exemption_reason:
				vmraid.throw(_("Row {0}: Please set at Tax Exemption Reason in Sales Taxes and Charges").format(row.idx),
					title=_("E-Invoicing Information Missing"))

	for schedule in doc.payment_schedule:
		if schedule.mode_of_payment and not schedule.mode_of_payment_code:
			schedule.mode_of_payment_code = vmraid.get_cached_value('Mode of Payment',
	...
	if not len(doc.payment_schedule):
		...
		for schedule in doc.payment_schedule:
			if not schedule.mode_of_payment:
				vmraid.throw(_("Row {0}: Please set the Mode of Payment in Payment Schedule").format(schedule.idx),
					title=_("E-Invoicing Information Missing"))
			elif not vmraid.db.get_value("Mode of Payment", schedule.mode_of_payment, "mode_of_payment_code"):
				vmraid.throw(_("Row {0}: Please set the correct code on Mode of Payment {1}").format(schedule.idx, schedule.mode_of_payment),
					title=_("E-Invoicing Information Missing"))
...
def prepare_and_attach_invoice(doc, replace=False):
	...
	invoice_xml = vmraid.render_template('erpadda/regional/italy/e-invoice.xml',
...
def validate_address(address_name):
	...
	for field in fields:
		if not data.get(field):
			vmraid.throw(_("Please set {0} for address {1}").format(field.replace('-',''), address_name),
				title=_("E-Invoicing Information Missing"))
