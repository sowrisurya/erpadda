...
from erpadda.regional.italy import fiscal_regimes, tax_exemption_reasons, mode_of_payment_codes, vat_collectability_options
	...
	custom_fields = {
		'Company': [
			...
			dict(fieldname='vat_collectability', label='VAT Collectability',
				...
				options="\n".join(map(lambda x: vmraid.safe_decode(x, encoding='utf-8'), vat_collectability_options))),
		...
		'Mode of Payment': [
			...
			fieldtype='Select', insert_after='included_in_print_rate', print_hide=1,
			options="\n".join(map(lambda x: vmraid.safe_decode(x, encoding='utf-8'), mode_of_payment_codes)))
		],
		'Payment Schedule': [
			dict(fieldname='mode_of_payment_code', label='Code',
