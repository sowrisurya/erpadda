...
import vmraid, re, json
from vmraid import _
import erpadda
from vmraid.utils import cstr, flt, cint, date_diff, nowdate, round_based_on_smallest_currency_fraction, money_in_words, getdate
from erpadda.regional.india import states, state_numbers
...
from erpadda.regional.india import number_state_mapping
...
def validate_gstin_for_india(doc, method):
	...
	if gst_category and gst_category == 'UIN Holders':
		if not GSTIN_UIN_FORMAT.match(doc.gstin):
			vmraid.throw(_("The input you've entered doesn't match the GSTIN format for UIN Holders or Non-Resident OIDAR Service Providers"),
				title=_("Invalid GSTIN"))
	else:
		if not GSTIN_FORMAT.match(doc.gstin):
			vmraid.throw(_("The input you've entered doesn't match the format of GSTIN."), title=_("Invalid GSTIN"))
		...
		if doc.gst_state_number != doc.gstin[:2]:
			vmraid.throw(_("First 2 digits of GSTIN should match with State number {0}.")
...
def validate_tax_category(doc, method):
	if doc.get('gst_state') and vmraid.db.get_value('Tax Category', {'gst_state': doc.gst_state, 'is_inter_state': doc.is_inter_state}):
		if doc.is_inter_state:
			vmraid.throw(_("Inter State tax category for GST State {0} already exists").format(doc.gst_state))
		else:
			vmraid.throw(_("Intra State tax category for GST State {0} already exists").format(doc.gst_state))
...
def validate_gstin_check_digit(gstin, label='GSTIN'):
	...
	if gstin[-1] != code_point_chars[((mod - (total % mod)) % mod)]:
		vmraid.throw(_("""Invalid {0}! The check digit validation has failed. Please ensure you've typed the {0} correctly.""").format(label))

def get_itemised_tax_breakup_header(item_doctype, tax_accounts):
	if vmraid.get_meta(item_doctype).has_field('gst_hsn_code'):
...
def get_itemised_tax_breakup_data(doc, account_wise=False):
	...
	for item, taxes in itemised_tax.items():
		...
		hsn_tax.setdefault(hsn_code, vmraid._dict())
...
def validate_document_name(doc, method=None):
	...
	if len(doc.name) > 16:
		vmraid.throw(_("Maximum length of document number should be 16 characters as per GST rules. Please change the naming series."))

	if not GST_INVOICE_NUMBER_FORMAT.match(doc.name):
		vmraid.throw(_("Document name should only contain alphanumeric values, dash(-) and slash(/) characters as per GST rules. Please change the naming series."))
...
def get_place_of_supply(party_details, doctype):
	if not vmraid.get_meta('Address').has_field('gst_state'): return
	...
	if address_name:
		address = vmraid.db.get_value("Address", address_name, ["gst_state", "gst_state_number", "gstin"], as_dict=1)
...
def get_tax_template_based_on_category(master_doctype, company, party_details):
	...
	default_tax = vmraid.db.get_value(master_doctype, {'company': company, 'tax_category': party_details.get('tax_category')},
		'name')
...
def get_tax_template(master_doctype, company, is_inter_state, state_code):
	tax_categories = vmraid.get_all('Tax Category', fields = ['name', 'is_inter_state', 'gst_state'],
	...
	for tax_category in tax_categories:
		if tax_category.gst_state == number_state_mapping[state_code] or \
			...
			default_tax = vmraid.db.get_value(master_doctype,
				{'company': company, 'disabled': 0, 'tax_category': tax_category.name}, 'name')
	return default_tax

def get_tax_template_for_sez(party_details, master_doctype, company, party_type):

	gst_details = vmraid.db.get_value(party_type, {'name': party_details.get(vmraid.scrub(party_type))},
			['gst_category', 'export_type'], as_dict=1)

	if gst_details:
		if gst_details.gst_category == 'SEZ' and gst_details.export_type == 'With Payment of Tax':
			default_tax = vmraid.db.get_value(master_doctype, {"company": company, "is_inter_state":1, "disabled":0,
...
def calculate_annual_eligible_hra_exemption(doc):
	basic_component, hra_component = vmraid.db.get_value('Company',  doc.company,  ["basic_component", "hra_component"])
	...
	if hra_component and basic_component:
		...
		if assignment:
			hra_component_exists = vmraid.db.exists("Salary Detail", {
			...
			vmraid.throw(_("Salary Structure must be submitted before submission of Tax Ememption Declaration"))
...
def download_ewb_json():
	...
	vmraid.local.response.filename = '{0}_e-WayBill_Data_{1}.json'.format(filename_prefix, vmraid.utils.random_string(5))

@vmraid.whitelist()
def get_gstins_for_company(company):
	company_gstins =[]
	if company:
		company_gstins = vmraid.db.sql("""select
...
def validate_doc(doc):
	...
	for fieldname in reqd_fields:
		if not doc.get(fieldname):
			vmraid.throw(_('{} is required to generate E-Way Bill JSON').format(
...
def get_transport_details(data, doc):
	...
	if doc.mode_of_transport == 'Road':
		if not doc.gst_transporter_id and not doc.vehicle_no:
			vmraid.throw(_('Either GST Transporter ID or Vehicle No is required if Mode of Transport is Road'))
		if doc.vehicle_no:
...
def get_gst_accounts(company, account_wise=False):
	...
	gst_settings_accounts = vmraid.get_all("GST Account",
		filters={"parent": "GST Settings", "company": company},
		fields=["cgst_account", "sgst_account", "igst_account", "cess_account"])

	if not gst_settings_accounts and not vmraid.flags.in_test:
...
def update_totals(gst_tax, base_gst_tax, doc):
	...
	doc.base_in_words = money_in_words(doc.base_grand_total, erpadda.get_company_currency(doc.company))
