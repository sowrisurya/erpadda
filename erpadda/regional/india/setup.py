...
def add_print_formats():
	...
	vmraid.reload_doc("accounts", "print_format", "gst_pos_invoice")
...
def make_fixtures(company=None):
	docs = []
	company = company.name if company else vmraid.db.get_value("Global Defaults", None, "default_company")
...
def set_tax_withholding_category(company):
	...
	for d in docs:
		try:
			...
			doc = vmraid.get_doc("Tax Withholding Category", d.get("name"))
...
def set_tds_account(docs, company):
	abbr = vmraid.get_value("Company", company, "abbr")
	parent_account = vmraid.db.get_value("Account", filters = {"account_name": "Duties and Taxes", "company": company})
	if parent_account:
		docs.extend([
			{
				"doctype": "Account",
