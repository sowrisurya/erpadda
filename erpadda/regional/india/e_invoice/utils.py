...
from vmraid.integrations.utils import make_post_request, make_get_request
from erpadda.regional.india.utils import get_gst_accounts, get_place_of_supply
from vmraid.utils.data import cstr, cint, format_date, flt, time_diff_in_seconds, now_datetime, add_to_date, get_link_to_form, getdate, time_diff_in_hours
...
def validate_eligibility(doc):
	...
	einvoicing_eligible_from = vmraid.db.get_single_value('E Invoice Settings', 'applicable_from') or '2021-04-01'
...
def validate_einvoice_fields(doc):
	...
	if doc.docstatus == 0 and doc._action == 'save':
		...
		vmraid.throw(_('You must cancel IRN before cancelling the document.'), title=_('Cancel Not Allowed'))
...
def get_transaction_details(invoice):
	...
	if not supply_type:
		...
		vmraid.throw(_('GST category should be one of {}, {}, {}, {}').format(rr, sez, overseas, export),
			title=_('Invalid Supply Type'))

	return vmraid._dict(dict(
		tax_scheme='GST',
...
def get_doc_details(invoice):
	if getdate(invoice.posting_date) < getdate('2021-01-01'):
		vmraid.throw(_('IRN generation is not allowed for invoices dated before 1st Jan 2021'), title=_('Not Allowed'))
...
def get_party_details(address_name, is_shipping_address=False):
	...
	party_address_details = vmraid._dict(dict(
...
def get_item_list(invoice):
	...
	for d in invoice.items:
		...
		item.batch_expiry_date = vmraid.db.get_value('Batch', d.batch_no, 'expiry_date') if d.batch_no else None
		item.batch_expiry_date = format_date(item.batch_expiry_date, 'dd/mm/yyyy') if item.batch_expiry_date else None
		item.is_service_item = 'N' if vmraid.db.get_value('Item', d.item_code, 'is_stock_item') else 'Y'
		item.serial_no = ""
...
def get_return_doc_reference(invoice):
	if not invoice.return_against:
		vmraid.throw(_('For generating IRN, reference to the original invoice is mandatory for a credit note. Please set {} field to generate e-invoice.')
			.format(vmraid.bold('Return Against')), title=_('Missing Field'))
...
def get_eway_bill_details(invoice):
	if invoice.is_return:
		vmraid.throw(_('E-Way Bill cannot be generated for Credit Notes & Debit Notes. Please clear fields in the Transporter Section of the invoice.'),
...
def validate_mandatory_fields(invoice):
	...
	if invoice.gst_category != 'Overseas' and not vmraid.db.get_value('Address', invoice.customer_address, 'gstin'):
		vmraid.throw(
...
def validate_totals(einvoice):
	...
	for item in item_list:
		...
		if abs(flt(item['AssAmt']) * flt(item['GstRt']) / 100) - (flt(item['CgstAmt']) + flt(item['SgstAmt']) + flt(item['IgstAmt'])) > 1:
			vmraid.throw(_('Row #{}: GST rate is invalid. Please remove tax rows with zero tax amount from taxes table.').format(item.idx))

	if abs(flt(value_details['AssVal']) - total_item_ass_value) > 1:
		vmraid.throw(_('Total Taxable Value of the items is not equal to the Invoice Net Total. Please check item taxes / discounts for any correction.'))

	if abs(flt(value_details['TotInvVal']) + flt(value_details['Discount']) - flt(value_details['OthChrg']) - total_item_value) > 1:
		vmraid.throw(_('Total Value of the items is not equal to the Invoice Grand Total. Please check item taxes / discounts for any correction.'))
	...
	if abs(flt(value_details['TotInvVal']) - calculated_invoice_value) > 1:
		vmraid.throw(_('Total Item Value + Taxes - Discount is not equal to the Invoice Grand Total. Please check taxes / discounts for any correction.'))
...
def make_einvoice(invoice):
	...
	# not yet implemented
	dispatch_details = period_details = export_details = vmraid._dict({})
...
def log_error(data=None):
	...
	vmraid.log_error(title=_('E Invoice Request Failed'), message=message)
...
def safe_json_load(json_string):
	...
	try:
		...
		vmraid.throw(_("Error in input data. Please check for any special characters near following input: <br> {}").format(snippet))
...
class GSPConnector():
	...
	def set_credentials(self):
		...
		if not self.e_invoice_settings.enable:
			vmraid.throw(_("E-Invoicing is disabled. Please enable it from {} to generate e-invoices.").format(get_link_to_form("E Invoice Settings", "E Invoice Settings")))
		...
		if self.invoice:
			...
			if credentials_for_gstin:
				...
				vmraid.throw(_('Cannot find e-invoicing credentials for selected Company GSTIN. Please check E-Invoice Settings'))
		else:
	...
	def get_seller_gstin(self):
		...
		if not gstin:
			vmraid.throw(_('Cannot retrieve Company GSTIN. Please select company address with valid GSTIN.'))
		return gstin
	...
	def fetch_auth_token(self):
		headers = {
			'gspappid': vmraid.conf.einvoice_client_id,
			'gspappsecret': vmraid.conf.einvoice_client_secret
		}
		res = {}
		try:
	...
	def cancel_irn(self, irn, reason, remark):
		...
		try:
			...
			if time_diff_in_hours(now_datetime(), self.invoice.ack_date) > 24:
				vmraid.throw(_('E-Invoice cannot be cancelled after 24 hours of IRN generation.'), title=_('Not Allowed'), exc=CancellationNotAllowed)
			if not irn:
				vmraid.throw(_('IRN not found. You must generate IRN before cancelling.'), title=_('Not Allowed'), exc=CancellationNotAllowed)
...
def schedule_bulk_generate_irn(docnames):
	...
	vmraid.publish_realtime("bulk_einvoice_generation_complete", {
		"user": vmraid.session.user,
		"failures": failures,
		"invoices": docnames
	})
...
def schedule_bulk_cancel_irn(docnames, reason, remark):
	...
	vmraid.publish_realtime("bulk_einvoice_cancellation_complete", {
		"user": vmraid.session.user,
		"failures": failures,
		"invoices": docnames
	})
