...
def get_header(filters, csv_class):
	...
	datev_settings = vmraid.get_doc('DATEV Settings', {'client': company})
	default_currency = vmraid.get_value('Company', company, 'default_currency')
	...
	header = [
		...
		vmraid.utils.formatdate(filters.get('fiscal_year_start'), 'yyyyMMdd'),
		# N = Length of account numbers (Sachkontenlänge)
		str(filters.get('account_number_length', 4)),
		# O = Transaction batch start date (YYYYMMDD)
		vmraid.utils.formatdate(filters.get('from_date'), 'yyyyMMdd') if csv_class.DATA_CATEGORY == DataCategory.TRANSACTIONS else '',
		# P = Transaction batch end date (YYYYMMDD)
		vmraid.utils.formatdate(filters.get('to_date'), 'yyyyMMdd') if csv_class.DATA_CATEGORY == DataCategory.TRANSACTIONS else '',
...
def zip_and_download(zip_filename, csv_files):
	...
	vmraid.response['filecontent'] = zip_buffer.getvalue()
	vmraid.response['filename'] = zip_filename
