...
from vmraid.utils import flt, formatdate, now_datetime, getdate
from datetime import date
from six import iteritems
from erpadda.regional.doctype.gstr_3b_report.gstr_3b_report import get_period
...
class Gstr1Report(object):
	...
	def get_items_based_on_tax_rate(self):
		...
		if unidentified_gst_accounts:
			vmraid.msgprint(_("Following accounts might be selected in GST Settings:")
				+ "<br>" + "<br>".join(unidentified_gst_accounts), alert=True)

		# Build itemised tax for export invoices where tax table is blank
		for invoice, items in iteritems(self.invoice_items):
			if invoice not in self.items_based_on_tax_rate and invoice not in unidentified_gst_accounts_invoice \
				and vmraid.db.get_value(self.doctype, invoice, "export_type") == "Without Payment of Tax":
	...
	for gst_in in res:
		...
		for number, invoice in iteritems(res[gst_in]):
			if not invoice[0]["place_of_supply"]:
				vmraid.throw(_("""{0} not entered in Invoice {1}.
					Please update and try again""").format(vmraid.bold("Place Of Supply"),
					vmraid.bold(invoice[0]['invoice_number'])))
...
def get_b2cs_json(data, gstin):
	...
	for d in data:
		if not d.get("place_of_supply"):
			vmraid.throw(_("""{0} not entered in some invoices.
				Please update and try again""").format(vmraid.bold("Place Of Supply")))
...
def get_company_gstin_number(company):
	...
	gstin = vmraid.get_all("Address", filters=filters, fields=["gstin"])

	if gstin:
		return gstin[0]["gstin"]
	else:
		vmraid.throw(_("Please set valid GSTIN No. in Company Address for company {0}").format(
			vmraid.bold(company)
		))
...
def download_json_file():
	...
	vmraid.response['filename'] = vmraid.scrub("{0} {1}".format(data['report_name'], data['report_type'])) + '.json'
