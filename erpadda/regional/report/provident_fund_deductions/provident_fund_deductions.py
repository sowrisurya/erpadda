...
def prepare_data(entry,component_type_dict):
	...
	employee_account_dict = vmraid._dict(vmraid.db.sql(""" select name, provident_fund_account from `tabEmployee`"""))
...
def get_data(filters):
	...
	salary_slips = vmraid.db.sql(""" select sal.name from `tabSalary Slip` sal
		where docstatus = 1 %s
		""" % (conditions), as_dict=1)

	component_type_dict = vmraid._dict(vmraid.db.sql(""" select name, component_type from `tabSalary Component`
		where component_type in ('Provident Fund', 'Additional Provident Fund', 'Provident Fund Loan')"""))

	if not len(component_type_dict):
		return []

	entry = vmraid.db.sql(""" select sal.name, sal.employee, sal.employee_name, ded.salary_component, ded.amount
...
@vmraid.whitelist()
def get_years():
	year_list = vmraid.db.sql_list("""select distinct YEAR(end_date) from `tabSalary Slip` ORDER BY YEAR(end_date) DESC""")
	if not year_list:
