...
def set_address_details(row, special_characters):
	...
	if row.get('company_address'):
		address_line1, address_line2, city, pincode, state = vmraid.db.get_value("Address", row.get('company_address'), ['address_line1', 'address_line2', 'city', 'pincode', 'state'])
	...
	if row.get('shipping_address_name'):
		address_line1, address_line2, city, pincode, state = vmraid.db.get_value("Address", row.get('shipping_address_name'), ['address_line1', 'address_line2', 'city', 'pincode', 'state'])
...
def set_taxes(row, filters):
	taxes = vmraid.get_list("Sales Taxes and Charges",
				filters={
					'parent': row.dn_id
				},
	...
	taxes_list = vmraid.get_list("GST Account",
		filters={
			"parent": "GST Settings",
