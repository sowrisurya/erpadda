...
def append_vat_on_sales(data, filters):
	...
	append_data(data, '4', _('Zero Rated'),
		vmraid.format(get_zero_rated_total(filters), 'Currency'), "-")

	append_data(data, '5', _('Exempt Supplies'),
		vmraid.format(get_exempt_total(filters), 'Currency'),"-")
	...
	for no, emirate in enumerate(emirates, 97):
		if emirate in amounts_by_emirate:
			...
			append_data(data, _('1{0}').format(chr(no)),
				...
				vmraid.format(0, 'Currency'), vmraid.format(0, 'Currency'))
...
def get_reverse_charge_total(filters):
	...
	try:
		return vmraid.db.get_all('Purchase Invoice',
...
def get_tourist_tax_return_total(filters):
	...
	query_filters.append(['docstatus', '=', 1])
	try:
		return vmraid.db.get_all('Sales Invoice',
			filters = query_filters,
			fields = ['sum(total)'],
			as_list=True,
			limit = 1
