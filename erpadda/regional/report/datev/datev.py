...
from erpadda.regional.germany.utils.datev.datev_csv import zip_and_download, get_datev_csv
...
def execute(filters=None):
	"""Entry point for vmraid."""
	data = []
	if filters and validate(filters):
		fn = 'temporary_against_account_number'
		filters[fn] = vmraid.get_value('DATEV Settings', filters.get('company'), fn)
...
def validate(filters):
	...
	if not company:
		vmraid.throw(_('<b>Company</b> is a mandatory filter.'))

	from_date = filters.get('from_date')
	if not from_date:
		vmraid.throw(_('<b>From Date</b> is a mandatory filter.'))

	to_date = filters.get('to_date')
	if not to_date:
		vmraid.throw(_('<b>To Date</b> is a mandatory filter.'))

	validate_fiscal_year(from_date, to_date, company)

	if not vmraid.db.exists('DATEV Settings', filters.get('company')):
		msg = 'Please create DATEV Settings for Company {}'.format(filters.get('company'))
		vmraid.log_error(msg, title='DATEV Settings missing')
		return False
...
def validate_fiscal_year(from_date, to_date, company):
	...
	if from_fiscal_year != to_fiscal_year:
		vmraid.throw(_('Dates {} and {} are not in the same fiscal year.').format(from_date, to_date))
...
def download_datev_csv(filters):
	...
	GET /api/method/erpadda.regional.report.datev.datev.download_datev_csv
	...
	coa = vmraid.get_value('Company', company, 'chart_of_accounts')
	filters['skr'] = '04' if 'SKR04' in coa else ('03' if 'SKR03' in coa else '')

	datev_settings = vmraid.get_doc('DATEV Settings', company)
	...
	zip_name = '{} DATEV.zip'.format(vmraid.utils.datetime.date.today())
