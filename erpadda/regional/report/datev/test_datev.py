...
from vmraid.utils import today, now_datetime, cstr
from erpadda.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice

from erpadda.regional.report.datev.datev import validate
from erpadda.regional.report.datev.datev import get_transactions
from erpadda.regional.report.datev.datev import get_customers
from erpadda.regional.report.datev.datev import get_suppliers
from erpadda.regional.report.datev.datev import get_account_names
from erpadda.regional.report.datev.datev import download_datev_csv

from erpadda.regional.germany.utils.datev.datev_csv import get_datev_csv, get_header
from erpadda.regional.germany.utils.datev.datev_constants import Transactions, DebtorsCreditors, AccountNames
...
def setup_fiscal_year():
	...
	if not vmraid.db.get_value("Fiscal Year", {"year": year}, "name"):
		try:
...
def make_customer_with_account(customer_name, company):
	acc_name = vmraid.db.get_value("Account", {
	...
	if not vmraid.db.exists("Customer", customer_name):
		...
		customer = vmraid.get_doc("Customer", customer_name)

	return customer

def make_item(item_code, company):
	warehouse_name = vmraid.db.get_value("Warehouse", {
...
def make_datev_settings(company):
	if not vmraid.db.exists("DATEV Settings", company.name):
		vmraid.get_doc({
...
class TestDatev(TestCase):
	def setUp(self):
		...
		warehouse = vmraid.db.get_value("Item Default", {
				"parent": item.name,
				"company": self.company.name
			}, "default_warehouse")
		...
		tax_account = vmraid.db.get_value("Account", {
				"account_number": "3806",
				"company": self.company.name
			}, "name")
	...
	def test_columns(self):
		def is_subset(get_data, allowed_keys):
			...
			allowed_set = set({vmraid.as_unicode(key) for key in allowed_keys})
	...
	def test_download(self):
		...
		zip_buffer = BytesIO()
		zip_buffer.write(vmraid.response['filecontent'])
