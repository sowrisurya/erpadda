...
from vmraid.utils import get_url, nowdate, date_diff
from vmraid.model.document import Document
from vmraid.contacts.doctype.contact.contact import get_default_contact

class EmailMissing(vmraid.ValidationError): pass

class GSTSettings(Document):
	def onload(self):
		data = vmraid._dict()
		data.total_addresses = vmraid.db.sql('''select count(*) from tabAddress where country = "India"''')
		data.total_addresses_with_gstin = vmraid.db.sql('''select distinct count(*)
...
def send_reminder():
	...
	vmraid.enqueue('erpadda.regional.doctype.gst_settings.gst_settings.send_gstin_reminder_to_all_parties')
	vmraid.msgprint(_('Email Reminders will be sent to all parties with email contacts'))

def send_gstin_reminder_to_all_parties():
	parties = []
	for address_name in vmraid.db.sql('''select name
		from tabAddress where country = "India" and ifnull(gstin, '')='' '''):
