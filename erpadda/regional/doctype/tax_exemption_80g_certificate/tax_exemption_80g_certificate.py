...
class TaxExemption80GCertificate(Document):
		...
		if self.recipient == 'Member':
			if getdate(self.date):
				...
				if not (fiscal_year.year_start_date <= getdate(self.date) \
					...
					vmraid.throw(_('The Certificate Date is not in the Fiscal Year {0}').format(vmraid.bold(self.fiscal_year)))

	def validate_duplicates(self):
		if self.recipient == 'Donor':
			certificate = vmraid.db.exists(self.doctype, {
				'donation': self.donation,
				'name': ('!=', self.name)
			})
			if certificate:
				vmraid.throw(_('An 80G Certificate {0} already exists for the donation {1}').format(
					get_link_to_form(self.doctype, certificate), vmraid.bold(self.donation)
				), title=_('Duplicate Certificate'))

	def validate_company_details(self):
		fields = ['company_80g_number', 'with_effect_from', 'pan_details']
		company_details = vmraid.db.get_value('Company', self.company, fields, as_dict=True)
		if not company_details.company_80g_number:
			vmraid.throw(_('Please set the {0} for company {1}').format(vmraid.bold('80G Number'),
				get_link_to_form('Company', self.company)))

		if not company_details.pan_details:
			vmraid.throw(_('Please set the {0} for company {1}').format(vmraid.bold('PAN Number'),
				get_link_to_form('Company', self.company)))
	...
	def get_payments(self):
		if not self.member:
			vmraid.throw(_('Please select a Member first.'))

		fiscal_year = get_fiscal_year(fiscal_year=self.fiscal_year, as_dict=True)

		memberships = vmraid.db.get_all('Membership', {
			'member': self.member,
		...
		if not memberships:
			vmraid.msgprint(_('No Membership Payments found against the Member {0}').format(self.member))
