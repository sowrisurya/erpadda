...
from vmraid.utils import getdate, get_link_to_form
...
class LowerDeductionCertificate(Document):
	...
	def validate_supplier_against_section_code(self):
		duplicate_certificate = vmraid.db.get_value('Lower Deduction Certificate', {'supplier': self.supplier, 'section_code': self.section_code}, ['name', 'valid_from', 'valid_upto'], as_dict=True)
		if duplicate_certificate and self.are_dates_overlapping(duplicate_certificate):
			...
			vmraid.throw(_("There is already a valid Lower Deduction Certificate {0} for Supplier {1} against Section Code {2} for this time period.")
				.format(certificate_link, vmraid.bold(self.supplier), vmraid.bold(self.section_code)))
