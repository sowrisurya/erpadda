...
class GSTR3BReport(Document):
	...
	def get_inter_state_supplies(self, state_number):
		inter_state_supply_tax = vmraid.db.sql(""" select t.account_head, t.tax_amount_after_discount_amount as tax_amount,
	...
	def get_inward_nil_exempt(self, state):
		inward_nil_exempt = vmraid.db.sql(""" select p.place_of_supply, sum(i.base_amount) as base_amount,
		...
		inward_nil_exempt += vmraid.db.sql("""SELECT sum(base_net_total) as base_amount, gst_category, place_of_supply
	...
	def get_company_gst_details(self):
		...
		gst_details =  vmraid.get_all("Address",
		...
		if gst_details:
			...
			vmraid.throw(_("Please enter GSTIN and state for the Company Address {0}").format(self.company_address))

	def get_account_heads(self):

		account_heads =  vmraid.get_all("GST Account",
		...
		if account_heads:
			...
			vmraid.throw(_("Please set account heads in GST Settings for Compnay {0}").format(self.company))
