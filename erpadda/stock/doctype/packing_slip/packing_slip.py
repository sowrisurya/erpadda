...
class PackingSlip(Document):
	...
	def validate_case_nos(self):
		...
		if not cint(self.from_case_no):
			...
			vmraid.msgprint(_("'To Case No.' cannot be less than 'From Case No.'"),
				raise_exception=1)

		res = vmraid.db.sql("""SELECT name FROM `tabPacking Slip`
		...
		if res:
			vmraid.throw(_("""Case No(s) already in use. Try from Case No {0}""").format(self.get_recommended_case_no()))
	...
	def get_details_for_packing(self):
		...
		res = vmraid.db.sql("""select item_code, sum(qty) as qty,
	...
	def recommend_new_qty(self, item, ps_item_qty, no_of_cases):
		...
		vmraid.throw(_("Quantity for Item {0} must be less than {1}").format(item.get("item_code"), item.get("recommended_qty")))
	...
	def update_item_details(self):
		...
		for d in self.get("items"):
			res = vmraid.db.get_value("Item", d.item_code,
	...
	def get_recommended_case_no(self):
		"""
			...
			note
		"""
		recommended_case_no = vmraid.db.sql("""SELECT MAX(to_case_no) FROM `tabPacking Slip`
