...
from vmraid.utils import flt, floor, nowdate, cint
...
class PutawayRule(Document):
	...
	def validate_duplicate_rule(self):
		existing_rule = vmraid.db.exists("Putaway Rule", {"item_code": self.item_code, "warehouse": self.warehouse})
		if existing_rule and existing_rule != self.name:
			vmraid.throw(_("Putaway Rule already exists for Item {0} in Warehouse {1}.")
	...
	def validate_warehouse_and_company(self):
		...
		if company != self.company:
			vmraid.throw(_("Warehouse {0} does not belong to Company {1}.")
				.format(vmraid.bold(self.warehouse), vmraid.bold(self.company)),
	...
	def validate_capacity(self):
		...
		if flt(self.stock_capacity) < flt(balance_qty):
			vmraid.throw(_("Warehouse Capacity for Item '{0}' must be greater than the existing stock level of {1} {2}.")
...
def get_available_putaway_capacity(rule):
	stock_capacity, item_code, warehouse = vmraid.db.get_value("Putaway Rule", rule,
...
def get_ordered_putaway_rules(item_code, company, source_warehouse=None):
	...
	rules = vmraid.get_all("Putaway Rule",
...
def show_unassigned_items_message(items_not_accomodated):
	...
	for entry in items_not_accomodated:
		...
		formatted_item_rows += """
			<td>{0}</td>
			<td>{1}</td>
		</tr>""".format(item_link, vmraid.bold(entry[1]))
