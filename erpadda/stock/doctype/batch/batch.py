...
from vmraid.utils import flt, cint, get_link_to_form
...
class Batch(Document):
	def autoname(self):
		...
		if not self.batch_id:
			create_new_batch, batch_number_series = vmraid.db.get_value('Item', self.item,
	...
	def before_save(self):
		has_expiry_date, shelf_life_in_days = vmraid.db.get_value('Item', self.item, ['has_expiry_date', 'shelf_life_in_days'])
		if not self.expiry_date and has_expiry_date and shelf_life_in_days:
			self.expiry_date = add_days(self.manufacturing_date, shelf_life_in_days)

		if has_expiry_date and not self.expiry_date:
			vmraid.throw(msg=_("Please set {0} for Batched Item {1}, which is used to set {2} on Submit.") \
				.format(vmraid.bold("Shelf Life in Days"),
...
def get_batch_qty(batch_no=None, warehouse=None, item_code=None, posting_date=None, posting_time=None):
	...
	if batch_no and warehouse:
		...
		out = float(vmraid.db.sql("""select sum(actual_qty)
			from `tabStock Ledger Entry`
			where warehouse=%s and batch_no=%s {0}""".format(cond),
			(warehouse, batch_no))[0][0] or 0)

	if batch_no and not warehouse:
		out = vmraid.db.sql('''select warehouse, sum(actual_qty) as qty
			from `tabStock Ledger Entry`
			where batch_no=%s
			group by warehouse''', batch_no, as_dict=1)

	if not batch_no and item_code and warehouse:
		out = vmraid.db.sql('''select batch_no, sum(actual_qty) as qty
			from `tabStock Ledger Entry`
...
def get_batches_by_oldest(item_code, warehouse):
	...
	batches_dates = [[batch, vmraid.get_value('Batch', batch.batch_no, 'expiry_date')] for batch in batches]
...
def split_batch(batch_no, item_code, warehouse, qty, new_batch_id=None):
	...
	company = vmraid.db.get_value('Stock Ledger Entry', dict(
			item_code=item_code,
			batch_no=batch_no,
			warehouse=warehouse
		), ['company'])

	stock_entry = vmraid.get_doc(dict(
		doctype='Stock Entry',
		purpose='Repack',
		company=company,
		items=[
...
def set_batch_nos(doc, warehouse_field, throw=False):
	...
	for d in doc.items:
		...
		has_batch_no = vmraid.db.get_value('Item', d.item_code, 'has_batch_no')
		...
		if has_batch_no and warehouse and qty > 0:
			if not d.batch_no:
				...
				if flt(batch_qty, d.precision("qty")) < flt(qty, d.precision("qty")):
					vmraid.throw(_("Row #{0}: The batch {1} has only {2} qty. Please select another batch which has {3} qty available or split the row into multiple rows, to deliver/issue from multiple batches").format(d.idx, d.batch_no, batch_qty, qty))
...
def get_batch_no(item_code, warehouse, qty=1, throw=False, serial_no=None):
	...
	if not batch_no:
		vmraid.msgprint(_('Please select a Batch for Item {0}. Unable to find a single batch that fulfills this requirement').format(vmraid.bold(item_code)))
		if throw:
...
def get_batches(item_code, warehouse, qty=1, throw=False, serial_no=None):
	...
	if serial_no and vmraid.get_cached_value('Item', item_code, 'has_batch_no'):
		...
		batch = vmraid.get_all("Serial No",
...
def validate_serial_no_with_batch(serial_nos, item_code):
	if vmraid.get_cached_value("Serial No", serial_nos[0], "item_code") != item_code:
		vmraid.throw(_("The serial no {0} does not belong to item {1}")
			.format(get_link_to_form("Serial No", serial_nos[0]), get_link_to_form("Item", item_code)))

	serial_no_link = ','.join([get_link_to_form("Serial No", sn) for sn in serial_nos])

	message = "Serial Nos" if len(serial_nos) > 1 else "Serial No"
	vmraid.throw(_("There is no batch found against the {0}: {1}")
