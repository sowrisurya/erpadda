...
from vmraid.utils import cint, flt, cstr, today, random_string, add_days
...
from erpadda.stock.doctype.serial_no.serial_no import SerialNoDuplicateError
...
class TestPurchaseReceipt(unittest.TestCase):
	...
	def test_reverse_purchase_receipt_sle(self):
		...
		vmraid.db.set_value('UOM', '_Test UOM', 'must_be_whole_number', 0)

		pr = make_purchase_receipt(qty=0.5)

		sl_entry = vmraid.db.get_all("Stock Ledger Entry", {"voucher_type": "Purchase Receipt",
		...
		sl_entry_cancelled = vmraid.db.get_all("Stock Ledger Entry", {"voucher_type": "Purchase Receipt",
			"voucher_no": pr.name}, ['actual_qty'], order_by='creation')

		self.assertEqual(len(sl_entry_cancelled), 2)
		self.assertEqual(sl_entry_cancelled[1].actual_qty, -0.5)

		vmraid.db.set_value('UOM', '_Test UOM', 'must_be_whole_number', 1)

	def test_make_purchase_invoice(self):
		if not vmraid.db.exists('Payment Terms Template', '_Test Payment Terms Template For Purchase Invoice'):
			vmraid.get_doc({
		...
		template = vmraid.db.get_value('Payment Terms Template', '_Test Payment Terms Template For Purchase Invoice')
		...
		vmraid.get_doc('Payment Terms Template', '_Test Payment Terms Template For Purchase Invoice').delete()
	...
	def test_purchase_receipt_no_gl_entry(self):
		...
		existing_bin_qty, existing_bin_stock_value = vmraid.db.get_value("Bin", {"item_code": "_Test Item",
		...
		stock_value_difference = vmraid.db.get_value("Stock Ledger Entry",
		...
		current_bin_stock_value = vmraid.db.get_value("Bin", {"item_code": "_Test Item",
	...
	def test_duplicate_serial_nos(self):
		...
		serial_nos = vmraid.db.get_value('Stock Ledger Entry',
	...
	def test_subcontracting(self):
		...
		vmraid.db.set_value("Buying Settings", None, "backflush_raw_materials_of_subcontract_based_on", "BOM")
	...
	def test_serial_no_supplier(self):
		...
		self.assertEqual(vmraid.db.get_value("Serial No", pr.get("items")[0].serial_no, "supplier"),
			pr.supplier)

		pr.cancel()
		self.assertFalse(vmraid.db.get_value("Serial No", pr.get("items")[0].serial_no, "warehouse"))
	...
	def test_rejected_serial_no(self):
		...
		for serial_no in accepted_serial_nos:
			self.assertEqual(vmraid.db.get_value("Serial No", serial_no, "warehouse"),
	...
	def test_purchase_return_partial(self):
		...
		# check sle
		outgoing_rate = vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Purchase Receipt",
	...
	def test_purchase_return_for_rejected_qty(self):
		...
		actual_qty = vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Purchase Receipt",
	...
	def test_closed_purchase_receipt(self):
		from erpadda.stock.doctype.purchase_receipt.purchase_receipt import update_purchase_receipt_status
	...
	def test_pr_billing_status(self):
		...
		from erpadda.buying.doctype.purchase_order.purchase_order \
	...
	def test_serial_no_against_purchase_receipt(self):
		...
		self.assertEqual(serial_no, vmraid.db.get_value("Serial No",
	...
	def test_make_purchase_invoice_from_pr_for_returned_qty(self):
		from erpadda.buying.doctype.purchase_order.test_purchase_order import create_purchase_order, create_pr_against_po
	...
	def test_subcontracted_pr_for_multi_transfer_batches(self):
		from erpadda.stock.doctype.stock_entry.test_stock_entry import make_stock_entry
		from erpadda.buying.doctype.purchase_order.purchase_order import make_rm_stock_entry, make_purchase_receipt
		from erpadda.buying.doctype.purchase_order.test_purchase_order import (update_backflush_based_on,
		...
		supplied_qty = vmraid.db.get_value("Purchase Order Item Supplied",
	...
	def test_po_to_pi_and_po_to_pr_worflow_full(self):
		...
		"""
		from erpadda.buying.doctype.purchase_order import test_purchase_order
...
def get_sl_entries(voucher_type, voucher_no):
	return vmraid.db.sql(""" select actual_qty, warehouse, stock_value_difference
		from `tabStock Ledger Entry` where voucher_type=%s and voucher_no=%s
		order by posting_time desc""", (voucher_type, voucher_no), as_dict=1)

def get_gl_entries(voucher_type, voucher_no):
	return vmraid.db.sql("""select account, debit, credit, cost_center, is_cancelled
...
def make_purchase_receipt(**args):
	...
	pr.append("items", {
		...
		"uom": uom,
		"cost_center": args.cost_center or vmraid.get_cached_value('Company',  pr.company,  'cost_center'),
		"asset_location": args.location or "Test Location"
	})

	if args.get_multiple_items:
		pr.items = []
		for item in get_items(warehouse= args.warehouse, cost_center = args.cost_center or vmraid.get_cached_value('Company', pr.company, 'cost_center')):
