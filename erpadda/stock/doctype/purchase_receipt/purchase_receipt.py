...
from vmraid.utils import flt, cint, nowdate
...
class PurchaseReceipt(BuyingController):
	...
	def get_already_received_qty(self, po, po_detail):
		qty = vmraid.db.sql("""select sum(qty) from `tabPurchase Receipt Item`
	...
	def get_po_qty_and_warehouse(self, po_detail):
		po_qty, po_warehouse = vmraid.db.get_value("Purchase Order Item", po_detail,
			["qty", "warehouse"])
	...
	def check_next_docstatus(self):
		submit_rv = vmraid.db.sql("""select t1.name
	...
	def on_cancel(self):
		...
		submitted = vmraid.db.sql("""select t1.name
			from `tabPurchase Invoice` t1,`tabPurchase Invoice Item` t2
			where t1.name = t2.parent and t2.purchase_receipt = %s and t1.docstatus = 1""",
			self.name)
		if submitted:
	...
	def get_current_stock(self):
		for d in self.get('supplied_items'):
			if self.supplier_warehouse:
				bin = vmraid.db.sql("select actual_qty from `tabBin` where item_code = %s and warehouse = %s", (d.rm_item_code, self.supplier_warehouse), as_dict = 1)
	...
	def get_gl_entries(self, warehouse_account=None):
		...
		for d in self.get("items"):
			if d.item_code in stock_items and flt(d.valuation_rate) and flt(d.qty):
				if warehouse_account.get(d.warehouse):
					stock_value_diff = vmraid.db.get_value("Stock Ledger Entry",
					...
					if divisional_loss:
						...
						cost_center = d.cost_center or vmraid.get_cached_value("Company", self.company, "cost_center")
		...
		if negative_expense_to_be_booked and valuation_tax:
			...
			negative_expense_booked_in_pi = vmraid.db.sql("""select name from `tabPurchase Invoice Item` pi
		...
		if warehouse_with_no_account:
			vmraid.msgprint(_("No accounting entries for the following warehouses") + ": \n" +
...
def update_billed_amount_based_on_po(po_detail, update_modified=True):
	...
	billed_against_po = vmraid.db.sql("""select sum(amount) from `tabPurchase Invoice Item`
	...
	pr_details = vmraid.db.sql("""select pr_item.name, pr_item.amount, pr_item.parent
	...
	for pr_item in pr_details:
		...
		billed_amt_agianst_pr = vmraid.db.sql("""select sum(amount) from `tabPurchase Invoice Item`
		...
		vmraid.db.set_value("Purchase Receipt Item", pr_item.name, "billed_amt", billed_amt_agianst_pr, update_modified=update_modified)
...
def update_billing_percentage(pr_doc, update_modified=True):
	...
	for item in pr_doc.items:
		return_data = vmraid.db.get_list("Purchase Receipt",
			fields = [
...
def get_invoiced_qty_map(purchase_receipt):
	...
	for pr_detail, qty in vmraid.db.sql("""select pr_detail, qty from `tabPurchase Invoice Item`
...
def get_returned_qty_map(purchase_receipt):
	...
	returned_qty_map = vmraid._dict(vmraid.db.sql("""select pr_item.purchase_receipt_item, abs(pr_item.qty) as qty
		from `tabPurchase Receipt Item` pr_item, `tabPurchase Receipt` pr
...
def get_item_account_wise_additional_cost(purchase_document):
	landed_cost_vouchers = vmraid.get_all("Landed Cost Purchase Receipt", fields=["parent"],
