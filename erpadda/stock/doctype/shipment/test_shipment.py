...
def get_shipment_customer_address(customer_name):
	...
	customer_address = vmraid.get_all("Address", fields=["name"], filters = {"address_title": address_title})
...
def get_shipment_company():
	...
	abbr = 'SC'
	companies = vmraid.get_all("Company", fields=["name"], filters = {"company_name": company_name})
	if len(companies):
		return companies[0]
	else:
