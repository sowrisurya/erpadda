...
from vmraid.utils import floor, flt, today, cint
...
from erpadda.selling.doctype.sales_order.sales_order import make_delivery_note as create_delivery_note_from_sales_order
	...
	def before_submit(self):
		for item in self.locations:
			...
			if not item.serial_no:
				vmraid.throw(_("Row #{0}: {1} does not have any available serial numbers in {2}").format(
					vmraid.bold(item.idx), vmraid.bold(item.item_code), vmraid.bold(item.warehouse)),
					title=_("Serial Nos Required"))
			if len(item.serial_no.split('\n')) == item.picked_qty:
				continue
			vmraid.throw(_('For item {0} at row {1}, count of serial numbers does not match with the picked quantity')
				.format(vmraid.bold(item.item_code), vmraid.bold(item.idx)), title=_("Quantity Mismatch"))
	...
	def set_item_locations(self, save=False):
		...
		if not self.get('locations') and self.docstatus == 1:
			for location in locations_replica:
				...
				self.append('locations', location)
			vmraid.msgprint(_("Please Restock Items and Update the Pick List to continue. To discontinue, cancel the Pick List."),
...
def get_available_item_locations(item_code, from_warehouses, required_qty, company, ignore_validation=False):
	...
	if remaining_qty > 0 and not ignore_validation:
		vmraid.msgprint(_('{0} units of Item {1} is not available.')
			.format(remaining_qty, vmraid.get_desk_link('Item', item_code)),
...
def get_available_item_locations_for_serialized_item(item_code, from_warehouses, required_qty, company):
	...
	serial_nos = vmraid.get_all('Serial No',
...
def get_available_item_locations_for_serial_and_batched_item(item_code, from_warehouses, required_qty, company):
	...
	for location in locations:
		...
		serial_nos = vmraid.get_list('Serial No',
			fields=['name'],
			filters=filters,
			limit=location.qty,
			order_by='purchase_date')
...
def get_available_item_locations_for_other_item(item_code, from_warehouses, required_qty, company):
	...
	item_locations = vmraid.get_all('Bin',
...
def create_delivery_note(source_name, target_doc=None):
	...
	for location in pick_list.locations:
		if location.sales_order_item:
			sales_order_item = vmraid.get_cached_doc('Sales Order Item', {'name':location.sales_order_item})
		else:
...
def target_document_exists(pick_list_name, purpose):
	if purpose == 'Delivery':
		return vmraid.db.exists('Delivery Note', {
...
def stock_entry_exists(pick_list_name):
	return vmraid.db.exists('Stock Entry', {
		'pick_list': pick_list_name
	})
...
def update_stock_entry_based_on_material_request(pick_list, stock_entry):
	for location in pick_list.locations:
		...
		if location.material_request_item:
			target_warehouse = vmraid.get_value('Material Request Item',
