...
from vmraid.utils import cstr, cint, flt, comma_or, getdate, nowdate, formatdate, format_time
from erpadda.stock.utils import get_incoming_rate
from erpadda.stock.stock_ledger import get_previous_sle, NegativeStockError, get_valuation_rate
from erpadda.stock.get_item_details import get_bin_details, get_default_cost_center, get_conversion_factor, get_reserved_qty_for_so
...
from erpadda.stock.doctype.batch.batch import get_batch_no, set_batch_nos, get_batch_qty
from erpadda.stock.doctype.item.item import get_item_defaults
from erpadda.manufacturing.doctype.bom.bom import validate_bom_no, add_additional_cost
from erpadda.stock.utils import get_bin
from vmraid.model.mapper import get_mapped_doc
from erpadda.stock.doctype.serial_no.serial_no import update_serial_nos_after_submit, get_serial_nos
from erpadda.stock.doctype.stock_reconciliation.stock_reconciliation import OpeningEntryAccountError
...
class IncorrectValuationRateError(vmraid.ValidationError): pass
class DuplicateEntryForWorkOrderError(vmraid.ValidationError): pass
class OperationsNotCompleteError(vmraid.ValidationError): pass
class MaxSampleAlreadyRetainedError(vmraid.ValidationError): pass
...
class StockEntry(StockController):
	...
	def set_job_card_data(self):
		if self.job_card and not self.work_order:
			data = vmraid.db.get_value('Job Card',
	...
	def validate_purpose(self):
		...
		if self.job_card and self.purpose not in ['Material Transfer for Manufacture', 'Repack']:
			vmraid.throw(_("For job card {0}, you can only make the 'Material Transfer for Manufacture' type stock entry")
				.format(self.job_card))

	def delete_linked_stock_entry(self):
		if self.purpose == "Send to Warehouse":
			for d in vmraid.get_all("Stock Entry", filters={"docstatus": 0,
	...
	def set_transfer_qty(self):
		for item in self.get("items"):
			if not flt(item.qty):
				vmraid.throw(_("Row {0}: Qty is mandatory").format(item.idx))
	...
	def update_cost_in_project(self):
		if (self.work_order and not vmraid.db.get_value("Work Order",
			self.work_order, "update_consumed_material_cost_in_project")):
			return

		if self.project:
			amount = vmraid.db.sql(""" select ifnull(sum(sed.amount), 0)
				from
			...
			additional_costs = vmraid.db.sql(""" select ifnull(sum(sed.base_amount), 0)
				from
	...
	def validate_item(self):
		...
		for item in self.get("items"):
			if flt(item.qty) and flt(item.qty) < 0:
				vmraid.throw(_("Row {0}: The item {1}, quantity must be positive number")
					.format(item.idx, vmraid.bold(item.item_code)))

			if item.item_code not in stock_items:
				vmraid.throw(_("{0} is not a stock Item").format(item.item_code))

			item_details = self.get_item_details(vmraid._dict(
			...
			if (self.purpose in ("Material Transfer", "Material Transfer for Manufacture")
				...
				vmraid.throw(_("Row #{0}: Please specify Serial No for Item {1}").format(item.idx, item.item_code),
			...
			if not vmraid.get_value('Work Order', self.work_order, 'skip_transfer'):
				...
				for item in self.items:
					if cstr(item.t_warehouse) == '':
						req_items = vmraid.get_all('Work Order Item',
	...
	def validate_fg_completed_qty(self):
		...
		for item_code, qty_list in iteritems(item_wise_qty):
			...
			if self.fg_completed_qty != total:
				vmraid.throw(_("The finished product {0} quantity {1} and For Quantity {2} cannot be different")
					.format(vmraid.bold(item_code), vmraid.bold(total), vmraid.bold(self.fg_completed_qty)))
	...
	def validate_difference_account(self):
		...
		for d in self.get("items"):
			if not d.expense_account:
				vmraid.throw(_("Please enter <b>Difference Account</b> or set default <b>Stock Adjustment Account</b> for company {0}")
					.format(vmraid.bold(self.company)))

			elif self.is_opening == "Yes" and vmraid.db.get_value("Account", d.expense_account, "report_type") == "Profit and Loss":
				vmraid.throw(_("Difference Account must be a Asset/Liability type account, since this Stock Entry is an Opening Entry"), OpeningEntryAccountError)
	...
	def check_if_operations_completed(self):
		...
		allowance_percentage = flt(vmraid.db.get_single_value("Manufacturing Settings",
		...
		for d in prod_order.get("operations"):
			...
			if total_completed_qty > flt(completed_qty):
				...
				if not job_card:
					vmraid.throw(_("Work Order {0}: Job Card not found for the operation {1}")
				...
				vmraid.throw(_("Row #{0}: Operation {1} is not completed for {2} qty of finished goods in Work Order {3}. Please update operation status via Job Card {4}.")
					.format(d.idx, vmraid.bold(d.operation), vmraid.bold(total_completed_qty), work_order_link, job_card_link), OperationsNotCompleteError)

	def check_duplicate_entry_for_work_order(self):
		other_ste = [t[0] for t in vmraid.db.get_values("Stock Entry",  {
		...
		if other_ste:
			production_item, qty = vmraid.db.get_value("Work Order",
				self.work_order, ["production_item", "qty"])
			args = other_ste + [production_item]
			fg_qty_already_entered = vmraid.db.sql("""select sum(transfer_qty)
	...
	def set_actual_qty(self):
		allow_negative_stock = cint(vmraid.db.get_value("Stock Settings", None, "allow_negative_stock"))
		...
		for d in self.get('items'):
			...
			if d.docstatus==1 and d.s_warehouse and not allow_negative_stock and flt(d.actual_qty, d.precision("actual_qty")) < flt(d.transfer_qty, d.precision("actual_qty")):
				vmraid.throw(_("Row {0}: Quantity not available for {4} in warehouse {1} at posting time of the entry ({2} {3})").format(d.idx,
					vmraid.bold(d.s_warehouse), formatdate(self.posting_date),
					format_time(self.posting_time), vmraid.bold(d.item_code))
					+ '<br><br>' + _("Available quantity is {0}, you need {1}").format(vmraid.bold(d.actual_qty),
						vmraid.bold(d.transfer_qty)),
					NegativeStockError, title=_('Insufficient Stock'))

	def set_serial_nos(self, work_order):
		previous_se = vmraid.db.get_value("Stock Entry", {"work_order": work_order,
				"purpose": "Material Transfer for Manufacture"}, "name")

		for d in self.get('items'):
			transferred_serial_no = vmraid.db.get_value("Stock Entry Detail",{"parent": previous_se,
		...
		for d in self.get('items'):
			...
			if not d.basic_rate and not d.allow_zero_valuation_rate:
				d.basic_rate = get_valuation_rate(d.item_code, d.t_warehouse,
					...
					currency=erpadda.get_company_currency(self.company), company=self.company,
	...
	def get_basic_rate_for_manufactured_item(self, finished_item_qty, outgoing_items_cost=0):
		...
		if vmraid.db.get_single_value("Manufacturing Settings", "material_consumption"):
	...
	def set_stock_entry_type(self):
		if self.purpose:
			self.stock_entry_type = vmraid.get_cached_value('Stock Entry Type',
				{'purpose': self.purpose}, 'name')

	def set_purpose_for_stock_entry(self):
		if self.stock_entry_type and not self.purpose:
			self.purpose = vmraid.get_cached_value('Stock Entry Type',
		...
		for warehouse in ['s_warehouse', 't_warehouse']:
			...
			for row in self.items:
				...
				for sn in get_serial_nos(row.serial_no):
					if sn in serial_nos:
						vmraid.throw(_('The serial no {0} has added multiple times in the stock entry {1}')
							.format(vmraid.bold(sn), self.name))
	...
	def validate_purchase_order(self):
		...
		backflush_raw_materials_based_on = vmraid.db.get_single_value("Buying Settings",
			"backflush_raw_materials_of_subcontract_based_on")

		qty_allowance = flt(vmraid.db.get_single_value("Buying Settings",
		...
		if (backflush_raw_materials_based_on == 'BOM'):
			...
			for se_item in self.items:
				...
				if not required_qty:
					bom_no = vmraid.db.get_value("Purchase Order Item",
						{"parent": self.purchase_order, "item_code": se_item.subcontracted_item},
						"bom")

					if se_item.allow_alternative_item:
						original_item_code = vmraid.get_value("Item Alternative", {"alternative_item_code": item_code}, "item_code")
				...
				if not required_qty:
					vmraid.throw(_("Item {0} not found in 'Raw Materials Supplied' table in Purchase Order {1}")
						.format(se_item.item_code, self.purchase_order))
				total_supplied = vmraid.db.sql("""select sum(transfer_qty)
				...
				if flt(total_supplied, precision) > flt(total_allowed, precision):
					vmraid.throw(_("Row {0}# Item {1} cannot be transferred more than {2} against Purchase Order {3}")
						.format(se_item.idx, se_item.item_code, total_allowed, self.purchase_order))
		elif backflush_raw_materials_based_on == "Material Transferred for Subcontract":
			for row in self.items:
				if not row.subcontracted_item:
					vmraid.throw(_("Row {0}: Subcontracted Item is mandatory for the raw material {1}")
	...
	def validate_bom(self):
		for d in self.get('items'):
			if d.bom_no and (d.t_warehouse != getattr(self, "pro_doc", vmraid._dict()).scrap_warehouse):
	...
	def validate_finished_goods(self):
		...
		production_item, wo_qty = vmraid.db.get_value("Work Order",
			self.work_order, ["production_item", "qty"])
		...
		for d in self.get('items'):
			if d.is_finished_item:
				if d.item_code != production_item:
					vmraid.throw(_("Finished Item {0} does not match with Work Order {1}")
						.format(d.item_code, self.work_order))
				elif flt(d.transfer_qty) > flt(self.fg_completed_qty):
					vmraid.throw(_("Quantity in row {0} ({1}) must be same as manufactured quantity {2}"). \
						format(d.idx, d.transfer_qty, self.fg_completed_qty))
				finished_items.append(d.item_code)

		if len(set(finished_items)) > 1:
			vmraid.throw(_("Multiple items cannot be marked as finished item"))
		...
		if self.purpose == "Manufacture":
			...
			if self.fg_completed_qty > allowed_qty:
				vmraid.throw(_("For quantity {0} should not be greater than work order quantity {1}")
	...
	def update_work_order(self):
		def _validate_work_order(pro_doc):
			...
			if pro_doc.status == 'Stopped':
				vmraid.throw(_("Transaction not allowed against stopped Work Order {0}").format(self.work_order))
	...
	def get_item_details(self, args=None, for_update=False):
		item = vmraid.db.sql("""select i.name, i.stock_uom, i.description, i.image, i.item_name, i.item_group,
		...
		if not item:
			vmraid.throw(_("Item {0} is not active or end of life has been reached").format(args.get("item_code")))
		...
		if self.purpose == 'Material Issue':
			ret["expense_account"] = (item.get("expense_account") or
				...
				vmraid.get_cached_value('Company',  self.company,  "default_expense_account"))
		...
		if self.purpose == "Send to Subcontractor" and self.get("purchase_order") and args.get('item_code'):
			subcontract_items = vmraid.get_all("Purchase Order Item Supplied",
	...
	def set_items_for_stock_in(self):
		...
		if self.outgoing_stock_entry and self.purpose == 'Material Transfer':
			...
			if doc.per_transferred == 100:
				vmraid.throw(_("Goods are already received against the outward entry {0}")
					.format(doc.name))
	...
	def get_items(self):
		...
		self.set_work_order_details()
		self.flags.backflush_based_on = vmraid.db.get_single_value("Manufacturing Settings",
			"backflush_raw_materials_based_on")

		if self.bom_no:

			backflush_based_on = vmraid.db.get_single_value("Manufacturing Settings",
				"backflush_raw_materials_based_on")
			...
			if self.purpose in ["Material Issue", "Material Transfer", "Manufacture", "Repack",
				...
				if self.work_order and self.purpose == "Material Transfer for Manufacture":
					...
					if self.purchase_order and self.purpose == "Send to Subcontractor":
						#Get PO Supplied Items Details
						item_wh = vmraid._dict(vmraid.db.sql("""
	...
	def get_bom_raw_materials(self, qty):
		...
		for item in itervalues(item_dict):
			...
			if item["allow_alternative_item"]:
				item["allow_alternative_item"] = vmraid.db.get_value('Work Order',
	...
	def get_unconsumed_raw_materials(self):
		...
		wo_items = vmraid.get_all('Work Order Item',
	...
	def get_transfered_raw_materials(self):
		...
		po_qty = vmraid.db.sql("""select qty, produced_qty, material_transferred_for_manufacturing from
			...
			if trans_qty and manufacturing_qty > (produced_qty + flt(self.fg_completed_qty)):
				...
				for d in backflushed_materials.get(item.item_code):
					if d.get(item.warehouse):
						...
						if consumed_qty and vmraid.db.get_single_value("Manufacturing Settings",
	...
	def get_pending_raw_materials(self, backflush_based_on=None):
		...
		overproduction_percentage = flt(vmraid.db.get_single_value("Manufacturing Settings",
			"overproduction_percentage_for_work_order"))
	...
	def get_pro_order_required_items(self, backflush_based_on=None):
		...
		pro_order = vmraid.get_doc("Work Order", self.work_order)
		if not vmraid.db.get_value("Warehouse", pro_order.wip_warehouse, "is_group"):
	...
	def add_to_stock_entry_detail(self, item_dict, bom_no=None):
		for d in item_dict:
			stock_uom = item_dict[d].get("stock_uom") or vmraid.db.get_value("Item", d, "stock_uom")
	...
	def validate_with_material_request(self):
		for item in self.get("items"):
			...
			if self.purpose == 'Material Transfer' and self.outgoing_stock_entry:
				parent_se = vmraid.get_value("Stock Entry Detail", item.ste_detail, ['material_request','material_request_item'],as_dict=True)
				if parent_se:
					material_request = parent_se.material_request
					material_request_item = parent_se.material_request_item

			if material_request:
				mreq_item = vmraid.db.get_value("Material Request Item",
					{"name": material_request_item, "parent": material_request},
					["item_code", "warehouse", "idx"], as_dict=True)
				if mreq_item.item_code != item.item_code:
					vmraid.throw(_("Item for row {0} does not match Material Request").format(item.idx),
					...
					if disabled == 0:
						...
						if expiry_date:
							if getdate(self.posting_date) > getdate(expiry_date):
								vmraid.throw(_("Batch {0} of Item {1} has expired.")
									.format(item.batch_no, item.item_code))
					else:
						vmraid.throw(_("Batch {0} of Item {1} is disabled.")
							.format(item.batch_no, item.item_code))
	...
	def update_purchase_order_supplied_items(self):
		...
		vmraid.db.sql("""UPDATE `tabPurchase Order Item Supplied` pos
			SET
	...
	def update_so_in_serial_number(self):
		so_name, item_code = vmraid.db.get_value("Work Order", self.work_order, ["sales_order", "production_item"])
		if so_name and item_code:
			qty_to_reserve = get_reserved_qty_for_so(so_name, item_code)
			if qty_to_reserve:
				reserved_qty = vmraid.db.sql("""select count(name) from `tabSerial No` where item_code=%s and
	...
	def update_transferred_qty(self):
		if self.purpose == 'Material Transfer' and self.outgoing_stock_entry:
			...
			for d in self.items:
				...
				transferred_qty = vmraid.get_all("Stock Entry Detail", fields = ["sum(qty) as qty"],
			...
			for data, transferred_qty in stock_entries.items():
				cond += """ WHEN (parent = %s and name = %s) THEN %s
					""" %(vmraid.db.escape(data[0]), vmraid.db.escape(data[1]), transferred_qty)

			if cond and stock_entries_child_list:
				vmraid.db.sql(""" UPDATE `tabStock Entry Detail`
					SET
		...
		if self.inspection_required:
			...
			for d in self.items:
				if d.quality_inspection:
					vmraid.db.set_value("Quality Inspection", d.quality_inspection, {
...
def make_stock_in_entry(source_name, target_doc=None):
	...
	def update_item(source_doc, target_doc, source_parent):
		...
		if source_doc.material_request_item and source_doc.material_request :
			...
			if add_to_transit:
				warehouse = vmraid.get_value('Material Request Item', source_doc.material_request_item, 'warehouse')
...
def get_used_alternative_items(purchase_order=None, work_order=None):
	...
	data = vmraid.db.sql(""" select sted.original_item, sted.uom, sted.conversion_factor,
...
def get_uom_details(item_code, uom, qty):
	...
	if not conversion_factor:
		vmraid.msgprint(_("UOM coversion factor required for UOM: {0} in Item: {1}")
...
def get_expired_batch_items():
	return vmraid.db.sql("""select b.item, sum(sle.actual_qty) as qty, sle.batch_no, sle.warehouse, sle.stock_uom\
...
def validate_sample_quantity(item_code, sample_quantity, qty, batch_no = None):
	if cint(qty) < cint(sample_quantity):
		vmraid.throw(_("Sample quantity {0} cannot be more than received quantity {1}").format(sample_quantity, qty))
	retention_warehouse = vmraid.db.get_single_value('Stock Settings', 'sample_retention_warehouse')
	retainted_qty = 0
	if batch_no:
	...
	if retainted_qty >= max_retain_qty:
		vmraid.msgprint(_("Maximum Samples - {0} have already been retained for Batch {1} and Item {2} in Batch {3}.").
	...
	if cint(sample_quantity) > cint(qty_diff):
		vmraid.msgprint(_("Maximum Samples - {0} can be retained for Batch {1} and Item {2}.").
