...
from erpadda.stock.doctype.stock_reconciliation.test_stock_reconciliation import create_stock_reconciliation
from erpadda.stock.doctype.item.test_item import set_item_variant_settings, make_item_variant, create_item
...
from erpadda.stock.doctype.stock_entry.stock_entry import move_sample_to_retention_warehouse, make_stock_in_entry
...
def get_sle(**args):
	...
	return vmraid.db.sql("""select * from `tabStock Ledger Entry` %s
...
class TestStockEntry(unittest.TestCase):
	...
	def _test_auto_material_request(self, item_code, material_request_type="Purchase", warehouse="_Test Warehouse - _TC"):
		...
		projected_qty, actual_qty = vmraid.db.get_value("Bin", {"item_code": item_code,
		...
		projected_qty = vmraid.db.get_value("Bin", {"item_code": item_code,
	...
	def test_material_receipt_gl_entry(self):
		...
		self.assertTrue(vmraid.db.sql("""select * from `tabStock Ledger Entry`
			where voucher_type='Stock Entry' and voucher_no=%s""", mr.name))

		self.assertTrue(vmraid.db.sql("""select * from `tabGL Entry`
			where voucher_type='Stock Entry' and voucher_no=%s""", mr.name))
	...
	def test_material_issue_gl_entry(self):
		...
		stock_value_diff = abs(vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Stock Entry",
	...
	def test_material_transfer_gl_entry(self):
		...
		if source_warehouse_account == target_warehouse_account:
			...
			self.assertFalse(vmraid.db.sql("""select * from `tabGL Entry`
	...
	def test_repack_with_additional_costs(self):
		...
		expenses_included_in_valuation = vmraid.get_value("Company", company, "expenses_included_in_valuation")
		...
		rm_stock_value_diff = abs(vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Stock Entry",
			"voucher_no": repack.name, "item_code": "_Test Item"}, "stock_value_difference"))

		fg_stock_value_diff = abs(vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Stock Entry",
	...
	def check_stock_ledger_entries(self, voucher_type, voucher_no, expected_sle):
		...
		sle = vmraid.db.sql("""select item_code, warehouse, actual_qty
	...
	def test_serial_move(self):
		...
		se.insert()
		se.submit()
		self.assertTrue(vmraid.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse 1 - _TC")

		se.cancel()
		self.assertTrue(vmraid.db.get_value("Serial No", serial_no, "warehouse"), "_Test Warehouse - _TC")
	...
	def test_warehouse_company_validation(self):
		...
		vmraid.get_doc("User", "test2@example.com")\
	...
	def test_work_order(self):
		from erpadda.manufacturing.doctype.work_order.work_order \
			import make_stock_entry as _make_stock_entry
		bom_no, bom_operation_cost = vmraid.db.get_value("BOM", {"item": "_Test FG Item 2",
	...
	def test_variant_work_order(self):
		bom_no = vmraid.db.get_value("BOM", {"item": "_Test Variant Item",
	...
	def test_retain_sample(self):
		...
		vmraid.db.set_value("Stock Settings", None, "sample_retention_warehouse", "Test Warehouse for Sample Retention - _TC")
	...
	# 	from erpadda.manufacturing.doctype.work_order.work_order \
	# 		import make_stock_entry as _make_stock_entry
	# 	bom_no = vmraid.db.get_value("BOM", {"item": "_Test FG Item 2",
	# 		"is_default": 1, "docstatus": 1})
	...
	# 	stock_entry = vmraid.get_doc(_make_stock_entry(work_order.name, "Material Consumption for Manufacture", 2))
	# 	for d in stock_entry.get('items'):
	...
	def test_gle_for_opening_stock_entry(self):
		...
		is_opening = vmraid.db.get_value("GL Entry",
	...
	def test_total_basic_amount_zero(self):
		se = vmraid.get_doc({"doctype":"Stock Entry",
