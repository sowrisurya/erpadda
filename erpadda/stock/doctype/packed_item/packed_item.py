...
def get_product_bundle_items(item_code):
	return vmraid.db.sql("""select t1.item_code, t1.qty, t1.uom, t1.description
...
def get_bin_qty(item, warehouse):
	det = vmraid.db.sql("""select actual_qty, projected_qty from `tabBin`
		where item_code = %s and warehouse = %s""", (item, warehouse), as_dict = 1)
	return det and det[0] or vmraid._dict()
...
def make_packing_list(doc):
	...
	for d in doc.get("items"):
		if vmraid.db.get_value("Product Bundle", {"new_item_code": d.item_code}):
