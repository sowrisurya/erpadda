...
class LandedCostVoucher(Document):
	...
	def get_items_from_purchase_receipts(self):
		...
		for pr in self.get("purchase_receipts"):
			if pr.receipt_document_type and pr.receipt_document:
				pr_items = vmraid.db.sql("""select pr_item.item_code, pr_item.description,
				...
				for d in pr_items:
					...
					item.cost_center = d.cost_center or \
						erpadda.get_default_cost_center(self.company)
	...
	def validate_purchase_receipts(self):
		...
		for d in self.get("purchase_receipts"):
			if vmraid.db.get_value(d.receipt_document_type, d.receipt_document, "docstatus") != 1:
				vmraid.throw(_("Receipt document must be submitted"))
			else:
		...
		for item in self.get("items"):
			if not item.receipt_document:
				...
				vmraid.throw(_("Item Row {0}: {1} {2} does not exist in above '{1}' table")
					.format(item.idx, item.receipt_document_type, item.receipt_document))

			if not item.cost_center:
				vmraid.throw(_("Row {0}: Cost center is required for an item {1}")
	...
	def validate_applicable_charges_for_item(self):
		...
		if not total:
			vmraid.throw(_("Total {0} for all items is zero, may be you should change 'Distribute Charges Based On'").format(based_on))

		total_applicable_charges = sum([flt(d.applicable_charges) for d in self.get("items")])

		precision = get_field_precision(vmraid.get_meta("Landed Cost Item").get_field("applicable_charges"),
		...
		if abs(diff) < (2.0 / (10**precision)):
			...
			vmraid.throw(_("Total Applicable Charges in Purchase Receipt Items table must be same as Total Taxes and Charges"))
	...
	def validate_asset_qty_and_status(self, receipt_document_type, receipt_document):
		for item in self.get('items'):
			if item.is_fixed_asset:
				...
				docs = vmraid.db.get_all('Asset', filters={ receipt_document_type: item.receipt_document,
					'item_code': item.item_code }, fields=['name', 'docstatus'])
				if not docs or len(docs) != item.qty:
					vmraid.throw(_('There are not enough asset created or linked to {0}. Please create or link {1} Assets with respective document.').format(
						item.receipt_document, item.qty))
				if docs:
					for d in docs:
						if d.docstatus == 1:
							vmraid.throw(_('{2} <b>{0}</b> has submitted Assets. Remove Item <b>{1}</b> from table to continue.').format(
	...
	def update_rate_in_serial_no_for_non_asset_items(self, receipt_document):
		for item in receipt_document.get("items"):
			if not item.is_fixed_asset and item.serial_no:
				...
				if serial_nos:
					vmraid.db.sql("update `tabSerial No` set purchase_rate=%s where name in ({0})"
