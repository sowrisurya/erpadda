...
class TestLandedCostVoucher(unittest.TestCase):
	def test_landed_cost_voucher(self):
		...
		last_sle = vmraid.db.get_value("Stock Ledger Entry", {
		...
		pr_lc_value = vmraid.db.get_value("Purchase Receipt Item", {"parent": pr.name}, "landed_cost_voucher_amount")
		self.assertEqual(pr_lc_value, 25.0)

		last_sle_after_landed_cost = vmraid.db.get_value("Stock Ledger Entry", {
				"voucher_type": pr.doctype,
				"voucher_no": pr.name,
				"item_code": "_Test Item",
				"warehouse": "Stores - TCP1"
	...
	def test_landed_cost_voucher_against_purchase_invoice(self):
		...
		pi = make_purchase_invoice(update_stock=1, posting_date=vmraid.utils.nowdate(),
			posting_time=vmraid.utils.nowtime(), cash_bank_account="Cash - TCP1",
		...
		pi_lc_value = vmraid.db.get_value("Purchase Invoice Item", {"parent": pi.name},
	...
	def test_landed_cost_voucher_for_serialized_item(self):
		vmraid.db.sql("delete from `tabSerial No` where name in ('SN001', 'SN002', 'SN003', 'SN004', 'SN005')")
		...
		serial_no = vmraid.db.get_value("Serial No", "SN001",
	...
	def test_multi_currency_lcv(self):
		...
		gl_entries = vmraid.get_all("GL Entry", fields=["account", "credit", "credit_in_account_currency"],
