...
class DeliveryNote(SellingController):
	...
	def set_actual_qty(self):
		for d in self.get('items'):
			if d.item_code and d.warehouse:
				actual_qty = vmraid.db.sql("""select actual_qty from `tabBin`
	...
	def validate_with_previous_doc(self):
		...
		if cint(vmraid.db.get_single_value('Selling Settings', 'maintain_same_sales_rate')) \
	...
	def update_current_stock(self):
		if self.get("_action") and self._action != "update_after_submit":
			for d in self.get('items'):
				d.actual_qty = vmraid.db.get_value("Bin", {"item_code": d.item_code,
					"warehouse": d.warehouse}, "actual_qty")

			for d in self.get('packed_items'):
				bin_qty = vmraid.db.get_value("Bin", {"item_code": d.item_code,
	...
	def check_credit_limit(self):
		...
		bypass_credit_limit_check_at_sales_order = cint(vmraid.db.get_value("Customer Credit Limit",
	...
	def validate_packed_qty(self):
		...
		for d in self.get("items"):
			if flt(d.get('qty')) != flt(d.get('packed_qty')):
				vmraid.msgprint(_("Packed quantity must equal quantity for Item {0} in row {1}").format(d.item_code, d.idx))
				has_error = True
		if has_error:
	...
	def check_next_docstatus(self):
		...
		submit_in = vmraid.db.sql("""select t1.name
	...
	def cancel_packing_slips(self):
		...
		"""
		res = vmraid.db.sql("""SELECT name FROM `tabPacking Slip` WHERE delivery_note = %s
	...
	def make_return_invoice(self):
		try:
			...
			vmraid.throw(_("Could not create Credit Note automatically, please uncheck 'Issue Credit Note' and submit again"))

def update_billed_amount_based_on_so(so_detail, update_modified=True):
	# Billed against Sales Order directly
	billed_against_so = vmraid.db.sql("""select sum(amount) from `tabSales Invoice Item`
		where so_detail=%s and (dn_detail is null or dn_detail = '') and docstatus=1""", so_detail)
	billed_against_so = billed_against_so and billed_against_so[0][0] or 0

	# Get all Delivery Note Item rows against the Sales Order Item row
	dn_details = vmraid.db.sql("""select dn_item.name, dn_item.amount, dn_item.si_detail, dn_item.parent
	...
	for dnd in dn_details:
		...
		if dnd.si_detail:
			...
			billed_amt_agianst_dn = vmraid.db.sql("""select sum(amount) from `tabSales Invoice Item`
		...
		vmraid.db.set_value("Delivery Note Item", dnd.name, "billed_amt", billed_amt_agianst_dn, update_modified=update_modified)
...
def get_invoiced_qty_map(delivery_note):
	...
	for dn_detail, qty in vmraid.db.sql("""select dn_detail, qty from `tabSales Invoice Item`
...
def get_returned_qty_map(delivery_note):
	...
	returned_qty_map = vmraid._dict(vmraid.db.sql("""select dn_item.dn_detail, abs(dn_item.qty) as qty
		from `tabDelivery Note Item` dn_item, `tabDelivery Note` dn
...
def make_shipment(source_name, target_doc=None):
	def postprocess(source, target):
		user = vmraid.db.get_value("User", vmraid.session.user, ['email', 'full_name', 'phone', 'mobile_no'], as_dict=1)
		...
		contact = vmraid.db.get_value("Contact", source.contact_person, ['email_id', 'phone', 'mobile_no'], as_dict=1)
...
def make_inter_company_transaction(doctype, source_name, target_doc=None):
	from erpadda.accounts.doctype.sales_invoice.sales_invoice import (validate_inter_company_transaction,
