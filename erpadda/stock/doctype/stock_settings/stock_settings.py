...
class StockSettings(Document):
	def validate(self):
		...
		if submitted_stock_frozen > stock_frozen_limit:
			...
			vmraid.msgprint (_("`Freeze Stocks Older Than` should be smaller than %d days.") %stock_frozen_limit)

		# show/hide barcode field
		for name in ["barcode", "barcodes", "scan_barcode"]:
			vmraid.make_property_setter({'fieldname': name, 'property': 'hidden',
	...
	def validate_warehouses(self):
		...
		for field in warehouse_fields:
			if vmraid.db.get_value("Warehouse", self.get(field), "is_group"):
				vmraid.throw(_("Group Warehouses cannot be used in transactions. Please change the value of {0}") \
					.format(vmraid.bold(self.meta.get_field(field).label)), title =_("Incorrect Warehouse"))
	...
	def cant_change_valuation_method(self):
		...
		if db_valuation_method and db_valuation_method != self.valuation_method:
			...
			sle = vmraid.db.sql("""select name from `tabStock Ledger Entry` sle
				where exists(select name from tabItem
					where name=sle.item_code and (valuation_method is null or valuation_method='')) limit 1
			""")

			if sle:
				vmraid.throw(_("Can't change the valuation method, as there are transactions against some items which do not have its own valuation method"))
	...
	def validate_clean_description_html(self):
		if int(self.clean_description_html or 0) \
			...
			# changed to text
			vmraid.enqueue('erpadda.stock.doctype.stock_settings.stock_settings.clean_all_descriptions', now=vmraid.flags.in_test)
