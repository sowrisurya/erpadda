...
from erpadda.stock.utils import get_stock_balance, get_incoming_rate, get_available_serial_nos
from erpadda.stock.doctype.batch.batch import get_batch_qty

class OpeningEntryAccountError(vmraid.ValidationError): pass
class EmptyStockReconciliationItemsError(vmraid.ValidationError): pass
...
class StockReconciliation(StockController):
	...
	def validate(self):
		if not self.expense_account:
			self.expense_account = vmraid.get_cached_value('Company',  self.company,  "stock_adjustment_account")
	...
	def remove_items_with_no_change(self):
		...
		if not items:
			vmraid.throw(_("None of the items have any change in quantity or value."),
		...
		for row_num, row in enumerate(self.items):
			...
			if row.qty and row.valuation_rate in ["", None]:
				...
				if not row.valuation_rate:
					...
					buying_rate = vmraid.db.get_value("Item Price", {"item_code": row.item_code,
	...
	def validate_item(self, item_code, row):
		...
		try:
			...
			if item.has_serial_no and not row.serial_no and not item.serial_no_series:
				raise vmraid.ValidationError(_("Serial no(s) required for serialized item {0}").format(item_code))
		...
		for row in self.items:
			...
			if item.has_serial_no or item.has_batch_no:
				...
				if row.serial_no or row.batch_no:
					vmraid.throw(_("Row #{0}: Item {1} is not a Serialized/Batched Item. It cannot have a Serial No/Batch No against it.") \
				...
				if row.qty and not row.valuation_rate and not row.allow_zero_valuation_rate:
					vmraid.throw(_("Valuation Rate required for Item {0} at row {1}").format(row.item_code, row.idx))
	...
	def make_sle_on_cancel(self):
		...
		if sl_entries:
			...
			sl_entries.reverse()
			allow_negative_stock = vmraid.db.get_value("Stock Settings", None, "allow_negative_stock")
			self.make_sl_entries(sl_entries, allow_negative_stock=allow_negative_stock)
	...
	def validate_expense_account(self):
		...
		elif self.purpose == "Opening Stock" or not vmraid.db.sql("""select name from `tabStock Ledger Entry` limit 1"""):
			if vmraid.db.get_value("Account", self.expense_account, "report_type") == "Profit and Loss":
				vmraid.throw(_("Difference Account must be a Asset/Liability type account, since this Stock Reconciliation is an Opening Entry"), OpeningEntryAccountError)
...
def get_items(warehouse, posting_date, posting_time, company):
	...
	for d in set(items):
		...
		if vmraid.db.get_value("Item", d[0], "disabled") == 0:
			res.append({
				"item_code": d[0],
				"warehouse": d[2],
				"qty": stock_bal[0],
...
def get_stock_balance_for(item_code, warehouse,
	...
	item_dict = vmraid.db.get_value("Item", item_code,
...
def get_difference_account(purpose, company):
	if purpose == 'Stock Reconciliation':
		...
		account = vmraid.db.get_value('Account', {'is_group': 0,
