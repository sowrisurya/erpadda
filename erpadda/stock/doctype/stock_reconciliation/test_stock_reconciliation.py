...
from erpadda.stock.doctype.stock_reconciliation.stock_reconciliation import EmptyStockReconciliationItemsError, get_items
	...
	def _test_reco_sle_gle(self, valuation_method):
		...
		for d in input_data:
			...
			if qty_after_transaction == last_sle.get("qty_after_transaction") \
				...
				# no gl entries
				self.assertTrue(vmraid.db.get_value("Stock Ledger Entry",
...
def set_valuation_method(item_code, valuation_method):
	...
	for warehouse in vmraid.get_all("Warehouse", filters={"company": "_Test Company"}, fields=["name", "is_group"]):
