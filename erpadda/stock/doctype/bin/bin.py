...
class Bin(Document):
	...
	def update_stock(self, args, allow_negative_stock=False, via_landed_cost_voucher=False):
		'''Called from erpadda.stock.utils.update_bin'''
		self.update_qty(args)

		if args.get("actual_qty") or args.get("voucher_type") == "Stock Reconciliation":
			from erpadda.stock.stock_ledger import update_entries_after, update_qty_in_future_sle
	...
	def update_reserved_qty_for_production(self):
		'''Update qty reserved for production from Production Item tables
			in open work orders'''
		self.reserved_qty_for_production = vmraid.db.sql('''
			SELECT
	...
	def update_reserved_qty_for_sub_contracting(self):
		#reserved qty
		reserved_qty_for_sub_contract = vmraid.db.sql('''
		...
		materials_transferred = vmraid.db.sql("""
			select
...
def on_doctype_update():
	vmraid.db.add_index("Bin", ["item_code", "warehouse"])
