...
from erpadda.stock.doctype.quality_inspection_template.quality_inspection_template \
...
class QualityInspection(Document):
	...
	def get_item_specification_details(self):
		if not self.quality_inspection_template:
			self.quality_inspection_template = vmraid.db.get_value('Item',
	...
	def set_status_based_on_acceptance_formula(self, reading):
		if not reading.acceptance_formula:
			vmraid.throw(_("Row #{0}: Acceptance Criteria Formula is required.").format(reading.idx),
		...
		try:
			...
			vmraid.throw(_("Row #{0}: {1} is not a valid reading field. Please refer to the field description.")
				.format(reading.idx, field),
				title=_("Invalid Formula"))
		except Exception:
			vmraid.throw(_("Row #{0}: Acceptance Criteria Formula is incorrect.").format(reading.idx),
				title=_("Invalid Formula"))
...
def quality_inspection_query(doctype, txt, searchfield, start, page_len, filters):
	return vmraid.get_all('Quality Inspection',
		limit_start=start,
