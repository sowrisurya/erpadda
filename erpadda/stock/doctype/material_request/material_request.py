...
from vmraid.utils import cstr, flt, getdate, new_line_sep, nowdate, add_days, get_link_to_form
...
class MaterialRequest(BuyingController):
	...
	def validate_qty_against_so(self):
		...
		for so_no in so_items.keys():
			for item in so_items[so_no].keys():
				already_indented = vmraid.db.sql("""select sum(qty)
					from `tabMaterial Request Item`
					where item_code = %s and sales_order = %s and
					docstatus = 1 and parent != %s""", (item, so_no, self.name))
				already_indented = already_indented and flt(already_indented[0][0]) or 0

				actual_so_qty = vmraid.db.sql("""select sum(stock_qty) from `tabSales Order Item`
					where parent = %s and item_code = %s and docstatus = 1""", (so_no, item))
				actual_so_qty = actual_so_qty and flt(actual_so_qty[0][0]) or 0

				if actual_so_qty and (flt(so_items[so_no][item]) + already_indented > actual_so_qty):
					vmraid.throw(_("Material Request of maximum {0} can be made for Item {1} against Sales Order {2}").format(actual_so_qty - already_indented, item, so_no))
	...
	def check_modified_date(self):
		mod_db = vmraid.db.sql("""select modified from `tabMaterial Request` where name = %s""",
			self.name)
	...
	def update_requested_qty(self, mr_item_rows=None):
		...
		for d in self.get("items"):
			if (not mr_item_rows or d.name in mr_item_rows) and [d.item_code, d.warehouse] not in item_wh_list \
					and vmraid.db.get_value("Item", d.item_code, "is_stock_item") == 1 and d.warehouse:
	...
	def update_requested_qty_in_production_plan(self):
		...
		for d in self.get('items'):
			if d.production_plan and d.material_request_plan_item:
				...
				vmraid.db.set_value('Material Request Plan Item',
		...
		for mr, mr_item_rows in material_request_map.items():
			if mr and mr_item_rows:
				...
				if mr_obj.status in ["Stopped", "Cancelled"]:
					vmraid.throw(_("{0} {1} is cancelled or stopped").format(_("Material Request"), mr),
						vmraid.InvalidStatusError)
...
def get_items_based_on_default_supplier(supplier):
	supplier_items = [d.parent for d in vmraid.db.get_all("Item Default",
...
def get_material_requests_based_on_supplier(doctype, txt, searchfield, start, page_len, filters):
	...
	material_requests = vmraid.db.sql("""select distinct mr.name, transaction_date,company
...
def get_default_supplier_query(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.db.sql("""select default_supplier
...
def raise_work_orders(material_request):
	...
	errors =[]
	work_orders = []
	default_wip_warehouse = vmraid.db.get_single_value("Manufacturing Settings", "default_wip_warehouse")
	...
	if work_orders:
		...
		if len(work_orders) > 1:
			msgprint(_("The following {0} were created: {1}")
				.format(vmraid.bold(_("Work Orders")), '<br>' + ', '.join(work_orders_list)))
		else:
			msgprint(_("The {0} {1} created sucessfully")
				.format(vmraid.bold(_("Work Order")), work_orders_list[0]))

	if errors:
		vmraid.throw(_("Work Order cannot be created for following reason: <br> {0}")
