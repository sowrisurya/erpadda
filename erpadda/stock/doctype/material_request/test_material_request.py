...
import vmraid, unittest, erpadda
from vmraid.utils import flt, today
from erpadda.stock.doctype.material_request.material_request \
...
class TestMaterialRequest(unittest.TestCase):
	def test_make_purchase_order(self):
		...
		self.assertRaises(vmraid.ValidationError, make_purchase_order,
			mr.name)
	...
	def _get_requested_qty(self, item_code, warehouse):
		return flt(vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse}, "indented_qty"))

	def test_make_stock_entry_for_material_issue(self):
		mr = vmraid.copy_doc(test_records[0]).insert()

		self.assertRaises(vmraid.ValidationError, make_stock_entry,
			mr.name)
	...
	def test_completed_qty_for_issue(self):
		def _get_requested_qty():
			return flt(vmraid.db.get_value("Bin", {"item_code": "_Test Item Home Desktop 100",
	...
	def test_material_request_type_manufacture(self):
		...
		requested_qty = vmraid.db.sql("""select indented_qty from `tabBin` where \
		...
		new_requested_qty = vmraid.db.sql("""select indented_qty from `tabBin` where \
			item_code= %s and warehouse= %s """, (mr.items[0].item_code, mr.items[0].warehouse))[0][0]
	...
	def test_multi_uom_for_purchase(self):
		...
		if not vmraid.db.get_value('UOM Conversion Detail',
