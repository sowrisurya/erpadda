...
class DeliveryTrip(Document):
	def __init__(self, *args, **kwargs):
		...
		self.default_distance_uom = vmraid.db.get_single_value("Global Defaults", "default_distance_unit") or "Meter"
		self.uom_conversion_factor = vmraid.db.get_value("UOM Conversion Factor",
...
def notify_customers(delivery_trip):
	...
	for stop in delivery_trip.delivery_stops:
		contact_info = vmraid.db.get_value("Contact", stop.contact, ["first_name", "last_name", "email_id"], as_dict=1)

		context.update({"items": []})
		if stop.delivery_note:
			items = vmraid.get_all("Delivery Note Item", filters={"parent": stop.delivery_note, "docstatus": 1}, fields=["*"])
		...
		if contact_info and contact_info.email_id:
			...
			vmraid.sendmail(recipients=contact_info.email_id,
...
def get_attachments(delivery_stop):
	if not (vmraid.db.get_single_value("Delivery Settings", "send_with_attachment") and delivery_stop.delivery_note):
		return []

	dispatch_attachment = vmraid.db.get_single_value("Delivery Settings", "dispatch_attachment")
	attachments = vmraid.attach_print("Delivery Note", delivery_stop.delivery_note,
