...
from vmraid.utils import cint, cstr, flt, add_days, nowdate, getdate, get_link_to_form
from erpadda.stock.get_item_details import get_reserved_qty_for_so

from vmraid import _, ValidationError
...
class SerialNo(StockController):
	...
	def validate(self):
		if self.get("__islocal") and self.warehouse and not self.via_stock_ledger:
			vmraid.throw(_("New Serial No cannot have Warehouse. Warehouse must be set by Stock Entry or Purchase Receipt"), SerialNoCannotCreateDirectError)
	...
	def validate_warehouse(self):
		if not self.get("__islocal"):
			item_code, warehouse = vmraid.db.get_value("Serial No",
				self.name, ["item_code", "warehouse"])
			if not self.via_stock_ledger and item_code != self.item_code:
				vmraid.throw(_("Item Code cannot be changed for Serial No."),
					SerialNoCannotCannotChangeError)
			if not self.via_stock_ledger and warehouse != self.warehouse:
				vmraid.throw(_("Warehouse cannot be changed for Serial No."),
					SerialNoCannotCannotChangeError)
	...
	def validate_item(self):
		...
		if item.has_serial_no!=1:
			vmraid.throw(_("Item {0} is not setup for Serial Nos. Check Item master").format(self.item_code))
		...
		if purchase_sle:
			...
			if purchase_sle.voucher_type in ("Purchase Receipt", "Purchase Invoice"):
				self.supplier, self.supplier_name = \
					vmraid.db.get_value(purchase_sle.voucher_type, purchase_sle.voucher_no,
		...
		if delivery_sle:
			...
			if delivery_sle.voucher_type  in ("Delivery Note", "Sales Invoice"):
				self.customer, self.customer_name = \
					vmraid.db.get_value(delivery_sle.voucher_type, delivery_sle.voucher_no,
	...
	def on_trash(self):
		sl_entries = vmraid.db.sql("""select serial_no from `tabStock Ledger Entry`
		...
		if sle_exists:
			vmraid.throw(_("Cannot delete Serial No {0}, as it is used in stock transactions").format(self.name))

	def before_rename(self, old, new, merge=False):
		if merge:
			vmraid.throw(_("Sorry, Serial Nos cannot be merged"))

	def after_rename(self, old, new, merge=False):
		"""rename serial_no text fields"""
		for dt in vmraid.db.sql("""select parent from tabDocField
			where fieldname='serial_no' and fieldtype in ('Text', 'Small Text', 'Long Text')"""):

			for item in vmraid.db.sql("""select name, serial_no from `tab%s`
				where serial_no like %s""" % (dt[0], vmraid.db.escape('%' + old + '%'))):

				serial_nos = map(lambda i: new if i.upper()==old.upper() else i, item[1].split('\n'))
				vmraid.db.sql("""update `tab%s` set serial_no = %s
...
def validate_serial_no(sle, item_det):
	...
	if item_det.has_serial_no==0:
		if serial_nos:
			vmraid.throw(_("Item {0} is not setup for Serial Nos. Column must be blank").format(sle.item_code),
				SerialNoNotRequiredError)
	elif not sle.is_cancelled:
		if serial_nos:
			if cint(sle.actual_qty) != flt(sle.actual_qty):
				vmraid.throw(_("Serial No {0} quantity {1} cannot be a fraction").format(sle.item_code, sle.actual_qty))

			if len(serial_nos) and len(serial_nos) != abs(cint(sle.actual_qty)):
				vmraid.throw(_("{0} Serial Numbers required for Item {1}. You have provided {2}.").format(abs(sle.actual_qty), sle.item_code, len(serial_nos)),
					SerialNoQtyError)

			if len(serial_nos) != len(set(serial_nos)):
				vmraid.throw(_("Duplicate Serial No entered for Item {0}").format(sle.item_code), SerialNoDuplicateError)

			for serial_no in serial_nos:
				if vmraid.db.exists("Serial No", serial_no):
					sr = vmraid.db.get_value("Serial No", serial_no, ["name", "item_code", "batch_no", "sales_order",
					...
					if sr.item_code!=sle.item_code:
						if not allow_serial_nos_with_different_item(serial_no, sle):
							vmraid.throw(_("Serial No {0} does not belong to Item {1}").format(serial_no,
					...
					if cint(sle.actual_qty) > 0 and has_serial_no_exists(sr, sle):
						...
						vmraid.throw(_("Serial No {0} has already been received in the {1} #{2}")
							.format(vmraid.bold(serial_no), sr.purchase_document_type, doc_name), SerialNoDuplicateError)
					...
					if cint(sle.actual_qty) < 0:
						if sr.warehouse!=sle.warehouse:
							vmraid.throw(_("Serial No {0} does not belong to Warehouse {1}").format(serial_no,
						...
						if sle.voucher_type in ("Delivery Note", "Sales Invoice"):
							...
							if sr.batch_no and sr.batch_no != sle.batch_no:
								vmraid.throw(_("Serial No {0} does not belong to Batch {1}").format(serial_no,
									sle.batch_no), SerialNoBatchError)

							if not sle.is_cancelled and not sr.warehouse:
								vmraid.throw(_("Serial No {0} does not belong to any Warehouse")
							...
							if sr.sales_order:
								if sle.voucher_type == "Sales Invoice":
									if not vmraid.db.exists("Sales Invoice Item", {"parent": sle.voucher_no,
									...
									if not vmraid.db.exists("Delivery Note Item", {"parent": sle.voucher_no,
										"item_code": sle.item_code, "against_sales_order": sr.sales_order}):
										invoice = vmraid.db.get_value("Delivery Note Item", {"parent": sle.voucher_no,
											"item_code": sle.item_code}, "against_sales_invoice")
										if not invoice or vmraid.db.exists("Sales Invoice Item",
							...
							if sle.voucher_type == "Sales Invoice":
								sales_order = vmraid.db.get_value("Sales Invoice Item", {"parent": sle.voucher_no,
								...
								sales_order = vmraid.get_value("Delivery Note Item", {"parent": sle.voucher_no,
									"item_code": sle.item_code}, "against_sales_order")
								if sales_order and get_reserved_qty_for_so(sales_order, sle.item_code):
									validate_so_serial_no(sr, sales_order)
								else:
									sales_invoice = vmraid.get_value("Delivery Note Item", {"parent": sle.voucher_no,
										"item_code": sle.item_code}, "against_sales_invoice")
									if sales_invoice:
										sales_order = vmraid.db.get_value("Sales Invoice Item", {
			...
			vmraid.throw(_("Serial Nos Required for Serialized Item {0}").format(sle.item_code),
...
def check_serial_no_validity_on_cancel(serial_no, sle):
	sr = vmraid.db.get_value("Serial No", serial_no, ["name", "warehouse", "company", "status"], as_dict=1)
...
def validate_material_transfer_entry(sle_doc):
	...
	if (sle_doc.voucher_type == "Stock Entry" and not sle_doc.is_cancelled and
		vmraid.get_cached_value("Stock Entry", sle_doc.voucher_no, "purpose") == "Material Transfer"):
...
def validate_so_serial_no(sr, sales_order):
	if not sr.sales_order or sr.sales_order!= sales_order:
		...
		vmraid.throw(_("""{0} Serial No {1} cannot be delivered""")
			.format(msg, sr.name))
...
def update_serial_nos(sle, item_det):
	...
	if not sle.is_cancelled and not sle.serial_no and cint(sle.actual_qty) > 0 \
		...
		vmraid.db.set(sle, "serial_no", serial_nos)
		validate_serial_no(sle, item_det)
	if sle.serial_no:
...
def get_item_details(item_code):
	return vmraid.db.sql("""select name, has_batch_no, docstatus,
...
def update_serial_nos_after_submit(controller, parentfield):
	stock_ledger_entries = vmraid.db.sql("""select voucher_detail_no, serial_no, actual_qty, warehouse
...
def update_maintenance_status():
	serial_nos = vmraid.db.sql('''select name from `tabSerial No` where (amc_expiry_date<%s or
...
def get_delivery_note_serial_no(item_code, qty, delivery_note):
	serial_nos = ''
	dn_serial_nos = vmraid.db.sql_list(""" select name from `tabSerial No`
...
def get_pos_reserved_serial_nos(filters):
	...
	pos_transacted_sr_nos = vmraid.db.sql("""select item.serial_no as serial_no
...
def fetch_serial_numbers(filters, qty, do_not_include=[]):
	...
	excluded_sr_nos = ", ".join(["" + vmraid.db.escape(sr) + "" for sr in do_not_include]) or "''"
	serial_numbers = vmraid.db.sql("""
