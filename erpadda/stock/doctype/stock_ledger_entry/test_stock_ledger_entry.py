...
from erpadda.stock.doctype.landed_cost_voucher.test_landed_cost_voucher import create_landed_cost_voucher
...
class TestStockLedgerEntry(unittest.TestCase):
	def setUp(self):
		...
		vmraid.db.sql("delete from `tabStock Ledger Entry` where item_code in (%s)" % (', '.join(['%s']*len(items))), items)
		vmraid.db.sql("delete from `tabBin` where item_code in (%s)" % (', '.join(['%s']*len(items))), items)
	...
	def test_item_cost_reposting(self):
		...
		create_stock_reconciliation(
			...
			rate=100,
			company=company,
			expense_account = "Stock Adjustment - _TC" if vmraid.get_all("Stock Ledger Entry") else "Temporary Opening - _TC",
	...
	def test_purchase_return_valuation_reposting(self):
		...
		# check sle
		outgoing_rate, stock_value_difference = vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Purchase Receipt",
	...
	def test_sales_return_valuation_reposting(self):
		...
		outgoing_rate = abs(vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Delivery Note",
	...
	def test_sub_contracted_item_costing(self):
		...
		# check outgoing_rate for DN after reposting
		incoming_rate = vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Purchase Receipt",
	...
	def test_back_dated_entry_not_allowed(self):
		...
		vmraid.db.set_value("Stock Settings", None,
		...
		try:
			...
			vmraid.db.set_value("Stock Settings", None, "role_allowed_to_create_edit_back_dated_transactions", None)
			vmraid.set_user("Administrator")
			user.remove_roles("Stock Manager")
