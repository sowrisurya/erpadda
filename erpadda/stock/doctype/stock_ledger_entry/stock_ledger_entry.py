...
from vmraid.utils import flt, getdate, add_days, formatdate, get_datetime, date_diff
...
class StockFreezeError(vmraid.ValidationError): pass
class BackDatedStockTransaction(vmraid.ValidationError): pass
...
class StockLedgerEntry(Document):
	...
	def calculate_batch_qty(self):
		if self.batch_no:
			batch_qty = vmraid.db.get_value("Stock Ledger Entry",
	...
	def actual_amt_check(self):
		if self.batch_no and not self.get("allow_negative_stock"):
			batch_bal_after_transaction = flt(vmraid.db.sql("""select sum(actual_qty)
				from `tabStock Ledger Entry`
				where warehouse=%s and item_code=%s and batch_no=%s""",
				(self.warehouse, self.item_code, self.batch_no))[0][0])

			if batch_bal_after_transaction < 0:
				vmraid.throw(_("Stock balance in Batch {0} will become negative {1} for Item {2} at Warehouse {3}")
	...
	def validate_item(self):
		item_det = vmraid.db.sql("""select name, item_name, has_batch_no, docstatus,
		...
		if self.voucher_type != 'Stock Reconciliation':
			if item_det.has_batch_no == 1:
				...
				if not self.batch_no:
					...
					vmraid.throw(_("{0} is not a valid Batch Number for Item {1}").format(self.batch_no, batch_item))
		...
		if item_det.has_variants:
			vmraid.throw(_("Stock cannot exist for Item {0} since has variants").format(self.item_code),
				ItemTemplateCannotHaveStock)

		self.stock_uom = item_det.stock_uom

	def check_stock_frozen_date(self):
		stock_frozen_upto = vmraid.db.get_value('Stock Settings', None, 'stock_frozen_upto') or ''
		if stock_frozen_upto:
			stock_auth_role = vmraid.db.get_value('Stock Settings', None,'stock_auth_role')
			if getdate(self.posting_date) <= getdate(stock_frozen_upto) and not stock_auth_role in vmraid.get_roles():
				vmraid.throw(_("Stock transactions before {0} are frozen").format(formatdate(stock_frozen_upto)), StockFreezeError)

		stock_frozen_upto_days = int(vmraid.db.get_value('Stock Settings', None, 'stock_frozen_upto_days') or 0)
		if stock_frozen_upto_days:
			stock_auth_role = vmraid.db.get_value('Stock Settings', None,'stock_auth_role')
			older_than_x_days_ago = (add_days(getdate(self.posting_date), stock_frozen_upto_days) <= date.today())
			if older_than_x_days_ago and not stock_auth_role in vmraid.get_roles():
				vmraid.throw(_("Not allowed to update stock transactions older than {0}").format(stock_frozen_upto_days), StockFreezeError)
	...
	def validate_with_last_transaction_posting_time(self):
		authorized_role = vmraid.db.get_single_value("Stock Settings", "role_allowed_to_create_edit_back_dated_transactions")
		if authorized_role:
			...
			if authorized_users and vmraid.session.user not in authorized_users:
				...
				if last_transaction_time and get_datetime(cur_doc_posting_datetime) < get_datetime(last_transaction_time):
					msg = _("Last Stock Transaction for item {0} under warehouse {1} was on {2}.").format(vmraid.bold(self.item_code),
						vmraid.bold(self.warehouse), vmraid.bold(last_transaction_time))
...
def on_doctype_update():
	if not vmraid.db.has_index('tabStock Ledger Entry', 'posting_sort_index'):
		vmraid.db.commit()
		vmraid.db.add_index("Stock Ledger Entry",
