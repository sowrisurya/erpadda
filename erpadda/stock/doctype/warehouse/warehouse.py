...
from vmraid.utils import cint, nowdate
...
class Warehouse(NestedSet):
	...
	def onload(self):
		...
		if self.company and cint(vmraid.db.get_value("Company", self.company, "enable_perpetual_inventory")):
	...
	def on_trash(self):
		# delete bin
		bins = vmraid.db.sql("select * from `tabBin` where warehouse = %s",
			self.name, as_dict=1)
		for d in bins:
			if d['actual_qty'] or d['reserved_qty'] or d['ordered_qty'] or \
				...
				vmraid.db.sql("delete from `tabBin` where name = %s", d['name'])
	...
	def check_if_sle_exists(self):
		return vmraid.db.sql("""select name from `tabStock Ledger Entry`
			where warehouse = %s limit 1""", self.name)

	def check_if_child_exists(self):
		return vmraid.db.sql("""select name from `tabWarehouse`
	...
	def before_rename(self, old_name, new_name, merge=False):
		...
		new_warehouse = erpadda.encode_company_abbr(new_name, self.company)

		if merge:
			if not vmraid.db.exists("Warehouse", new_warehouse):
				vmraid.throw(_("Warehouse {0} does not exist").format(new_warehouse))

			if self.company != vmraid.db.get_value("Warehouse", new_warehouse, "company"):
				vmraid.throw(_("Both Warehouse must belong to same Company"))
	...
	def recalculate_bin_qty(self, new_name):
		...
		repost_stock_for_items = vmraid.db.sql_list("""select distinct item_code
			from tabBin where warehouse=%s""", new_name)

		# Delete all existing bins to avoid duplicate bins for the same item and warehouse
		vmraid.db.sql("delete from `tabBin` where warehouse=%s", new_name)
...
def get_children(doctype, parent=None, company=None, is_root=False):
	...
	for wh in warehouses:
		...
		if company:
			wh["company_currency"] = vmraid.db.get_value('Company', company, 'default_currency')
	return warehouses
...
def convert_to_group_or_ledger():
	...
	return vmraid.get_doc("Warehouse", args.docname).convert_to_group_or_ledger()

def get_child_warehouses(warehouse):
	lft, rgt = vmraid.get_cached_value("Warehouse", warehouse, ["lft", "rgt"])

	return vmraid.db.sql_list("""select name from `tabWarehouse`
		where lft >= %s and rgt <= %s""", (lft, rgt))

def get_warehouses_based_on_account(account, company=None):
	warehouses = []
	for d in vmraid.get_all("Warehouse", fields = ["name", "is_group"],
	...
	if (not warehouses and company and
		vmraid.get_cached_value("Company", company, "default_inventory_account") == account):
		warehouses = [d.name for d in vmraid.get_all("Warehouse", filters={'is_group': 0})]

	if not warehouses:
		vmraid.throw(_("Warehouse not found against the account {0}")
			.format(account))
