...
def get_consumed_details(filters):
	...
	for d in vmraid.db.sql("""select sle.item_code, i.item_name, i.description,
...
def get_suppliers_details(filters):
	...
	for d in vmraid.db.sql("""select pr.supplier, pri.item_code from
...
def get_material_transfer_vouchers():
	return vmraid.db.sql_list("""select name from `tabStock Entry` where
