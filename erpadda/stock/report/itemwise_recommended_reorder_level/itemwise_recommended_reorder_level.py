...
def get_item_info(filters):
	...
	return vmraid.db.sql("""select name, item_name, description, brand, item_group,
...
def get_delivered_items(condition):
	dn_items = vmraid.db.sql("""select dn_item.item_code, sum(dn_item.stock_qty) as dn_qty
		from `tabDelivery Note` dn, `tabDelivery Note Item` dn_item
		where dn.name = dn_item.parent and dn.docstatus = 1 %s
		group by dn_item.item_code""" % (condition), as_dict=1)

	si_items = vmraid.db.sql("""select si_item.item_code, sum(si_item.stock_qty) as si_qty
