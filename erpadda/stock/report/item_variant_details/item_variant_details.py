...
def get_data(item):
	...
	variant_results = vmraid.db.sql("""select name from `tabItem`
		where variant_of = %s""", item, as_dict=1)
	if not variant_results:
		vmraid.msgprint(_("There isn't any item variant for the selected item"))
		return []
	else:
		variants = ", ".join([vmraid.db.escape(variant['name']) for variant in variant_results])
	...
	attribute_list = [d[0] for d in vmraid.db.sql("""select attribute
...
def get_open_sales_orders_map(variants):
	open_sales_orders = vmraid.db.sql("""
		select
			count(*) as count,
			item_code
		from
