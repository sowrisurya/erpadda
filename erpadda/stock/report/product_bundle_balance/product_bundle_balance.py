	...
	for parent_item in parent_items:
		...
		for warehouse, company in iteritems(warehouse_company_map):
			...
			for child_item_detail in required_items:
				child_item_balance = stock_balance.get(child_item_detail.item_code, vmraid._dict()).get(warehouse, vmraid._dict())
				child_row = {
					"indent": 1,
...
def get_stock_ledger_entries(filters, items):
	...
	item_conditions_sql = ' and sle.item_code in ({})' \
		.format(', '.join([vmraid.db.escape(i) for i in items]))
...
def get_sle_conditions(filters):
	...
	if filters.get("warehouse"):
		warehouse_details = vmraid.db.get_value("Warehouse", filters.get("warehouse"), ["lft", "rgt"], as_dict=1)
