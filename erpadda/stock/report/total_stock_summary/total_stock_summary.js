	...
	"filters": [
		{
			...
			"change": function() {
				let group_by = vmraid.query_report.get_filter_value("group_by")
				let company_filter = vmraid.query_report.get_filter("company")
				if (group_by == "Company") {
					company_filter.df.reqd = 0;
					company_filter.df.hidden = 1;
					vmraid.query_report.set_filter_value("company", "");
