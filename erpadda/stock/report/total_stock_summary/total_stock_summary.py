...
def get_total_stock(filters):
	...
	if filters.get("group_by") == "Warehouse":
		if filters.get("company"):
			conditions += " AND warehouse.company = %s" % vmraid.db.escape(filters.get("company"), percent=False)
...
def validate_filters(filters):
	if filters.get("group_by") == 'Company' and \
		...
		vmraid.throw(_("Please set Company filter blank if Group By is 'Company'"))
