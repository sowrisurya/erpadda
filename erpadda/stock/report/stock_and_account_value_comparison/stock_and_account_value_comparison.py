...
from erpadda.accounts.utils import get_stock_accounts
...
def execute(filters=None):
	if not erpadda.is_perpetual_inventory_enabled(filters.company):
		vmraid.throw(_("Perpetual inventory required for the company {0} to view this report.")
...
def get_stock_ledger_data(report_filters, filters):
	...
	return vmraid.get_all("Stock Ledger Entry", filters=filters,
...
def get_gl_data(report_filters, filters):
	...
	gl_entries = vmraid.get_all("GL Entry", filters=filters,
		fields = ["name", "voucher_type", "voucher_no",
