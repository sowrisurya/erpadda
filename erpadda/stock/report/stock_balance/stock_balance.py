...
from vmraid.utils import flt, cint, getdate, now, date_diff
from erpadda.stock.utils import add_additional_uom_columns
...
from erpadda.stock.report.stock_ageing.stock_ageing import get_fifo_queue, get_average_age
...
def get_conditions(filters):
	...
	if filters.get("to_date"):
		conditions += " and sle.posting_date <= %s" % vmraid.db.escape(filters.get("to_date"))
	else:
		vmraid.throw(_("'To Date' is required"))

	if filters.get("company"):
		conditions += " and sle.company = %s" % vmraid.db.escape(filters.get("company"))

	if filters.get("warehouse"):
		warehouse_details = vmraid.db.get_value("Warehouse",
...
def get_stock_ledger_entries(filters, items):
	...
	if items:
		item_conditions_sql = ' and sle.item_code in ({})'\
			.format(', '.join([vmraid.db.escape(i, percent=False) for i in items]))
...
def get_items(filters):
	...
	items = []
	if conditions:
		items = vmraid.db.sql_list("""select name from `tabItem` item where {}"""
...
def get_item_details(items, sle, filters):
	...
	if filters.get("include_uom"):
		...
		cf_join = "left join `tabUOM Conversion Detail` ucd on ucd.parent=item.name and ucd.uom=%s" \
			% vmraid.db.escape(filters.get("include_uom"))
...
def get_item_reorder_details(items):
	...
	if items:
		item_reorder_details = vmraid.db.sql("""
			select parent, warehouse, warehouse_reorder_qty, warehouse_reorder_level
			from `tabItem Reorder`
			where parent in ({0})
		""".format(', '.join([vmraid.db.escape(i, percent=False) for i in items])), as_dict=1)
...
def validate_filters(filters):
	if not (filters.get("item_code") or filters.get("warehouse")):
		...
		if sle_count > 500000:
			vmraid.throw(_("Please set filter based on Item or Warehouse due to a large amount of entries."))

def get_variants_attributes():
	'''Return all item variant attributes.'''
	return [i.name for i in vmraid.get_all('Item Attribute')]

def get_variant_values_for(items):
	'''Returns variant values for items.'''
	attribute_map = {}
	for attr in vmraid.db.sql('''select parent, attribute, attribute_value
