...
from erpadda.stock.utils import update_included_uom_in_report, is_reposting_item_valuation_in_progress
...
def execute(filters=None):
	...
	for bin in bin_list:
		...
		company = warehouse_company.setdefault(bin.warehouse,
			vmraid.db.get_value("Warehouse", bin.warehouse, "company"))
...
def get_bin_list(filters):
	...
	if filters.warehouse:
		warehouse_details = vmraid.db.get_value("Warehouse", filters.warehouse, ["lft", "rgt"], as_dict=1)
	...
	bin_list = vmraid.db.sql("""select item_code, warehouse, actual_qty, planned_qty, indented_qty,
...
def get_item_map(item_code, include_uom):
	...
	for ir in vmraid.db.sql("""select * from `tabItem Reorder` {condition}""".format(condition=condition), as_dict=1):
