...
import json
from vmraid.utils import flt, cstr, nowdate, nowtime, get_link_to_form

from six import string_types

class InvalidWarehouseCompany(vmraid.ValidationError): pass
...
def get_serial_nos_data_after_transactions(args):
	serial_nos = []
	data = vmraid.db.sql(""" SELECT serial_no, actual_qty
...
def get_latest_stock_qty(item_code, warehouse=None):
	...
	actual_qty = vmraid.db.sql("""select sum(actual_qty) from tabBin
...
def get_latest_stock_balance():
	bin_map = {}
	for d in vmraid.db.sql("""SELECT item_code, warehouse, stock_value as stock_value
...
def update_bin(args, allow_negative_stock=False, via_landed_cost_voucher=False):
	is_stock_item = vmraid.db.get_value('Item', args.get("item_code"), 'is_stock_item')
	if is_stock_item:
		bin = get_bin(args.get("item_code"), args.get("warehouse"))
		bin.update_stock(args, allow_negative_stock, via_landed_cost_voucher)
		return bin
	else:
		vmraid.msgprint(_("Item {0} ignored since it is not a stock item").format(args.get("item_code")))

@vmraid.whitelist()
def get_incoming_rate(args, raise_error_if_no_rate=True):
	"""Get Incoming Rate based on valuation method"""
	from erpadda.stock.stock_ledger import get_previous_sle, get_valuation_rate
	...
	if not in_rate:
		...
		in_rate = get_valuation_rate(args.get('item_code'), args.get('warehouse'),
			...
			currency=erpadda.get_company_currency(args.get('company')), company=args.get('company'),
...
def get_avg_purchase_rate(serial_nos):
	...
	return flt(vmraid.db.sql("""select avg(purchase_rate) from `tabSerial No`
		where name in (%s)""" % ", ".join(["%s"] * len(serial_nos)),
		tuple(serial_nos))[0][0])

def get_valuation_method(item_code):
	"""get valuation method from item or default"""
	val_method = vmraid.db.get_value('Item', item_code, 'valuation_method')
	if not val_method:
		val_method = vmraid.db.get_value("Stock Settings", None, "valuation_method") or "FIFO"
	return val_method
...
def validate_warehouse_company(warehouse, company):
	warehouse_company = vmraid.db.get_value("Warehouse", warehouse, "company")
	if warehouse_company and warehouse_company != company:
		vmraid.throw(_("Warehouse {0} does not belong to company {1}").format(warehouse, company),
			InvalidWarehouseCompany)

def is_group_warehouse(warehouse):
	if vmraid.db.get_value("Warehouse", warehouse, "is_group"):
		vmraid.throw(_("Group node warehouse is not allowed to select for transactions"))

def validate_disabled_warehouse(warehouse):
	if vmraid.db.get_value("Warehouse", warehouse, "disabled"):
		vmraid.throw(_("Disabled Warehouse {0} cannot be used for this transaction.").format(get_link_to_form('Warehouse', warehouse)))
...
def get_available_serial_nos(args):
	return vmraid.db.sql(""" SELECT name from `tabSerial No`
...
def get_incoming_outgoing_rate_for_cancel(item_code, voucher_type, voucher_no, voucher_detail_no):
	outgoing_rate = vmraid.db.sql("""SELECT abs(stock_value_difference / actual_qty)
		FROM `tabStock Ledger Entry`
...
def is_reposting_item_valuation_in_progress():
	reposting_in_progress = vmraid.db.exists("Repost Item Valuation",
		{'docstatus': 1, 'status': ['in', ['Queued','In Progress']]})
	if reposting_in_progress:
		vmraid.msgprint(_("Item valuation reposting in progress. Report might show incorrect item valuation."), alert=1)