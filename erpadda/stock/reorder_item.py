...
import erpadda
import json
from vmraid.utils import flt, nowdate, add_days, cint
...
def _reorder_item():
	...
	warehouse_company = vmraid._dict(vmraid.db.sql("""select name, company from `tabWarehouse`
		where disabled=0"""))
	default_company = (erpadda.get_default_company() or
		vmraid.db.sql("""select name from tabCompany limit 1""")[0][0])

	items_to_consider = vmraid.db.sql_list("""select name from `tabItem` item
...
def get_item_warehouse_projected_qty(items_to_consider):
	...
	for item_code, warehouse, projected_qty in vmraid.db.sql("""select item_code, warehouse, projected_qty
...
def create_material_request(material_requests):
	...
	if mr_list:
		if getattr(vmraid.local, "reorder_email_notify", None) is None:
			vmraid.local.reorder_email_notify = cint(vmraid.db.get_value('Stock Settings', None,
...
def send_email_notification(mr_list):
	...
	email_list = vmraid.db.sql_list("""select distinct r.parent
		from `tabHas Role` r, tabUser p
		where p.name = r.parent and p.enabled = 1 and p.docstatus < 2
		and r.role in ('Purchase Manager','Stock Manager')
		and p.name not in ('Administrator', 'All', 'Guest')""")

	msg = vmraid.render_template("templates/emails/reorder_item.html", {
		"mr_list": mr_list
	})

	vmraid.sendmail(recipients=email_list,
