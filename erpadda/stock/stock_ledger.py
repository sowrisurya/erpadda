...
from vmraid.utils import cint, flt, cstr, now, get_link_to_form
from vmraid.model.meta import get_field_precision
from erpadda.stock.utils import get_valuation_method, get_incoming_outgoing_rate_for_cancel
from erpadda.stock.utils import get_bin
import json
from six import iteritems

# future reposting
class NegativeStockError(vmraid.ValidationError): pass
class SerialNoExistsInFutureTransaction(vmraid.ValidationError):
	pass

_exceptions = vmraid.local('stockledger_exceptions')
# _exceptions = []

def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	if sl_entries:
		from erpadda.stock.utils import update_bin
...
def validate_cancellation(args):
	if args[0].get("is_cancelled"):
		repost_entry = vmraid.db.get_value("Repost Item Valuation", {
		...
		if repost_entry:
			if repost_entry.status == 'In Progress':
				vmraid.throw(_("Cannot cancel the transaction. Reposting of item valuation on submission is not completed yet."))
...
def set_as_cancel(voucher_type, voucher_no):
	vmraid.db.sql("""update `tabStock Ledger Entry` set is_cancelled=1,
...
def get_args_for_voucher(voucher_type, voucher_no):
	return vmraid.db.get_all("Stock Ledger Entry",
...
class update_entries_after(object):
	...
	def __init__(self, args, allow_zero_rate=False, allow_negative_stock=None, via_landed_cost_voucher=False, verbose=1):
		...
		self.allow_negative_stock = allow_negative_stock \
			or cint(vmraid.db.get_single_value("Stock Settings", "allow_negative_stock"))
	...
	def get_precision(self):
		...
		self.precision = get_field_precision(vmraid.get_meta("Stock Ledger Entry").get_field("stock_value"),
	...
	def get_future_entries_to_fix(self):
		...
		args = self.data[self.args.warehouse].previous_sle \
			or vmraid._dict({"item_code": self.item_code, "warehouse": self.args.warehouse})
	...
	def get_dependent_entries_to_fix(self, entries_to_fix, sle):
		...
		args = self.data[dependant_sle.warehouse].previous_sle \
			or vmraid._dict({"item_code": self.item_code, "warehouse": dependant_sle.warehouse})
	...
	def get_incoming_outgoing_rate_from_transaction(self, sle):
		...
		# Sales and Purchase Return
			...
			if vmraid.get_cached_value(sle.voucher_type, sle.voucher_no, "is_return"):
				from erpadda.controllers.sales_and_purchase_return import get_rate_for_return # don't move this import to top
				...
				# check in item table
				item_code, incoming_rate = vmraid.db.get_value(sle.voucher_type + " Item",
				...
				if item_code == sle.item_code:
					...
					rate = vmraid.db.get_value(ref_doctype, {"parent_detail_docname": sle.voucher_detail_no,
	...
	def update_rate_on_stock_entry(self, sle, outgoing_rate):
		...
		stock_entry = vmraid.get_doc("Stock Entry", sle.voucher_no, for_update=True)
	...
	def update_rate_on_delivery_and_sales_return(self, sle, outgoing_rate):
		...
		if item_code == sle.item_code:
			vmraid.db.set_value(sle.voucher_type + " Item", sle.voucher_detail_no, "incoming_rate", outgoing_rate)
		else:
			# packed item
			vmraid.db.set_value("Packed Item",
	...
	def update_rate_on_purchase_receipt(self, sle, outgoing_rate):
		if vmraid.db.exists(sle.voucher_type + " Item", sle.voucher_detail_no):
			vmraid.db.set_value(sle.voucher_type + " Item", sle.voucher_detail_no, "base_net_rate", outgoing_rate)
		else:
			vmraid.db.set_value("Purchase Receipt Item Supplied", sle.voucher_detail_no, "rate", outgoing_rate)

		# Recalculate subcontracted item's rate in case of subcontracted purchase receipt/invoice
		if vmraid.db.get_value(sle.voucher_type, sle.voucher_no, "is_subcontracted"):
	...
	def get_incoming_value_for_serial_nos(self, sle, serial_nos):
		...
		all_serial_nos = vmraid.get_all("Serial No",
	...
	def raise_exceptions(self):
		...
		for warehouse, exceptions in iteritems(self.exceptions):
			...
			if ((exceptions[0]["voucher_type"], exceptions[0]["voucher_no"]) in
				vmraid.local.flags.currently_saving):

				msg = _("{0} units of {1} needed in {2} to complete this transaction.").format(
					abs(deficiency), vmraid.get_desk_link('Item', exceptions[0]["item_code"]),
		...
		if msg_list:
			message = "\n\n".join(msg_list)
			if self.verbose:
				vmraid.throw(message, NegativeStockError, title='Insufficient Stock')
			else:
...
def get_stock_ledger_entries(previous_sle, operator=None,
	...
	if check_serial_no and previous_sle.get("serial_no"):
		...
		""").format(vmraid.db.escape(serial_no), vmraid.db.escape('{}\n%'.format(serial_no)),
			vmraid.db.escape('%\n{}'.format(serial_no)), vmraid.db.escape('%\n{}\n%'.format(serial_no)))
...
def get_sle_by_voucher_detail_no(voucher_detail_no, excluded_sle=None):
	return vmraid.db.get_value('Stock Ledger Entry',
...
def get_valuation_rate(item_code, warehouse, voucher_type, voucher_no,
	...
	if not valuation_rate:
		...
		if not valuation_rate:
			# try in price list
			valuation_rate = vmraid.db.get_value('Item Price',
	...
	if not allow_zero_rate and not valuation_rate and raise_error_if_no_rate \
		...
		solutions += "<li>" + _("If not, you can Cancel / Submit this entry") + " {0} ".format(vmraid.bold("after")) + _("performing either one below:") + "</li>"
	...
	if args.actual_qty < 0 and not allow_negative_stock:
		...
		if sle:
			message = _("{0} units of {1} needed in {2} on {3} {4} for {5} to complete this transaction.").format(
				...
				vmraid.get_desk_link(sle[0]["voucher_type"], sle[0]["voucher_no"]))
