...
import vmraid
from vmraid.utils import flt, cstr, nowdate, nowtime
...
def get_balance_qty_from_sle(item_code, warehouse):
	balance_qty = vmraid.db.sql("""select qty_after_transaction from `tabStock Ledger Entry`
...
def set_stock_balance_as_per_serial_no(item_code=None, posting_date=None, posting_time=None,
	...
	bin = vmraid.db.sql("""select bin.item_code, bin.warehouse, bin.actual_qty, item.stock_uom
		from `tabBin` bin, tabItem item
		where bin.item_code = item.name and item.has_serial_no = 1 %s""" % condition)

	for d in bin:
		serial_nos = vmraid.db.sql("""select count(name) from `tabSerial No`
			where item_code=%s and warehouse=%s and docstatus < 2""", (d[0], d[1]))

		sle = vmraid.db.sql("""select valuation_rate, company from `tabStock Ledger Entry`
			where item_code = %s and warehouse = %s and is_cancelled = 0
