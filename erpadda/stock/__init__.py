...
def get_warehouse_account_map(company=None):
	company_warehouse_account_map = company and vmraid.flags.setdefault('warehouse_account_map', {}).get(company)
	...
	if not warehouse_account_map or not company_warehouse_account_map or vmraid.flags.in_test:
		...
		for d in vmraid.get_all('Warehouse',
...
def get_warehouse_account(warehouse, warehouse_account=None):
	...
	if not account and warehouse.company:
		account = vmraid.db.get_value('Account',
			{'account_type': 'Stock', 'is_group': 0, 'company': warehouse.company}, 'name')

	if not account and warehouse.company and not warehouse.is_group:
		vmraid.throw(_("Please set Account in Warehouse {0} or Default Inventory Account in Company {1}")
