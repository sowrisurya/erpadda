...
from vmraid.utils import flt, cint, add_days, cstr, add_months, getdate
import json, copy
from erpadda.accounts.doctype.pricing_rule.pricing_rule import get_pricing_rule_for_item, set_transaction_type
...
def get_basic_details(args, item, overwrite_warehouse=True):
	...
	if args.get('doctype') == "Material Request" and not args.get('material_request_type'):
		args['material_request_type'] = vmraid.db.get_value('Material Request',
	...
	if args.get("manufacturer"):
		...
		data = vmraid.get_value("Item", item.name,
...
def get_item_warehouse(item, args, overwrite_warehouse, defaults={}):
	...
	if overwrite_warehouse or not args.warehouse:
		...
		if not warehouse:
			...
			warehouse_exists = vmraid.db.exists("Warehouse", {
...
def _get_item_tax_template(args, taxes, out=None, for_validate=False):
	...
	for tax in taxes:
		tax_company = vmraid.get_value("Item Tax Template", tax.item_tax_template, 'company')
...
def get_default_cost_center(args, item=None, item_group=None, brand=None, company=None):
	...
	if not cost_center and company:
		cost_center = vmraid.get_cached_value("Company",
			company, "cost_center")
...
def insert_item_price(args):
	...
	if vmraid.db.get_value("Price List", args.price_list, "currency", cache=True) == args.currency \
		and cint(vmraid.db.get_single_value("Stock Settings", "auto_insert_price_list_rate_if_missing")):
		if vmraid.has_permission("Item Price", "write"):
			...
			item_price = vmraid.db.get_value('Item Price',
			...
			if item_price and item_price.name:
				if item_price.price_list_rate != price_list_rate:
					...
					vmraid.msgprint(_("Item Price updated for {0} in Price List {1}").format(args.item_code,
				...
				item_price.insert()
				vmraid.msgprint(_("Item Price added for {0} in Price List {1}").format(args.item_code,
					args.price_list), alert=True)
...
def get_item_price(args, item_code, ignore_party=False):
	...
	return vmraid.db.sql(""" select name, price_list_rate, uom
...
def validate_conversion_rate(args, meta):
	...
	if (not args.conversion_rate
		and args.currency==vmraid.get_cached_value('Company',  args.company,  "default_currency")):
	...
	args.conversion_rate = flt(args.conversion_rate,
		get_field_precision(meta.get_field("conversion_rate"),
			vmraid._dict({"fields": args})))

	if args.price_list:
		if (not args.plc_conversion_rate
			and args.price_list_currency==vmraid.db.get_value("Price List", args.price_list, "currency", cache=True)):
...
def get_pos_profile(company, pos_profile=None, user=None):
	if pos_profile: return vmraid.get_cached_doc('POS Profile', pos_profile)
	...
	pos_profile = vmraid.db.sql("""SELECT pf.*
		FROM
...
def get_serial_nos_by_fifo(args, sales_order=None):
	if vmraid.db.get_single_value("Stock Settings", "automatically_set_serial_nos_based_on_fifo"):
		return "\n".join(vmraid.db.sql_list("""select name from `tabSerial No`
...
def get_conversion_factor(item_code, uom):
	...
	conversion_factor = vmraid.db.get_value("UOM Conversion Detail",
...
def get_projected_qty(item_code, warehouse):
	return {"projected_qty": vmraid.db.get_value("Bin",
		{"item_code": item_code, "warehouse": warehouse}, "projected_qty")}

@vmraid.whitelist()
def get_bin_details(item_code, warehouse, company=None):
	bin_details = vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse},
...
def get_company_total_stock(item_code, company):
	return vmraid.db.sql("""SELECT sum(actual_qty) from
...
def get_serial_no_details(item_code, warehouse, stock_qty, serial_no):
	args = vmraid._dict({"item_code":item_code, "warehouse":warehouse, "stock_qty":stock_qty, "serial_no":serial_no})
...
def get_batch_qty_and_serial_no(batch_no, stock_qty, warehouse, item_code, has_serial_no):
	...
	if (flt(batch_qty_and_serial_no.get('actual_batch_qty')) >= flt(stock_qty)) and has_serial_no:
		args = vmraid._dict({"item_code":item_code, "warehouse":warehouse, "stock_qty":stock_qty, "batch_no":batch_no})
		serial_no = get_serial_no(args)
...
def apply_price_list_on_item(args):
	...
	item_doc = vmraid.get_doc("Item", args.item_code)
...
def get_default_bom(item_code=None):
	if item_code:
		bom = vmraid.db.get_value("BOM", {"docstatus": 1, "is_default": 1, "is_active": 1, "item": item_code})
		if bom:
			return bom
...
def get_valuation_rate(item_code, company, warehouse=None):
	...
	if item.get("is_stock_item"):
		...
		return vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse},
			["valuation_rate"], as_dict=True) or {"valuation_rate": 0}

	elif not item.get("is_stock_item"):
		valuation_rate =vmraid.db.sql("""select sum(base_net_amount) / sum(qty*conversion_factor)
...
def get_so_reservation_for_item(args):
	...
	if args.get('against_sales_order'):
		...
		sales_order = vmraid.db.sql("""select sales_order from `tabSales Invoice Item` where
...
def get_reserved_qty_for_so(sales_order, item_code):
	reserved_qty = vmraid.db.sql("""select sum(qty) from `tabSales Order Item`
