...
vmraid.ui.form.on("Non Profit Settings", {
	refresh: function(frm) {
		...
		let docs_url = "https://docs.erpadda.com/docs/user/manual/en/non_profit/membership";

		frm.set_intro(__("You can learn more about memberships in the manual. ") + `<a href='${docs_url}'>${__('ERPAdda Docs')}</a>`, true);
	...
	setup_buttons_for_membership: function(frm) {
		...
		if (frm.doc.membership_webhook_secret) {
			...
			frm.add_custom_button(__("Copy Webhook URL"), () => {
				vmraid.utils.copy_to_clipboard(`https://${vmraid.boot.sitename}/api/method/erpadda.non_profit.doctype.membership.membership.trigger_razorpay_subscription`);
			}, __("Memberships"));
	...
	setup_buttons_for_donation: function(frm) {
		...
		if (frm.doc.donation_webhook_secret) {
			...
			frm.add_custom_button(__("Copy Webhook URL"), () => {
				vmraid.utils.copy_to_clipboard(`https://${vmraid.boot.sitename}/api/method/erpadda.non_profit.doctype.donation.donation.capture_razorpay_donations`);
			}, __("Donations"));
