...
from vmraid.utils import getdate, flt, get_link_to_form
from vmraid.email import sendmail_to_system_managers
from erpadda.non_profit.doctype.membership.membership import verify_signature

class Donation(Document):
	def validate(self):
		if not self.donor or not vmraid.db.exists('Donor', self.donor):
			# for web forms
	...
	def create_donor_for_website_user(self):
		donor_name = vmraid.get_value('Donor', dict(email=vmraid.session.user))

		if not donor_name:
			user = vmraid.get_doc('User', vmraid.session.user)
			donor = vmraid.get_doc(dict(
				doctype='Donor',
	...
	def create_payment_entry(self, date=None):
		...
		if not settings.donation_payment_account:
			vmraid.throw(_('You need to set <b>Payment Account</b> for Donation in {0}').format(
...
def capture_razorpay_donations(*args, **kwargs):
	...
	try:
		...
		log = vmraid.log_error(e, 'Donation Webhook Verification Error')
		notify_failure(log)
	...
	try:
		...
		log = vmraid.log_error(message, _('Error creating donation entry for {0}').format(donor.name))
		notify_failure(log)
		return { 'status': 'Failed', 'reason': e }
...
def create_donation(donor, payment):
	if not vmraid.db.exists('Mode of Payment', payment.method):
		create_mode_of_payment(payment.method)

	company = get_company_for_donations()
	donation = vmraid.get_doc({
		'doctype': 'Donation',
		'company': company,
		'donor': donor.name,
...
def get_donor(email):
	donors = vmraid.get_all('Donor',
		filters={'email': email},
		order_by='creation desc')

	try:
		return vmraid.get_doc('Donor', donors[0]['name'])
	except Exception:
		return None
...
def create_donor(payment):
	donor_details = vmraid._dict(payment)
	donor_type = vmraid.db.get_single_value('Non Profit Settings', 'default_donor_type')

	donor = vmraid.new_doc('Donor')
	donor.update({
...
def get_company_for_donations():
	company = vmraid.db.get_single_value('Non Profit Settings', 'donation_company')
	if not company:
...
def notify_failure(log):
	try:
		...
		sendmail_to_system_managers(_('[Important] [ERPAdda] Razorpay donation webhook failed, please check.'), content)
	except Exception:
		pass
