...
vmraid.ui.form.on('Restaurant Order Entry', {
	setup: function(frm) {
		let get_item_query = () => {
			return {
				query: 'erpadda.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant',
				filters: {
	...
	onload_post_render: function(frm) {
		if(!frm.item_selector) {
			frm.item_selector = new erpadda.ItemSelector({
				frm: frm,
				item_field: 'item',
				item_query: 'erpadda.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.item_query_restaurant',
				get_filters: () => {
	...
	restaurant_table: function(frm) {
		...
		return vmraid.call({
			method: 'erpadda.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.get_invoice',
			args: {
				table: frm.doc.restaurant_table
			},
			callback: (r) => {
	...
	sync: function(frm) {
		return vmraid.call({
			method: 'erpadda.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.sync',
			args: {
				table: frm.doc.restaurant_table,
				items: frm.doc.items
			},
	...
	make_invoice: function(frm) {
		frm.trigger('sync').then(() => {
			vmraid.prompt([
				...
				return vmraid.call({
					method: 'erpadda.restaurant.doctype.restaurant_order_entry.restaurant_order_entry.make_invoice',
					args: {
						table: frm.doc.restaurant_table,
