...
def get_invoice(table):
	...
	invoice_name = vmraid.get_value('Sales Invoice', dict(restaurant_table = table, docstatus=0))
	restaurant, menu_name = get_restaurant_and_menu_name(table)
	if invoice_name:
		invoice = vmraid.get_doc('Sales Invoice', invoice_name)
	else:
		invoice = vmraid.new_doc('Sales Invoice')
		invoice.naming_series = vmraid.db.get_value('Restaurant', restaurant, 'invoice_series_prefix')
		invoice.is_pos = 1
		default_customer = vmraid.db.get_value('Restaurant', restaurant, 'default_customer')
		if not default_customer:
			vmraid.throw(_('Please set default customer in Restaurant Settings'))
		invoice.customer = default_customer

	invoice.taxes_and_charges = vmraid.db.get_value('Restaurant', restaurant, 'default_tax_template')
	invoice.selling_price_list = vmraid.db.get_value('Price List', dict(restaurant_menu=menu_name, enabled=1))
...
def make_invoice(table, customer, mode_of_payment):
	...
	vmraid.msgprint(_('Invoice Created'), indicator='green', alert=True)
...
def item_query_restaurant(doctype='Item', txt='', searchfield='name', start=0, page_len=20, filters=None, as_dict=False):
	...
	items = vmraid.db.get_all('Restaurant Menu Item', ['item'], dict(parent = menu))
	del filters['table']
...
def get_restaurant_and_menu_name(table):
	if not table:
		vmraid.throw(_('Please select a table'))

	restaurant = vmraid.db.get_value('Restaurant Table', table, 'restaurant')
	menu = vmraid.db.get_value('Restaurant', restaurant, 'active_menu')

	if not menu:
		vmraid.throw(_('Please set an active menu for Restaurant {0}').format(restaurant))
