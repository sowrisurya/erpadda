...
def get_data(data, filters):
	po = get_po(filters)
	po_transferred_qty_map = vmraid._dict(get_transferred_quantity([v.name for v in po]))
...
def get_po(filters):
	...
	return vmraid.get_all("Purchase Order", filters=record_filters, fields=["name", "transaction_date", "supplier"])
...
def get_stock_entry(po):
	return vmraid.get_all("Stock Entry", filters=[
...
def get_stock_entry_detail(se):
	return vmraid.get_all("Stock Entry Detail", filters=[
			["parent", "in", se]
		],
		fields=["parent", "item_code", "qty"])

def get_purchase_order_item_supplied(po):
	return vmraid.get_all("Purchase Order Item Supplied", filters=[
			('parent', 'IN', po)
