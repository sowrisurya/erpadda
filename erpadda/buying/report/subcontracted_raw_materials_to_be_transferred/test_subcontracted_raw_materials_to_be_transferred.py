...
from erpadda.buying.report.subcontracted_raw_materials_to_be_transferred.subcontracted_raw_materials_to_be_transferred import execute
import json, vmraid, unittest
...
class TestSubcontractedItemToBeTransferred(unittest.TestCase):
	...
	def test_pending_and_transferred_qty(self):
		...
		col, data = execute(filters=vmraid._dict({'supplier': po.supplier,
		   'from_date': vmraid.utils.get_datetime(vmraid.utils.add_to_date(po.transaction_date, days=-10)),
...
def transfer_subcontracted_raw_materials(po):
	...
	se = vmraid.get_doc(make_rm_stock_entry(po, rm_item_string))
