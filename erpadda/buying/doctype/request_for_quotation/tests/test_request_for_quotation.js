...
QUnit.test("test: request_for_quotation", function(assert) {
	...
	vmraid.run_serially([
		() => {
			...
			return vmraid.tests.make('Request for Quotation', [
				...
				{items: [
					[
						...
						{"qty": 5},
						{"schedule_date": vmraid.datetime.add_days(vmraid.datetime.now_date(),20)},
		...
		() => {
			...
			assert.ok(cur_frm.doc.items[0].warehouse == 'All Warehouses - '+vmraid.get_abbr(vmraid.defaults.get_default("Company")), "Warehouse correct");
		...
		() => vmraid.click_button('Send Supplier Emails'),
		() => vmraid.timeout(6),
		() => {
