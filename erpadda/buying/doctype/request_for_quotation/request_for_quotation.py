...
from vmraid.utils import get_url, cint
...
class RequestforQuotation(BuyingController):
	...
	def validate_supplier_list(self):
		for d in self.suppliers:
			...
			if prevent_rfqs:
				...
				vmraid.throw(_("RFQs are not allowed for {0} due to a scorecard standing of {1}").format(d.supplier, standing))
			warn_rfqs = vmraid.db.get_value("Supplier", d.supplier, 'warn_rfqs')
			if warn_rfqs:
				standing = vmraid.db.get_value("Supplier Scorecard",d.supplier, 'status')
				vmraid.msgprint(_("{0} currently has a {1} Supplier Scorecard standing, and RFQs to this supplier should be issued with caution.").format(d.supplier, standing), title=_("Caution"), indicator='orange')
	...
	def validate_email_id(self, args):
		if not args.email_id:
			vmraid.throw(_("Row {0}: For Supplier {1}, Email Address is Required to send an email").format(args.idx, vmraid.bold(args.supplier)))
	...
	def update_supplier_part_no(self, supplier):
		...
		for item in self.items:
			item.supplier_part_no = vmraid.db.get_value('Item Supplier',
...
def check_portal_enabled(reference_doctype):
	if not vmraid.db.get_value('Portal Menu Item',
		{'reference_doctype': reference_doctype}, 'enabled'):
		vmraid.throw(_("The Access to Request for Quotation From Portal is Disabled. To Allow Access, Enable it in Portal Settings."))
...
def get_supplier_contacts(doctype, txt, searchfield, start, page_len, filters):
	return vmraid.db.sql("""select `tabContact`.name from `tabContact`, `tabDynamic Link`
...
def make_supplier_quotation_from_rfq(source_name, target_doc=None, for_supplier=None):
	def postprocess(source, target_doc):
		if for_supplier:
			...
			target_doc.buying_price_list = args.buying_price_list or vmraid.db.get_value('Buying Settings', None, 'buying_price_list')
...
def create_supplier_quotation(doc):
	...
	try:
		sq_doc = vmraid.get_doc({
			...
			"buying_price_list": doc.get('buying_price_list') or vmraid.db.get_value('Buying Settings', None, 'buying_price_list')
		})
...
def create_rfq_items(sq_doc, supplier, data):
	sq_doc.append('items', {
		...
		"supplier_part_no": vmraid.db.get_value("Item Supplier", {'parent': data.item_code, 'supplier': supplier}, "supplier_part_no"),
...
def get_supplier_tag():
	if not vmraid.cache().hget("Supplier", "Tags"):
		filters = {"document_type": "Supplier"}
		tags = list(set([tag.tag for tag in vmraid.get_all("Tag Link", filters=filters, fields=["tag"]) if tag]))
		vmraid.cache().hset("Supplier", "Tags", tags)

	return vmraid.cache().hget("Supplier", "Tags")
