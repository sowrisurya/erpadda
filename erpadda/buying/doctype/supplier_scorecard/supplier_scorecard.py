...
from vmraid.utils import nowdate, get_last_day, getdate, add_days, add_years
from erpadda.buying.doctype.supplier_scorecard_period.supplier_scorecard_period import make_supplier_scorecard
...
def get_timeline_data(doctype, name):
	...
	for sc in scorecards:
		start_date, end_date, total_score = vmraid.db.get_value('Supplier Scorecard Period', sc.name, ['start_date', 'end_date', 'total_score'])
...
def make_all_scorecards(docname):
	...
	while (start_date < todays) and (end_date <= todays):
		...
		end_date = get_scorecard_date(sc.period, start_date)
	if scp_count > 0:
		vmraid.msgprint(_("Created {0} scorecards for {1} between: ").format(scp_count, sc.supplier) + str(first_start_date) + " - " + str(last_end_date))
	return scp_count
