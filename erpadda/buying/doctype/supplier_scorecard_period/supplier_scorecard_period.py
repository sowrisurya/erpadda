...
from erpadda.buying.doctype.supplier_scorecard_criteria.supplier_scorecard_criteria import get_variables
...
class SupplierScorecardPeriod(Document):
	...
	def calculate_criteria(self):
		for crit in self.criteria:
			try:
				crit.score = min(crit.max_score, max( 0 ,vmraid.safe_eval(self.get_eval_statement(crit.formula),  None, {'max':max, 'min': min})))
			except Exception:
				vmraid.throw(_("Could not solve criteria score function for {0}. Make sure the formula is valid.").format(crit.criteria_name),vmraid.ValidationError)
				crit.score = 0
	...
	def calculate_weighted_score(self, weighing_function):
		try:
			weighed_score = vmraid.safe_eval(self.get_eval_statement(weighing_function),  None, {'max':max, 'min': min})
		except Exception:
			vmraid.throw(_("Could not solve weighted score function. Make sure the formula is valid."),vmraid.ValidationError)
			weighed_score = 0
...
def make_supplier_scorecard(source_name, target_doc=None):
	def update_criteria_fields(obj, target, source_parent):
		target.max_score, target.formula = vmraid.db.get_value('Supplier Scorecard Criteria',
