...
from vmraid.utils import flt, nowdate, add_days, getdate
...
class SupplierQuotation(BuyingController):
	...
	def update_rfq_supplier_status(self, include_me):
		...
		for rfq in rfq_list:
			...
			doc_sup = vmraid.get_all('Request for Quotation Supplier', filters=
				{'parent': doc.name, 'supplier': self.supplier}, fields=['name', 'quote_status'])

			doc_sup = doc_sup[0] if doc_sup else None
			if not doc_sup:
				vmraid.throw(_("Supplier {0} not found in {1}").format(self.supplier,
			...
			for item in doc.items:
				...
				vmraid.db.set_value('Request for Quotation Supplier', doc_sup.name, 'quote_status', quote_status)
