...
from erpadda.stock.doctype.serial_no.serial_no import SerialNoRequiredError, SerialNoQtyError
from erpadda.stock.doctype.batch.batch import UnableToSelectBatchError
...
def make_purchase_receipt():
	if random.random() < 0.6:
		...
		for po in po_list:
			pr = vmraid.get_doc(make_purchase_receipt(po))

			if pr.is_subcontracted=="Yes":
				pr.supplier_warehouse = "Supplier - WPL"

			pr.posting_date = vmraid.flags.current_date
			pr.insert()
			try:
				pr.submit()
...
def make_delivery_note():
	...
	if random.random() < 0.6:
		...
		for so in list(set([r[0] for r in query_report.run(report)["result"]
			...
			dn = vmraid.get_doc(make_delivery_note(so))
			dn.posting_date = vmraid.flags.current_date
			for d in dn.get("items"):
				if not d.expense_account:
					d.expense_account = ("Cost of Goods Sold - {0}".format(
						vmraid.get_cached_value('Company',  dn.company,  'abbr')))
...
def make_stock_reconciliation():
	...
	from erpadda.stock.doctype.stock_reconciliation.stock_reconciliation \
		import OpeningEntryAccountError, EmptyStockReconciliationItemsError

	if random.random() < 0.4:
		stock_reco = vmraid.new_doc("Stock Reconciliation")
		stock_reco.posting_date = vmraid.flags.current_date
		stock_reco.company = erpadda.get_default_company()
...
def submit_draft_stock_entries():
	...
	vmraid.db.commit()
	for st in vmraid.db.get_values("Stock Entry", {"docstatus":0}, "name"):
		try:
			ste = vmraid.get_doc("Stock Entry", st[0])
			ste.posting_date = vmraid.flags.current_date
			ste.save()
			ste.submit()
			vmraid.db.commit()
...
def make_sales_return_records():
	if random.random() < 0.1:
		for data in vmraid.get_all('Delivery Note', fields=["name"], filters={"docstatus": 1}):
			if random.random() < 0.1:
				try:
...
def make_purchase_return_records():
	if random.random() < 0.1:
		for data in vmraid.get_all('Purchase Receipt', fields=["name"], filters={"docstatus": 1}):
			if random.random() < 0.1:
				try:
