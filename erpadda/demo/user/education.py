...
from erpadda.education.api import get_student_group_students, make_attendance_records, enroll_student, \
								get_fee_schedule, collect_fees, get_course


def work():
	vmraid.set_user(vmraid.db.get_global('demo_education_user'))
	for d in range(20):
		approve_random_student_applicant()
		enroll_random_student(vmraid.flags.current_date)
	# if vmraid.flags.current_date.weekday()== 0:
	# 	make_course_schedule(vmraid.flags.current_date, vmraid.utils.add_days(vmraid.flags.current_date, 5))	
	mark_student_attendance(vmraid.flags.current_date)
...
def approve_random_student_applicant():
	...
	if random_student:
		...
		vmraid.db.set_value("Student Applicant", random_student, "application_status", status[weighted_choice([9,3])])
...
def assign_student_group(student, student_name, program, courses, batch):
	...
	for d in vmraid.get_list("Student Group", fields=("name"), filters={"program": program, "course":("in", course_list), "disabled": 0}):
		student_group = vmraid.get_doc("Student Group", d.name)
		...
		student_group.save()
	student_batch = vmraid.get_list("Student Group", fields=("name"), filters={"program": program, "group_based_on":"Batch", "batch":batch, "disabled": 0})[0]
	student_batch_doc = vmraid.get_doc("Student Group", student_batch.name)
...
def mark_student_attendance(current_date):
	...
	for d in vmraid.db.get_list("Student Group", filters={"group_based_on": "Batch", "disabled": 0}):
...
def make_fees():
	for d in range(1,10):
		...
		collect_fees(random_fee, vmraid.db.get_value("Fees", random_fee, "outstanding_amount"))	

def make_assessment_plan(date):
	for d in range(1,4):
		random_group = get_random("Student Group", {"group_based_on": "Course", "disabled": 0}, True)
		doc = vmraid.new_doc("Assessment Plan")
...
def make_course_schedule(start_date, end_date):
	for d in vmraid.db.get_list("Student Group"):
		cs = vmraid.new_doc("Scheduling Tool")
