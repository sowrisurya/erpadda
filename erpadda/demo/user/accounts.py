...
from erpadda.demo.user.sales import make_sales_order
...
def work():
	...
	if random.random() <= 0.6:
		...
		for so in list(set([r[0] for r in query_report.run(report)["result"]
			...
			try:
				si = vmraid.get_doc(make_sales_invoice(so))
				si.posting_date = vmraid.flags.current_date
				for d in si.get("items"):
					if not d.income_account:
						d.income_account = "Sales - {}".format(vmraid.get_cached_value('Company',  si.company,  'abbr'))
				si.insert()
				si.submit()
				vmraid.db.commit()
	...
	if random.random() <= 0.6:
		...
		for pr in list(set([r[0] for r in query_report.run(report)["result"]
			if r[0]!="Total"]))[:random.randint(1, 5)]:
			try:
				pi = vmraid.get_doc(make_purchase_invoice(pr))
				pi.posting_date = vmraid.flags.current_date
	...
	if random.random() < 0.4:
		...
		if sales_invoice_name:
			si = vmraid.get_doc("Sales Invoice", sales_invoice_name)
...
def make_payment_entries(ref_doctype, report):
	...
	outstanding_invoices = vmraid.get_all(ref_doctype, fields=["name"],
		filters={
	...
	for inv in outstanding_invoices[:random.randint(1, 2)]:
		...
		pe.posting_date = vmraid.flags.current_date
		pe.reference_no = random_string(6)
		pe.reference_date = vmraid.flags.current_date
		pe.insert()
		pe.submit()
		vmraid.db.commit()
	...
	for inv in outstanding_invoices[:1]:
		jv = vmraid.get_doc(get_payment_entry_against_invoice(ref_doctype, inv.name))
		jv.posting_date = vmraid.flags.current_date
		jv.cheque_no = random_string(6)
		jv.cheque_date = vmraid.flags.current_date
		jv.insert()
		jv.submit()
		vmraid.db.commit()
...
def make_pos_invoice():
	...
	for data in vmraid.get_all('Sales Order', fields=["name"],
		filters = [["per_billed", "<", "100"]]):
		si = vmraid.get_doc(make_sales_invoice(data.name))
		si.is_pos =1
		si.posting_date = vmraid.flags.current_date
		for d in si.get("items"):
			if not d.income_account:
