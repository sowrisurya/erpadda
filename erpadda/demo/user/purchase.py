...
import vmraid, random, json, erpadda
from vmraid.utils.make_random import how_many, get_random
...
from erpadda.stock.doctype.material_request.material_request import make_request_for_quotation
from erpadda.buying.doctype.request_for_quotation.request_for_quotation import \
			 make_supplier_quotation_from_rfq

def work():
	vmraid.set_user(vmraid.db.get_global('demo_purchase_user'))
	...
	if random.random() < 0.6:
		for rfq in vmraid.get_all('Request for Quotation',
			filters={'status': 'Open'}, limit=random.randint(1, 6)):
			if not vmraid.get_all('Supplier Quotation',
				filters={'request_for_quotation': rfq.name}, limit=1):
				rfq = vmraid.get_doc('Request for Quotation', rfq.name)
	...
	party_account_currency = get_party_account_currency("Supplier", supplier, erpadda.get_default_company())
	...
	if random.random() < 0.5:
		from erpadda.stock.doctype.material_request.material_request import make_supplier_quotation

		report = "Material Requests for which Supplier Quotations are not created"
		for row in query_report.run(report)["result"][:random.randint(1, 3)]:
			if row[0] != "Total":
				sq = vmraid.get_doc(make_supplier_quotation(row[0]))
				sq.transaction_date = vmraid.flags.current_date
...
def make_material_request(item_code, qty):
	...
	variant_of = vmraid.db.get_value('Item', item_code, 'variant_of') or item_code

	if vmraid.db.get_value('BOM', {'item': variant_of, 'is_default': 1, 'is_active': 1}):
		mr.material_request_type = 'Manufacture'
	else:
		mr.material_request_type = "Purchase"

	mr.transaction_date = vmraid.flags.current_date
	mr.schedule_date = vmraid.utils.add_days(mr.transaction_date, 7)

	mr.append("items", {
		"doctype": "Material Request Item",
		"schedule_date": vmraid.utils.add_days(mr.transaction_date, 7),
		"item_code": item_code,
		"qty": qty
	})
	mr.insert()
...
def make_subcontract():
	...
	if item_code:
		...
		po.transaction_date = vmraid.flags.current_date # added
		po.schedule_date = vmraid.utils.add_days(vmraid.flags.current_date, 7)

		item_code = get_random("Item", {"is_sub_contracted_item": 1})

		po.append("items", {
			"item_code": item_code,
			"schedule_date": vmraid.utils.add_days(vmraid.flags.current_date, 7),
		...
		stock_entry = vmraid.get_doc(make_rm_stock_entry(po.name, json.dumps([rm_items])))
