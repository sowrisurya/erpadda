...
from erpadda.healthcare.doctype.lab_test.lab_test import create_test_from_template
...
def make_appointment():
	...
	while i <= 4:
		...
		department = vmraid.get_value("Healthcare Practitioner", practitioner, "department")
		patient = get_random("Patient")
		patient_sex = vmraid.get_value("Patient", patient, "sex")
...
def consulation_on_appointment():
	for i in range(3):
		...
		appointment = vmraid.get_doc("Patient Appointment",appointment)
...
def lab_test_on_encounter():
	...
	while i <= 2:
		...
		test_rx = vmraid.get_doc("Lab Prescription", test_rx)
		encounter = vmraid.get_doc("Patient Encounter", test_rx.parent)
...
def import_drug():
	...
	data = json.loads(open(vmraid.get_app_path('erpadda', 'demo', 'data', 'drug_list.json')).read())
	for d in data:
		doc = vmraid.new_doc("Item")
		doc.update(d)
		doc.insert()
