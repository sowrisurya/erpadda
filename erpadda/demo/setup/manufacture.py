...
import random, json
import vmraid
from vmraid.utils import nowdate, add_days
...
def setup_workstation():
	...
	for w in workstations:
		vmraid.get_doc({
			"doctype": "Workstation",
			"workstation_name": w,
			"holiday_list": vmraid.get_all("Holiday List")[0].name,
...
def show_item_groups_in_website():
	...
	products = vmraid.get_doc("Item Group", "Products")
	products.show_in_website = 1
	products.route = 'products'
	products.save()

def setup_asset():
	assets = json.loads(open(vmraid.get_app_path('erpadda', 'demo', 'data', 'asset.json')).read())
	for d in assets:
		asset = vmraid.new_doc('Asset')
		asset.update(d)
...
def setup_item():
	items = json.loads(open(vmraid.get_app_path('erpadda', 'demo', 'data', 'item.json')).read())
	for i in items:
		...
		if hasattr(item, 'item_defaults') and item.item_defaults[0].default_warehouse:
			...
			warehouse = vmraid.get_all('Warehouse', filters={'warehouse_name': item.item_defaults[0].default_warehouse}, limit=1)
			if warehouse:
