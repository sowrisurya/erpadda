...
from vmraid.utils import flt, now_datetime, cstr, random_string
...
from erpadda.demo.domains import data
from vmraid import _

def setup(domain):
	vmraid.flags.in_demo = 1
	...
	employees = vmraid.get_all('Employee',  fields=['name', 'date_of_joining'])
...
def complete_setup(domain='Manufacturing'):
	...
	if not vmraid.get_all('Company', limit=1):
		setup_complete({
			"full_name": "Test User",
			"email": "test_demo@erpadda.com",
		...
		if company:
			...
			company_doc.db_set('default_payroll_payable_account',
				vmraid.db.get_value('Account', dict(account_name='Payroll Payable')))
...
def setup_holiday_list():
	...
	vmraid.set_value("Company", erpadda.get_default_company(), "default_holiday_list", holiday_list.name)


def setup_user():
	vmraid.db.sql('delete from tabUser where name not in ("Guest", "Administrator")')
	for u in json.loads(open(vmraid.get_app_path('erpadda', 'demo', 'data', 'user.json')).read()):
...
def setup_employee():
	vmraid.db.set_value("HR Settings", None, "emp_created_by", "Naming Series")
	vmraid.db.commit()

	for d in vmraid.get_all('Salary Component'):
		salary_component = vmraid.get_doc('Salary Component', d.name)
		salary_component.append('accounts', dict(
			company=erpadda.get_default_company(),
			account=vmraid.get_value('Account', dict(account_name=('like', 'Salary%')))
		))
		salary_component.save()

	import_json('Employee')
	holiday_list = vmraid.db.get_value("Holiday List", {"holiday_list_name": str(now_datetime().year)}, 'name')
	vmraid.db.sql('''update tabEmployee set holiday_list={0}'''.format(holiday_list))

def setup_salary_structure(employees, salary_slip_based_on_timesheet=0):
	ss = vmraid.new_doc('Salary Structure')
	...
	ss.payment_account = vmraid.get_value('Account',
		{'account_type': 'Cash', 'company': erpadda.get_default_company(),'is_group':0}, "name")
	...
	for e in employees:
		sa  = vmraid.new_doc("Salary Structure Assignment")
		sa.employee = e.name
...
def setup_user_roles(domain):
	user = vmraid.get_doc('User', 'demo@erpadda.com')
	...
	if not vmraid.db.get_global('demo_hr_user'):
		user = vmraid.get_doc('User', 'CaitlinSnow@example.com')
		user.add_roles('HR User', 'HR Manager', 'Accounts User')
		vmraid.db.set_global('demo_hr_user', user.name)
		update_employee_department(user.name, 'Human Resources')
		for d in vmraid.get_all('User Permission', filters={"user": "CaitlinSnow@example.com"}):
			vmraid.delete_doc('User Permission', d.name)

	if not vmraid.db.get_global('demo_sales_user_1'):
		user = vmraid.get_doc('User', 'VandalSavage@example.com')
		user.add_roles('Sales User')
		update_employee_department(user.name, 'Sales')
		vmraid.db.set_global('demo_sales_user_1', user.name)

	if not vmraid.db.get_global('demo_sales_user_2'):
		user = vmraid.get_doc('User', 'GraceChoi@example.com')
		user.add_roles('Sales User', 'Sales Manager', 'Accounts User')
		update_employee_department(user.name, 'Sales')
		vmraid.db.set_global('demo_sales_user_2', user.name)

	if not vmraid.db.get_global('demo_purchase_user'):
		user = vmraid.get_doc('User', 'MaxwellLord@example.com')
		user.add_roles('Purchase User', 'Purchase Manager', 'Accounts User', 'Stock User')
		update_employee_department(user.name, 'Purchase')
		vmraid.db.set_global('demo_purchase_user', user.name)

	if not vmraid.db.get_global('demo_manufacturing_user'):
		user = vmraid.get_doc('User', 'NeptuniaAquaria@example.com')
		...
		vmraid.db.set_global('demo_manufacturing_user', user.name)

	if not vmraid.db.get_global('demo_stock_user'):
		user = vmraid.get_doc('User', 'HollyGranger@example.com')
		user.add_roles('Manufacturing User', 'Stock User', 'Purchase User', 'Accounts User')
		update_employee_department(user.name, 'Production')
		vmraid.db.set_global('demo_stock_user', user.name)

	if not vmraid.db.get_global('demo_accounts_user'):
		user = vmraid.get_doc('User', 'BarryAllen@example.com')
		user.add_roles('Accounts User', 'Accounts Manager', 'Sales User', 'Purchase User')
		update_employee_department(user.name, 'Accounts')
		vmraid.db.set_global('demo_accounts_user', user.name)

	if not vmraid.db.get_global('demo_projects_user'):
		user = vmraid.get_doc('User', 'PeterParker@example.com')
		user.add_roles('HR User', 'Projects User')
		update_employee_department(user.name, 'Management')
		vmraid.db.set_global('demo_projects_user', user.name)

	if domain == "Education":
		if not vmraid.db.get_global('demo_education_user'):
			user = vmraid.get_doc('User', 'ArthurCurry@example.com')
			user.add_roles('Academics User')
			update_employee_department(user.name, 'Management')
			vmraid.db.set_global('demo_education_user', user.name)

	#Add Expense Approver
	user = vmraid.get_doc('User', 'ClarkKent@example.com')
	user.add_roles('Expense Approver')

def setup_leave_allocation():
	year = now_datetime().year
	for employee in vmraid.get_all('Employee', fields=['name']):
		leave_types = vmraid.get_all("Leave Type", fields=['name', 'max_continuous_days_allowed'])
		for leave_type in leave_types:
			if not leave_type.max_continuous_days_allowed:
				leave_type.max_continuous_days_allowed = 10

		leave_allocation = vmraid.get_doc({
...
def setup_mode_of_payment():
	company_abbr = vmraid.get_cached_value('Company',  erpadda.get_default_company(),  "abbr")
	account_dict = {'Cash': 'Cash - '+ company_abbr , 'Bank': 'National Bank - '+ company_abbr}
	for payment_mode in vmraid.get_all('Mode of Payment', fields = ["name", "type"]):
		if payment_mode.type:
			mop = vmraid.get_doc('Mode of Payment', payment_mode.name)
			mop.append('accounts', {
				'company': erpadda.get_default_company(),
...
def setup_account():
	...
	data = json.loads(open(vmraid.get_app_path('erpadda', 'demo', 'data',
		'account.json')).read())
	for d in data:
		doc = vmraid.new_doc('Account')
		doc.update(d)
		doc.parent_account = vmraid.db.get_value('Account', {'account_name': doc.parent_account})
		doc.insert()
...
def setup_account_to_expense_type():
	...
	for expense_type in expense_types:
		doc = vmraid.get_doc("Expense Claim Type", expense_type["name"])
		doc.append("accounts", {
			"company" : erpadda.get_default_company(),
...
def setup_budget():
	fiscal_years = vmraid.get_all("Fiscal Year", order_by="year_start_date")[-2:]
	...
	for fy in fiscal_years:
		...
		expense_ledger_count = vmraid.db.count("Account", {"is_group": "0", "root_type": "Expense"})
...
def setup_pos_profile():
	company_abbr = vmraid.get_cached_value('Company',  erpadda.get_default_company(),  "abbr")
	pos = vmraid.new_doc('POS Profile')
	pos.user = vmraid.db.get_global('demo_accounts_user')
...
def setup_role_permissions():
	...
	for doctype, roles in role_permissions.items():
		for role in roles:
			if not vmraid.db.get_value('Custom DocPerm',
...
def import_json(doctype, submit=False, values=None):
	vmraid.flags.in_import = True
	data = json.loads(open(vmraid.get_app_path('erpadda', 'demo', 'data',
		vmraid.scrub(doctype) + '.json')).read())
	for d in data:
...
def update_employee_department(user_id, department):
	employee = vmraid.db.get_value('Employee', {"user_id": user_id}, 'name')
	department = vmraid.db.get_value('Department', {'department_name': department}, 'name')
	vmraid.db.set_value('Employee', employee, 'department', department)
