...
def get_log_data(filters):
	fy = vmraid.db.get_value('Fiscal Year', filters.get('fiscal_year'), ['year_start_date', 'year_end_date'], as_dict=True)
	data = vmraid.db.sql("""select
...
def get_service_expense(logname):
	expense_amount = vmraid.db.sql("""select sum(expense_amount)
