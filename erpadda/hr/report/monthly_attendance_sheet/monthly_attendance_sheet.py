...
import vmraid
from vmraid.utils import cstr, cint, getdate
...
def execute(filters=None):
	...
	default_holiday_list = vmraid.get_cached_value('Company',  filters.get("company"),  "default_holiday_list")
	...
	leave_types = vmraid.db.get_list("Leave Type")
	leave_list = None
...
def add_data(employee_map, att_map, filters, holiday_map, conditions, default_holiday_list, leave_types=None):
	...
	for emp in employee_map:
		...
		if filters.summarized_view:
			leave_details = vmraid.db.sql("""select leave_type, status, count(*) as count from `tabAttendance`\
				where leave_type is not NULL %s group by leave_type, status""" % conditions, filters, as_dict=1)

			time_default_counts = vmraid.db.sql("""select (select count(*) from `tabAttendance` where \
...
def get_attendance_list(conditions, filters):
	attendance_list = vmraid.db.sql("""select employee, day(attendance_date) as day_of_month,
	...
	for d in attendance_list:
		att_map.setdefault(d.employee, vmraid._dict()).setdefault(d.day_of_month, "")
...
def get_employee_details(group_by, company):
	...
	query = """select name, employee_name, designation, department, branch, company,
		holiday_list from `tabEmployee` where company = %s """ % vmraid.db.escape(company)

	if group_by:
		group_by = group_by.lower()
		query += " order by " + group_by + " ASC"

	employee_details = vmraid.db.sql(query , as_dict=1)
...
def get_holiday(holiday_list, month):
	...
	for d in holiday_list:
		if d:
			holiday_map.setdefault(d, vmraid.db.sql('''select day(holiday_date), weekly_off from `tabHoliday`
...
def get_attendance_years():
	year_list = vmraid.db.sql_list("""select distinct YEAR(attendance_date) from tabAttendance ORDER BY YEAR(attendance_date) DESC""")
	if not year_list:
