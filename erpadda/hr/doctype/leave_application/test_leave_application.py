...
from erpadda.hr.doctype.leave_application.leave_application import LeaveDayBlockedError, OverlapError, NotAnOptionalHoliday, get_leave_balance_on
from vmraid.permissions import clear_user_permissions_for_doctype
from vmraid.utils import add_days, nowdate, now_datetime, getdate, add_months
...
from erpadda.hr.doctype.leave_policy_assignment.leave_policy_assignment import create_assignment_for_multiple_employees
...
class TestLeaveApplication(unittest.TestCase):
	def setUp(self):
		for dt in ["Leave Application", "Leave Allocation", "Salary Slip", "Leave Ledger Entry"]:
			vmraid.db.sql("DELETE FROM `tab%s`" % dt) #nosec

	@classmethod
	def setUpClass(cls):
		set_leave_approver()
		vmraid.db.sql("delete from tabAttendance where employee='_T-Employee-00001'")

	def tearDown(self):
		vmraid.set_user("Administrator")

	def _clear_roles(self):
		vmraid.db.sql("""delete from `tabHas Role` where parent in
	...
	def test_overwrite_attendance(self):
		...
		attendance = vmraid.get_all('Attendance', ['name', 'status', 'attendance_date'],
	...
	def test_block_list(self):
		...
		vmraid.db.set_value("Department", "_Test Department - _TC",
	...
	def test_leaves_allowed(self):
		...
		leave_type = vmraid.get_doc(dict(
	...
	def test_applicable_after(self):
		...
		leave_type_1 = vmraid.get_doc(dict(
	...
	def test_earned_leaves_creation(self):
		...
		leave_policy_assignments = create_assignment_for_multiple_employees([employee.name], vmraid._dict(data))

		vmraid.get_doc("Leave Policy Assignment", leave_policy_assignments[0]).grant_leave_alloc_for_employee()
	...
	def test_current_leave_on_submit(self):
		...
		allocation = vmraid.get_doc(dict(
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_application.submit()
		leave_ledger_entry = vmraid.get_all('Leave Ledger Entry', fields='*', filters=dict(transaction_name=leave_application.name))
		...
		self.assertFalse(vmraid.db.exists("Leave Ledger Entry", {'transaction_name':leave_application.name}))
	...
	def test_ledger_entry_creation_on_intermediate_allocation_expiry(self):
		...
		leave_ledger_entry = vmraid.get_all('Leave Ledger Entry', '*', filters=dict(transaction_name=leave_application.name))
...
def get_leave_period():
	leave_period_name = vmraid.db.exists({
		"doctype": "Leave Period",
		"company": "_Test Company"
	})
	if leave_period_name:
		return vmraid.get_doc("Leave Period", leave_period_name[0][0])
	else:
