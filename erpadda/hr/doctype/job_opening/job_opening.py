...
from erpadda.hr.doctype.staffing_plan.staffing_plan import get_designation_counts, get_active_staffing_plan_details
	...
	def validate_current_vacancies(self):
		...
		if self.staffing_plan and self.planned_vacancies:
			...
			if self.planned_vacancies <= current_count:
				vmraid.throw(_("Job Openings for designation {0} already open or hiring completed as per Staffing Plan {1}").format(
