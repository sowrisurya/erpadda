...
import vmraid
from vmraid.utils import flt, date_diff, formatdate, add_days, today, getdate
from vmraid import _
from vmraid.model.document import Document
from erpadda.hr.utils import set_employee_name, get_leave_period
from erpadda.hr.doctype.leave_ledger_entry.leave_ledger_entry import expire_allocation, create_leave_ledger_entry

class OverlapError(vmraid.ValidationError): pass
class BackDatedAllocationError(vmraid.ValidationError): pass
class OverAllocationError(vmraid.ValidationError): pass
class LessAllocationError(vmraid.ValidationError): pass
class ValueMultiplierError(vmraid.ValidationError): pass
...
class LeaveAllocation(Document):
	...
	def validate_leave_allocation_days(self):
		...
		max_leaves_allowed = flt(vmraid.db.get_value("Leave Type", self.leave_type, "max_leaves_allowed"))
		if max_leaves_allowed > 0:
			...
			if leave_allocated > max_leaves_allowed:
				vmraid.throw(_("Total allocated leaves are more days than maximum allocation of {0} leave type for employee {1} in the period")
	...
	def update_leave_policy_assignments_when_no_allocations_left(self):
		allocations = vmraid.db.get_list("Leave Allocation", filters = {
			"docstatus": 1,
			"leave_policy_assignment": self.leave_policy_assignment
		})
		if len(allocations) == 0:
			vmraid.db.set_value("Leave Policy Assignment", self.leave_policy_assignment ,"leaves_allocated", 0)
	...
	def validate_lwp(self):
		if vmraid.db.get_value("Leave Type", self.leave_type, "is_lwp"):
			vmraid.throw(_("Leave Type {0} cannot be allocated since it is leave without pay").format(self.leave_type))
	...
	def validate_allocation_overlap(self):
		...
		if leave_allocation:
			vmraid.msgprint(_("{0} already allocated for Employee {1} for period {2} to {3}")
				.format(self.leave_type, self.employee, formatdate(self.from_date), formatdate(self.to_date)))

			vmraid.throw(_('Reference') + ': <a href="/app/Form/Leave Allocation/{0}">{0}</a>'
	...
	def validate_back_dated_allocation(self):
		...
		if future_allocation:
			vmraid.throw(_("Leave cannot be allocated before {0}, as leave balance has already been carry-forwarded in the future leave allocation record {1}")
	...
	def set_total_leaves_allocated(self):
		...
		if not self.total_leaves_allocated \
			...
			and not vmraid.db.get_value("Leave Type", self.leave_type, "is_compensatory"):
				vmraid.throw(_("Total leaves allocated is mandatory for Leave Type {0}")
	...
	def set_carry_forwarded_leaves_in_previous_allocation(self, on_cancel=False):
		...
		if previous_allocation:
			vmraid.db.set_value("Leave Allocation", previous_allocation.name,
	...
	def create_leave_ledger_entry(self, submit=True):
		if self.unused_leaves:
			expiry_days = vmraid.db.get_value("Leave Type", self.leave_type, "expire_carry_forwarded_leaves_after_days")
...
def get_previous_allocation(from_date, leave_type, employee):
	...
	return vmraid.db.get_value("Leave Allocation",
		filters={
			'to_date': ("<", from_date),
...
def get_carry_forwarded_leaves(employee, leave_type, date, carry_forward=None):
	...
	if carry_forward and previous_allocation:
		...
		if unused_leaves:
			max_carry_forwarded_leaves = vmraid.db.get_value("Leave Type",
...
def get_unused_leaves(employee, leave_type, from_date, to_date):
	...
	leaves = vmraid.get_all("Leave Ledger Entry", filters={
		'employee': employee,
		'leave_type': leave_type,
		'from_date': ('>=', from_date),
		'to_date': ('<=', to_date)
