...
import vmraid
import json
from vmraid.utils import cint, getdate, formatdate, today
	...
	def validate_days(self):
		...
		for day in self.get("holidays"):
			if not (getdate(self.from_date) <= getdate(day.holiday_date) <= getdate(self.to_date)):
				vmraid.throw(_("The holiday on {0} is not between From Date and To Date").format(formatdate(day.holiday_date)))
...
def get_events(start, end, filters=None):
	...
	return vmraid.get_list('Holiday List',
...
def is_holiday(holiday_list, date=today()):
	...
	"""
	if holiday_list:
		return bool(vmraid.get_all('Holiday List',
