...
def get_approvers(doctype, txt, searchfield, start, page_len, filters):
	...
	department_details = {}
	department_list = []
	employee = vmraid.get_value("Employee", filters.get("employee"), ["employee_name","department", "leave_approver", "expense_approver", "shift_request_approver"], as_dict=True)

	employee_department = filters.get("department") or employee.department
	if employee_department:
		department_details = vmraid.db.get_value("Department", {"name": employee_department}, ["lft", "rgt"], as_dict=True)
	if department_details:
		department_list = vmraid.db.sql("""select name from `tabDepartment` where lft <= %s
			and rgt >= %s
			and disabled=0
	...
	if filters.get("doctype") == "Leave Application" and employee.leave_approver:
		approvers.append(vmraid.db.get_value("User", employee.leave_approver, ['name', 'first_name', 'last_name']))

	if filters.get("doctype") == "Expense Claim" and employee.expense_approver:
		approvers.append(vmraid.db.get_value("User", employee.expense_approver, ['name', 'first_name', 'last_name']))

	if filters.get("doctype") == "Shift Request" and employee.shift_request_approver:
		approvers.append(vmraid.db.get_value("User", employee.shift_request_approver, ['name', 'first_name', 'last_name']))
	...
	if department_list:
		for d in department_list:
			approvers += vmraid.db.sql("""select user.name, user.first_name, user.last_name from
	...
	if len(approvers) == 0:
		error_msg = _("Please set {0} for the Employee: {1}").format(field_name, vmraid.bold(employee.employee_name))
		if department_list:
