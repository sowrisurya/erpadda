...
import vmraid
from vmraid.utils import now, cint, get_datetime
from vmraid.model.document import Document
from vmraid import _

from erpadda.hr.doctype.shift_assignment.shift_assignment import get_actual_start_end_datetime_of_shift
...
class EmployeeCheckin(Document):
	...
	def validate_duplicate_log(self):
		doc = vmraid.db.exists('Employee Checkin', {
			'employee': self.employee,
			'time': self.time,
			'name': ['!=', self.name]})
		if doc:
			doc_link = vmraid.get_desk_link('Employee Checkin', doc)
			vmraid.throw(_('This employee already has a log with the same timestamp.{0}')
				.format("<Br>" + doc_link))
	...
	def fetch_shift(self):
		...
		if shift_actual_timings[0] and shift_actual_timings[1]:
			if shift_actual_timings[2].shift_type.determine_check_in_and_check_out == 'Strictly based on Log Type in Employee Checkin' and not self.log_type and not self.skip_auto_attendance:
				vmraid.throw(_('Log Type is required for check-ins falling in the shift: {0}.').format(shift_actual_timings[2].shift_type.name))
...
def add_log_based_on_employee_field(employee_field_value, timestamp, device_id=None, log_type=None, skip_auto_attendance=0, employee_fieldname='attendance_device_id'):
	...
	employee = vmraid.db.get_values("Employee", {employee_fieldname: employee_field_value}, ["name", "employee_name", employee_fieldname], as_dict=True)
	if employee:
		employee = employee[0]
	else:
		vmraid.throw(_("No Employee found for the given employee field value. '{}': {}").format(employee_fieldname,employee_field_value))
...
def mark_attendance_and_link_log(logs, attendance_status, attendance_date, working_hours=None, late_entry=False, early_exit=False, in_time=None, out_time=None, shift=None):
	...
	if attendance_status == 'Skip':
		vmraid.db.sql("""update `tabEmployee Checkin`
