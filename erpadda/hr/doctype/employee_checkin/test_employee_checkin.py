...
import vmraid
from vmraid.utils import now_datetime, nowdate, to_timedelta
import unittest
from datetime import timedelta

from erpadda.hr.doctype.employee_checkin.employee_checkin import add_log_based_on_employee_field, mark_attendance_and_link_log, calculate_working_hours
...
class TestEmployeeCheckin(unittest.TestCase):
	...
	def test_mark_attendance_and_link_log(self):
		...
		logs_count = vmraid.db.count('Employee Checkin', {'name':['in', log_names], 'skip_auto_attendance':1})
		...
		log_names = [log.name for log in logs]
		logs_count = vmraid.db.count('Employee Checkin', {'name':['in', log_names], 'attendance':attendance.name})
		self.assertEqual(logs_count, 4)
		attendance_count = vmraid.db.count('Attendance', {'status':'Present', 'working_hours':8.2,
