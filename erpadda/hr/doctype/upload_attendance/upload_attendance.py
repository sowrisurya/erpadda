...
import vmraid
from vmraid.utils import cstr, add_days, date_diff, getdate
...
def add_header(w):
	status = ", ".join((vmraid.get_meta("Attendance").get_field("status").options or "").strip().split("\n"))
	w.writerow(["Notes:"])
...
def get_active_employees():
	employees = vmraid.db.get_all('Employee',
...
def get_existing_attendance_records(args):
	attendance = vmraid.db.sql("""select name, attendance_date, employee, status, leave_type, naming_series
...
def import_attendances(rows):
	...
	for i, row in enumerate(rows):
		...
		try:
			...
			vmraid.publish_realtime('import_attendance', dict(
				progress=i,
				total=len(rows)
			))
