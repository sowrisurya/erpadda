...
import vmraid
import unittest
from vmraid.utils import today, add_months, add_days
...
class TestCompensatoryLeaveRequest(unittest.TestCase):
	def setUp(self):
		...
		vmraid.db.sql(''' delete from `tabAttendance` where attendance_date in {0} '''.format((today(), add_days(today(), -1)))) #nosec
	...
	def test_leave_allocation_update_on_submit(self):
		...
		leaves_allocated = vmraid.db.get_value('Leave Allocation', {
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_ledger_entry = vmraid.get_all('Leave Ledger Entry', fields='*', filters=filters, order_by = 'creation desc')
...
def get_compensatory_leave_request(employee, leave_date=today()):
	prev_comp_leave_req = vmraid.db.get_value('Compensatory Leave Request',
...
def mark_attendance(employee, date=today(), status='Present'):
	if not vmraid.db.exists(dict(doctype='Attendance', employee=employee.name, attendance_date=date, status='Present')):
