...
from vmraid.utils import date_diff, add_days, getdate, cint, format_date
from vmraid.model.document import Document
from erpadda.hr.utils import validate_dates, validate_overlap, get_leave_period, \
...
class CompensatoryLeaveRequest(Document):
	...
	def validate_attendance(self):
		attendance = vmraid.get_all('Attendance',
			filters={
	...
	def validate_holidays(self):
		...
		if len(holidays) < date_diff(self.work_end_date, self.work_from_date) + 1:
			if date_diff(self.work_end_date, self.work_from_date):
				msg = _("The days between {0} to {1} are not valid holidays.").format(vmraid.bold(format_date(self.work_from_date)), vmraid.bold(format_date(self.work_end_date)))
			else:
	...
	def on_submit(self):
		...
		if leave_period:
			...
			vmraid.throw(_("There is no leave period in between {0} and {1}").format(format_date(self.work_from_date), format_date(self.work_end_date)))
