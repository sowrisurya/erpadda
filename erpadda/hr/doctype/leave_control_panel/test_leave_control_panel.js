...
QUnit.test("Test: Leave control panel [HR]", function (assert) {
	...
	vmraid.run_serially([
		...
		() => vmraid.set_route("Form", "Leave Control Panel"),
		() => vmraid.timeout(1),
		...
		() => cur_frm.set_value("from_date", vmraid.datetime.add_months(today_date, -2)),
		() => cur_frm.set_value("to_date", vmraid.datetime.add_days(today_date, -1)),	// for two months [not today]
		() => cur_frm.set_value("no_of_days", 3),
		// allocate leaves
		() => vmraid.click_button('Allocate'),
		() => vmraid.timeout(1),
