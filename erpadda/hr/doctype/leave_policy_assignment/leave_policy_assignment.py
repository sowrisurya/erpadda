...
from vmraid.utils import getdate, date_diff, comma_and, formatdate, get_datetime, flt
...
class LeavePolicyAssignment(Document):
	...
	def set_dates(self):
		if self.assignment_based_on == "Leave Period":
			self.effective_from, self.effective_to = vmraid.db.get_value("Leave Period", self.leave_period, ["from_date", "to_date"])
	...
	def validate_policy_assignment_overlap(self):
		leave_policy_assignments = vmraid.get_all("Leave Policy Assignment", filters = {
		...
		if len(leave_policy_assignments):
			vmraid.throw(_("Leave Policy: {0} already assigned for Employee {1} for period {2} to {3}")
	...
	def get_new_leaves(self, leave_type, new_leaves_allocated, leave_type_details, date_of_joining):
		...
		precision = get_field_precision(vmraid.get_meta("Leave Allocation").get_field("new_leaves_allocated"))
	...
	def get_leaves_for_passed_months(self, leave_type, new_leaves_allocated, leave_type_details, date_of_joining):
		...
		from_date = vmraid.db.get_value("Leave Period", self.leave_period, "from_date")
...
def grant_leave_for_multiple_employees(leave_policy_assignments):
	...
	for assignment in leave_policy_assignments:
		try:
			vmraid.get_doc("Leave Policy Assignment", assignment).grant_leave_alloc_for_employee()
		except Exception:
...
def automatically_allocate_leaves_based_on_leave_policy():
	today = getdate()
	automatically_allocate_leaves_based_on_leave_policy = vmraid.db.get_single_value(
		'HR Settings', 'automatically_allocate_leaves_based_on_leave_policy'
	)

	pending_assignments = vmraid.get_list(
