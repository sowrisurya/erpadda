...
from vmraid.utils import add_days, today, flt, DATE_FORMAT, getdate
...
def validate_leave_allocation_against_leave_application(ledger):
	...
	if leave_application_records:
		vmraid.throw(_("Leave allocation {0} is linked with the Leave Application {1}").format(
...
def get_previous_expiry_ledger_entry(ledger):
	...
	creation_date = vmraid.db.get_value("Leave Ledger Entry", filters={
	...
	return vmraid.db.get_value("Leave Ledger Entry", filters={
...
def process_expired_allocation():
	...
	leave_type_records = vmraid.db.get_values("Leave Type", filters={
...
def get_remaining_leaves(allocation):
	...
	return vmraid.db.get_value("Leave Ledger Entry",
		filters={
