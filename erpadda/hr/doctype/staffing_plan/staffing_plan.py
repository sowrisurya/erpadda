...
from vmraid.utils import getdate, nowdate, cint, flt
from vmraid.utils.nestedset import get_descendants_of

class SubsidiaryCompanyError(vmraid.ValidationError): pass
class ParentCompanyError(vmraid.ValidationError): pass
...
class StaffingPlan(Document):
	...
	def validate_overlap(self, staffing_plan_detail):
		...
		# and spd.vacancies>0 ?
		overlap = vmraid.db.sql("""select spd.parent
		...
		if overlap and overlap [0][0]:
			vmraid.throw(_("Staffing Plan {0} already exist for designation {1}")
	...
	def validate_with_parent_plan(self, staffing_plan_detail):
		...
		if cint(staffing_plan_detail.vacancies) > cint(parent_plan_details[0].vacancies) or \
			...
			vmraid.throw(_("You can only plan for upto {0} vacancies and budget {1} \
		...
		all_sibling_details = vmraid.db.sql("""select sum(spd.vacancies) as vacancies,
		...
		if (cint(parent_plan_details[0].vacancies) < \
			...
			vmraid.throw(_("{0} vacancies and {1} budget for {2} already planned for subsidiary companies of {3}. \
	...
	def validate_with_subsidiary_plans(self, staffing_plan_detail):
		...
		children_details = vmraid.db.sql("""select sum(spd.vacancies) as vacancies,
			sum(spd.total_estimated_cost) as total_estimated_cost
			from `tabStaffing Plan Detail` spd join `tabStaffing Plan` sp on spd.parent=sp.name
			where spd.designation=%s and sp.docstatus=1
			and sp.to_date >= %s and sp.from_date <=%s
		...
		if children_details and \
			...
			vmraid.throw(_("Subsidiary companies have already planned for {1} vacancies at a budget of {2}. \
				Staffing Plan for {0} should allocate more vacancies and budget for {3} than planned for its subsidiary companies"
				.format(self.company,
					cint(children_details.vacancies),
					children_details.total_estimated_cost,
					vmraid.bold(staffing_plan_detail.designation))), SubsidiaryCompanyError)
...
def get_designation_counts(designation, company):
	...
	employee_counts["employee_count"] = vmraid.db.get_value("Employee",
		filters={
	...
	employee_counts['job_openings'] = vmraid.db.get_value("Job Opening",
		filters={
			'designation': designation,
			'status': 'Open',
			'company': ('in', company_set)
