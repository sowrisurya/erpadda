...
class JobOffer(Document):
	...
	def validate(self):
		...
		job_offer = vmraid.db.exists("Job Offer",{"job_applicant": self.job_applicant, "docstatus": ["!=", 2]})
		if job_offer and job_offer != self.name:
			vmraid.throw(_("Job Offer: {0} is already for Job Applicant: {1}").format(vmraid.bold(job_offer), vmraid.bold(self.job_applicant)))
	...
	def get_job_offer(self, from_date, to_date):
		...
		return vmraid.get_all("Job Offer", filters={
...
def make_employee(source_name, target_doc=None):
	def set_missing_values(source, target):
		target.personal_email, target.first_name = vmraid.db.get_value("Job Applicant", \
