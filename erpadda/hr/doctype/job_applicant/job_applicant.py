...
from vmraid.utils import comma_and, validate_email_address
...
class JobApplicant(Document):
	...
	def autoname(self):
		...
		if not keys:
			vmraid.throw(_("Name or Email is mandatory"), vmraid.NameError)
		self.name = " - ".join(keys)
	...
	def check_email_id_is_unique(self):
		if self.email_id:
			names = vmraid.db.sql_list("""select name from `tabJob Applicant`
				where email_id=%s and name!=%s and job_title=%s""", (self.email_id, self.name, self.job_title))

			if names:
				vmraid.throw(_("Email Address must be unique, already exists for {0}").format(comma_and(names)), vmraid.DuplicateEntryError)
