...
from vmraid.utils import get_fullname, flt, cstr, get_link_to_form
from vmraid.model.document import Document
from erpadda.hr.utils import set_employee_name, share_doc_with_approver
...
class InvalidExpenseApproverError(vmraid.ValidationError): pass
class ExpenseApproverIdentityError(vmraid.ValidationError): pass

class ExpenseClaim(AccountsController):
	def onload(self):
		self.get("__onload").make_payment_via_journal_entry = vmraid.db.get_single_value('Accounts Settings',
	...
	def set_payable_account(self):
		if not self.payable_account and not self.is_paid:
			self.payable_account = vmraid.get_cached_value('Company', self.company, 'default_expense_claim_payable_account')
	...
	def validate_account_details(self):
		for data in self.expenses:
			if not data.cost_center:
				vmraid.throw(_("Row {0}: {1} is required in the expenses table to book an expense claim.")
					.format(data.idx, vmraid.bold("Cost Center")))
	...
	def validate_advances(self):
		...
		for d in self.get("advances"):
			ref_doc = vmraid.db.get_value("Employee Advance", d.employee_advance,
			...
			if d.allocated_amount and flt(d.allocated_amount) > flt(d.unclaimed_amount):
				vmraid.throw(_("Row {0}# Allocated amount {1} cannot be greater than unclaimed amount {2}")
	...
	def validate_sanctioned_amount(self):
		for d in self.get('expenses'):
			if flt(d.sanctioned_amount) > flt(d.amount):
				vmraid.throw(_("Sanctioned Amount cannot be greater than Claim Amount in Row {0}.").format(d.idx))
...
def update_reimbursed_amount(doc, jv=None):
	...
	amt = vmraid.db.sql("""select ifnull(sum(debit_in_account_currency), 0) - ifnull(sum(credit_in_account_currency), 0)as amt
	...
	vmraid.db.set_value("Expense Claim", doc.name , "total_amount_reimbursed", amt)
...
def get_expense_claim_account(expense_claim_type, company):
	account = vmraid.db.get_value("Expense Claim Account",
		{"parent": expense_claim_type, "company": company}, "default_account")
	if not account:
		vmraid.throw(_("Set the default account for the {0} {1}")
			.format(vmraid.bold("Expense Claim Type"), get_link_to_form("Expense Claim Type", expense_claim_type)))
...
def get_advances(employee, advance_id=None):
	if not advance_id:
		condition = 'docstatus=1 and employee={0} and paid_amount > 0 and paid_amount > claimed_amount + return_amount'.format(vmraid.db.escape(employee))
	else:
		condition = 'name={0}'.format(vmraid.db.escape(advance_id))
