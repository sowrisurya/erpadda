...
import vmraid
import unittest
from vmraid.utils import random_string, nowdate
...
test_records = vmraid.get_test_records('Expense Claim')
...
class TestExpenseClaim(unittest.TestCase):
	def test_total_expense_claim_for_project(self):
		vmraid.db.sql("""delete from `tabTask` where project = "_Test Project 1" """)
		vmraid.db.sql("""delete from `tabProject` where name = "_Test Project 1" """)
	...
	def test_rejected_expense_claim(self):
		...
		gl_entry = vmraid.get_all('GL Entry', {'voucher_type': 'Expense Claim', 'voucher_no': expense_claim.name})
...
def make_expense_claim(payable_account, amount, sanctioned_amount, company, account, project=None, task_name=None, do_not_submit=False, taxes=None):
	...
	currency, cost_center = vmraid.db.get_value('Company', company, ['default_currency', 'cost_center'])
	expense_claim = {
		"doctype": "Expense Claim",
		"employee": employee,
		"payable_account": payable_account,
