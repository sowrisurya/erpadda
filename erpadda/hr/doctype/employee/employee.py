...
from vmraid.utils import getdate, validate_email_address, today, add_years, format_datetime, cstr
from vmraid.model.naming import set_name_by_naming_series
from vmraid import throw, _, scrub
...
from erpadda.hr.doctype.job_offer.job_offer import get_staffing_plan_detail

class EmployeeUserDisabledError(vmraid.ValidationError): pass
class EmployeeLeftValidationError(vmraid.ValidationError): pass
...
class Employee(NestedSet):
	...
	def update_user_permissions(self):
		...
		employee_user_permission_exists = vmraid.db.exists('User Permission', {
			'allow': 'Employee',
			'for_value': self.name,
			'user': self.user_id
		})
	...
	def validate_status(self):
		if self.status == 'Left':
			reports_to = vmraid.db.get_all('Employee',
				filters={'reports_to': self.name, 'status': "Active"},
				fields=['name','employee_name']
			)
			if reports_to:
				link_to_employees = [vmraid.utils.get_link_to_form('Employee', employee.name, label=employee.employee_name) for employee in reports_to]
				message = _("The following employees are currently still reporting to {0}:").format(vmraid.bold(self.employee_name))
	...
	def on_trash(self):
		...
		if vmraid.db.exists("Employee Transfer", {'new_employee_id': self.name, 'docstatus': 1}):
			emp_transfer = vmraid.get_doc("Employee Transfer", {'new_employee_id': self.name, 'docstatus': 1})
	...
	def validate_onboarding_process(self):
		employee_onboarding = vmraid.get_all("Employee Onboarding",
...
def get_timeline_data(doctype, name):
	...
	return dict(vmraid.db.sql('''select unix_timestamp(attendance_date), count(*)
...
def send_birthday_reminders():
	...
	if int(vmraid.db.get_single_value("HR Settings", "stop_birthday_reminders") or 0):
		return
...
def get_holiday_list_for_employee(employee, raise_exception=True):
	...
	if not holiday_list and raise_exception:
		vmraid.throw(_('Please set a default Holiday List for Employee {0} or Company {1}').format(employee, company))
...
def is_holiday(employee, date=None, raise_exception=True):
	...
	if holiday_list:
		return vmraid.get_all('Holiday List', dict(name=holiday_list, holiday_date=date)) and True or False
...
def get_all_employee_emails(company):
	...
	employee_list = vmraid.get_all('Employee',
		fields=['name','employee_name'],
		filters={
			'status': 'Active',
			'company': company
	...
	for employee in employee_list:
		if not employee:
			continue
		user, company_email, personal_email = vmraid.db.get_value('Employee',
...
def get_employee_emails(employee_list):
	...
	for employee in employee_list:
		if not employee:
			continue
		user, company_email, personal_email = vmraid.db.get_value('Employee', employee,
...
def get_children(doctype, parent=None, company=None, is_root=False, is_tree=False):
	...
	employees = vmraid.get_list(doctype, fields=fields,
		filters=filters, order_by='name')

	for employee in employees:
		is_expandable = vmraid.get_all(doctype, filters=[
