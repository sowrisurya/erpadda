...
def get_employees(date, department = None, branch = None, company = None):
	...
	employee_list = vmraid.get_list("Employee", fields=["employee", "employee_name"], filters=filters, order_by="employee_name")
	marked_employee = {}
	for emp in vmraid.get_list("Attendance", fields=["employee", "status"],
...
def mark_employee_attendance(employee_list, status, date, leave_type=None, company=None):
	...
	for employee in employee_list:
		...
		if not company:
			company = vmraid.db.get_value("Employee", employee['employee'], "Company")

		attendance=vmraid.get_doc(dict(
			doctype='Attendance',
