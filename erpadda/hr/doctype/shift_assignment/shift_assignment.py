...
from vmraid.utils import cint, cstr, date_diff, flt, formatdate, getdate, now_datetime, nowdate
...
class ShiftAssignment(Document):
	def validate(self):
		...
		if self.end_date and self.end_date <= self.start_date:
			vmraid.throw(_("End Date must not be lesser than Start Date"))
	...
	def throw_overlap_error(self, shift_details):
		...
		if shift_details.docstatus == 1 and shift_details.status == "Active":
			msg = _("Employee {0} already has Active Shift {1}: {2}").format(vmraid.bold(self.employee), vmraid.bold(self.shift_type), vmraid.bold(shift_details.name))
...
def add_assignments(events, start, end, conditions=None):
	...
	for d in records:
		...
		while daily_event_start <= daily_event_end:
			start_timing = vmraid.utils.get_datetime(daily_event_start)+ shift_timing_map[d.shift_type]['start_time']
			end_timing = vmraid.utils.get_datetime(daily_event_start)+ shift_timing_map[d.shift_type]['end_time']
...
def get_shift_type_timing(shift_types):
	shift_timing_map = {}
	data = vmraid.get_all("Shift Type", filters = {"name": ("IN", shift_types)}, fields = ['name', 'start_time', 'end_time'])
...
def get_employee_shift(employee, for_date=nowdate(), consider_default_shift=False, next_shift_direction=None):
	...
	shift_assignment_details = vmraid.db.get_value('Shift Assignment', {'employee':employee, 'start_date':('<=', for_date), 'docstatus': '1', 'status': "Active"}, ['shift_type', 'end_date'])
	...
	if shift_type_name:
		holiday_list_name = vmraid.db.get_value('Shift Type', shift_type_name, 'holiday_list')
	...
	if not shift_type_name and next_shift_direction:
		...
		if consider_default_shift and default_shift:
			...
			dates = vmraid.db.get_all('Shift Assignment',
				['start_date', 'end_date'],
