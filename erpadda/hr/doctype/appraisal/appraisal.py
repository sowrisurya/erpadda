...
class Appraisal(Document):
	...
	def validate_existing_appraisal(self):
		chk = vmraid.db.sql("""select name from `tabAppraisal` where employee=%s
			and (status='Submitted' or status='Completed')
			and ((start_date>=%s and start_date<=%s)
			or (end_date>=%s and end_date<=%s))""",
			(self.employee,self.start_date,self.end_date,self.start_date,self.end_date))
		if chk:
			vmraid.throw(_("Appraisal {0} created for Employee {1} in the given date range").format(chk[0][0], self.employee_name))
	...
	def calculate_total(self):
		...
		if int(total_w) != 100:
			vmraid.throw(_("Total weightage assigned should be 100%.<br>It is {0}").format(str(total_w) + "%"))

		if vmraid.db.get_value("Employee", self.employee, "user_id") != \
				vmraid.session.user and total == 0:
