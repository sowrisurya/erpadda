...
class DailyWorkSummary(Document):
	def send_mails(self, dws_group, emails):
		...
		vmraid.sendmail(recipients=emails,
	...
	def get_message_details(self):
		...
		dws_group = vmraid.get_doc('Daily Work Summary Group',
		...
		for d in replies:
			user = vmraid.db.get_values("User",
				{"email": d.sender},
			...
			try:
				if original_image:
					file_name = vmraid.get_list('File',
			...
			if d.text_content:
				d.content = vmraid.utils.md_to_html(
					EmailReplyParser.parse_reply(d.text_content)
				)

		did_not_reply = [(vmraid.db.get_value("User", {"email": email}, "full_name") or email)
...
def get_users_email(doc):
	return [d.email for d in doc.users
		if vmraid.db.get_value("User", d.user, "enabled")]
