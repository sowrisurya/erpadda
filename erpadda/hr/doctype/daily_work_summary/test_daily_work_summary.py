...
class TestDailyWorkSummary(unittest.TestCase):
	...
	def test_incoming(self):
		...
		daily_work_summary = vmraid.get_doc('Daily Work Summary',
			vmraid.get_all('Daily Work Summary')[0].name)
	...
	def setup_and_prepare_test(self, hour=None):
		...
		self.users = vmraid.get_all('User',
			fields=['email'],
			filters=dict(email=('!=', 'test@example.com')))
		self.setup_groups(hour)

		from erpadda.hr.doctype.daily_work_summary_group.daily_work_summary_group \
			import trigger_emails
		trigger_emails()

		# check if emails are created

		self.emails = vmraid.db.sql("""select r.recipient, q.message, q.message_id \
	...
	def setup_groups(self, hour=None):
		...
		groups = vmraid.get_doc(dict(doctype="Daily Work Summary Group",
