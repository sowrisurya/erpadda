...
from vmraid.utils import cint, getdate, get_datetime
from erpadda.hr.doctype.shift_assignment.shift_assignment import get_actual_start_end_datetime_of_shift, get_employee_shift
from erpadda.hr.doctype.employee_checkin.employee_checkin import mark_attendance_and_link_log, calculate_working_hours
...
class ShiftType(Document):
	...
	def process_auto_attendance(self):
		...
		logs = vmraid.db.get_list('Employee Checkin', fields="*", filters=filters, order_by="employee,time")
	...
	def mark_absent_for_dates_with_no_attendance(self, employee):
		...
		"""
		date_of_joining, relieving_date, employee_creation = vmraid.db.get_value("Employee", employee, ["date_of_joining", "relieving_date", "creation"])
	...
	def get_assigned_employee(self, from_date=None, consider_default_shift=False):
		...
		assigned_employees = vmraid.get_all('Shift Assignment', 'employee', filters, as_list=True)
		assigned_employees = [x[0] for x in assigned_employees]

		if consider_default_shift:
			filters = {'default_shift': self.name}
			default_shift_employees = vmraid.get_all('Employee', 'name', filters, as_list=True)
...
def get_filtered_date_list(employee, start_date, end_date, filter_attendance=True, holiday_list=None):
	...
	dates = vmraid.db.sql("""select * from
