...
class EmployeeTransfer(Document):
	def validate(self):
		if vmraid.get_value("Employee", self.employee, "status") == "Left":
			vmraid.throw(_("Cannot transfer Employee with status Left"))

	def before_submit(self):
		if getdate(self.transfer_date) > getdate():
			vmraid.throw(_("Employee Transfer cannot be submitted before Transfer Date "),
	...
	def on_cancel(self):
		...
		if self.create_new_employee_id:
			if self.new_employee_id:
				vmraid.throw(_("Please delete the Employee {0} to cancel this document").format(
