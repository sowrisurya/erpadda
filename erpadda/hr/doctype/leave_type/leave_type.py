...
class LeaveType(Document):
	def validate(self):
		if self.is_lwp:
			...
			if leave_allocation:
				vmraid.throw(_('Leave application is linked with leave allocations {0}. Leave application cannot be set as leave without pay').format(", ".join(leave_allocation))) #nosec
