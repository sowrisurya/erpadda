...
import vmraid
import unittest
from vmraid.utils import today, add_months
...
class TestLeaveEncashment(unittest.TestCase):
	def setUp(self):
		...
		leave_policy_assignments = create_assignment_for_multiple_employees([self.employee], vmraid._dict(data))
	...
	def test_creation_of_leave_ledger_entry_on_submit(self):
		...
		leave_ledger_entry = vmraid.get_all('Leave Ledger Entry', fields='*', filters=dict(transaction_name=leave_encashment.name))
		...
		vmraid.db.sql("Delete from `tabAdditional Salary` WHERE ref_docname = %s", (leave_encashment.name) )

		leave_encashment.cancel()
		self.assertFalse(vmraid.db.exists("Leave Ledger Entry", {'transaction_name':leave_encashment.name}))
