...
class ItemVariantExistsError(vmraid.ValidationError): pass
class InvalidItemAttributeValueError(vmraid.ValidationError): pass
class ItemTemplateCannotHaveStock(vmraid.ValidationError): pass
...
def validate_is_incremental(numeric_attribute, attribute, value, item):
	...
	if not (is_in_range and is_incremental):
		vmraid.throw(_("Value for Attribute {0} must be within the range of {1} to {2} in the increments of {3} for Item {4}")\
			.format(attribute, from_range, to_range, increment, item),
			InvalidItemAttributeValueError, title=_('Invalid Attribute'))

def validate_item_attribute_value(attributes_list, attribute, attribute_value, item, from_variant=True):
	allow_rename_attribute_value = vmraid.db.get_single_value('Item Variant Settings', 'allow_rename_attribute_value')
	if allow_rename_attribute_value:
		pass
	elif attribute_value not in attributes_list:
		if from_variant:
			vmraid.throw(_("{0} is not a valid Value for Attribute {1} of Item {2}.").format(
				vmraid.bold(attribute_value), vmraid.bold(attribute), vmraid.bold(item)), InvalidItemAttributeValueError, title=_("Invalid Value"))
		else:
			msg = _("The value {0} is already assigned to an existing Item {1}.").format(
				vmraid.bold(attribute_value), vmraid.bold(item))
			msg += "<br>" + _("To still proceed with editing this Attribute Value, enable {0} in Item Variant Settings.").format(vmraid.bold("Allow Rename Attribute Value"))
...
def get_attribute_values(item):
	if not vmraid.flags.attribute_values:
		...
		for t in vmraid.get_all('Item Variant Attribute',
...
def find_variant(template, args, variant_item_code=None):
	conditions = ["""(iv_attribute.attribute={0} and iv_attribute.attribute_value={1})"""\
		.format(vmraid.db.escape(key), vmraid.db.escape(cstr(value))) for key, value in args.items()]

	conditions = " or ".join(conditions)

	from erpadda.portal.product_configurator.utils import get_item_codes_by_attributes
...
def enqueue_multiple_variant_creation(item, args):
	...
	if total_variants < 10:
		...
		vmraid.enqueue("erpadda.controllers.item_variant.create_multiple_variants",
			item=item, args=args, now=vmraid.flags.in_test);
		return 'queued'
...
def make_variant_item_code(template_item_code, template_item_name, variant):
	...
	for attr in variant.attributes:
		item_attribute = vmraid.db.sql("""select i.numeric_values, v.abbr
