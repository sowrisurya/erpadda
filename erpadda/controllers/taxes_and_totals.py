...
class calculate_taxes_and_totals(object):
		...
		for item in self.doc.get('items'):
			if item.item_code and item.get('item_tax_template'):
				...
				if item.item_tax_template not in taxes:
					vmraid.throw(_("Row {0}: Invalid Item Tax Template for item {1}").format(
	...
	def calculate_total_advance(self):
		if self.doc.docstatus < 2:
			...
			if invoice_total > 0 and self.doc.total_advance > invoice_total:
				vmraid.throw(_("Advance amount cannot be greater than {0} {1}")
	...
	def update_paid_amount_for_return(self, total_amount_to_pay):
		default_mode_of_payment = vmraid.db.get_value('POS Payment Method',
	...
	for tax in taxes:
		...
		if item_tax_map:
			for item_code, tax_data in item_tax_map.items():
				...
				itemised_tax[item_code][tax.description] = vmraid._dict(dict(
					tax_rate = tax_rate,
