...
def get_data(filters, conditions):
	...
	year_start_date, year_end_date = vmraid.db.get_value("Fiscal Year",
	...
	if filters.get("group_by"):
		...
		if filters.get('based_on') in ['Item','Customer','Supplier']:
			...
			inc = 1
		data1 = vmraid.db.sql(""" select %s from `tab%s` t1, `tab%s Item` t2 %s
		...
		for d in range(len(data1)):
			...
			row = vmraid.db.sql("""select DISTINCT(%s) from `tab%s` t1, `tab%s Item` t2 %s
			...
			for i in range(len(row)):
				...
				row1 = vmraid.db.sql(""" select %s , %s from `tab%s` t1, `tab%s Item` t2 %s
							where t2.parent = t1.name and t1.company = %s and %s between %s and %s
		...
		data = vmraid.db.sql(""" select %s from `tab%s` t1, `tab%s Item` t2 %s
					where t2.parent = t1.name and t1.company = %s and %s between %s and %s and
					t1.docstatus = 1 %s %s
					group by %s
				""" %
