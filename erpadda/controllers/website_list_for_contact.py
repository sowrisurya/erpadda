...
def get_list_context(context=None):
	return {
		...
		"currency_symbols": json.dumps(dict(vmraid.db.sql("""select name, symbol
...
def get_list_for_transactions(doctype, txt, filters, limit_start, limit_page_length=20,
	...
	if or_filters:
		for r in vmraid.get_list(doctype, fields=fields,filters=filters, or_filters=or_filters,
...
def rfq_transaction_list(parties_doctype, doctype, parties, limit_start, limit_page_length):
	data = vmraid.db.sql("""select distinct parent as name, supplier from `tab{doctype}`
