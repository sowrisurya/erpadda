...
class OverAllowanceError(vmraid.ValidationError): pass
...
class StatusUpdater(Document):
		...
		if self.doctype in status_map:
			...
			for s in sl:
				if not s[1]:
					...
					if vmraid.safe_eval(s[1][5:], None, { "self": self.as_dict(), "getdate": getdate,
							"nowdate": nowdate, "get_value": vmraid.db.get_value }):
						self.status = s[0]
						break
		...
		for args in self.status_updater:
			...
			for d in self.get_all_children():
				...
				if d.doctype == args['source_dt'] and d.get(args["join_field"]):
					...
					item = vmraid.db.sql("""select item_code, `{target_ref_field}`,
	...
	def check_overflow_with_allowance(self, item, args):
		...
		role_allowed_to_over_deliver_receive = vmraid.db.get_single_value('Stock Settings', 'role_allowed_to_over_deliver_receive')
		...
		if overflow_percent - allowance > 0.01 and role not in vmraid.get_roles():
	...
	def limits_crossed_error(self, args, item, qty_or_amount):
		...
		vmraid.throw(_('This document is over limit by {0} {1} for item {4}. Are you making another {3} against the same {2}?')
			.format(
	...
	def _update_children(self, args, update_modified):
		...
		for d in self.get_all_children():
			...
			if args.get('second_source_dt') and args.get('second_source_field') \
				...
				args['second_source_condition'] = vmraid.db.sql(""" select ifnull((select sum(%(second_source_field)s)
			...
			if args['detail_id']:
				...
				args["source_dt_value"] = vmraid.db.sql("""
				...
				vmraid.db.sql("""update `tab%(target_dt)s`
	...
	def _update_percent_field(self, args, update_modified=True):
		...
		if args.get('target_parent_field'):
			vmraid.db.sql("""update `tab%(target_parent_dt)s`
	...
	def _update_modified(self, args, update_modified):
		...
		if update_modified:
			args['update_modified'] = ', modified = now(), modified_by = {0}'\
				.format(vmraid.db.escape(vmraid.session.user))
	...
	def update_billing_status(self, zero_amount_refdoc, ref_dt, ref_fieldname):
		for ref_dn in zero_amount_refdoc:
			ref_doc_qty = flt(vmraid.db.sql("""select ifnull(sum(qty), 0) from `tab%s Item`
				where parent=%s""" % (ref_dt, '%s'), (ref_dn))[0][0])

			billed_qty = flt(vmraid.db.sql("""select ifnull(sum(qty), 0)
...
def get_allowance_for(item_code, item_allowance=None, global_qty_allowance=None, global_amount_allowance=None, qty_or_amount="qty"):
	...
	qty_allowance, over_billing_allowance = \
		vmraid.db.get_value('Item', item_code, ['over_delivery_receipt_allowance', 'over_billing_allowance'])

	if qty_or_amount == "qty" and not qty_allowance:
		if global_qty_allowance == None:
			global_qty_allowance = flt(vmraid.db.get_single_value('Stock Settings', 'over_delivery_receipt_allowance'))
		qty_allowance = global_qty_allowance
	elif qty_or_amount == "amount" and not over_billing_allowance:
		if global_amount_allowance == None:
			global_amount_allowance = flt(vmraid.db.get_single_value('Accounts Settings', 'over_billing_allowance'))
