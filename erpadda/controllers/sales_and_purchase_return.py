...
from vmraid.utils import flt, get_datetime, format_datetime

class StockOverReturnError(vmraid.ValidationError): pass
	...
	if not vmraid.db.exists(doc.doctype, doc.return_against):
		...
		if ref_doc.company == doc.company and ref_doc.get(party_type) == doc.get(party_type) and ref_doc.docstatus == 1:
			...
			if get_datetime(return_posting_datetime) < get_datetime(ref_posting_datetime):
				vmraid.throw(_("Posting timestamp must be after {0}").format(format_datetime(ref_posting_datetime)))

			# validate same exchange rate
			if doc.conversion_rate != ref_doc.conversion_rate:
				vmraid.throw(_("Exchange Rate must be same as {0} {1} ({2})")
					.format(doc.doctype, doc.return_against, ref_doc.conversion_rate))

			# validate update stock
			if doc.doctype == "Sales Invoice" and doc.update_stock and not ref_doc.update_stock:
					vmraid.throw(_("'Update Stock' can not be checked because items are not delivered via {0}")
...
def validate_returned_items(doc):
	...
	for d in vmraid.db.sql("""select {0} from `tab{1} Item` where parent = %s"""
		.format(select_fields, doc.doctype), doc.return_against, as_dict=1):
			valid_items = get_ref_item_dict(valid_items, d)

	if doc.doctype in ("Delivery Note", "Sales Invoice"):
		for d in vmraid.db.sql("""select item_code, qty, serial_no, batch_no from `tabPacked Item`
	...
	for d in doc.get("items"):
		if d.item_code and (flt(d.qty) < 0 or flt(d.get('received_qty')) < 0):
			if d.item_code not in valid_items:
				vmraid.throw(_("Row # {0}: Returned Item {1} does not exist in {2} {3}")
				...
				if ref.rate and doc.doctype in ("Delivery Note", "Sales Invoice") and flt(d.rate) > ref.rate:
					vmraid.throw(_("Row # {0}: Rate cannot be greater than the rate used in {1} {2}")
						.format(d.idx, doc.doctype, doc.return_against))

				elif ref.batch_no and d.batch_no not in ref.batch_no:
					vmraid.throw(_("Row # {0}: Batch No must be same as {1} {2}")
						.format(d.idx, doc.doctype, doc.return_against))
					...
					if not d.serial_no:
						...
						for s in serial_nos:
							if s not in ref.serial_no:
								vmraid.throw(_("Row # {0}: Serial No {1} does not match with {2} {3}")
									.format(d.idx, s, doc.doctype, doc.return_against))

				if warehouse_mandatory and vmraid.db.get_value("Item", d.item_code, "is_stock_item") \
...
def validate_quantity(doc, args, ref, valid_items, already_returned_items):
	...
	stock_qty_precision = get_field_precision(vmraid.get_meta(doc.doctype + " Item")
	...
	for column in fields:
		...
		if reference_qty:
			if flt(args.get(column)) > 0:
				...
				vmraid.throw(_("Item {0} has already been returned")
					.format(args.item_code), StockOverReturnError)
			elif abs(flt(current_stock_qty, stock_qty_precision)) > max_returnable_qty:
				vmraid.throw(_("Row # {0}: Cannot return more than {1} for Item {2}")
					.format(args.idx, max_returnable_qty, args.item_code), StockOverReturnError)

def get_ref_item_dict(valid_items, ref_item_row):
	from erpadda.stock.doctype.serial_no.serial_no import get_serial_nos

	valid_items.setdefault(ref_item_row.item_code, vmraid._dict({
		"qty": 0,
		"rate": 0,
		"stock_qty": 0,
		"rejected_qty": 0,
...
def get_already_returned_items(doc):
	...
	for d in data:
		items.setdefault(d.item_code, vmraid._dict({
			"qty": d.get("qty"),
...
def get_returned_qty_map_for_row(row_name, doctype):
	...
	data = vmraid.db.get_list(doctype,
		fields = fields,
		filters = [
			[doctype, "docstatus", "=", 1],
			[doctype, "is_return", "=", 1],
...
def make_return_doc(doctype, source_name, target_doc=None):
	...
	default_warehouse_for_sales_return = vmraid.db.get_value("Company", company, "default_warehouse_for_sales_return")
...
def get_rate_for_return(voucher_type, voucher_no, item_code, return_against=None,
	...
	return flt(vmraid.db.get_value("Stock Ledger Entry", filters, select_field))
...
def get_filters(voucher_type, voucher_no, voucher_detail_no, return_against, item_code, return_against_item_field, item_row):
	...
	if item_row:
		...
		reference_voucher_detail_no = vmraid.db.get_value(voucher_type + " Item", voucher_detail_no, return_against_item_field)
