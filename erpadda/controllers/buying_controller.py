...
from vmraid.utils import flt,cint, cstr, getdate
...
from erpadda.buying.utils import validate_for_items, update_last_purchase_rate
from erpadda.stock.stock_ledger import get_valuation_rate
from erpadda.stock.doctype.stock_entry.stock_entry import get_used_alternative_items
from erpadda.stock.doctype.serial_no.serial_no import get_auto_serial_nos, auto_make_serial_nos, get_serial_nos
...
class BuyingController(StockController):
	...
	def set_supplier_from_item_default(self):
		if self.meta.get_field("supplier") and not self.supplier:
			for d in self.get("items"):
				supplier = vmraid.db.get_value("Item Default",
	...
	def validate_asset_return(self):
		...
		not_cancelled_asset = [d.name for d in vmraid.db.get_all("Asset", {
			purchase_doc_field: self.return_against,
			"docstatus": 1
		})]
		if self.is_return and len(not_cancelled_asset):
			vmraid.throw(_("{} has submitted assets linked to it. You need to cancel the assets to create purchase return.")
	...
	def set_landed_cost_voucher_amount(self):
		for d in self.get("items"):
			lc_voucher_data = vmraid.db.sql("""select sum(applicable_charges), cost_center
	...
	def validate_from_warehouse(self):
		for item in self.get('items'):
			if item.get('from_warehouse') and (item.get('from_warehouse') == item.get('warehouse')):
				vmraid.throw(_("Row #{0}: Accepted Warehouse and Supplier Warehouse cannot be same").format(item.idx))

			if item.get('from_warehouse') and self.get('is_subcontracted') == 'Yes':
				vmraid.throw(_("Row #{0}: Cannot select Supplier Warehouse while suppling raw materials to subcontractor").format(item.idx))
			...
			if not cint(self.get("is_return")):
				...
				if self.is_internal_transfer():
					if rate != d.rate:
						...
						d.discount_amount = 0
						vmraid.msgprint(_("Row {0}: Item rate has been updated as per valuation rate since its an internal stock transfer")
	...
	def get_supplied_items_cost(self, item_row_id, reset_outgoing_rate=True):
		...
		for d in self.get("supplied_items"):
			if d.reference_name == item_row_id:
				if reset_outgoing_rate and vmraid.db.get_value('Item', d.rm_item_code, 'is_stock_item'):
	...
	def validate_for_subcontracting(self):
		if not self.is_subcontracted and self.sub_contracted_items:
			vmraid.throw(_("Please enter 'Is Subcontracted' as Yes or No"))
			...
			if self.doctype == "Purchase Order":
				for supplied_item in self.get("supplied_items"):
					if not supplied_item.reserve_warehouse:
						vmraid.throw(_("Reserved Warehouse is mandatory for Item {0} in Raw Materials supplied").format(vmraid.bold(supplied_item.rm_item_code)))
	...
	def update_raw_materials_supplied_based_on_stock_entries(self):
		...
		for item in self.get('items'):
			...
			if not fg_yet_to_be_received:
				vmraid.throw(_("Row #{0}: Item {1} is already fully received in Purchase Order {2}")
					.format(item.idx, vmraid.bold(item.item_code),
						vmraid.utils.get_link_to_form("Purchase Order", item.purchase_order)),
			...
			for raw_material in transferred_raw_materials + non_stock_items:
				...
				if fg_yet_to_be_received == item.qty:
					...
					if vmraid.get_cached_value('UOM', raw_material.stock_uom, 'must_be_whole_number'):
	...
	def update_raw_materials_supplied_based_on_bom(self, item, raw_material_table):
		...
		item_wh = vmraid._dict(vmraid.db.sql("""select i.item_code, id.default_warehouse
		...
		for bom_item in bom_items:
			if self.doctype == "Purchase Order":
				...
				if vmraid.db.get_value("Warehouse", reserve_warehouse, "company") != self.company:
			...
			if self.doctype in ["Purchase Receipt", "Purchase Invoice"]:
				...
				if item.batch_no and vmraid.db.get_value("Item", rm.rm_item_code, "has_batch_no") and not rm.batch_no:
	...
	def sub_contracted_items(self):
		if not hasattr(self, "_sub_contracted_items"):
			...
			item_codes = list(set(item.item_code for item in
				self.get("items")))
			if item_codes:
				items = vmraid.get_all('Item', filters={
					'name': ['in', item_codes],
		...
		for d in self.get("items"):
			if flt(d.rejected_qty) and not d.rejected_warehouse:
				...
				if not d.rejected_warehouse:
					vmraid.throw(_("Row #{0}: Rejected Warehouse is mandatory against rejected Item {1}").format(d.idx, d.item_code))
	...
	def validate_accepted_rejected_qty(self):
		for d in self.get("items"):
			...
			if (flt(val, d.precision("received_qty")) != flt(d.received_qty, d.precision("received_qty"))):
				vmraid.throw(_("Accepted + Rejected Qty must be equal to Received quantity for Item {0}").format(d.item_code))
	...
	def validate_negative_quantity(self, item_row, field_list):
		...
		for fieldname in field_list:
			if flt(item_row[fieldname]) < 0:
				vmraid.throw(_("Row #{0}: {1} can not be negative for item {2}").format(item_row['idx'],
					vmraid.get_meta(item_row.doctype).get_label(fieldname), item_row['item_code']))
		...
		for po, po_item_rows in po_map.items():
			if po and po_item_rows:
				...
				if po_obj.status in ["Closed", "Cancelled"]:
					vmraid.throw(_("{0} {1} is cancelled or closed").format(_("Purchase Order"), po),
				...
				if item_data.get('auto_create_assets'):
					...
					if item_data.get('asset_naming_series'):
						...
						if len(created_assets) > 5:
							...
							messages.append(_('{} Assets created for {}').format(len(created_assets), vmraid.bold(d.item_code)))
						else:
							...
							messages.append(
								_('Asset{} {assets_link} created for {}').format(is_plural, vmraid.bold(d.item_code), assets_link=assets_link)
							)
					else:
						vmraid.throw(_("Row {}: Asset Naming Series is mandatory for the auto creation for item {}")
							.format(d.idx, vmraid.bold(d.item_code)))
				else:
					messages.append(_("Assets not created for {0}. You will have to create asset manually.")
						.format(vmraid.bold(d.item_code)))
	...
	def make_asset(self, row):
		...
		item_data = vmraid.db.get_value('Item',
				...
				for asset in assets:
					...
					if self.docstatus in [0, 1] and not asset.get(field):
						...
						if asset.docstatus == 1 and delete_asset:
							vmraid.throw(_('Cannot cancel this document as it is linked with submitted asset {0}. Please cancel it to continue.')
...
def get_items_from_bom(item_code, bom, exploded_item=1):
	...
	bom_items = vmraid.db.sql("""select t2.item_code, t2.name,
...
def get_subcontracted_raw_materials_from_se(purchase_order, fg_item):
	...
	"""
	raw_materials = vmraid.db.multisql({
...
def get_backflushed_subcontracted_raw_materials(purchase_orders):
	purchase_receipts = vmraid.get_all("Purchase Receipt Item",
	...
	backflushed_raw_materials_map = vmraid._dict()
	for args, references in iteritems(distinct_purchase_receipts):
		...
		for data in purchase_receipt_supplied_items:
			...
			if pr_key not in backflushed_raw_materials_map:
				backflushed_raw_materials_map.setdefault(pr_key, vmraid._dict({
					"qty": 0.0,
					"serial_no": [],
					"batch_no": [],
					"consumed_batch": {}
...
def get_supplied_items(item_code, purchase_receipt, references):
	return vmraid.get_all("Purchase Receipt Item Supplied",
		fields=["rm_item_code", "main_item_code", "consumed_qty", "serial_no", "batch_no"],
		filters={"main_item_code": item_code, "parent": purchase_receipt, "reference_name": ("in", references)})

def get_asset_item_details(asset_items):
	asset_items_data = {}
	for d in vmraid.get_all('Item', fields = ["name", "auto_create_assets", "asset_naming_series"],
...
def validate_item_type(doc, fieldname, message):
	...
	invalid_items = [d[0] for d in vmraid.db.sql("""
...
def set_serial_nos(raw_material, consumed_serial_nos, qty):
	...
	if serial_nos and qty <= len(serial_nos):
		raw_material.serial_no = '\n'.join(list(serial_nos)[0:vmraid.utils.cint(qty)])
...
def get_transferred_batch_qty_map(purchase_order, fg_item):
	...
	transferred_batches = vmraid.db.sql("""
		SELECT
			sed.batch_no,
			SUM(sed.qty) AS qty,
			sed.item_code,
...
def get_backflushed_batch_qty_map(purchase_order, fg_item):
	...
	backflushed_batches = vmraid.db.sql("""
		SELECT
			pris.batch_no,
