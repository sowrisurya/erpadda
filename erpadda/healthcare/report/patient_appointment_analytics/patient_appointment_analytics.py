...
class Analytics(object):
	...
	def get_appointments_based_on_healthcare_practitioner(self):
		...
		self.entries = vmraid.db.get_all('Patient Appointment',
	...
	def get_periodic_data(self):
		self.appointment_periodic_data = vmraid._dict()

		for d in self.entries:
			period = self.get_period(d.get('appointment_date'))
			if self.filters.tree_type == 'Healthcare Practitioner':
				self.appointment_periodic_data.setdefault(d.practitioner, vmraid._dict()).setdefault(period, 0.0)
				self.appointment_periodic_data[d.practitioner][period] += 1

			elif self.filters.tree_type == 'Medical Department':
				self.appointment_periodic_data.setdefault(d.department, vmraid._dict()).setdefault(period, 0.0)
