...
from vmraid.utils import getdate, get_timespan_date_range
import json

@vmraid.whitelist()
def get_therapy_sessions_count(patient):
	total = vmraid.db.count('Therapy Session', filters={
		'docstatus': 1,
		'patient': patient
	})

	month_start = datetime.today().replace(day=1)
	this_month = vmraid.db.count('Therapy Session', filters={
...
def get_therapy_sessions_distribution_data(patient, field):
	if field == 'therapy_type':
		result = vmraid.db.get_all('Therapy Session',
		...
		data = vmraid.db.get_all('Therapy Session',  filters={
			'docstatus': 1,
			'patient': patient
		}, as_list=True)
		therapy_sessions = [entry[0] for entry in data]

		result = vmraid.db.get_all('Exercise',
			filters = {
