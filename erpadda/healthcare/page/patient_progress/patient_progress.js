vmraid.pages['patient-progress'].on_page_load = function(wrapper) {
...
class PatientProgress {
	...
	show() {
		vmraid.breadcrumbs.add('Healthcare');
		this.sidebar.empty();

		let me = this;
		let patient = vmraid.ui.form.make_control({
			parent: me.sidebar,
			df: {
				fieldtype: 'Link',
				options: 'Patient',
		...
		if (vmraid.route_options && !this.patient) {
			patient.set_value(vmraid.route_options.patient);
			this.patient_id = vmraid.route_options.patient;
		}
	...
	make_patient_profile() {
		...
		this.main_section.empty().append(vmraid.render_template('patient_progress'));
	...
	get_patient_info() {
		return vmraid.xcall('vmraid.client.get', {
			doctype: 'Patient',
	...
	get_therapy_sessions_count() {
		return vmraid.xcall(
			'erpadda.healthcare.page.patient_progress.patient_progress.get_therapy_sessions_count', {
	...
	render_patient_details() {
		this.get_patient_info().then(() => {
			this.get_therapy_sessions_count().then(() => {
				$('.patient-info').empty().append(vmraid.render_template('patient_progress_sidebar', {
	...
	setup_patient_profile_links() {
		this.wrapper.find('.patient-profile-link').on('click', () => {
			vmraid.set_route('Form', 'Patient', this.patient_id);
		});
		...
		this.wrapper.find('.therapy-plan-link').on('click', () => {
			...
			vmraid.set_route('List', 'Therapy Plan');
		});
		...
		this.wrapper.find('.patient-history').on('click', () => {
			...
			vmraid.set_route('patient_history');
		});
	}

	render_heatmap() {
		this.heatmap = new vmraid.Chart('.patient-heatmap', {
			type: 'heatmap',
	...
	update_heatmap_data(date_from) {
		vmraid.xcall('erpadda.healthcare.page.patient_progress.patient_progress.get_patient_heatmap_data', {
			patient: this.patient_id,
			...
			let filters = [
				{
					...
					options: vmraid.dashboard_utils.get_years_since_creation(this.patient.creation),
					action: (selected_item) => {
						this.update_heatmap_data(vmraid.datetime.obj_to_str(selected_item));
					}
				},
			];
			vmraid.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.heatmap-container');
		});
	}

	render_percentage_chart(field, title) {
		vmraid.xcall(
			'erpadda.healthcare.page.patient_progress.patient_progress.get_therapy_sessions_distribution_data', {
				patient: this.patient_id,
				field: field
			}
		).then(chart => {
			if (chart.labels.length) {
				this.percentage_chart = new vmraid.Chart('.therapy-session-percentage-chart', {
					title: title,
					type: 'percentage',
					data: {
						labels: chart.labels,
	...
	create_percentage_chart_filters() {
		...
		vmraid.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.percentage-chart-container');
	}
	...
	create_time_span_filters(action_method, parent) {
		...
		vmraid.dashboard_utils.render_chart_filters(filters, 'chart-filter', chart_control, 1);
	}
	...
	render_date_range_fields(action_method, parent) {
		...
		if (!date_field.length) {
			...
			let date_range_field = vmraid.ui.form.make_control({
				df: {
					fieldtype: 'DateRange',
					fieldname: 'from_date',
	...
	show_therapy_progress() {
		let me = this;
		let therapy_type = vmraid.ui.form.make_control({
	...
	render_therapy_progress_chart(time_span='Last Month') {
		...
		vmraid.xcall(
			'erpadda.healthcare.page.patient_progress.patient_progress.get_therapy_progress_data', {
				patient: this.patient_id,
			...
			if (!chart.labels.length) {
				...
				if (!this.therapy_line_chart) {
					this.therapy_line_chart = new vmraid.Chart(parent, {
						type: 'axis-mixed',
						height: 250,
						data: data,
						lineOptions: {
	...
	show_assessment_results() {
		let me = this;
		let assessment_template = vmraid.ui.form.make_control({
	...
	render_assessment_result_chart(time_span='Last Month') {
		...
		vmraid.xcall(
			'erpadda.healthcare.page.patient_progress.patient_progress.get_patient_assessment_data', {
				patient: this.patient_id,
			...
			if (!chart.labels.length) {
				...
				if (!this.assessment_line_chart) {
					this.assessment_line_chart = new vmraid.Chart(parent, {
						type: 'axis-mixed',
						height: 250,
						data: data,
						lineOptions: {
	...
	show_therapy_assessment_correlation() {
		let me = this;
		let assessment = vmraid.ui.form.make_control({
	...
	render_therapy_assessment_correlation_chart(time_span='Last Month') {
		...
		vmraid.xcall(
			'erpadda.healthcare.page.patient_progress.patient_progress.get_therapy_assessment_correlation_data', {
				patient: this.patient_id,
			...
			if (!chart.labels.length) {
				...
				if (!this.correlation_chart) {
					this.correlation_chart = new vmraid.Chart(parent, {
						type: 'axis-mixed',
						height: 300,
						data: data,
						axisOptions: {
	...
	show_assessment_parameter_progress() {
		let me = this;
		let parameter = vmraid.ui.form.make_control({
	...
	render_assessment_parameter_progress_chart(time_span='Last Month') {
		...
		vmraid.xcall(
			'erpadda.healthcare.page.patient_progress.patient_progress.get_assessment_parameter_data', {
				patient: this.patient_id,
			...
			if (!chart.labels.length) {
				...
				if (!this.parameter_chart) {
					this.parameter_chart = new vmraid.Chart(parent, {
						type: 'line',
						height: 250,
						data: data,
						lineOptions: {
