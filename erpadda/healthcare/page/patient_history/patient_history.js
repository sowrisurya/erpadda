vmraid.provide('vmraid.patient_history');
vmraid.pages['patient_history'].on_page_load = function(wrapper) {
	let me = this;
	...
	let pid = '';
	page.main.html(vmraid.render_template('patient_history', {}));
	...
	this.page.main.on('click', '.btn-more', function() {
		...
		if (me.page.main.find('.'+docname).parent().find('.document-html').attr('data-fetched') == '1') {
			...
			if (doctype && docname) {
				...
				vmraid.call({
					method: 'erpadda.healthcare.utils.render_doc_as_html',
					args:{
						doctype: doctype,
						docname: docname,
...
let setup_filters = function(patient, me) {
	...
	vmraid.xcall(
		'erpadda.healthcare.page.patient_history.patient_history.get_patient_history_doctypes'
	).then(document_types => {
		let doctype_filter = vmraid.ui.form.make_control({
...
let get_documents = function(patient, me, document_types="", selected_date_range="") {
	...
	vmraid.call({
		'method': 'erpadda.healthcare.page.patient_history.patient_history.get_feed',
		args: filters,
		callback: function(r) {
			let data = r.message;
			if (data.length) {
...
let add_to_records = function(me, data) {
	...
	for (i=0; i<data.length; i++) {
		if (data[i].reference_doctype) {
			...
			if (vmraid.user_info(data[i].owner).image) {
				data[i].imgsrc = vmraid.utils.get_file_link(vmraid.user_info(data[i].owner).image);
			} else {
				data[i].imgsrc = false;
			}
			...
			time_line_heading += data[i].reference_doctype + " - " +
				`<a onclick="vmraid.set_route('Form', '${data[i].reference_doctype}', '${data[i].reference_name}');">
...
let add_date_separator = function(data) {
	let date = vmraid.datetime.str_to_obj(data.communication_date);
	let pdate = '';
	let diff = vmraid.datetime.get_day_diff(vmraid.datetime.get_today(), vmraid.datetime.obj_to_str(date));
	...
	if (diff < 1) {
		...
		pdate = __('on ') + vmraid.datetime.global_date_format(date);
	}
	data.date_sep = pdate;
	return data;
};
...
let show_patient_vital_charts = function(patient, me, btn_show_id, pts, title) {
	vmraid.call({
		method: 'erpadda.healthcare.utils.get_patient_vitals',
		args:{
			patient: patient
		},
		callback: function(r) {
			if (r.message) {
				...
				new vmraid.Chart('.patient_vital_charts', {
					data: {
						labels: labels,
						datasets: datasets
					},
