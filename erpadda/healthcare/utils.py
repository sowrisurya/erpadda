...
from vmraid.utils import time_diff_in_hours, rounded
from six import string_types
from erpadda.healthcare.doctype.healthcare_settings.healthcare_settings import get_income_account
from erpadda.healthcare.doctype.fee_validity.fee_validity import create_fee_validity
from erpadda.healthcare.doctype.lab_test.lab_test import create_multiple

@vmraid.whitelist()
def get_healthcare_services_to_invoice(patient, company):
	patient = vmraid.get_doc('Patient', patient)
	items_to_invoice = []
	if patient:
...
def validate_customer_created(patient):
	if not vmraid.db.get_value('Patient', patient.name, 'customer'):
...
def get_appointments_to_invoice(patient, company):
	...
	patient_appointments = vmraid.get_list(
			'Patient Appointment',
			fields = '*',
	...
	for appointment in patient_appointments:
		...
		if appointment.procedure_template:
			if vmraid.db.get_value('Clinical Procedure Template', appointment.procedure_template, 'is_billable'):
		...
		# Consultation Appointments, should check fee validity
			...
			if vmraid.db.get_single_value('Healthcare Settings', 'enable_free_follow_ups') and \
				vmraid.db.exists('Fee Validity Reference', {'appointment': appointment.name}):
			...
			if not encounter.appointment:
				...
				if encounter.practitioner:
					if encounter.inpatient_record and \
						vmraid.db.get_single_value('Healthcare Settings', 'do_not_bill_inpatient_encounters'):
						continue
...
def get_lab_tests_to_invoice(patient, company):
	...
	lab_tests = vmraid.get_list(
		'Lab Test',
		fields=['name', 'template'],
		filters={'patient': patient.name, 'company': company, 'invoiced': False, 'docstatus': 1}
	)
	for lab_test in lab_tests:
		item, is_billable = vmraid.get_cached_value('Lab Test Template', lab_test.template, ['item', 'is_billable'])
		if is_billable:
	...
	lab_prescriptions = vmraid.db.sql(
		'''
			SELECT
	...
	for prescription in lab_prescriptions:
		item, is_billable = vmraid.get_cached_value('Lab Test Template', prescription.lab_test_code, ['item', 'is_billable'])
...
def get_clinical_procedures_to_invoice(patient, company):
	...
	for procedure in procedures:
		if not procedure.appointment:
			item, is_billable = vmraid.get_cached_value('Clinical Procedure Template', procedure.procedure_template, ['item', 'is_billable'])
		...
		if procedure.invoice_separately_as_consumables and procedure.consume_stock \
			...
			service_item = vmraid.db.get_single_value('Healthcare Settings', 'clinical_procedure_consumable_item')
			if not service_item:
				...
				vmraid.throw(msg, title=_('Missing Configuration'))
	...
	procedure_prescriptions = vmraid.db.sql(
		'''
			SELECT
	...
	for prescription in procedure_prescriptions:
		item, is_billable = vmraid.get_cached_value('Clinical Procedure Template', prescription.procedure, ['item', 'is_billable'])
		if is_billable:
			clinical_procedures_to_invoice.append({
...
def get_inpatient_services_to_invoice(patient, company):
	services_to_invoice = []
	inpatient_services = vmraid.db.sql(
		'''
			SELECT
				io.*
			FROM
	...
	for inpatient_occupancy in inpatient_services:
		service_unit_type = vmraid.db.get_value('Healthcare Service Unit', inpatient_occupancy.service_unit, 'service_unit_type')
		service_unit_type = vmraid.get_cached_doc('Healthcare Service Unit Type', service_unit_type)
...
def get_therapy_plans_to_invoice(patient, company):
	...
	for plan in therapy_plans:
		therapy_plans_to_invoice.append({
			...
			'service': vmraid.db.get_value('Therapy Plan Template', plan.therapy_plan_template, 'linked_item')
		})
...
def get_therapy_sessions_to_invoice(patient, company):
	...
	for therapy in therapy_sessions:
		if not therapy.appointment:
			if therapy.therapy_type and vmraid.db.get_value('Therapy Type', therapy.therapy_type, 'is_billable'):
				therapy_sessions_to_invoice.append({
					'reference_type': 'Therapy Session',
					'reference_name': therapy.name,
					'service': vmraid.db.get_value('Therapy Type', therapy.therapy_type, 'item')
				})
...
def get_appointment_type_service_item(appointment_type, department, is_inpatient):
	from erpadda.healthcare.doctype.appointment_type.appointment_type import get_service_item_based_on_department
...
def get_practitioner_service_item(practitioner, is_inpatient):
	...
	if is_inpatient:
		service_item, practitioner_charge = vmraid.db.get_value('Healthcare Practitioner', practitioner, ['inpatient_visit_charge_item', 'inpatient_visit_charge'])
	else:
		service_item, practitioner_charge = vmraid.db.get_value('Healthcare Practitioner', practitioner, ['op_consulting_charge_item', 'op_consulting_charge'])
...
def get_healthcare_service_item(is_inpatient):
	...
	if is_inpatient:
		service_item = vmraid.db.get_single_value('Healthcare Settings', 'inpatient_visit_charge_item')
	else:
		service_item = vmraid.db.get_single_value('Healthcare Settings', 'op_consulting_charge_item')
...
def get_practitioner_charge(practitioner, is_inpatient):
	if is_inpatient:
		practitioner_charge = vmraid.db.get_value('Healthcare Practitioner', practitioner, 'inpatient_visit_charge')
	else:
		practitioner_charge = vmraid.db.get_value('Healthcare Practitioner', practitioner, 'op_consulting_charge')
	...
	if doc.items:
		for item in doc.items:
			if item.get('reference_dt') and item.get('reference_dn'):
				if vmraid.get_meta(item.reference_dt).has_field('invoiced'):
					set_invoiced(item, method, doc.name)

	if method=='on_submit' and vmraid.db.get_single_value('Healthcare Settings', 'create_lab_test_on_si_submit'):
...
def set_invoiced(item, method, ref_invoice=None):
	...
	if item.reference_dt == 'Clinical Procedure':
		...
		if service_item == item.item_code:
			vmraid.db.set_value(item.reference_dt, item.reference_dn, 'consumption_invoiced', invoiced)
		else:
			vmraid.db.set_value(item.reference_dt, item.reference_dn, 'invoiced', invoiced)
	else:
		vmraid.db.set_value(item.reference_dt, item.reference_dn, 'invoiced', invoiced)

	if item.reference_dt == 'Patient Appointment':
		if vmraid.db.get_value('Patient Appointment', item.reference_dn, 'procedure_template'):
...
def validate_invoiced_on_submit(item):
	if item.reference_dt == 'Clinical Procedure' and \
		vmraid.db.get_single_value('Healthcare Settings', 'clinical_procedure_consumable_item') == item.item_code:
		is_invoiced = vmraid.db.get_value(item.reference_dt, item.reference_dn, 'consumption_invoiced')
	else:
		is_invoiced = vmraid.db.get_value(item.reference_dt, item.reference_dn, 'invoiced')
	if is_invoiced:
		vmraid.throw(_('The item referenced by {0} - {1} is already invoiced').format(
			item.reference_dt, item.reference_dn))


def manage_prescriptions(invoiced, ref_dt, ref_dn, dt, created_check_field):
	created = vmraid.db.get_value(ref_dt, ref_dn, created_check_field)
	if created:
		# Fetch the doc created for the prescription
		doc_created = vmraid.db.get_value(dt, {'prescription': ref_dn})
		vmraid.db.set_value(dt, doc_created, 'invoiced', invoiced)


def check_fee_validity(appointment):
	if not vmraid.db.get_single_value('Healthcare Settings', 'enable_free_follow_ups'):
		return

	validity = vmraid.db.exists('Fee Validity', {
	...
	validity = vmraid.get_doc('Fee Validity', validity)
	return validity
...
def manage_fee_validity(appointment):
	...
	if fee_validity:
		if appointment.status == 'Cancelled' and fee_validity.visited > 0:
			...
			vmraid.db.delete('Fee Validity Reference', {'appointment': appointment.name})
...
def manage_doc_for_appointment(dt_from_appointment, appointment, invoiced):
	dn_from_appointment = vmraid.db.get_value(
		dt_from_appointment,
		filters={'appointment': appointment}
	)
	if dn_from_appointment:
		vmraid.db.set_value(dt_from_appointment, dn_from_appointment, 'invoiced', invoiced)


@vmraid.whitelist()
def get_drugs_to_invoice(encounter):
	encounter = vmraid.get_doc('Patient Encounter', encounter)
	if encounter:
		patient = vmraid.get_doc('Patient', encounter.patient)
		if patient:
			if patient.customer:
				items_to_invoice = []
				for drug_line in encounter.drug_prescription:
					if drug_line.drug_code:
						qty = 1
						if vmraid.db.get_value('Item', drug_line.drug_code, 'stock_uom') == 'Nos':
...
def get_children(doctype, parent, company, is_root=False):
	...
	hc_service_units = vmraid.get_list(doctype, fields=fields, filters=filters)
...
def get_patient_vitals(patient, from_date=None, to_date=None):
	...
	vitals = vmraid.db.get_all('Vital Signs', filters={
			'docstatus': 1,
			'patient': patient
