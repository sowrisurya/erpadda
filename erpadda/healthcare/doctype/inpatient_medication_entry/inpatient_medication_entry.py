...
from vmraid.utils import flt, get_link_to_form, getdate, nowtime
from erpadda.stock.utils import get_latest_stock_qty
...
class InpatientMedicationEntry(Document):
	...
	def get_medication_orders(self):
		...
		if orders:
			...
			vmraid.msgprint(_('No pending medication orders found for selected criteria'))
	...
	def on_submit(self):
		...
		if self.update_stock:
			...
			success_msg += _('Stock Entry {0} created and ').format(
				vmraid.bold(get_link_to_form('Stock Entry', stock_entry)))

		self.update_medication_orders()
		success_msg += _('Inpatient Medication Orders updated successfully')
		vmraid.msgprint(success_msg, title=_('Success'), indicator='green')

	def validate_medication_orders(self):
		for entry in self.medication_orders:
			docstatus, is_completed = vmraid.db.get_value('Inpatient Medication Order Entry', entry.against_imoe,
				['docstatus', 'is_completed'])

			if docstatus == 2:
				vmraid.throw(_('Row {0}: Cannot create Inpatient Medication Entry against cancelled Inpatient Medication Order {1}').format(
					entry.idx, get_link_to_form(entry.against_imo)))

			if is_completed:
				vmraid.throw(_('Row {0}: This Medication Order is already marked as completed').format(
					entry.idx))
	...
	def update_medication_orders(self, on_cancel=False):
		...
		for order, count in order_entry_map.items():
			medication_order = vmraid.get_doc('Inpatient Medication Order', order)
			completed_orders = flt(count)
			current_value = vmraid.db.get_value('Inpatient Medication Order', order, 'completed_orders')
	...
	def check_stock_qty(self):
		...
		if drug_shortage:
			message = _('Quantity not available for the following items in warehouse {0}. ').format(vmraid.bold(self.warehouse))
			...
			for drug, shortage_qty in drug_shortage.items():
				...
				formatted_item_rows += """
					<td>{0}</td>
					<td>{1}</td>
				</tr>""".format(item_link, vmraid.bold(shortage_qty))
			...
			vmraid.throw(message, title=_('Insufficient Stock'), is_minimizable=True, wide=True)
	...
	def make_stock_entry(self):
		...
		for entry in self.medication_orders:
			...
			se_child.uom = vmraid.db.get_value('Item', entry.drug_code, 'stock_uom')
	...
	def cancel_stock_entries(self):
		stock_entries = vmraid.get_all('Stock Entry', {'inpatient_medication_entry': self.name})
		for entry in stock_entries:
			doc = vmraid.get_doc('Stock Entry', entry.name)
			doc.cancel()
...
def get_current_healthcare_service_unit(inpatient_record):
	ip_record = vmraid.get_doc('Inpatient Record', inpatient_record)
...
def make_difference_stock_entry(docname):
	doc = vmraid.get_doc('Inpatient Medication Entry', docname)
	...
	for drug, shortage_qty in drug_shortage.items():
		...
		se_child.item_name = vmraid.db.get_value('Item', drug, 'stock_uom')
		se_child.uom = vmraid.db.get_value('Item', drug, 'stock_uom')
		se_child.stock_uom = se_child.uom
