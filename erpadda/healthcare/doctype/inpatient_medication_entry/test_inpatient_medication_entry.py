...
import vmraid
import unittest
from vmraid.utils import add_days, getdate, now_datetime
from erpadda.healthcare.doctype.inpatient_record.test_inpatient_record import create_patient, create_inpatient, get_healthcare_service_unit, mark_invoiced_inpatient_occupancy
...
from erpadda.healthcare.doctype.inpatient_medication_order.test_inpatient_medication_order import create_ipmo, create_ipme
from erpadda.healthcare.doctype.inpatient_medication_entry.inpatient_medication_entry import get_drug_shortage_map, make_difference_stock_entry
...
class TestInpatientMedicationEntry(unittest.TestCase):
	def setUp(self):
		...
		vmraid.db.sql("""delete from `tabInpatient Medication Order`""")
		vmraid.db.sql("""delete from `tabInpatient Medication Entry`""")
	...
	def test_ipme_with_stock_update(self):
		...
		is_order_completed = vmraid.db.get_value('Inpatient Medication Order Entry',
			ipme.medication_orders[0].against_imoe, 'is_completed')
		self.assertEqual(is_order_completed, 1)

		# test stock entry
		stock_entry = vmraid.db.exists('Stock Entry', {'inpatient_medication_entry': ipme.name})
		self.assertTrue(stock_entry)

		# check references
		stock_entry = vmraid.get_doc('Stock Entry', stock_entry)
	...
	def tearDown(self):
		# cleanup - Discharge
		schedule_discharge(vmraid.as_json({'patient': self.patient}))
		...
		for entry in vmraid.get_all('Inpatient Medication Entry'):
			doc = vmraid.get_doc('Inpatient Medication Entry', entry.name)
			doc.cancel()

		for entry in vmraid.get_all('Inpatient Medication Order'):
			doc = vmraid.get_doc('Inpatient Medication Order', entry.name)
			doc.cancel()

def make_stock_entry(warehouse=None):
	vmraid.db.set_value('Company', '_Test Company', {
