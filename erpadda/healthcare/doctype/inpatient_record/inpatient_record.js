...
vmraid.ui.form.on('Inpatient Record', {
	setup: function(frm) {
	...
	let dialog = new vmraid.ui.Dialog({
		...
		fields: [
			...
			{fieldtype: 'Datetime', label: 'Admission Datetime', fieldname: 'check_in',
				reqd: 1, default: vmraid.datetime.now_datetime()
			},
			{fieldtype: 'Date', label: 'Expected Discharge', fieldname: 'expected_discharge',
				default: frm.doc.expected_length_of_stay ? vmraid.datetime.add_days(vmraid.datetime.now_datetime(), frm.doc.expected_length_of_stay) : ''
			}
		],
...
let transfer_patient_dialog = function(frm) {
	let dialog = new vmraid.ui.Dialog({
		...
		fields: [
			...
			{fieldtype: 'Datetime', label: 'Check In', fieldname: 'check_in', reqd: 1, default: vmraid.datetime.now_datetime()}
		],
		...
		primary_action : function() {
			...
			if(check_in > vmraid.datetime.now_datetime()){
				vmraid.msgprint({
	...
	dialog.fields_dict['leave_from'].get_query = function(){
		return {
			query : 'erpadda.healthcare.doctype.inpatient_record.inpatient_record.get_leave_from',
