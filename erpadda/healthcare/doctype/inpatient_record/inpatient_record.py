...
from vmraid.utils import today, now_datetime, getdate, get_datetime, get_link_to_form
from vmraid.model.document import Document
from vmraid.desk.reportview import get_match_cond

class InpatientRecord(Document):
	def after_insert(self):
		vmraid.db.set_value('Patient', self.patient, 'inpatient_record', self.name)
		vmraid.db.set_value('Patient', self.patient, 'inpatient_status', self.status)

		if self.admission_encounter: # Update encounter
			vmraid.db.set_value('Patient Encounter', self.admission_encounter, 'inpatient_record', self.name)
			vmraid.db.set_value('Patient Encounter', self.admission_encounter, 'inpatient_status', self.status)

	def validate(self):
		self.validate_dates()
		self.validate_already_scheduled_or_admitted()
		if self.status == "Discharged":
			vmraid.db.set_value("Patient", self.patient, "inpatient_status", None)
			vmraid.db.set_value("Patient", self.patient, "inpatient_record", None)
	...
	def validate_dates(self):
		if (getdate(self.expected_discharge) < getdate(self.scheduled_date)) or \
			...
			vmraid.throw(_('Expected and Discharge dates cannot be less than Admission Schedule date'))
		...
		for entry in self.inpatient_occupancies:
			if entry.check_in and entry.check_out and \
				...
				vmraid.throw(_('Row #{0}: Check Out datetime cannot be less than Check In datetime').format(entry.idx))
...
def schedule_inpatient(args):
	...
	if not admission_order or not admission_order['patient'] or not admission_order['admission_encounter']:
		vmraid.throw(_('Missing required details, did not create Inpatient Record'))
	...
	# Patient details
	patient = vmraid.get_doc('Patient', admission_order['patient'])
	...
	encounter = vmraid.get_doc('Patient Encounter', admission_order['admission_encounter'])
...
def schedule_discharge(args):
	...
	inpatient_record_id = vmraid.db.get_value('Patient', discharge_order['patient'], 'inpatient_record')
	if inpatient_record_id:
		inpatient_record = vmraid.get_doc('Inpatient Record', inpatient_record_id)
		check_out_inpatient(inpatient_record)
		set_details_from_ip_order(inpatient_record, discharge_order)
		inpatient_record.status = 'Discharge Scheduled'
		inpatient_record.save(ignore_permissions = True)
		vmraid.db.set_value('Patient', discharge_order['patient'], 'inpatient_status', inpatient_record.status)
		vmraid.db.set_value('Patient Encounter', inpatient_record.discharge_encounter, 'inpatient_status', inpatient_record.status)
	...
	if inpatient_record.inpatient_occupancies:
		for inpatient_occupancy in inpatient_record.inpatient_occupancies:
			if inpatient_occupancy.left != 1:
				...
				vmraid.db.set_value("Healthcare Service Unit", inpatient_occupancy.service_unit, "occupancy_status", "Vacant")
...
def validate_inpatient_invoicing(inpatient_record):
	if vmraid.db.get_single_value("Healthcare Settings", "allow_discharge_despite_unbilled_services"):
		return
	...
	if pending_invoices:
		...
		vmraid.throw(message, title=_("Unbilled Services"), is_minimizable=True, wide=True)
...
def get_unbilled_inpatient_docs(doc, inpatient_record):
	return vmraid.db.get_list(doc, filters = {'patient': inpatient_record.patient,
...
def admit_patient(inpatient_record, service_unit, check_in, expected_discharge=None):
	...
	vmraid.db.set_value('Patient', inpatient_record.patient, 'inpatient_status', 'Admitted')
	vmraid.db.set_value('Patient', inpatient_record.patient, 'inpatient_record', inpatient_record.name)
...
def transfer_patient(inpatient_record, service_unit, check_in):
	...
	vmraid.db.set_value("Healthcare Service Unit", service_unit, "occupancy_status", "Occupied")
...
def get_leave_from(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.db.sql(query.format(**{
		"docname":	docname,
