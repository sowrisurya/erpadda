...
class TherapyPlan(Document):
	...
	def set_therapy_details_from_template(self):
		...
		therapy_plan_template = vmraid.get_doc('Therapy Plan Template', self.therapy_plan_template)
...
def make_therapy_session(therapy_plan, patient, therapy_type, company, appointment=None):
	therapy_type = vmraid.get_doc('Therapy Type', therapy_type)

	therapy_session = vmraid.new_doc('Therapy Session')
...
def make_sales_invoice(reference_name, patient, company, therapy_plan_template):
	...
	si.company = company
	si.patient = patient
	si.customer = vmraid.db.get_value('Patient', patient, 'customer')

	item = vmraid.db.get_value('Therapy Plan Template', therapy_plan_template, 'linked_item')
