...
from vmraid.utils import getdate, cstr, get_link_to_form
...
class LabTest(Document):
	...
	def after_insert(self):
		if self.prescription:
			vmraid.db.set_value('Lab Prescription', self.prescription, 'lab_test_created', 1)
			if vmraid.db.get_value('Lab Prescription', self.prescription, 'invoiced'):
				self.invoiced = True
		...
		for item in self.normal_test_items:
			if item.result_value and item.secondary_uom and item.conversion_factor:
				try:
					...
					vmraid.msgprint(_('Row #{0}: Result for Secondary UOM not calculated'.format(item.idx)), title = _('Warning'))
		...
		if self.normal_test_items:
			for item in self.normal_test_items:
				if not item.result_value and not item.allow_blank and item.require_result_value:
					vmraid.throw(_('Row #{0}: Please enter the result value for {1}').format(
						item.idx, vmraid.bold(item.lab_test_name)), title=_('Mandatory Results'))

		if self.descriptive_test_items:
			for item in self.descriptive_test_items:
				if not item.result_value and not item.allow_blank and item.require_result_value:
					vmraid.throw(_('Row #{0}: Please enter the result value for {1}').format(
						item.idx, vmraid.bold(item.lab_test_particulars)), title=_('Mandatory Results'))


def create_test_from_template(lab_test):
	template = vmraid.get_doc('Lab Test Template', lab_test.template)
	patient = vmraid.get_doc('Patient', lab_test.patient)
...
def update_status(status, name):
	if name and status:
		vmraid.db.set_value('Lab Test', name, {
			'status': status,
...
def create_multiple(doctype, docname):
	if not doctype or not docname:
		vmraid.throw(_('Sales Invoice or Patient Encounter is required to create Lab Tests'), title=_('Insufficient Data'))
	...
	if lab_test_created:
		vmraid.msgprint(_('Lab Test(s) {0} created successfully').format(lab_test_created), indicator='green')
	else:
		vmraid.msgprint(_('No Lab Tests created'))
		...
		for item in encounter.lab_test_prescription:
			if not item.lab_test_created:
				...
				if template:
					...
					vmraid.db.set_value('Lab Prescription', item.name, 'lab_test_created', 1)
...
def create_lab_test_from_invoice(sales_invoice):
	...
	invoice = vmraid.get_doc('Sales Invoice', sales_invoice)
	if invoice and invoice.patient:
		patient = vmraid.get_doc('Patient', invoice.patient)
		for item in invoice.items:
			lab_test_created = 0
			if item.reference_dt == 'Lab Prescription':
				lab_test_created = vmraid.db.get_value('Lab Prescription', item.reference_dn, 'lab_test_created')
			...
			if lab_test_created != 1:
				...
				if template:
					...
					lab_test.save(ignore_permissions = True)
					if item.reference_dt != 'Lab Prescription':
						vmraid.db.set_value('Sales Invoice Item', item.name, 'reference_dt', 'Lab Test')
						vmraid.db.set_value('Sales Invoice Item', item.name, 'reference_dn', lab_test.name)
...
def get_lab_test_template(item):
	template_id = vmraid.db.exists('Lab Test Template', {'item': item})
	if template_id:
		return vmraid.get_doc('Lab Test Template', template_id)
	return False

def create_lab_test_doc(invoiced, practitioner, patient, template, company):
	lab_test = vmraid.new_doc('Lab Test')
...
def create_sample_doc(template, patient, invoice, company = None):
	if template.sample:
		sample_exists = vmraid.db.exists({
		...
		if sample_exists:
			...
			sample_collection = vmraid.get_doc('Sample Collection', sample_exists[0][0])
			...
			if template.sample_details:
				...
				vmraid.db.set_value('Sample Collection', sample_collection.name, 'sample_details', sample_details)

			vmraid.db.set_value('Sample Collection', sample_collection.name, 'sample_qty', quantity)

		else:
			# Create Sample Collection for template, copy vals from Invoice
			sample_collection = vmraid.new_doc('Sample Collection')
			if invoice:
...
def create_sample_collection(lab_test, template, patient, invoice):
	if vmraid.get_cached_value('Healthcare Settings', None, 'create_sample_collection_for_lab_test'):
		sample_collection = create_sample_doc(template, patient, invoice, lab_test.company)
		if sample_collection:
			lab_test.sample = sample_collection.name
			sample_collection_doc = get_link_to_form('Sample Collection', sample_collection.name)
			vmraid.msgprint(_('Sample Collection {0} has been created').format(sample_collection_doc),
...
def load_result_format(lab_test, template, prescription, invoice):
	if template.lab_test_template_type == 'Single':
		...
		for lab_test_group in template.lab_test_groups:
			...
			if lab_test_group.lab_test_template:
				template_in_group = vmraid.get_doc('Lab Test Template', lab_test_group.lab_test_template)
	...
	if template.lab_test_template_type != 'No Result':
		if prescription:
			...
			if invoice:
				vmraid.db.set_value('Lab Prescription', prescription, 'invoiced', True)
		lab_test.save(ignore_permissions=True) # Insert the result
		return lab_test

@vmraid.whitelist()
def get_employee_by_user_id(user_id):
	emp_id = vmraid.db.exists('Employee', { 'user_id': user_id })
	if emp_id:
		return vmraid.get_doc('Employee', emp_id)
	return None
