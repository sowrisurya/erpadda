...
import unittest
import vmraid
from vmraid.utils import getdate, nowtime
...
from erpadda.healthcare.doctype.patient_medical_record.test_patient_medical_record import create_lab_test_template as create_blood_test_template

class TestLabTest(unittest.TestCase):
	def test_lab_test_item(self):
		lab_template = create_lab_test_template()
		self.assertTrue(vmraid.db.exists('Item', lab_template.item))
		self.assertEqual(vmraid.db.get_value('Item Price', {'item_code':lab_template.item}, 'price_list_rate'), lab_template.lab_test_rate)

		lab_template.disabled = 1
		lab_template.save()
		self.assertEqual(vmraid.db.get_value('Item', lab_template.item, 'disabled'), 1)
	...
	def test_descriptive_lab_test(self):
		...
		lab_test.save()
		self.assertRaises(vmraid.ValidationError, lab_test.submit)

	def test_sample_collection(self):
		vmraid.db.set_value('Healthcare Settings', 'Healthcare Settings', 'create_sample_collection_for_lab_test', 1)
		lab_template = create_lab_test_template()
		...
		self.assertTrue(vmraid.db.exists('Sample Collection', {'sample': lab_template.sample}))

		vmraid.db.set_value('Healthcare Settings', 'Healthcare Settings', 'create_sample_collection_for_lab_test', 0)
		lab_test = create_lab_test(lab_template)
		lab_test.descriptive_test_items[0].result_value = 12
		lab_test.descriptive_test_items[1].result_value = 1
		lab_test.descriptive_test_items[2].result_value = 2.3
...
def create_lab_test_template(test_sensitivity=0, sample_collection=1):
	...
	if vmraid.db.exists('Lab Test Template', 'Insulin Resistance'):
		return vmraid.get_doc('Lab Test Template', 'Insulin Resistance')
	template = vmraid.new_doc('Lab Test Template')
...
def create_lab_test_sample():
	blood_sample = vmraid.db.exists('Lab Test Sample', 'Blood Sample')
	if blood_sample:
		return blood_sample

	sample = vmraid.new_doc('Lab Test Sample')
...
def create_patient_encounter():
	...
	patient_encounter = vmraid.new_doc('Patient Encounter')
