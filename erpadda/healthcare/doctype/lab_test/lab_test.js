...
vmraid.ui.form.on('Lab Test', {
	...
	refresh: function (frm) {
		...
		if (vmraid.defaults.get_default('lab_test_approval_required') && vmraid.user.has_role('LabTest Approver')) {
		...
		if (frm.doc.docstatus === 1 && frm.doc.sms_sent === 0 && frm.doc.status !== 'Rejected' ) {
			frm.add_custom_button(__('Send SMS'), function () {
				vmraid.call({
					method: 'erpadda.healthcare.doctype.healthcare_settings.healthcare_settings.get_sms_text',
...
vmraid.ui.form.on('Lab Test', 'patient', function (frm) {
	if (frm.doc.patient) {
		vmraid.call({
...
vmraid.ui.form.on('Normal Test Result', {
	normal_test_items_remove: function () {
		vmraid.msgprint(__('Not permitted, configure Lab Test Template as required'));
		cur_frm.reload_doc();
	}
});

vmraid.ui.form.on('Descriptive Test Result', {
...
var status_update = function (approve, frm) {
	...
	vmraid.call({
		method: 'erpadda.healthcare.doctype.lab_test.lab_test.update_status',
...
var get_lab_test_prescribed = function (frm) {
	if (frm.doc.patient) {
		vmraid.call({
			method: 'erpadda.healthcare.doctype.lab_test.lab_test.get_lab_test_prescribed',
			args: { patient: frm.doc.patient },
			callback: function (r) {
		...
		vmraid.msgprint(__('Please select Patient to get Lab Tests'));
	}
};
...
var send_sms = function (vals, frm) {
	...
	if (!number || !message) {
		vmraid.throw(__('Did not send SMS, missing patient mobile number or message content.'));
	}
	vmraid.call({
