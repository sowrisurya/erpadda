...
vmraid.ui.form.on('Appointment Type', {
...
vmraid.ui.form.on('Appointment Type Service Item', {
			...
			vmraid.call({
				...
				callback: function(data) {
					if (data.message.price_list_rate) {
						vmraid.model.set_value(cdt, cdn, 'op_consulting_charge', data.message.price_list_rate);
					}
				}
			});
		}
	...
	inpatient_visit_charge_item: function(frm, cdt, cdn) {
		...
		if (frm.doc.price_list && d.inpatient_visit_charge_item) {
			vmraid.call({
				...
				callback: function (data) {
					if (data.message.price_list_rate) {
						vmraid.model.set_value(cdt, cdn, 'inpatient_visit_charge', data.message.price_list_rate);
					}
				}
			});
		}
