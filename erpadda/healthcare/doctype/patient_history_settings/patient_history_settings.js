...
vmraid.ui.form.on('Patient History Settings', {
	...
	show_field_selector_dialog: function(frm, doc, doctype, doc_fields) {
		...
		d.set_primary_action(__('Save'), () => {
			...
			vmraid.model.with_doctype(doc.document_type, function() {
				for (let idx in values) {
					let value = values[idx];

					let field = vmraid.get_meta(doc.document_type).fields.filter((df) => df.fieldname == value)[0];
					if (field) {
				...
				d.refresh();
				vmraid.model.set_value(doctype, doc.name, 'selected_fields', JSON.stringify(selected_fields));
			});
	...
	get_date_field_for_dt: function(frm, row) {
		frm.call({
			...
			callback: function(data) {
				if (data.message) {
					vmraid.model.set_value('Patient History Custom Document Type',
...
vmraid.ui.form.on('Patient History Custom Document Type', {
...
vmraid.ui.form.on('Patient History Standard Document Type', {
	add_edit_fields: function(frm, cdt, cdn) {
		let row = locals[cdt][cdn];
		if (row.document_type) {
