...
from vmraid.utils import get_time, flt
from vmraid.model.mapper import get_mapped_doc
from vmraid import _
from vmraid.utils import cstr, getdate, get_link_to_form
...
class TherapySession(Document):
	...
	def validate_duplicate(self):
		...
		if overlaps:
			...
			vmraid.throw(overlapping_details, title=_('Therapy Sessions Overlapping'))
	...
	def update_sessions_count_in_therapy_plan(self, on_cancel=False):
		therapy_plan = vmraid.get_doc('Therapy Plan', self.therapy_plan)
...
def create_therapy_session(source_name, target_doc=None):
	def set_missing_values(source, target):
		therapy_type = vmraid.get_doc('Therapy Type', source.therapy_type)
...
def invoice_therapy_session(source_name, target_doc=None):
	def set_missing_values(source, target):
		target.customer = vmraid.db.get_value('Patient', source.patient, 'customer')
...
def get_therapy_item(therapy, item):
	item.item_code = vmraid.db.get_value('Therapy Type', therapy.therapy_type, 'item')
	...
	item.cost_center = vmraid.get_cached_value('Company', therapy.company, 'cost_center')
