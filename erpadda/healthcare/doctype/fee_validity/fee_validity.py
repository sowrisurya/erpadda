...
class FeeValidity(Document):
	...
	def set_start_date(self):
		...
		for appointment in self.ref_appointments:
			appointment_date = vmraid.db.get_value('Patient Appointment', appointment.appointment, 'appointment_date')
...
def create_fee_validity(appointment):
	...
	fee_validity = vmraid.new_doc('Fee Validity')
	fee_validity.practitioner = appointment.practitioner
	fee_validity.patient = appointment.patient
	fee_validity.max_visits = vmraid.db.get_single_value('Healthcare Settings', 'max_visits') or 1
	valid_days = vmraid.db.get_single_value('Healthcare Settings', 'valid_days') or 1
...
def check_is_new_patient(appointment):
	validity_exists = vmraid.db.exists('Fee Validity', {
	...
	appointment_exists = vmraid.db.get_all('Patient Appointment', {
