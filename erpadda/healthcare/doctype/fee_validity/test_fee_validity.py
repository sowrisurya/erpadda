...
from erpadda.healthcare.doctype.patient_appointment.test_patient_appointment import create_healthcare_docs, create_appointment, create_healthcare_service_items
...
class TestFeeValidity(unittest.TestCase):
	def setUp(self):
		...
		vmraid.db.sql("""delete from `tabPatient`""")
		make_pos_profile()

	def test_fee_validity(self):
		item = create_healthcare_service_items()
		healthcare_settings = vmraid.get_single("Healthcare Settings")
		...
		invoiced = vmraid.db.get_value("Patient Appointment", appointment.name, "invoiced")
