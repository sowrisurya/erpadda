...
from erpadda.healthcare.doctype.patient_appointment.test_patient_appointment import create_encounter, create_healthcare_docs, create_appointment
...
class TestPatientMedicalRecord(unittest.TestCase):
	...
	def test_medical_record(self):
		...
		# check for encounter
		medical_rec = vmraid.db.exists('Patient Medical Record', {'status': 'Open', 'reference_name': encounter.name})
		self.assertTrue(medical_rec)

		vital_signs = create_vital_signs(appointment)
		# check for vital signs
		medical_rec = vmraid.db.exists('Patient Medical Record', {'status': 'Open', 'reference_name': vital_signs.name})
		self.assertTrue(medical_rec)
		...
		medical_rec = vmraid.db.exists('Patient Medical Record', {'status': 'Open', 'reference_name': procedure.name})
		self.assertTrue(medical_rec)

		template = create_lab_test_template(medical_department)
		lab_test = create_lab_test(template.name, patient)
		# check for lab test
		medical_rec = vmraid.db.exists('Patient Medical Record', {'status': 'Open', 'reference_name': lab_test.name})
		self.assertTrue(medical_rec)


def create_procedure(appointment):
	if appointment:
		procedure = vmraid.new_doc('Clinical Procedure')
...
def create_vital_signs(appointment):
	vital_signs = vmraid.new_doc('Vital Signs')
...
def create_lab_test_template(medical_department):
	if vmraid.db.exists('Lab Test Template', 'Blood Test'):
		return vmraid.get_doc('Lab Test Template', 'Blood Test')
...
def create_lab_test(template, patient):
	...
	lab_test.patient_sex = vmraid.db.get_value('Patient', patient, 'sex')
