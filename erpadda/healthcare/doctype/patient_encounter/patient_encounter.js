...
vmraid.ui.form.on('Patient Encounter', {
	setup: function(frm) {
		...
		if (!frm.doc.__islocal) {
			...
			frm.add_custom_button(__('Patient History'), function() {
				if (frm.doc.patient) {
					...
					vmraid.msgprint(__('Please select Patient'));
				}
			},'View');
			...
			if (frm.doc.drug_prescription && frm.doc.inpatient_record && frm.doc.inpatient_status === "Admitted") {
				frm.add_custom_button(__('Inpatient Medication Order'), function() {
					vmraid.model.open_mapped_doc({
						method: 'erpadda.healthcare.doctype.patient_encounter.patient_encounter.make_ip_medication_order',
						frm: frm
					});
				}, 'Create');
			}
...
var schedule_inpatient = function(frm) {
	var dialog = new vmraid.ui.Dialog({
		...
		primary_action : function() {
			...
			vmraid.call({
				method: 'erpadda.healthcare.doctype.inpatient_record.inpatient_record.schedule_inpatient',
				args: {
					args: args
				},
				callback: function(data) {
...
var schedule_discharge = function(frm) {
	var dialog = new vmraid.ui.Dialog ({
		...
		primary_action : function() {
			...
			vmraid.call ({
				method: 'erpadda.healthcare.doctype.inpatient_record.inpatient_record.schedule_discharge',
				args: {args},
				callback: function(data) {
					if(!data.exc){
						frm.reload_doc();
...
let create_medical_record = function(frm) {
	...
	vmraid.new_doc('Patient Medical Record');
};
...
let create_procedure = function(frm) {
	...
	vmraid.new_doc('Clinical Procedure');
};

vmraid.ui.form.on('Drug Prescription', {
	dosage: function(frm, cdt, cdn){
		vmraid.model.set_value(cdt, cdn, 'update_schedule', 1);
		let child = locals[cdt][cdn];
		if (child.dosage) {
			vmraid.model.set_value(cdt, cdn, 'interval_uom', 'Day');
			vmraid.model.set_value(cdt, cdn, 'interval', 1);
		}
	},
	...
	interval_uom: function(frm, cdt, cdn) {
		...
		if (child.interval_uom == 'Hour') {
			vmraid.model.set_value(cdt, cdn, 'dosage', null);
		}
	}
});
