...
vmraid.ui.form.on('Patient Appointment', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		frm.set_query('service_unit', function() {
			return {
				query: 'erpadda.controllers.queries.get_healthcare_service_units',
				filters: {
		...
		if (frm.is_new()) {
			frm.page.set_primary_action(__('Check Availability'), function() {
				if (!frm.doc.patient) {
					...
					vmraid.call({
						method: 'erpadda.healthcare.doctype.patient_appointment.patient_appointment.check_payment_fields_reqd',
		...
		if (frm.doc.patient) {
			frm.add_custom_button(__('Patient History'), function() {
				vmraid.route_options = {'patient': frm.doc.patient};
			...
			if (frm.doc.procedure_template) {
				frm.add_custom_button(__('Clinical Procedure'), function(){
					vmraid.model.open_mapped_doc({
						method: 'erpadda.healthcare.doctype.clinical_procedure.clinical_procedure.make_procedure',
						frm: frm,
					});
				}, __('Create'));
				...
				frm.add_custom_button(__('Therapy Session'),function(){
					vmraid.model.open_mapped_doc({
						method: 'erpadda.healthcare.doctype.therapy_session.therapy_session.create_therapy_session',
						frm: frm,
					})
				}, 'Create');
			} else {
				frm.add_custom_button(__('Patient Encounter'), function() {
					vmraid.model.open_mapped_doc({
						method: 'erpadda.healthcare.doctype.patient_appointment.patient_appointment.make_encounter',
						frm: frm,
					});
				}, __('Create'));
			}
	...
	set_payment_details: function(frm) {
		vmraid.db.get_single_value('Healthcare Settings', 'automate_appointment_invoicing').then(val => {
			if (val) {
				vmraid.call({
					method: 'erpadda.healthcare.utils.get_service_item_and_practitioner_charge',
					args: {
						doc: frm.doc
					},
					callback: function(data) {
						if (data.message) {
							vmraid.model.set_value(frm.doctype, frm.docname, 'paid_amount', data.message.practitioner_charge);
							vmraid.model.set_value(frm.doctype, frm.docname, 'billing_item', data.message.service_item);
						}
					}
				});
			}
	...
	therapy_type: function(frm) {
		if (frm.doc.therapy_type) {
			vmraid.db.get_value('Therapy Type', frm.doc.therapy_type, 'default_duration', (r) => {
	...
	get_prescribed_therapies: function(frm) {
		if (frm.doc.patient) {
			vmraid.call({
				method: "erpadda.healthcare.doctype.patient_appointment.patient_appointment.get_prescribed_therapies",
				args: {patient: frm.doc.patient},
				callback: function(r) {
					if (r.message) {
...
let check_and_set_availability = function(frm) {
	...
	function show_slots(d, fd) {
		if (d.get_value('appointment_date') && d.get_value('practitioner')) {
			...
			vmraid.call({
				method: 'erpadda.healthcare.doctype.patient_appointment.patient_appointment.get_availability_data',
				args: {
...
let get_prescribed_procedure = function(frm) {
	if (frm.doc.patient) {
		vmraid.call({
			method: 'erpadda.healthcare.doctype.patient_appointment.patient_appointment.get_procedure_prescribed',
			args: {patient: frm.doc.patient},
			callback: function(r) {
...
let create_vital_signs = function(frm) {
	if (!frm.doc.patient) {
		vmraid.throw(__('Please select patient'));
	}
	...
	vmraid.new_doc('Vital Signs');
};

let update_status = function(frm, status){
	let doc = frm.doc;
	vmraid.confirm(__('Are you sure you want to cancel this appointment?'),
		function() {
			vmraid.call({
				method: 'erpadda.healthcare.doctype.patient_appointment.patient_appointment.update_status',
