...
import json
from vmraid.utils import getdate, get_time, flt
...
from erpadda.healthcare.doctype.healthcare_settings.healthcare_settings import get_receivable_account, get_income_account
from erpadda.healthcare.utils import check_fee_validity, get_service_item_and_practitioner_charge, manage_fee_validity
...
class PatientAppointment(Document):
	...
	def validate_overlaps(self):
		...
		if overlaps:
			...
			vmraid.throw(overlapping_details, title=_('Appointments Overlapping'))

	def validate_service_unit(self):
		if self.inpatient_record and self.service_unit:
			from erpadda.healthcare.doctype.inpatient_medication_entry.inpatient_medication_entry import get_current_healthcare_service_unit

			is_inpatient_occupancy_unit = vmraid.db.get_value('Healthcare Service Unit', self.service_unit,
				'inpatient_occupancy')
			...
			if is_inpatient_occupancy_unit and service_unit != self.service_unit:
				msg = _('Patient {0} is not admitted in the service unit {1}').format(vmraid.bold(self.patient), vmraid.bold(self.service_unit)) + '<br>'
				...
				vmraid.throw(msg, title=_('Invalid Healthcare Service Unit'))
	...
	def set_payment_details(self):
		if vmraid.db.get_single_value('Healthcare Settings', 'automate_appointment_invoicing'):
	...
	def validate_customer_created(self):
		if vmraid.db.get_single_value('Healthcare Settings', 'automate_appointment_invoicing'):
			if not vmraid.db.get_value('Patient', self.patient, 'customer'):
				msg = _("Please set a Customer linked to the Patient")
				msg +=  " <b><a href='/app/Form/Patient/{0}'>{0}</a></b>".format(self.patient)
				vmraid.throw(msg, title=_('Customer Not Found'))

	def update_prescription_details(self):
		if self.procedure_prescription:
			vmraid.db.set_value('Procedure Prescription', self.procedure_prescription, 'appointment_booked', 1)
			if self.procedure_template:
				comments = vmraid.db.get_value('Procedure Prescription', self.procedure_prescription, 'comments')
				if comments:
					vmraid.db.set_value('Patient Appointment', self.name, 'notes', comments)

	def update_fee_validity(self):
		fee_validity = manage_fee_validity(self)
		if fee_validity:
			vmraid.msgprint(_('{0} has fee validity till {1}').format(self.patient, fee_validity.valid_till))
	...
	def get_therapy_types(self):
		...
		therapy_types = []
		doc = vmraid.get_doc('Therapy Plan', self.therapy_plan)
...
def check_payment_fields_reqd(patient):
	automate_invoicing = vmraid.db.get_single_value('Healthcare Settings', 'automate_appointment_invoicing')
	free_follow_ups = vmraid.db.get_single_value('Healthcare Settings', 'enable_free_follow_ups')
	if automate_invoicing:
		if free_follow_ups:
			fee_validity = vmraid.db.exists('Fee Validity', {'patient': patient, 'status': 'Pending'})
			if fee_validity:
...
def invoice_appointment(appointment_doc):
	automate_invoicing = vmraid.db.get_single_value('Healthcare Settings', 'automate_appointment_invoicing')
	appointment_invoiced = vmraid.db.get_value('Patient Appointment', appointment_doc.name, 'invoiced')
	enable_free_follow_ups = vmraid.db.get_single_value('Healthcare Settings', 'enable_free_follow_ups')
	if enable_free_follow_ups:
		fee_validity = check_fee_validity(appointment_doc)
		if fee_validity and fee_validity.status == 'Completed':
			fee_validity = None
		elif not fee_validity:
			if vmraid.db.exists('Fee Validity Reference', {'appointment': appointment_doc.name}):
				return
...
def create_sales_invoice(appointment_doc):
	...
	sales_invoice.customer = vmraid.get_value('Patient', appointment_doc.patient, 'customer')
	...
	vmraid.msgprint(_('Sales Invoice {0} created').format(sales_invoice.name), alert=True)
	vmraid.db.set_value('Patient Appointment', appointment_doc.name, {
		'invoiced': 1,
...
def check_is_new_patient(patient, name=None):
	...
	has_previous_appointment = vmraid.db.exists('Patient Appointment', filters)
...
def get_appointment_item(appointment_doc, item):
	...
	item.cost_center = vmraid.get_cached_value('Company', appointment_doc.company, 'cost_center')
	item.rate = charge
	item.amount = charge
	item.qty = 1
...
def cancel_appointment(appointment_id):
	appointment = vmraid.get_doc('Patient Appointment', appointment_id)
...
def check_sales_invoice_exists(appointment):
	sales_invoice = vmraid.db.get_value('Sales Invoice Item', {
		'reference_dt': 'Patient Appointment',
		'reference_dn': appointment.name
	}, 'parent')

	if sales_invoice:
		sales_invoice = vmraid.get_doc('Sales Invoice', sales_invoice)
...
def get_availability_data(date, practitioner):
	...
	practitioner_doc = vmraid.get_doc('Healthcare Practitioner', practitioner)
	...
	if practitioner_doc.practitioner_schedules:
		...
		vmraid.throw(_('{0} does not have a Healthcare Practitioner Schedule. Add it in Healthcare Practitioner master').format(
			practitioner), title=_('Practitioner Schedule Not Found'))


	if not slot_details:
		# TODO: return available slots in nearby dates
		vmraid.throw(_('Healthcare Practitioner not available on {0}').format(weekday), title=_('Not Available'))
...
def check_employee_wise_availability(date, practitioner_doc):
	...
	if practitioner_doc.employee:
		...
		employee = vmraid.db.get_value('Employee', {'user_id': practitioner_doc.user_id}, 'name')

	if employee:
		# check holiday
		if is_holiday(employee, date):
			vmraid.throw(_('{0} is a holiday'.format(date)), title=_('Not Available'))

		# check leave status
		leave_record = vmraid.db.sql("""select half_day from `tabLeave Application`
			where employee = %s and %s between from_date and to_date
			and docstatus = 1""", (employee, date), as_dict=True)
		if leave_record:
			if leave_record[0].half_day:
				vmraid.throw(_('{0} is on a Half day Leave on {1}').format(practitioner_doc.name, date), title=_('Not Available'))
			else:
				vmraid.throw(_('{0} is on Leave on {1}').format(practitioner_doc.name, date), title=_('Not Available'))
...
def get_available_slots(practitioner_doc, date):
	...
	for schedule_entry in practitioner_doc.practitioner_schedules:
		if schedule_entry.schedule:
			practitioner_schedule = vmraid.get_doc('Practitioner Schedule', schedule_entry.schedule)
		else:
			vmraid.throw(_('{0} does not have a Healthcare Practitioner Schedule. Add it in Healthcare Practitioner').format(
				vmraid.bold(practitioner)), title=_('Practitioner Schedule Not Found'))
		...
		if practitioner_schedule:
			...
			if available_slots:
				...
				if schedule_entry.service_unit:
					...
					allow_overlap = vmraid.get_value('Healthcare Service Unit', schedule_entry.service_unit, 'overlap_appointments')
...
def update_status(appointment_id, status):
	vmraid.db.set_value('Patient Appointment', appointment_id, 'status', status)
	appointment_booked = True
	if status == 'Cancelled':
		appointment_booked = False
		cancel_appointment(appointment_id)

	procedure_prescription = vmraid.db.get_value('Patient Appointment', appointment_id, 'procedure_prescription')
	if procedure_prescription:
		vmraid.db.set_value('Procedure Prescription', procedure_prescription, 'appointment_booked', appointment_booked)


def send_confirmation_msg(doc):
	if vmraid.db.get_single_value('Healthcare Settings', 'send_appointment_confirmation'):
		message = vmraid.db.get_single_value('Healthcare Settings', 'appointment_confirmation_msg')
		try:
			send_message(doc, message)
		except Exception:
			vmraid.log_error(vmraid.get_traceback(), _('Appointment Confirmation Message Not Sent'))
			vmraid.msgprint(_('Appointment Confirmation Message Not Sent'), indicator='orange')
...
def send_appointment_reminder():
	if vmraid.db.get_single_value('Healthcare Settings', 'send_appointment_reminder'):
		remind_before = datetime.datetime.strptime(vmraid.db.get_single_value('Healthcare Settings', 'remind_before'), '%H:%M:%S')
		reminder_dt = datetime.datetime.now() + datetime.timedelta(
			hours=remind_before.hour, minutes=remind_before.minute, seconds=remind_before.second)

		appointment_list = vmraid.db.get_all('Patient Appointment', {
		...
		for appointment in appointment_list:
			doc = vmraid.get_doc('Patient Appointment', appointment.name)
			message = vmraid.db.get_single_value('Healthcare Settings', 'appointment_reminder_msg')
			send_message(doc, message)
			vmraid.db.set_value('Patient Appointment', doc.name, 'reminded', 1)

def send_message(doc, message):
	patient_mobile = vmraid.db.get_value('Patient', doc.patient, 'mobile')
	if patient_mobile:
		...
		message = vmraid.render_template(message, context)
		number = [patient_mobile]
		try:
			send_sms(number, message)
		except Exception as e:
			vmraid.msgprint(_('SMS not sent, please check SMS Settings'), alert=True)
...
def update_appointment_status():
	...
	appointments = vmraid.get_all('Patient Appointment', {
		'status': ('not in', ['Closed', 'Cancelled'])
	}, as_dict=1)

	for appointment in appointments:
		vmraid.get_doc('Patient Appointment', appointment.name).set_status()
