...
vmraid.ui.form.on('Exercise Type', {
	refresh: function(frm) {
		let wrapper = frm.fields_dict.steps_html.wrapper;

		frm.ExerciseEditor = new erpadda.ExerciseEditor(frm, wrapper);
	}
});

erpadda.ExerciseEditor = class ExerciseEditor {
	...
	show_add_card_dialog(frm) {
		...
		let d = new vmraid.ui.Dialog({
			...
			primary_action: function() {
				...
				let step = vmraid.model.add_child(frm.doc, 'Exercise Type Step', 'steps_table');
	...
	show_edit_card_dialog(frm, id) {
		let new_dialog = new vmraid.ui.Dialog({
