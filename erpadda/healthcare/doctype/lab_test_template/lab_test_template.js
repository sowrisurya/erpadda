...
vmraid.ui.form.on('Lab Test Template', {
...
let change_template_code = function(doc) {
	let d = new vmraid.ui.Dialog({
		...
		primary_action: function() {
			...
			if (values) {
				vmraid.call({
					'method': 'erpadda.healthcare.doctype.lab_test_template.lab_test_template.change_test_code_from_template',
					'args': {lab_test_code: values.lab_test_code, doc: doc},
					callback: function (data) {
						vmraid.set_route('Form', 'Lab Test Template', data.message);
					}
				});
			}
			d.hide();
...
vmraid.ui.form.on('Lab Test Template', 'lab_test_name', function(frm) {
	frm.doc.change_in_item = 1;
});

vmraid.ui.form.on('Lab Test Template', 'lab_test_rate', function(frm) {
	frm.doc.change_in_item = 1;
});

vmraid.ui.form.on('Lab Test Template', 'lab_test_group', function(frm) {
	frm.doc.change_in_item = 1;
});

vmraid.ui.form.on('Lab Test Template', 'lab_test_description', function(frm) {
	frm.doc.change_in_item = 1;
});

vmraid.ui.form.on('Lab Test Groups', 'template_or_new_line', function (frm, cdt, cdn) {
	let child = locals[cdt][cdn];
	if (child.template_or_new_line == 'Add New Line') {
		vmraid.model.set_value(cdt, cdn, 'lab_test_template', '');
		vmraid.model.set_value(cdt, cdn, 'lab_test_description', '');
	}
});
