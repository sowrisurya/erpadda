...
class TestInpatientMedicationOrder(unittest.TestCase):
	...
	def test_order_creation(self):
		...
		prescription_dosage = vmraid.get_doc('Prescription Dosage', '1-1-1')
	...
	def test_inpatient_validation(self):
		...
		self.assertRaises(vmraid.ValidationError, ipmo.insert)
	...
	def test_status(self):
		...
		filters = vmraid._dict(from_date=add_days(getdate(), -1), to_date=add_days(getdate(), -1), from_time='', to_time='')
...
def create_dosage_form():
	if not vmraid.db.exists('Dosage Form', 'Tablet'):
		vmraid.get_doc({
			'doctype': 'Dosage Form',
			'dosage_form': 'Tablet'
		}).insert()
...
def create_drug(item=None):
	...
	drug = vmraid.db.exists('Item', {'item_code': 'Dextromethorphan'})
	if not drug:
		drug = vmraid.get_doc({
			'doctype': 'Item',
...
def create_ipmo(patient):
	orders = get_orders()
	ipmo = vmraid.new_doc('Inpatient Medication Order')
...
def create_ipme(filters, update_stock=0):
	ipme = vmraid.new_doc('Inpatient Medication Entry')
