...
vmraid.ui.form.on('Clinical Procedure Template', {
...
let change_template_code = function(doc) {
	let d = new vmraid.ui.Dialog({
		...
		primary_action: function() {
			...
			if (values) {
				vmraid.call({
					'method': 'erpadda.healthcare.doctype.clinical_procedure_template.clinical_procedure_template.change_item_code_from_template',
...
vmraid.ui.form.on('Clinical Procedure Item', {
	qty: function(frm, cdt, cdn) {
		let d = locals[cdt][cdn];
		vmraid.model.set_value(cdt, cdn, 'transfer_qty', d.qty * d.conversion_factor);
	},
	...
	item_code: function(frm, cdt, cdn) {
		...
		if (d.item_code) {
			...
			return vmraid.call({
				method: 'erpadda.healthcare.doctype.clinical_procedure_template.clinical_procedure_template.get_item_details',
				args: {args: args},
				callback: function(r) {
					if (r.message) {
						let d = locals[cdt][cdn];
...
vmraid.tour['Clinical Procedure Template'] = [
	{
		fieldname: 'template',
	...
	},
	{
		...
		description: __('Check this if the Clinical Procedure utilises consumables. Click ') + "<a href='https://docs.erpadda.com/docs/user/manual/en/healthcare/clinical_procedure_template#22-manage-procedure-consumables' target='_blank'>here</a>" + __(' to know more')
