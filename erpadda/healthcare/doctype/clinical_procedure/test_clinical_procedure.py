...
import unittest
import vmraid
from erpadda.healthcare.doctype.patient_appointment.test_patient_appointment import create_healthcare_docs, create_clinical_procedure_template
...
class TestClinicalProcedure(unittest.TestCase):
	def test_procedure_template_item(self):
		...
		self.assertTrue(vmraid.db.exists('Item', procedure_template.item))

		procedure_template.disabled = 1
		procedure_template.save()
		self.assertEqual(vmraid.db.get_value('Item', procedure_template.item, 'disabled'), 1)
	...
	def test_consumables(self):
		...
		# check consumption
		self.assertTrue(vmraid.db.exists('Stock Entry', result))


def create_consumable():
	if vmraid.db.exists('Item', 'Syringe'):
		return vmraid.get_doc('Item', 'Syringe')
	consumable = vmraid.new_doc('Item')
