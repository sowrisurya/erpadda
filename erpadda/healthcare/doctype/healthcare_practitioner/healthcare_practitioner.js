...
vmraid.ui.form.on('Healthcare Practitioner', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		vmraid.dynamic_link = {doc: frm.doc, fieldname: 'name', doctype: 'Healthcare Practitioner'};
...
vmraid.ui.form.on('Healthcare Practitioner', 'user_id',function(frm) {
	if (frm.doc.user_id) {
		vmraid.call({
			...
			callback: function (data) {
				...
				vmraid.model.get_value('Employee', {'user_id': frm.doc.user_id}, 'name',
					function(data) {
						if (data) {
							if (!frm.doc.employee || frm.doc.employee != data.name)
								vmraid.model.set_value(frm.doctype, frm.docname, 'employee', data.name);
						} else {
							vmraid.model.set_value(frm.doctype, frm.docname, 'employee', '');
						}
					}
				);

				if (!frm.doc.first_name || frm.doc.first_name != data.message.first_name)
					vmraid.model.set_value(frm.doctype,frm.docname, 'first_name', data.message.first_name);
				if (!frm.doc.middle_name || frm.doc.middle_name != data.message.middle_name)
					vmraid.model.set_value(frm.doctype,frm.docname, 'middle_name', data.message.middle_name);
				if (!frm.doc.last_name || frm.doc.last_name != data.message.last_name)
					vmraid.model.set_value(frm.doctype,frm.docname, 'last_name', data.message.last_name);
				if (!frm.doc.mobile_phone || frm.doc.mobile_phone != data.message.mobile_no)
					vmraid.model.set_value(frm.doctype,frm.docname, 'mobile_phone', data.message.mobile_no);
			}
		});
	}
});

vmraid.ui.form.on('Healthcare Practitioner', 'employee', function(frm) {
	if (frm.doc.employee){
		vmraid.call({
			'method': 'vmraid.client.get',
			args: {
			...
			callback: function (data) {
				...
				if (!frm.doc.designation || frm.doc.designation != data.message.designation)
					vmraid.model.set_value(frm.doctype,frm.docname, 'designation', data.message.designation);
				if (!frm.doc.first_name || !frm.doc.user_id){
					vmraid.model.set_value(frm.doctype,frm.docname, 'first_name', data.message.first_name);
					vmraid.model.set_value(frm.doctype,frm.docname, 'middle_name', '');
					vmraid.model.set_value(frm.doctype,frm.docname, 'last_name', data.message.last_name);
				}
				if (!frm.doc.mobile_phone || !frm.doc.user_id)
					vmraid.model.set_value(frm.doctype,frm.docname, 'mobile_phone', data.message.cell_number);
				if (!frm.doc.address || frm.doc.address != data.message.current_address)
					vmraid.model.set_value(frm.doctype,frm.docname, 'address', data.message.current_address);
			}
		});
	}
});

vmraid.tour['Healthcare Practitioner'] = [
	{
		fieldname: 'employee',
		title: __('Employee'),
