...
vmraid.ui.form.on('Patient', {
	refresh: function (frm) {
		...
		if (vmraid.defaults.get_default('patient_name_by') != 'Naming Series') {
			frm.toggle_display('naming_series', false);
		} else {
			erpadda.toggle_naming_series();
		}

		if (vmraid.defaults.get_default('collect_registration_fee') && frm.doc.status == 'Disabled') {
			frm.add_custom_button(__('Invoice Patient Registration'), function () {
				invoice_registration(frm);
			});
		}

		if (frm.doc.patient_name && vmraid.user.has_role('Physician')) {
			frm.add_custom_button(__('Patient History'), function() {
				vmraid.route_options = {'patient': frm.doc.name};
				vmraid.set_route('patient_history');
			},'View');
		}

		if (!frm.doc.__islocal && (vmraid.user.has_role('Nursing User') || vmraid.user.has_role('Physician'))) {
...
vmraid.ui.form.on('Patient', 'dob', function(frm) {
	if (frm.doc.dob) {
		let today = new Date();
		let birthDate = new Date(frm.doc.dob);
		if (today < birthDate){
			vmraid.msgprint(__('Please select a valid Date'));
			vmraid.model.set_value(frm.doctype,frm.docname, 'dob', '');
		}
		else {
...
vmraid.ui.form.on('Patient Relation', {
...
let create_vital_signs = function (frm) {
	if (!frm.doc.name) {
		vmraid.throw(__('Please save the patient first'));
	}
	vmraid.route_options = {
		'patient': frm.doc.name,
	};
...
let create_encounter = function (frm) {
	...
	vmraid.new_doc('Patient Encounter');
};
		...
		callback: function(data) {
			if (!data.exc) {
				if (data.message.invoice) {
					vmraid.set_route('Form', 'Sales Invoice', data.message.invoice);
				}
				cur_frm.reload_doc();
			}
		}
