...
vmraid.ui.form.on('Job Card', {
	...
	prepare_timer_buttons: function(frm) {
		...
		if (!frm.doc.job_started) {
			frm.add_custom_button(__("Start"), () => {
				if (!frm.doc.employee) {
					vmraid.prompt({fieldtype: 'Link', label: __('Employee'), options: "Employee",
			...
			frm.add_custom_button(__("Resume"), () => {
				vmraid.flags.resume_job = 1;
				frm.events.start_job(frm);
			}).addClass("btn-primary");
		} else {
			frm.add_custom_button(__("Pause"), () => {
				vmraid.flags.pause_job = 1;
				frm.set_value("status", "On Hold");
				frm.events.complete_job(frm);
			});

			frm.add_custom_button(__("Complete"), () => {
				let completed_time = vmraid.datetime.now_datetime();
	...
	start_job: function(frm) {
		let row = vmraid.model.add_child(frm.doc, 'Job Card Time Log', 'time_logs');
	...
	complete_job: function(frm, completed_time, completed_qty) {
		frm.doc.time_logs.forEach(d => {
			if (d.from_time && !d.to_time) {
				d.to_time = completed_time || vmraid.datetime.now_datetime();
				d.completed_qty = completed_qty || 0;

				if(vmraid.flags.pause_job) {
