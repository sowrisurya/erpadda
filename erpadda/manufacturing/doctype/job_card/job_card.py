...
from vmraid.utils import (flt, cint, time_diff_in_hours, get_datetime, getdate,
...
class OperationMismatchError(vmraid.ValidationError): pass
class OperationSequenceError(vmraid.ValidationError): pass
class JobCardCancelError(vmraid.ValidationError): pass
...
class JobCard(Document):
	...
	def validate_time_logs(self):
		...
		if self.get('time_logs'):
			for d in self.get('time_logs'):
				...
				if data:
					vmraid.throw(_("Row {0}: From Time and To Time of {1} is overlapping with {2}")
	...
	def get_overlap_for(self, args, check_next_available_slot=False):
		...
		if self.workstation:
			production_capacity = vmraid.get_cached_value("Workstation",
		...
		existing = vmraid.db.sql("""select jc.name as name, jctl.to_time from
	...
	def check_workstation_time(self, row):
		...
		if (not workstation_doc.working_hours or
			cint(vmraid.db.get_single_value("Manufacturing Settings", "allow_overtime"))):
	...
	def get_required_items(self):
		...
		for d in doc.required_items:
			if not d.operation:
				vmraid.throw(_("Row {0} : Operation is required against the raw material item {1}")
	...
	def validate_job_card(self):
		if not self.time_logs:
			vmraid.throw(_("Time logs are required for {0} {1}")
		...
		if self.for_quantity and self.total_completed_qty != self.for_quantity:
			...
			vmraid.throw(_("The {0} ({1}) must be equal to {2} ({3})")
	...
	def validate_produced_quantity(self, for_quantity, wo):
		...
		if wo.produced_qty > for_quantity:
			first_part_msg = (_("The {0} {1} is used to calculate the valuation cost for the finished good {2}.")
				.format(vmraid.bold(_("Job Card")), vmraid.bold(self.name), vmraid.bold(self.production_item)))
			...
			vmraid.throw(_("{0} {1}").format(first_part_msg, second_part_msg),
	...
	def get_current_operation_data(self):
		return vmraid.get_all('Job Card',
	...
	def set_transferred_qty_in_job_card(self, ste_doc):
		for row in ste_doc.items:
			...
			qty = vmraid.db.sql(""" SELECT SUM(qty) from `tabStock Entry Detail` sed, `tabStock Entry` se
	...
	def set_transferred_qty(self, update_status=False):
		...
		if self.items:
			self.transferred_qty = vmraid.db.get_value('Stock Entry', {
				'job_card': self.name,
		...
		if self.work_order:
			...
			if doc.transfer_material_against == 'Job Card' and not doc.skip_transfer:
				...
				if completed:
					job_cards = vmraid.get_all('Job Card', filters = {'work_order': self.work_order,
	...
	def validate_operation_id(self):
		if (self.get("operation_id") and self.get("operation_row_number") and self.operation and self.work_order and
			vmraid.get_cached_value("Work Order Operation", self.operation_row_number, "name") != self.operation_id):
			work_order = bold(get_link_to_form("Work Order", self.work_order))
			vmraid.throw(_("Operation {0} does not belong to the work order {1}")
	...
	def validate_sequence_id(self):
		...
		data = vmraid.get_all("Work Order Operation",
		...
		for row in data:
			if row.status != "Completed" and row.completed_qty < current_operation_qty:
				vmraid.throw(_("{0}, complete the operation {1} before the operation {2}.")
					.format(message, bold(row.operation), bold(self.operation)), OperationSequenceError)

@vmraid.whitelist()
def get_operation_details(work_order, operation):
	if work_order and operation:
		return vmraid.get_all("Work Order Operation", fields = ["name", "idx"],
			filters = {
				"parent": work_order,
				"operation": operation
			}
...
def get_operations(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.get_all("Work Order Operation",
		filters = args,
...
def get_job_details(start, end, filters=None):
	...
	job_cards = vmraid.db.sql(""" SELECT `tabJob Card`.name, `tabJob Card`.work_order,
