...
from erpadda.stock.get_item_details import get_price_list_rate
...
class BOM(WebsiteGenerator):
	...
	def autoname(self):
		names = vmraid.db.sql_list("""select name from `tabBOM` where item=%s""", self.item)
		...
		if vmraid.db.exists("BOM", name):
			...
			if conflicting_bom.item != self.item:
				msg = (_("A BOM with name {0} already exists for item {1}.")
					.format(vmraid.bold(name), vmraid.bold(conflicting_bom.item)))

				vmraid.throw(_("{0}{1} Did you rename the item? Please contact Administrator / Tech support")
					.format(msg, "<br>"))
	...
	def get_routing(self):
		if self.routing:
			...
			for row in vmraid.get_all("BOM Operation", fields = fields,
	...
	def validate_bom_currecny(self, item):
		if item.get('bom_no') and vmraid.db.get_value('BOM', item.get('bom_no'), 'currency') != self.currency:
			vmraid.throw(_("Row {0}: Currency of the BOM #{1} should be equal to the selected currency {2}")
	...
	def get_rm_rate(self, arg):
		...
		if arg.get('scrap_items'):
			...
			if not vmraid.db.get_value('Item', arg["item_code"], 'is_customer_provided_item') and not arg.get('sourced_by_supplier'):
				if arg.get('bom_no') and self.set_rate_of_sub_assembly_item_based_on_bom:
					...
					if not rate:
						if self.rm_cost_as_per == "Price List":
							vmraid.msgprint(_("Price not found for item {0} in price list {1}")
								.format(arg["item_code"], self.buying_price_list), alert=True)
						else:
							vmraid.msgprint(_("{0} not found for item {1}")
	...
	def update_cost(self, update_parent=True, from_child_bom=False, save=True):
		...
		if self.total_cost != existing_bom_cost and update_parent:
			parent_boms = vmraid.db.sql_list("""select distinct parent from `tabBOM Item`
		...
		if not from_child_bom:
			vmraid.msgprint(_("Cost Updated"))

	def update_parent_cost(self):
		if self.total_cost:
			cost = self.total_cost / self.quantity

			vmraid.db.sql("""update `tabBOM Item` set rate=%s, amount=stock_qty*%s
				where bom_no = %s and docstatus < 2 and parenttype='BOM'""",
				(cost, cost, self.name))

	def get_bom_unitcost(self, bom_no):
		bom = vmraid.db.sql("""select name, base_total_cost/quantity as unit_cost from `tabBOM`
	...
	def manage_default_bom(self):
		...
		elif not vmraid.db.exists(dict(doctype='BOM', docstatus=1, item=self.item, is_default=1)) \
			and self.is_active:
	...
	def validate_currency(self):
		if self.rm_cost_as_per == 'Price List':
			...
			if price_list_currency not in (self.currency, self.company_currency()):
				vmraid.throw(_("Currency of the price list {0} must be {1} or {2}")
	...
	def check_recursion(self, bom_list=[]):
		...
		bom_nos = vmraid.get_all('BOM Item', fields=["bom_no"],
		...
		if not raise_exception:
			bom_nos = vmraid.get_all('BOM Item', fields=["parent"],
		...
		if raise_exception:
			vmraid.throw(_("BOM recursion: {0} cannot be parent or child of {1}").format(self.name, self.name))

	def traverse_tree(self, bom_list=None):
		def _get_children(bom_no):
			children = vmraid.cache().hget('bom_children', bom_no)
			if children is None:
				children = vmraid.db.sql_list("""SELECT `bom_no` FROM `tabBOM Item`
	...
	def calculate_op_cost(self):
		...
		for d in self.get('operations'):
			if d.workstation:
				if not d.hour_rate:
					hour_rate = flt(vmraid.db.get_value("Workstation", d.workstation, "hour_rate"))
	...
	def get_exploded_items(self):
		...
		for d in self.get('items'):
			if d.bom_no:
				...
				self.add_to_cur_exploded_items(vmraid._dict({
					'item_code'		: d.item_code,
	...
	def validate_bom_links(self):
		if not self.is_active:
			act_pbom = vmraid.db.sql("""select distinct bom_item.parent from `tabBOM Item` bom_item
...
def get_bom_item_rate(args, bom_doc):
	if bom_doc.rm_cost_as_per == 'Valuation Rate':
		...
		rate = ( flt(args.get('last_purchase_rate')) \
			or vmraid.db.get_value("Item", args['item_code'], "last_purchase_rate")) \
...
def get_valuation_rate(args):
	...
	if valuation_rate <= 0:
		last_valuation_rate = vmraid.db.sql("""select valuation_rate
...
def get_bom_items_as_dict(bom, company, qty=1, fetch_exploded=1, fetch_scrap_items=0, include_non_stock_items=False, fetch_qty_in_stock_uom=True):
	...
	if cint(fetch_exploded):
		...
		items = vmraid.db.sql(query, { "parent": bom, "qty": qty, "bom": bom, "company": company }, as_dict=True)
...
def validate_bom_no(item, bom_no):
	...
	if item:
		...
		if bom.item.lower() == item.lower() or \
			bom.item.lower() == cstr(vmraid.db.get_value("Item", item, "variant_of")).lower():
 				rm_item_exists = True
...
def get_children(doctype, parent=None, is_root=False, **filters):
	...
	if vmraid.form_dict.parent:
		bom_doc = vmraid.get_doc("BOM", vmraid.form_dict.parent)
		vmraid.has_permission("BOM", doc=bom_doc, throw=True)

		bom_items = vmraid.get_all('BOM Item',
...
def get_boms_in_bottom_up_order(bom_no=None):
	...
	if bom_no:
		...
		# get all leaf BOMs
		bom_list = vmraid.db.sql_list("""select name from `tabBOM` bom
...
def add_additional_cost(stock_entry, work_order):
	...
	expenses_included_in_valuation = vmraid.get_cached_value("Company", work_order.company,
...
def add_non_stock_items_cost(stock_entry, work_order, expense_account):
	...
	non_stock_items = vmraid.get_all('Item',
...
def get_bom_diff(bom1, bom2):
	...
	if bom1 == bom2:
		vmraid.throw(_("BOM 1 {0} and BOM 2 {1} should not be same")
			.format(vmraid.bold(bom1), vmraid.bold(bom2)))
...
def item_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if txt:
		...
		barcodes = vmraid.get_all("Item Barcode",
