...
from vmraid.utils import (flt, cint, getdate, formatdate,
	comma_and, time_diff_in_seconds, to_timedelta, add_days)
from vmraid.model.document import Document
from dateutil.parser import parse

class WorkstationHolidayError(vmraid.ValidationError): pass
class NotInWorkingHoursError(vmraid.ValidationError): pass
...
class Workstation(Document):
	...
	def validate_overlap_for_operation_timings(self):
		...
		for d in self.get("working_hours"):
			existing = vmraid.db.sql_list("""select idx from `tabWorkstation Working Hour`
			...
			if existing:
				vmraid.throw(_("Row #{0}: Timings conflicts with row {1}").format(d.idx, comma_and(existing)), OverlapError)

	def update_bom_operation(self):
		bom_list = vmraid.db.sql("""select DISTINCT parent from `tabBOM Operation`
			where workstation = %s""", self.name)
		for bom_no in bom_list:
			vmraid.db.sql("""update `tabBOM Operation` set hour_rate = %s
	...
	def validate_workstation_holiday(self, schedule_date, skip_holiday_list_check=False):
		if not skip_holiday_list_check and (not self.holiday_list or
			cint(vmraid.db.get_single_value("Manufacturing Settings", "allow_production_on_holidays"))):
...
def get_default_holiday_list():
	return vmraid.get_cached_value('Company',  vmraid.defaults.get_user_default("Company"),  "default_holiday_list")

def check_if_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	if from_datetime and to_datetime:
		if not cint(vmraid.db.get_value("Manufacturing Settings", "None", "allow_production_on_holidays")):
...
def is_within_operating_hours(workstation, operation, from_datetime, to_datetime):
	...
	vmraid.throw(_("Operation {0} longer than any available working hours in workstation {1}, break down the operation into multiple operations").format(operation, workstation.name), NotInWorkingHoursError)
...
def check_workstation_for_holiday(workstation, from_datetime, to_datetime):
	...
	if holiday_list and from_datetime and to_datetime:
		applicable_holidays = []
		for d in vmraid.db.sql("""select holiday_date from `tabHoliday` where parent = %s
		...
		if applicable_holidays:
			vmraid.throw(_("Workstation is closed on the following dates as per Holiday List: {0}")
