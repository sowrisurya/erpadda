...
erpadda.work_order = {
		...
		if (doc.docstatus === 1) {
			if (doc.status != 'Stopped' && doc.status != 'Completed') {
				frm.add_custom_button(__('Stop'), function() {
					erpadda.work_order.stop_work_order(frm, "Stopped");
				}, __("Status"));
			} else if (doc.status == 'Stopped') {
				frm.add_custom_button(__('Re-open'), function() {
					erpadda.work_order.stop_work_order(frm, "Resumed");
				}, __("Status"));
			}
	...
	stop_work_order: function(frm, status) {
		vmraid.call({
			method: "erpadda.manufacturing.doctype.work_order.work_order.stop_unstop",
			args: {
				work_order: frm.doc.name,
				status: status
			},
