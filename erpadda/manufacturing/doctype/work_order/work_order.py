...
from vmraid.utils import flt, get_datetime, getdate, date_diff, cint, nowdate, get_link_to_form, time_diff_in_hours
from vmraid.model.document import Document
from erpadda.manufacturing.doctype.bom.bom import validate_bom_no, get_bom_items_as_dict, get_bom_item_rate
from dateutil.relativedelta import relativedelta
from erpadda.stock.doctype.item.item import validate_end_of_life, get_item_defaults
...
from erpadda.manufacturing.doctype.manufacturing_settings.manufacturing_settings import get_mins_between_operations
from erpadda.stock.stock_balance import get_planned_qty, update_bin_qty
from vmraid.utils.csvutils import getlink
from erpadda.stock.utils import get_bin, validate_warehouse_company, get_latest_stock_qty
from erpadda.utilities.transaction_base import validate_uom_is_integer
from vmraid.model.mapper import get_mapped_doc

class OverProductionError(vmraid.ValidationError): pass
class CapacityError(vmraid.ValidationError): pass
class StockOverProductionError(vmraid.ValidationError): pass
class OperationTooLongError(vmraid.ValidationError): pass
class ItemHasVariantError(vmraid.ValidationError): pass
...
class WorkOrder(Document):
	...
	def set_default_warehouse(self):
		if not self.wip_warehouse:
			self.wip_warehouse = vmraid.db.get_single_value("Manufacturing Settings", "default_wip_warehouse")
	...
	def validate_work_order_against_so(self):
		# already ordered qty
		ordered_qty_against_so = vmraid.db.sql("""select sum(qty) from `tabWork Order`
		...
		so_item_qty = vmraid.db.sql("""select sum(stock_qty) from `tabSales Order Item`
			where parent = %s and item_code = %s""",
			(self.sales_order, self.production_item))[0][0]
		# get qty from Packing Item table
		dnpi_qty = vmraid.db.sql("""select sum(qty) from `tabPacked Item`
		...
		if total_qty > so_qty + (allowance_percentage/100 * so_qty):
			vmraid.throw(_("Cannot produce more Item {0} than Sales Order quantity {1}")
	...
	def get_status(self, status=None):
		...
		if self.docstatus==0:
			...
			if status != 'Stopped':
				stock_entries = vmraid._dict(vmraid.db.sql("""select purpose, sum(fg_completed_qty)
	...
	def update_work_order_qty(self):
		...
		for purpose, fieldname in (("Manufacture", "produced_qty"),
			...
			qty = flt(vmraid.db.sql("""select sum(fg_completed_qty)
				from `tabStock Entry` where work_order=%s and docstatus=1
				and purpose=%s""", (self.name, purpose))[0][0])

			completed_qty = self.qty + (allowance_percentage/100 * self.qty)
			if qty > completed_qty:
				vmraid.throw(_("{0} ({1}) cannot be greater than planned quantity ({2}) in Work Order {3}").format(\
	...
	def update_production_plan_status(self):
		...
		if self.production_plan_item:
			total_qty = vmraid.get_all("Work Order", fields = "sum(produced_qty) as produced_qty",
	...
	def create_job_card(self):
		...
		for i, row in enumerate(self.operations):
			...
			if not row.workstation:
				vmraid.throw(_("Row {0}: select the workstation against the operation {1}")
			...
			if enable_capacity_planning and job_card_doc:
				...
				if date_diff(row.planned_start_time, original_start_time) > plan_days:
					...
					vmraid.throw(_("Unable to find the time slot in the next {0} days for the operation {1}.")
	...
	def validate_cancel(self):
		...
		stock_entry = vmraid.db.sql("""select name from `tabStock Entry`
			where work_order = %s and docstatus = 1""", self.name)
		if stock_entry:
			vmraid.throw(_("Cannot cancel because submitted Stock Entry {0} exists").format(vmraid.utils.get_link_to_form('Stock Entry', stock_entry[0][0])))
	...
	def update_ordered_qty(self):
		if self.production_plan and self.production_plan_item:
			...
			vmraid.db.set_value('Production Plan Item',
	...
	def update_work_order_qty_in_so(self):
		...
		if self.product_bundle_item:
			total_bundle_qty = vmraid.db.sql(""" select sum(qty) from
				`tabProduct Bundle Item` where parent = %s""", (vmraid.db.escape(self.product_bundle_item)))[0][0]
		...
		qty = vmraid.db.sql(""" select sum(qty) from
		...
		vmraid.db.set_value('Sales Order Item',
			self.sales_order_item, 'work_order_qty', flt(work_order_qty/total_bundle_qty, 2))

	def update_completed_qty_in_material_request(self):
		if self.material_request:
			vmraid.get_doc("Material Request", self.material_request).update_completed_qty([self.material_request_item])
	...
	def set_work_order_operations(self):
		...
		if self.use_multi_level_bom:
			bom_list = vmraid.get_doc("BOM", self.bom_no).traverse_tree()
		else:
			bom_list = [self.bom_no]

		operations = vmraid.db.sql("""
			select
	...
	def calculate_time(self):
		bom_qty = vmraid.db.get_value("BOM", self.bom_no, "quantity")
	...
	def get_holidays(self, workstation):
		...
		if holiday_list not in holidays:
			holiday_list_days = [getdate(d[0]) for d in vmraid.get_all("Holiday", fields=["holiday_date"],
	...
	def update_operation_status(self):
		allowance_percentage = flt(vmraid.db.get_single_value("Manufacturing Settings", "overproduction_percentage_for_work_order"))
	...
	def set_actual_dates(self):
		if self.get("operations"):
			...
			data = vmraid.get_all("Stock Entry",
	...
	def validate_production_item(self):
		if vmraid.db.get_value("Item", self.production_item, "has_variants"):
	...
	def update_transaferred_qty_for_required_items(self):
		...
		for d in self.required_items:
			transferred_qty = vmraid.db.sql('''select sum(qty)
	...
	@vmraid.whitelist()
	def make_bom(self):
		data = vmraid.db.sql(""" select sed.item_code, sed.qty, sed.s_warehouse
...
def get_bom_operations(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.get_all('BOM Operation',
...
def get_item_details(item, project = None, skip_bom_info=False):
	...
	if not res["bom_no"]:
		if project:
			...
			vmraid.msgprint(_("Default BOM not found for Item {0} and Project {1}").format(item, project), alert=1)
		else:
			vmraid.throw(_("Default BOM for {0} not found").format(item))

	bom_data = vmraid.db.get_value('BOM', res['bom_no'],
...
def make_stock_entry(work_order_id, purpose, qty=None):
	...
	if work_order.bom_no:
		stock_entry.inspection_required = vmraid.db.get_value('BOM',
...
def validate_operation_data(row):
	if row.get("qty") <= 0:
		vmraid.throw(_("Quantity to Manufacture can not be zero for the operation {0}")
			.format(
	...
	if row.get("qty") > row.get("pending_qty"):
		vmraid.throw(_("For operation {0}: Quantity ({1}) can not be greter than pending quantity({2})")
			.format(
				vmraid.bold(row.get("operation")),
...
def create_job_card(work_order, row, qty=0, enable_capacity_planning=False, auto_create=False):
	...
	if auto_create:
		...
		doc.insert()
		vmraid.msgprint(_("Job card {0} created").format(get_link_to_form("Job Card", doc.name)), alert=True)
