...
import vmraid, json, copy
from vmraid import msgprint, _
from six import string_types, iteritems

from vmraid.model.document import Document
from vmraid.utils import cstr, flt, cint, nowdate, add_days, comma_and, now_datetime, ceil
from vmraid.utils.csvutils import build_csv_response
from erpadda.manufacturing.doctype.bom.bom import validate_bom_no, get_children
...
class ProductionPlan(Document):
	...
	def get_so_items(self):
		...
		items = vmraid.db.sql("""select distinct parent, item_code, warehouse,
		...
		packed_items = vmraid.db.sql("""select distinct pi.parent, pi.item_code, pi.warehouse as warehouse,
	...
	def get_mr_items(self):
		...
		if not self.get("material_requests") or not self.get_so_mr_list("material_request", "material_requests"):
			vmraid.throw(_("Please fill the Material Requests table"), title=_("Material Requests Required"))
		...
		items = vmraid.db.sql("""select distinct parent, name, item_code, warehouse, description,
	...
	def delete_draft_work_order(self):
		for d in vmraid.get_all('Work Order', fields = ["name"],
	...
	def get_production_items(self):
		...
		for d in self.po_items:
			...
			item_details.update({
				"project": self.project or vmraid.db.get_value("Sales Order", d.sales_order, "project")
			})
	...
	def create_work_order(self, item):
		from erpadda.manufacturing.doctype.work_order.work_order import OverProductionError, get_default_warehouse
...
def get_exploded_items(item_details, company, bom_no, include_non_stock_items, planned_qty=1):
	for d in vmraid.db.sql("""select bei.item_code, item.default_bom as bom,
...
def get_material_request_items(row, sales_order, company,
	...
	if row['purchase_uom'] != row['stock_uom']:
		if not row['conversion_factor']:
			vmraid.throw(_("UOM Conversion factor ({0} -> {1}) not found for item: {2}")
...
def get_bin_details(row, company, for_warehouse=None, all_warehouse=False):
	...
	return vmraid.db.sql(""" select ifnull(sum(projected_qty),0) as projected_qty,
...
def get_items_for_material_requests(doc, warehouses=None):
	...
	if not po_items or not [row.get('item_code') for row in po_items if row.get('item_code')]:
		vmraid.throw(_("Items to Manufacture are required to pull the Raw Materials associated with it."),
