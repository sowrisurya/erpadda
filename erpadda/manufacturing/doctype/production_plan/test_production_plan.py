...
import vmraid
import unittest
from vmraid.utils import nowdate, now_datetime, flt
from erpadda.stock.doctype.item.test_item import create_item
from erpadda.manufacturing.doctype.production_plan.production_plan import get_sales_orders
...
class TestProductionPlan(unittest.TestCase):
	def setUp(self):
		for item in ['Test Production Item 1', 'Subassembly Item 1',
			...
			sr = vmraid.db.get_value('Stock Reconciliation Item',
	...
	def test_production_plan(self):
		...
		material_requests = vmraid.get_all('Material Request Item', fields = ['distinct parent'],
			filters = {'production_plan': pln.name}, as_list=1)

		self.assertTrue(len(material_requests), 2)

		pln.make_work_order()
		work_orders = vmraid.get_all('Work Order', fields = ['name'],
			filters = {'production_plan': pln.name}, as_list=1)
	...
	def test_production_plan_sales_orders(self):
		...
		work_order = vmraid.db.get_value('Work Order', {'sales_order': sales_order,
	...
	def test_pp_to_mr_customer_provided(self):
		...
		material_request = vmraid.db.get_value('Material Request Item', {'production_plan': production_plan.name, 'item_code': 'CUST-0987'}, 'parent')
	...
	def test_production_plan_with_multi_level_bom(self):
		...
		to_produce_qty = vmraid.db.get_value("Work Order",
