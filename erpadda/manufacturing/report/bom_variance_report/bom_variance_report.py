...
def get_data(filters):
	...
	results = []
	for d in vmraid.db.sql(""" select name as work_order, qty, produced_qty, production_item, bom_no
		from `tabWork Order` where produced_qty > qty and docstatus = 1 and {0}""".format(cond), as_dict=1):
		results.append(d)

		for data in vmraid.get_all('Work Order Item', fields=["item_code as raw_material_code",
...
def get_work_orders(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.db.sql("""select name from `tabWork Order`
