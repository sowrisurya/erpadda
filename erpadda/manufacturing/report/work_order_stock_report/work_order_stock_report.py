...
def get_item_list(wo_list, filters):
	...
	for wo_details in wo_list:
		...
		for wo_item_details in vmraid.db.get_values("Work Order Item",
			{"parent": wo_details.name}, ["item_code", "source_warehouse"], as_dict=1):

			item_list = vmraid.db.sql("""SELECT
...
def get_work_orders():
	out =  vmraid.get_all("Work Order", filters={"docstatus": 1, "status": ( "!=","Completed")},
