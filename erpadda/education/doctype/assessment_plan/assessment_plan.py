...
class AssessmentPlan(Document):
	...
	def validate_max_score(self):
		...
		if self.maximum_assessment_score != max_score:
			vmraid.throw(_("Sum of Scores of Assessment Criteria needs to be {0}.").format(self.maximum_assessment_score))

	def validate_assessment_criteria(self):
		assessment_criteria_list = vmraid.db.sql_list(''' select apc.assessment_criteria
		...
		for d in self.assessment_criteria:
			if d.assessment_criteria in assessment_criteria_list:
				vmraid.throw(_("You have already assessed for the assessment criteria {}.")
					.format(vmraid.bold(d.assessment_criteria)))
