...
from erpadda.education.report.course_wise_assessment_report.course_wise_assessment_report import get_formatted_result
from erpadda.education.report.course_wise_assessment_report.course_wise_assessment_report import get_child_assessment_groups
...
def preview_report_card(doc):
	...
	if not (doc.student_name and doc.student_batch):
		program_enrollment = vmraid.get_all("Program Enrollment", fields=["student_batch_name", "student_name"],
	...
	template = "erpadda/education/doctype/student_report_generation_tool/student_report_generation_tool.html"
	base_template_path = "vmraid/www/printview.html"

	from vmraid.www.printview import get_letter_head
	letterhead = get_letter_head(vmraid._dict({"letter_head": doc.letterhead}), not doc.add_letterhead)

	html = vmraid.render_template(template,
		{
			"doc": doc,
	...
	final_template = vmraid.render_template(base_template_path, {"body": html, "title": "Report Card"})
...
def get_courses_criteria(courses):
	...
	for course in courses:
		course_criteria[course] = [d.assessment_criteria for d in vmraid.get_all("Course Assessment Criteria",
...
def get_attendance_count(student, academic_year, academic_term=None):
	if academic_year:
		from_date, to_date = vmraid.db.get_value("Academic Year", academic_year, ["year_start_date", "year_end_date"])
	elif academic_term:
		from_date, to_date = vmraid.db.get_value("Academic Term", academic_term, ["term_start_date", "term_end_date"])
	if from_date and to_date:
		attendance = dict(vmraid.db.sql('''select status, count(student) as no_of_days
