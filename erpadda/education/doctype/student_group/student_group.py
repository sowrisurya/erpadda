...
class StudentGroup(Document):
	...
	def validate_strength(self):
		...
		if self.max_strength and len(self.students) > self.max_strength:
			vmraid.throw(_("""Cannot enroll more than {0} students for this student group.""").format(self.max_strength))
	...
	def validate_students(self):
		...
		for d in self.students:
			...
			if (self.group_based_on == "Batch") and cint(vmraid.defaults.get_defaults().validate_batch)\
				and d.student not in students:
				vmraid.throw(_("{0} - {1} is not enrolled in the Batch {2}").format(d.group_roll_number, d.student_name, self.batch))

			if (self.group_based_on == "Course") and cint(vmraid.defaults.get_defaults().validate_course)\
				and (d.student not in students):
				vmraid.throw(_("{0} - {1} is not enrolled in the Course {2}").format(d.group_roll_number, d.student_name, self.course))
...
def fetch_students(doctype, txt, searchfield, start, page_len, filters):
	if filters.get("group_based_on") != "Activity":
		...
		student_group_student = vmraid.db.sql_list('''select student from `tabStudent Group Student` where parent=%s''',
		...
		return vmraid.db.sql("""select name, title from tabStudent
