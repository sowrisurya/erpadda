...
from vmraid.utils import get_link_to_form, getdate, formatdate
...
class StudentAttendance(Document):
	...
	def validate_mandatory(self):
		if not (self.student_group or self.course_schedule):
			vmraid.throw(_('{0} or {1} is mandatory').format(vmraid.bold('Student Group'),
				vmraid.bold('Course Schedule')), title=_('Mandatory Fields'))
	...
	def set_student_group(self):
		if self.course_schedule:
			self.student_group = vmraid.db.get_value('Course Schedule', self.course_schedule, 'student_group')
	...
	def validate_student(self):
		...
		if student_group and self.student not in student_group_students:
			...
			vmraid.throw(_('Student {0}: {1} does not belong to Student Group {2}').format(
	...
	def validate_duplication(self):
		...
		if self.course_schedule:
			attendance_record = vmraid.db.exists('Student Attendance', {
		...
		if attendance_record:
			...
			vmraid.throw(_('Student Attendance record {0} already exists against the Student {1}')
				.format(record, vmraid.bold(self.student)), title=_('Duplicate Entry'))

	def validate_is_holiday(self):
		holiday_list = get_holiday_list()
		if is_holiday(holiday_list, self.date):
			vmraid.throw(_('Attendance cannot be marked for {0} as it is a holiday.').format(
...
def get_holiday_list(company=None):
	...
	if not holiday_list:
		vmraid.throw(_('Please set a default Holiday List for Company {0}').format(vmraid.bold(get_default_company())))
	return holiday_list
