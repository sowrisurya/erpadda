...
from vmraid.utils import cint, flt, cstr
...
class FeeSchedule(Document):
	...
	def get_dashboard_info(self):
		...
		fees_amount = vmraid.db.sql("""select sum(grand_total), sum(outstanding_amount) from tabFees
	...
	def calculate_total_and_program(self):
		...
		for d in self.student_groups:
			...
			if self.program and student_group_program and self.program != student_group_program:
				vmraid.msgprint(_("Program in the Fee Structure and Student Group {0} are different.")
	...
	def create_fees(self):
		...
		vmraid.publish_realtime("fee_schedule_progress",
			{"progress": "0", "reload": 1}, user=vmraid.session.user)

		total_records = sum([int(d.total_students) for d in self.student_groups])
		if total_records > 10:
			vmraid.msgprint(_('''Fee records will be created in the background.
...
def generate_fee(fee_schedule):
	...
	for d in doc.student_groups:
		...
		for student in students:
			try:
				...
				fees_doc.submit()
				created_records += 1
				vmraid.publish_realtime("fee_schedule_progress", {"progress": str(int(created_records * 100/total_records))}, user=vmraid.session.user)
	...
	vmraid.publish_realtime("fee_schedule_progress",
		{"progress": "100", "reload": 1}, user=vmraid.session.user)
