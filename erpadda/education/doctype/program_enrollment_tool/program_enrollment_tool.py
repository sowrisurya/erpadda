...
class ProgramEnrollmentTool(Document):
	...
	def get_students(self):
		...
		if not self.get_students_from:
			...
			if self.get_students_from == "Student Applicant":
				students = vmraid.db.sql('''select name as student_applicant, title as student_name from `tabStudent Applicant`
				...
				students = vmraid.db.sql('''select student, student_name, student_batch_name, student_category from `tabProgram Enrollment`
	...
	def enroll_students(self):
		...
		for i, stud in enumerate(self.students):
			vmraid.publish_realtime("program_enrollment_tool", dict(progress=[i+1, total]), user=vmraid.session.user)
			if stud.student:
