...
from erpadda.education.api import get_assessment_details
...
class AssessmentResult(Document):
	...
	def validate_duplicate(self):
		assessment_result = vmraid.get_list("Assessment Result", filters={"name": ("not in", [self.name]),
			"student":self.student, "assessment_plan":self.assessment_plan, "docstatus":("!=", 2)})
		if assessment_result:
			vmraid.throw(_("Assessment Result record {0} already exists.").format(getlink("Assessment Result",assessment_result[0].name)))
