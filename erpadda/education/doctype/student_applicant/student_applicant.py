...
from vmraid.utils import getdate, add_years, nowdate, date_diff
...
class StudentApplicant(Document):
	...
	def on_update_after_submit(self):
		...
		if student:
			vmraid.throw(_("Cannot change status as student {0} is linked with student application {1}").format(student[0].name, self.name))

	def on_submit(self):
		if self.paid and not self.student_admission:
			vmraid.throw(_("Please select Student Admission which is mandatory for the paid student applicant"))
...
def get_student_admission_data(student_admission, program):
	...
	student_admission = vmraid.db.sql("""select sa.admission_start_date, sa.admission_end_date,
