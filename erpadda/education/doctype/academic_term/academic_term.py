...
class AcademicTerm(Document):
    ...
    def validate(self):
        ...
        if self.term_start_date and self.term_end_date \
            ...
            vmraid.throw(_("The Term End Date cannot be earlier than the Term Start Date. Please correct the dates and try again."))
        ...
        if self.term_start_date and getdate(year.year_start_date) and (getdate(self.term_start_date) < getdate(year.year_start_date)):
            vmraid.throw(_("The Term Start Date cannot be earlier than the Year Start Date of the Academic Year to which the term is linked (Academic Year {}). Please correct the dates and try again.").format(self.academic_year))

        if self.term_end_date and getdate(year.year_end_date) and (getdate(self.term_end_date) > getdate(year.year_end_date)):
            vmraid.throw(_("The Term End Date cannot be later than the Year End Date of the Academic Year to which the term is linked (Academic Year {}). Please correct the dates and try again.").format(self.academic_year))


def validate_duplication(self):
    term = vmraid.db.sql("""select name from `tabAcademic Term` where academic_year= %s and term_name= %s
    and docstatus<2 and name != %s""", (self.academic_year, self.term_name, self.name))
    if term:
        vmraid.throw(_("An academic term with this 'Academic Year' {0} and 'Term Name' {1} already exists. Please modify these entries and try again.").format(self.academic_year,self.term_name))
