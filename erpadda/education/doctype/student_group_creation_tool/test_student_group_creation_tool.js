...
QUnit.test('Test: Student Group Creation Tool', function(assert){
	...
	vmraid.run_serially([
		...
		() => vmraid.set_route('Form', 'Student Group Creation Tool'),
		() => vmraid.timeout(0.5),
		() => {
			...
			vmraid.tests.click_button('Get Courses');
		},
		() => vmraid.timeout(1),
		() => {
		...
		() => vmraid.timeout(0.5),
		() => vmraid.tests.click_button("Create Student Groups"),
		() => vmraid.timeout(0.5),
