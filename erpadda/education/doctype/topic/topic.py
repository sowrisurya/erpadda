...
class Topic(Document):
	def get_contents(self):
		try:
			...
			content_data = [vmraid.get_doc(topic_content.content_type, topic_content.content) for topic_content in topic_content_list]
...
def add_topic_to_courses(topic, courses, mandatory=False):
	...
	for entry in courses:
		...
		course.save()
	vmraid.db.commit()
	vmraid.msgprint(_('Topic {0} has been added to all the selected courses successfully.').format(vmraid.bold(topic)),
...
def add_content_to_topics(content_type, content, topics):
	...
	for entry in topics:
		...
		topic.save()
	vmraid.db.commit()
	vmraid.msgprint(_('{0} {1} has been added to all the selected topics successfully.').format(content_type, vmraid.bold(content)),
