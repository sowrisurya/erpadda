...
import datetime
from vmraid.utils import today, to_timedelta
...
class TestCourseSchedule(unittest.TestCase):
	def test_student_group_conflict(self):
		...
		cs2 = make_course_schedule_test_record(schedule_date=cs1.schedule_date, from_time= cs1.from_time, 
			to_time= cs1.to_time, instructor="_Test Instructor 2", room=vmraid.get_all("Room")[1].name, do_not_save= 1)
	...
	def test_instructor_conflict(self):
		...
		cs2 = make_course_schedule_test_record(from_time= cs1.from_time, to_time= cs1.to_time, 
			student_group="Course-TC101-2014-2015 (_Test Academic Term)", room=vmraid.get_all("Room")[1].name, do_not_save= 1)
		self.assertRaises(OverlapError, cs2.save)
	...
	def test_no_conflict(self):
		...
		make_course_schedule_test_record(from_time= cs1.from_time, to_time= cs1.to_time, 
			student_group="Course-TC102-2014-2015 (_Test Academic Term)", instructor="_Test Instructor 2", room=vmraid.get_all("Room")[1].name)
