...
import vmraid
import unittest
from vmraid.utils import getdate, add_days, add_months
...
class TestStudentLeaveApplication(unittest.TestCase):
	...
	def test_attendance_record_creation(self):
		...
		attendance_record = vmraid.db.exists('Student Attendance', {'leave_application': leave_application.name, 'status': 'Absent'})
		self.assertTrue(attendance_record)

		# mark as present
		date = add_days(getdate(), -1)
		leave_application = create_leave_application(date, date, 1)
		attendance_record = vmraid.db.exists('Student Attendance', {'leave_application': leave_application.name, 'status': 'Present'})
		self.assertTrue(attendance_record)
	...
	def test_attendance_record_cancellation(self):
		...
		attendance_status = vmraid.db.get_value('Student Attendance', {'leave_application': leave_application.name}, 'docstatus')
	...
	def test_holiday(self):
		...
		self.assertIsNone(vmraid.db.exists('Student Attendance', {'leave_application': leave_application.name, 'date': add_days(today, 1)}))
