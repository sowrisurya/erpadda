...
from vmraid.utils import get_link_to_form, getdate, date_diff, flt
...
class StudentLeaveApplication(Document):
	...
	def validate_duplicate(self):
		data = vmraid.db.sql("""select name from `tabStudent Leave Application`
			where
		...
		if data:
			...
			vmraid.throw(_('Leave application {0} already exists against the student {1}')
				.format(link, vmraid.bold(self.student)), title=_('Duplicate Entry'))
	...
	def update_attendance(self):
		...
		for dt in daterange(getdate(self.from_date), getdate(self.to_date)):
			...
			attendance = vmraid.db.exists('Student Attendance', {
				'student': self.student,
				'date': date,
				'docstatus': ('!=', 2)
			})
