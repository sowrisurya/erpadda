...
def validate_overlap_for(doc, doctype, fieldname, value=None):
	...
	if existing:
		vmraid.throw(_("This {0} conflicts with {1} for {2} {3}").format(doc.doctype, existing.name,
...
def get_overlap_for(doc, doctype, fieldname, value=None):
	...
	existing = vmraid.db.sql("""select name, from_time, to_time from `tab{0}`
...
def validate_duplicate_student(students):
	...
	for stud in students:
		if stud.student in unique_students:
			vmraid.throw(_("Student {0} - {1} appears Multiple times in row {2} & {3}")
...
def get_enrollment(master, document, student):
	...
	"""
	if master == 'program':
		enrollments = vmraid.get_all("Program Enrollment", filters={'student':student, 'program': document, 'docstatus': 1})
	if master == 'course':
		enrollments = vmraid.get_all("Course Enrollment", filters={'student':student, 'course': document})
...
def add_activity(course, content_type, content, program):
	...
	student = get_current_student()
	if not student:
		return vmraid.throw(_("Student with email {0} does not exist").format(vmraid.session.user), vmraid.DoesNotExistError)
...
def get_program_completion(program):
	topics = vmraid.db.sql("""select `tabCourse Topic`.topic, `tabCourse Topic`.parent
...
def check_content_completion(content_name, content_type, enrollment_name):
	activity = vmraid.get_all("Course Activity", filters={'enrollment': enrollment_name, 'content_type': content_type, 'content': content_name})
	if activity:
		return True
	else:
		return False
...
def check_quiz_completion(quiz, enrollment_name):
	attempts = vmraid.get_all("Quiz Activity",
		filters={
