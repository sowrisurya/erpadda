...
from vmraid.utils import flt, cstr, getdate
...
def get_course(program):
	...
	'''
	courses = vmraid.db.sql('''select course, course_name from `tabProgram Course` where parent=%s''',
			(program), as_dict=1)
	return courses
...
def enroll_student(source_name):
	...
	program_enrollment.program = vmraid.db.get_value("Student Applicant", source_name, "program")
...
def check_attendance_records_exist(course_schedule=None, student_group=None, date=None):
	...
	if course_schedule:
		...
		return vmraid.get_list("Student Attendance", filters={"student_group": student_group, "date": date})
...
def mark_attendance(students_present, students_absent, course_schedule=None, student_group=None, date=None):
	...
	if student_group:
		...
		if academic_year:
			year_start_date, year_end_date = vmraid.db.get_value('Academic Year', academic_year, ['year_start_date', 'year_end_date'])
			if getdate(date) < getdate(year_start_date) or getdate(date) > getdate(year_end_date):
				vmraid.throw(_('Attendance cannot be marked outside of Academic Year {0}').format(academic_year))
...
def get_student_guardians(student):
	...
	:param student: Student.
	"""
	guardians = vmraid.get_list("Student Guardian", fields=["guardian"] ,
...
def get_student_group_students(student_group, include_inactive=0):
	...
	"""
	if include_inactive:
		students = vmraid.get_list("Student Group Student", fields=["student", "student_name"] ,
...
def get_fee_structure(program, academic_term=None):
	...
	"""
	fee_structure = vmraid.db.get_values("Fee Structure", {"program": program,
...
def get_fee_components(fee_structure):
	...
	"""
	if fee_structure:
		fs = vmraid.get_list("Fee Component", fields=["fees_category", "description", "amount"] , filters={"parent": fee_structure}, order_by= "idx")
		return fs
...
def get_fee_schedule(program, student_category=None):
	...
	"""
	fs = vmraid.get_list("Program Fee", fields=["academic_term", "fee_structure", "due_date", "amount"] ,
...
def get_course_schedule_events(start, end, filters=None):
	...
	data = vmraid.db.sql("""select name, course, color,
...
def get_assessment_criteria(course):
	...
	:param Course: Course
	"""
	return vmraid.get_list("Course Assessment Criteria", \
...
def get_assessment_details(assessment_plan):
	...
	"""
	return vmraid.get_list("Assessment Plan Criteria", \
...
def get_result(student, assessment_plan):
	...
	:param Assessment Plan: Assessment Plan
	"""
	results = vmraid.get_all("Assessment Result", filters={"student": student,
...
def get_grade(grading_scale, percentage):
	...
	if not hasattr(vmraid.local, 'grading_scale'):
		grading_scale = vmraid.get_all("Grading Scale Interval", fields=["grade_code", "threshold"], filters={"parent": grading_scale})
...
def get_assessment_result_doc(student, assessment_plan):
	assessment_result = vmraid.get_all("Assessment Result", filters={"student": student,
			"assessment_plan": assessment_plan, "docstatus": ("!=", 2)})
