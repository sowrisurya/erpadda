...
import vmraid
from vmraid.utils import cstr, cint, getdate, get_first_day, get_last_day, date_diff, add_days
...
def get_attendance_list(from_date, to_date, student_group, students_list):
	attendance_list = vmraid.db.sql('''select student, date, status
...
def get_attendance_years():
	year_list = vmraid.db.sql_list('''select distinct YEAR(date) from `tabStudent Attendance` ORDER BY YEAR(date) DESC''')
	if not year_list:
