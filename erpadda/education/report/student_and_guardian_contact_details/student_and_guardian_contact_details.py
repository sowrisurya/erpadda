...
def execute(filters=None):
	...
	program_enrollments = vmraid.get_list("Program Enrollment", fields=["student", "student_name"],
...
def get_guardian_map(student_list):
	...
	guardian_mobile_no = dict(vmraid.db.sql("""select name, mobile_number from `tabGuardian`
			where name in (%s)""" % ", ".join(['%s']*len(guardian_list)), tuple(guardian_list)))

	guardian_email_id = dict(vmraid.db.sql("""select name, email_address from `tabGuardian`
			where name in (%s)""" % ", ".join(['%s']*len(guardian_list)), tuple(guardian_list)))
...
def get_student_roll_no(academic_year, program, batch):
	student_group = vmraid.get_all("Student Group",
		filters={"academic_year":academic_year, "program":program, "batch":batch, "disabled": 0})
	if student_group:
		roll_no_dict = dict(vmraid.db.sql('''select student, group_roll_number from `tabStudent Group Student` where parent=%s''',
