...
def execute(filters=None):
	...
	if is_holiday(holiday_list, filters.get("date")):
		msgprint(_("No attendance has been marked for {0} as it is a Holiday").format(vmraid.bold(formatdate(filters.get("date")))))
...
def get_active_student_group():
	active_student_groups = vmraid.db.sql("""select name from `tabStudent Group` where group_based_on = "Batch"
		and academic_year=%s order by name""", (vmraid.defaults.get_defaults().academic_year), as_dict=1)
	return active_student_groups

def get_student_group_strength(student_group):
	student_group_strength = vmraid.db.sql("""select count(*) from `tabStudent Group Student`
		where parent = %s and active=1""", student_group)[0][0]
	return student_group_strength

def get_student_attendance(student_group, date):
	student_attendance = vmraid.db.sql("""select count(*) as count, status from `tabStudent Attendance` where
