...
from vmraid.utils import (flt, getdate, get_first_day, add_months, add_days, formatdate, cstr, cint)
...
def get_fiscal_year_data(from_fiscal_year, to_fiscal_year):
	fiscal_year = vmraid.db.sql("""select min(year_start_date) as year_start_date,
...
def set_gl_entries_by_account(
	...
	accounts = vmraid.db.sql_list("""select name from `tabAccount`
		where lft >= %s and rgt <= %s and company = %s""", (root_lft, root_rgt, company))

	if accounts:
		additional_conditions += " and account in ({})"\
			.format(", ".join([vmraid.db.escape(d) for d in accounts]))
		...
		if filters.get("include_default_book_entries"):
			gl_filters["company_fb"] = vmraid.db.get_value("Company",
		...
		gl_entries = vmraid.db.sql("""select posting_date, account, debit, credit, is_opening, fiscal_year, debit_in_account_currency, credit_in_account_currency, account_currency from `tabGL Entry`
			where company=%(company)s
			{additional_conditions}
			and posting_date <= %(to_date)s
			and is_cancelled = 0
