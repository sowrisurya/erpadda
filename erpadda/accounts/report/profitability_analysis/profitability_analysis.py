...
from erpadda.accounts.report.financial_statements import filter_accounts, filter_out_zero_value_rows
...
def get_accounts_data(based_on, company):
	if based_on == 'cost_center':
		return vmraid.db.sql("""select name, parent_cost_center as parent_account, cost_center_name as account_name, lft, rgt
...
def prepare_data(accounts, filters, total_row, parent_children_map, based_on):
	...
	for d in accounts:
		...
		if based_on == 'cost_center':
			cost_center_doc = vmraid.get_doc("Cost Center",d.name)
			if not cost_center_doc.enable_distributed_cost_center:
				DCC_allocation = vmraid.db.sql("""SELECT parent, sum(percentage_allocation) as percentage_allocation
...
def set_gl_entries_by_account(company, from_date, to_date, based_on, gl_entries_by_account,
	...
	gl_entries = vmraid.db.sql("""select posting_date, {based_on} as based_on, debit, credit,
