...
from erpadda.accounts.doctype.tax_withholding_category.tax_withholding_category \
...
def get_result(filters):
	...
	if filters.supplier:
		filters.supplier = vmraid.db.get_list('Supplier',
	...
	for supplier in filters.supplier:
		tds = vmraid.get_doc("Tax Withholding Category", supplier.tax_withholding_category)
...
def get_invoice_and_tds_amount(supplier, account, company, from_date, to_date, fiscal_year):
	...
	tds_deducted = 0
	if vouchers:
		tds_deducted = flt(vmraid.db.sql("""
			select sum(credit)
			from `tabGL Entry`
