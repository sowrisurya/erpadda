# Copyright (c) 2015, VMRaid Technologies Pvt. Ltd.
# License: GNU General Public License v3. See license.txt

from __future__ import unicode_literals
import vmraid, erpadda
from vmraid import _, scrub
from vmraid.utils import getdate, nowdate, flt, cint, formatdate, cstr, now, time_diff_in_seconds
...
class ReceivablePayableReport(object):
	...
	def run(self, args):
		...
		self.set_defaults()
		self.party_naming_by = vmraid.db.get_value(args.get("naming_by")[0], None, args.get("naming_by")[1])
		self.get_columns()
		self.get_data()
		self.get_chart_data()
	...
	def set_defaults(self):
		...
		self.company_currency = vmraid.get_cached_value('Company',  self.filters.get("company"), "default_currency")
	...
	def get_cost_center_conditions(self, conditions):
		...
		cost_center_list = [center.name for center in vmraid.get_list("Cost Center", filters = {'lft': (">=", lft), 'rgt': ("<=", rgt)})]
	...
	def add_common_filters(self, conditions, values, party_type_field):
		...
		accounts = [d.name for d in vmraid.get_all("Account",
	...
	def get_party_details(self, party):
		if not party in self.party_details:
			if self.party_type == 'Customer':
				self.party_details[party] = vmraid.db.get_value('Customer', party, ['customer_name',
					'territory', 'customer_group', 'customer_primary_contact'], as_dict=True)
			else:
				self.party_details[party] = vmraid.db.get_value('Supplier', party, ['supplier_name',
