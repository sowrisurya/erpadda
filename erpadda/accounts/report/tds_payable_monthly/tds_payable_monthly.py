...
def execute(filters=None):
	filters["invoices"] = vmraid.cache().hget("invoices", vmraid.session.user)
...
def get_supplier_map(filters):
	...
	supplier_detail = vmraid.db.get_all('Supplier',
	...
	for d in supplier_detail:
		if d.get("tax_withholding_category") not in tds_docs:
			tds_docs[d.get("tax_withholding_category")] = \
				vmraid.get_doc("Tax Withholding Category", d.get("tax_withholding_category"))
...
def get_gle_map(filters):
	...
	gle_map = {}
	gle = vmraid.db.get_all('GL Entry',\
...
def get_tds_invoices():
	...
	suppliers = [d.name for d in vmraid.db.get_list("Supplier",
		{"tax_withholding_category": ["!=", ""]}, ["name"])]

	invoices = vmraid.db.get_list("Purchase Invoice",
		{"supplier": ["in", suppliers]}, ["name", "supplier"])

	invoices = [d for d in invoices if d.supplier]
	vmraid.cache().hset("invoices", vmraid.session.user, invoices)
