...
class GrossProfitGenerator(object):
		...
		for row in self.si_list:
			...
			if row.update_stock:
				...
				product_bundles = self.product_bundles.get("Delivery Note", {})\
					.get(row.delivery_note, vmraid._dict())
	...
	def get_returned_invoice_items(self):
		...
		for inv in returned_invoices:
			self.returned_invoices.setdefault(inv.return_against, vmraid._dict())\
	...
	def get_last_purchase_rate(self, item_code, row):
		condition = ''
		if row.project:
			condition += " AND a.project=%s" % (vmraid.db.escape(row.project))
		elif row.cost_center:
			condition += " AND a.cost_center=%s" % (vmraid.db.escape(row.cost_center))
		if self.filters.to_date:
			condition += " AND modified='%s'" % (self.filters.to_date)

		last_purchase_rate = vmraid.db.sql("""
	...
	def load_stock_ledger_entries(self):
		res = vmraid.db.sql("""select item_code, voucher_type, voucher_no,
	...
	def load_product_bundle(self):
		...
		for d in vmraid.db.sql("""select parenttype, parent, parent_item,
			item_code, warehouse, -1*qty as total_qty, parent_detail_docname
			from `tabPacked Item` where docstatus=1""", as_dict=True):
			self.product_bundles.setdefault(d.parenttype, vmraid._dict()).setdefault(d.parent,
				vmraid._dict()).setdefault(d.parent_item, []).append(d)

	def load_non_stock_items(self):
		self.non_stock_items = vmraid.db.sql_list("""select name from tabItem
