...
from vmraid.utils import flt, cint, getdate
from erpadda.accounts.report.utils import get_currency, convert_to_presentation_currency
from erpadda.accounts.report.financial_statements import get_fiscal_year_data, sort_accounts
from erpadda.accounts.report.balance_sheet.balance_sheet import (get_provisional_profit_loss,
	check_opening_balance, get_chart_data, get_report_summary as get_bs_summary)
from erpadda.accounts.report.profit_and_loss_statement.profit_and_loss_statement import (get_net_profit_loss,
	get_chart_data as get_pl_chart_data, get_report_summary as get_pl_summary)
from erpadda.accounts.report.cash_flow.cash_flow import (get_cash_flow_accounts, get_account_type_based_gl_data,
...
def get_data(companies, root_type, balance_must_be, fiscal_year, filters=None, ignore_closing_entries=False):
	...
	for root in vmraid.db.sql("""select lft, rgt from tabAccount
...
def get_company_currency(filters=None):
	return (filters.get('presentation_currency')
		or vmraid.get_cached_value('Company',  filters.company,  "default_currency"))
...
def get_subsidiary_companies(company):
	lft, rgt = vmraid.get_cached_value('Company',
		company,  ["lft", "rgt"])

	return vmraid.db.sql_list("""select name from `tabCompany`
		where lft >= {0} and rgt <= {1} order by lft, rgt""".format(lft, rgt))

def get_accounts(root_type, filters):
	return vmraid.db.sql(""" select name, is_group, company,
...
def set_gl_entries_by_account(from_date, to_date, root_lft, root_rgt, filters, gl_entries_by_account,
	...
	company_lft, company_rgt = vmraid.get_cached_value('Company',
		filters.get('company'),  ["lft", "rgt"])

	additional_conditions = get_additional_conditions(from_date, ignore_closing_entries, filters)
	companies = vmraid.db.sql(""" select name, default_currency from `tabCompany`
	...
	for d in companies:
		gl_entries = vmraid.db.sql("""select gl.posting_date, gl.account, gl.debit, gl.credit, gl.is_opening, gl.company,
...
def get_account_details(account):
	return vmraid.get_cached_value('Account', account, ['name', 'report_type', 'root_type', 'company',
