...
class PartyLedgerSummaryReport(object):
	...
	def get_data(self):
		company_currency = vmraid.get_cached_value('Company',  self.filters.get("company"),  "default_currency")
		invoice_dr_or_cr = "debit" if self.filters.party_type == "Customer" else "credit"
		reverse_dr_or_cr = "credit" if self.filters.party_type == "Customer" else "debit"

		self.party_data = vmraid._dict({})
		for gle in self.gl_entries:
			self.party_data.setdefault(gle.party, vmraid._dict({
				"party": gle.party,
	...
	def prepare_conditions(self):
		...
		if self.filters.party_type == "Customer":
			if self.filters.get("customer_group"):
				lft, rgt = vmraid.db.get_value("Customer Group",
			...
			if self.filters.get("territory"):
				lft, rgt = vmraid.db.get_value("Territory",
	...
	def get_return_invoices(self):
		...
		self.return_invoices = [d.name for d in vmraid.get_all(doctype, filters={"is_return": 1, "docstatus": 1,
