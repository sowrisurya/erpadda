...
def get_mode_of_payments(filters):
	...
	if invoice_list:
		inv_mop = vmraid.db.sql("""select a.owner,a.posting_date, ifnull(b.mode_of_payment, '') as mode_of_payment
...
def get_invoices(filters):
	conditions = get_conditions(filters)
	return vmraid.db.sql("""select a.name
		from `tabSales Invoice` a
...
def get_mode_of_payment_details(filters):
	...
	invoice_list_names = ",".join(['"' + invoice['name'] + '"' for invoice in invoice_list])
	if invoice_list:
		inv_mop_detail = vmraid.db.sql("""select a.owner, a.posting_date,
		...
		inv_change_amount = vmraid.db.sql("""select a.owner, a.posting_date,
