...
from vmraid.utils import date_diff, add_months, today, getdate, add_days, flt, get_last_day, get_first_day, cint, get_link_to_form, rounded
...
def validate_service_stop_date(doc):
	...
	old_stop_dates = {}
	old_doc = vmraid.db.get_all("{0} Item".format(doc.doctype),
...
def book_deferred_income_or_expense(doc, deferred_process, posting_date=None):
	...
	via_journal_entry = cint(vmraid.db.get_singles_value('Accounts Settings', 'book_deferred_entries_via_journal_entry'))
	submit_journal_entry = cint(vmraid.db.get_singles_value('Accounts Settings', 'submit_journal_entries'))
	book_deferred_entries_based_on = vmraid.db.get_singles_value('Accounts Settings', 'book_deferred_entries_based_on')
...
def process_deferred_accounting(posting_date=None):
	...
	if not cint(vmraid.db.get_singles_value('Accounts Settings', 'automatically_process_deferred_accounting_entry')):
		return
...
def make_gl_entries(doc, credit_account, debit_account, against,
	...
	if gl_entries:
		try:
			...
			vmraid.log_error(message=traceback)
...
def get_deferred_booking_accounts(doctype, voucher_detail_no, dr_or_cr):
	...
	if doctype == 'Sales Invoice':
		credit_account, debit_account = vmraid.db.get_value('Sales Invoice Item', {'name': voucher_detail_no},
			['income_account', 'deferred_revenue_account'])
	else:
		credit_account, debit_account = vmraid.db.get_value('Purchase Invoice Item', {'name': voucher_detail_no},
