...
from vmraid.utils import (add_days, getdate, formatdate, date_diff,
...
from erpadda.exceptions import PartyFrozen, PartyDisabled, InvalidAccountCurrency
from erpadda.accounts.utils import get_fiscal_year
from erpadda import get_company_currency

from six import iteritems, string_types

class DuplicatePartyAccountError(vmraid.ValidationError): pass
...
def _get_party_details(party=None, account=None, party_type="Customer", company=None, posting_date=None,
	...
	party_details = vmraid._dict(set_account_and_due_date(party, account, party_type, company, posting_date, bill_date, doctype))
	party = party_details[party_type.lower()]

	if not ignore_permissions and not (vmraid.has_permission(party_type, "read", party) or vmraid.has_permission(party_type, "select", party)):
	...
	if party_type == "Supplier" and party:
		party_details["supplier_tds"] = vmraid.get_value(party_type, party.name, "tax_withholding_category")
...
def get_default_price_list(party):
	...
	if party.doctype == "Customer":
		price_list =  vmraid.get_cached_value("Customer Group",
...
def set_price_list(party_details, party, party_type, given_price_list, pos=None):
	...
	if price_list:
		party_details.price_list_currency = vmraid.db.get_value("Price List", price_list, "currency", cache=True)
...
def get_party_account(party_type, party, company=None):
	...
	account = vmraid.db.get_value("Party Account",
...
def get_party_bank_account(party_type, party):
	return vmraid.db.get_value('Bank Account', {
...
def get_party_gle_currency(party_type, party, company):
	def generator():
		existing_gle_currency = vmraid.db.sql("""select account_currency from `tabGL Entry`
			where docstatus=1 and company=%(company)s and party_type=%(party_type)s and party=%(party)s
			limit 1""", { "company": company, "party_type": party_type, "party": party })

		return existing_gle_currency[0][0] if existing_gle_currency else None

	return vmraid.local_cache("party_gle_currency", (party_type, party, company), generator,
		regenerate_if_none=True)

def get_party_gle_account(party_type, party, company):
	def generator():
		existing_gle_account = vmraid.db.sql("""select account from `tabGL Entry`
			where docstatus=1 and company=%(company)s and party_type=%(party_type)s and party=%(party)s
			limit 1""", { "company": company, "party_type": party_type, "party": party })

		return existing_gle_account[0][0] if existing_gle_account else None

	return vmraid.local_cache("party_gle_account", (party_type, party, company), generator,
		regenerate_if_none=True)
...
def validate_party_gle_currency(party_type, party, company, party_account_currency=None):
	...
	if existing_gle_currency and party_account_currency != existing_gle_currency:
		vmraid.throw(_("{0} {1} has accounting entries in currency {2} for company {3}. Please select a receivable or payable account with currency {2}.")
			.format(vmraid.bold(party_type), vmraid.bold(party), vmraid.bold(existing_gle_currency), vmraid.bold(company)), InvalidAccountCurrency)
...
def validate_party_accounts(doc):
	...
	for account in doc.get("accounts"):
		if account.company in companies:
			vmraid.throw(_("There can only be 1 Account per Company in {0} {1}")
				.format(doc.doctype, doc.name), DuplicatePartyAccountError)
		else:
			companies.append(account.company)

		party_account_currency = vmraid.db.get_value("Account", account.account, "account_currency", cache=True)
		if vmraid.db.get_default("Company"):
			company_default_currency = vmraid.get_cached_value('Company',
				vmraid.db.get_default("Company"),  "default_currency")
		else:
		...
		if doc.get("default_currency") and party_account_currency and company_default_currency:
			if doc.default_currency != party_account_currency and doc.default_currency != company_default_currency:
				vmraid.throw(_("Billing currency must be equal to either default company's currency or party account currency"))
...
def validate_due_date(posting_date, due_date, party_type, party, company=None, bill_date=None, template_name=None):
	if getdate(due_date) < getdate(posting_date):
		...
		if default_due_date != posting_date and getdate(due_date) > getdate(default_due_date):
			is_credit_controller = vmraid.db.get_single_value("Accounts Settings", "credit_controller") in vmraid.get_roles()
			if is_credit_controller:
				msgprint(_("Note: Due / Reference Date exceeds allowed customer credit days by {0} day(s)")
					.format(date_diff(due_date, default_due_date)))
			else:
				vmraid.throw(_("Due / Reference Date cannot be after {0}")
					.format(formatdate(default_due_date)))

@vmraid.whitelist()
def get_address_tax_category(tax_category=None, billing_address=None, shipping_address=None):
	addr_tax_category_from = vmraid.db.get_single_value("Accounts Settings", "determine_address_tax_category_from")
...
def set_taxes(party, party_type, posting_date, company, customer_group=None, supplier_group=None, tax_category=None,
	...
	from erpadda.accounts.doctype.tax_rule.tax_rule import get_tax_template, get_party_details
	args = {
		party_type.lower(): party,
		"company": company
	}
...
def get_payment_terms_template(party_name, party_type, company=None):
	...
	template = None
	if party_type == 'Customer':
		customer = vmraid.get_cached_value("Customer", party_name,
			fieldname=['payment_terms', "customer_group"], as_dict=1)
		template = customer.payment_terms

		if not template and customer.customer_group:
			template = vmraid.get_cached_value("Customer Group",
				customer.customer_group, 'payment_terms')
	else:
		supplier = vmraid.get_cached_value("Supplier", party_name,
	...
	if party_type and party_name:
		if party_type in ("Customer", "Supplier"):
			...
			if party.disabled:
				...
				frozen_accounts_modifier = vmraid.db.get_single_value( 'Accounts Settings', 'frozen_accounts_modifier')
			...
			if vmraid.db.get_value("Employee", party_name, "status") == "Left":
...
def get_timeline_data(doctype, name):
	...
	data += vmraid.db.sql("""select {fields}
...
def get_dashboard_info(party_type, party, loyalty_program=None):
	...
	companies = vmraid.get_all(doctype, filters={
		'docstatus': 1,
		party_type.lower(): party
	...
	company_wise_grand_total = vmraid.get_all(doctype,
		filters={
			'docstatus': 1,
			party_type.lower(): party,
	...
	if party_type == "Customer":
		loyalty_point_details = vmraid._dict(vmraid.get_all("Loyalty Point Entry",
			filters={
				'customer': party,
				'expiry_date': ('>=', getdate()),
				},
	...
	company_wise_total_unpaid = vmraid._dict(vmraid.db.sql("""
...
def get_partywise_advanced_payment_amount(party_type, posting_date = None, future_payment=0, company=None):
	...
	data = vmraid.db.sql(""" SELECT party, sum({0}) as amount
		FROM `tabGL Entry`
		WHERE
