...
def get_party_tax_withholding_details(inv, tax_withholding_category=None):
	...
	if not tax_withholding_category:
		tax_withholding_category, pan_no = vmraid.db.get_value(party_type, party, ['tax_withholding_category', 'pan'])
	...
	if not tax_details:
		vmraid.throw(_('Please set associated account in Tax Withholding Category {0} against Company {1}')
			.format(tax_withholding_category, inv.company))

	if party_type == 'Customer' and not tax_details.cumulative_threshold:
		# TCS is only chargeable on sum of invoiced value
		vmraid.throw(_('Tax Withholding Category {} against Company {} for Customer {} should have Cumulative Threshold value.')
...
def get_tax_withholding_rates(tax_withholding, fiscal_year):
	...
	vmraid.throw(_("No Tax Withholding data found for the current Fiscal Year."))
...
def get_lower_deduction_certificate(fiscal_year, pan_no):
	ldc_name = vmraid.db.get_value('Lower Deduction Certificate', { 'pan_no': pan_no, 'fiscal_year': fiscal_year }, 'name')
	if ldc_name:
...
def get_deducted_tax(taxable_vouchers, fiscal_year, tax_details):
	...
	return vmraid.db.get_value('GL Entry', filters, field) or 0.0

def get_tds_amount(ldc, parties, inv, tax_details, fiscal_year_details, tax_deducted, vouchers):
	tds_amount = 0

	supp_credit_amt = vmraid.db.get_value('Purchase Invoice', {
		'name': ('in', vouchers), 'docstatus': 1, 'apply_tds': 1
	}, 'sum(net_total)') or 0.0

	supp_jv_credit_amt = vmraid.db.get_value('Journal Entry Account', {
...
def get_tcs_amount(parties, inv, tax_details, fiscal_year_details, vouchers, adv_vouchers):
	...
	invoiced_amt = vmraid.db.get_value('GL Entry', {
		'is_cancelled': 0,
		'party': ['in', parties],
		'company': inv.company,
	...
	advance_amt = vmraid.db.get_value('GL Entry', {
		'is_cancelled': 0,
		'party': ['in', parties],
		'company': inv.company,
	...
	credit_note_amt = vmraid.db.get_value('GL Entry', {
		'is_cancelled': 0,
		'credit': ['>', 0],
		'party': ['in', parties],
		'fiscal_year': fiscal_year,
...
def get_tds_amount_from_ldc(ldc, parties, fiscal_year, pan_no, tax_details, posting_date, net_total):
	tds_amount = 0
	limit_consumed = vmraid.db.get_value('Purchase Invoice', {
		'supplier': ('in', parties),
		'apply_tds': 1,
		'docstatus': 1
	}, 'sum(net_total)')
