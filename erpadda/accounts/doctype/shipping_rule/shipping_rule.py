...
class OverlappingConditionError(vmraid.ValidationError): pass
class FromGreaterThanToError(vmraid.ValidationError): pass
class ManyBlankToValuesError(vmraid.ValidationError): pass
	...
	def validate_countries(self, doc):
		...
		if self.countries:
			...
			if not shipping_country:
				vmraid.throw(_('Shipping Address does not have country, which is required for this Shipping Rule'))
			if shipping_country not in [d.country for d in self.countries]:
				vmraid.throw(_('Shipping rule not applicable for country {0} in Shipping Address').format(shipping_country))
