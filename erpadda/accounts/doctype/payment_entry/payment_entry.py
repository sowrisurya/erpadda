...
from vmraid import _, scrub, ValidationError
from vmraid.utils import flt, comma_or, nowdate, getdate
from erpadda.accounts.utils import get_outstanding_invoices, get_account_currency, get_balance_on
...
from erpadda.accounts.doctype.bank_account.bank_account import get_party_bank_account, get_bank_account_details
from erpadda.controllers.accounts_controller import AccountsController, get_supplier_block_status
from erpadda.accounts.doctype.invoice_discounting.invoice_discounting import get_party_account_based_on_invoice_discounting
...
class PaymentEntry(AccountsController):
	...
	def validate_duplicate_entry(self):
		...
		for d in self.get("references"):
			if (d.reference_doctype, d.reference_name, d.payment_term) in reference_names:
				vmraid.throw(_("Row #{0}: Duplicate entry in References {1} {2}")
		...
		for d in self.get("references"):
			if (flt(d.allocated_amount))> 0:
				if flt(d.allocated_amount) > flt(d.outstanding_amount):
					vmraid.throw(_("Row #{0}: Allocated Amount cannot be greater than outstanding amount.").format(d.idx))
		...
		if self.party:
			...
			if self.party_account and self.party_type in ("Customer", "Supplier"):
				self.validate_account_type(self.party_account,
					[erpadda.get_party_account_type(self.party_type)])
	...
	def validate_reference_documents(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype not in valid_reference_doctypes:
				vmraid.throw(_("Reference Doctype must be one of {0}")
				...
				if not vmraid.db.exists(d.reference_doctype, d.reference_name):
					...
					if d.reference_doctype != "Journal Entry":
						if self.party != ref_doc.get(scrub(self.party_type)):
							vmraid.throw(_("{0} {1} is not associated with {2} {3}")
					...
					if d.reference_doctype in ("Sales Invoice", "Purchase Invoice", "Expense Claim", "Fees"):
						...
						if ref_party_account != self.party_account:
								vmraid.throw(_("{0} {1} is associated with {2}, but Party Account is {3}")
									.format(d.reference_doctype, d.reference_name, ref_party_account, self.party_account))

					if ref_doc.docstatus != 1:
						vmraid.throw(_("{0} {1} must be submitted")
	...
	def validate_paid_invoices(self):
		...
		for d in self.get("references"):
			...
			if d.reference_doctype in ("Sales Invoice", "Purchase Invoice", "Fees"):
				outstanding_amount, is_return = vmraid.get_cached_value(d.reference_doctype, d.reference_name, ["outstanding_amount", "is_return"])
		...
		for k, v in no_oustanding_refs.items():
			vmraid.msgprint(
				_("{} - {} now have {} as they had no outstanding amount left before submitting the Payment Entry.")
					.format(k, vmraid.bold(", ".join([d.reference_name for d in v])), vmraid.bold("negative outstanding amount"))
	...
	def validate_journal_entry(self):
		for d in self.get("references"):
			if d.allocated_amount and d.reference_doctype == "Journal Entry":
				je_accounts = vmraid.db.sql("""select debit, credit from `tabJournal Entry Account`
					where account = %s and party=%s and docstatus = 1 and parent = %s
					and (reference_type is null or reference_type in ("", "Sales Order", "Purchase Order"))
					""", (self.party_account, self.party, d.reference_name), as_dict=True)

				if not je_accounts:
					vmraid.throw(_("Row #{0}: Journal Entry {1} does not have account {2} or already matched against another voucher")
		...
		for key, allocated_amount in iteritems(invoice_payment_amount_map):
			...
			if cancel:
				...
				if allocated_amount > outstanding:
					vmraid.throw(_('Cannot allocate more than {0} against payment term {1}').format(outstanding, key[0]))
	...
	def validate_payment_against_negative_invoice(self):
		if ((self.payment_type=="Pay" and self.party_type=="Customer")
			...
			if not total_negative_outstanding:
				vmraid.throw(_("Cannot {0} {1} {2} without any negative outstanding invoice")
					.format(self.payment_type, ("to" if self.party_type=="Customer" else "from"),
						self.party_type), InvalidPaymentEntry)

			elif paid_amount - additional_charges > total_negative_outstanding:
				vmraid.throw(_("Paid Amount cannot be greater than total negative outstanding amount {0}")
	...
	def add_party_gl_entries(self, gl_entries):
		if self.party_account:
			...
			dr_or_cr = "credit" if erpadda.get_party_account_type(self.party_type) == 'Receivable' else "debit"
		...
		if self.payment_type == "Receive" and self.party_type == "Donor" and self.party:
			for d in self.get("references"):
				if d.reference_doctype=="Donation" and d.reference_name:
					...
					vmraid.db.set_value("Donation", d.reference_name, "paid", is_paid)
...
def get_outstanding_reference_documents(args):
	...
	if args.get("voucher_type") and args.get("voucher_no"):
		condition = " and voucher_type={0} and voucher_no={1}"\
			.format(vmraid.db.escape(args["voucher_type"]), vmraid.db.escape(args["voucher_no"]))
	...
	if not data:
		vmraid.msgprint(_("No outstanding invoices found for the {0} {1} which qualify the filters you have specified.")
			.format(args.get("party_type").lower(), vmraid.bold(args.get("party"))))
...
def split_invoices_based_on_payment_terms(outstanding_invoices):
	...
	for idx, d in enumerate(outstanding_invoices):
		if d.voucher_type in ['Sales Invoice', 'Purchase Invoice']:
			payment_term_template = vmraid.db.get_value(d.voucher_type, d.voucher_no, 'payment_terms_template')
			if payment_term_template:
				allocate_payment_based_on_payment_terms = vmraid.db.get_value(
					'Payment Terms Template', payment_term_template, 'allocate_payment_based_on_payment_terms')
				if allocate_payment_based_on_payment_terms:
					payment_schedule = vmraid.get_all('Payment Schedule', filters={'parent': d.voucher_no}, fields=["*"])

					for payment_term in payment_schedule:
						if payment_term.outstanding > 0.1:
							invoice_ref_based_on_payment_terms.setdefault(idx, [])
							invoice_ref_based_on_payment_terms[idx].append(vmraid._dict({
								'due_date': d.due_date,
								'currency': d.currency,
	...
	if invoice_ref_based_on_payment_terms:
		for idx, ref in invoice_ref_based_on_payment_terms.items():
			...
			vmraid.msgprint(_("Spliting {} {} into {} rows as per payment terms").format(
...
def get_account_details(account, date, cost_center=None):
	...
	account_list = vmraid.get_list('Account', {
		'name': account
...
def get_company_defaults(company):
	...
	for fieldname in fields:
		if not ret[fieldname]:
			vmraid.throw(_("Please set default {0} in Company {1}")
...
def get_reference_details(reference_doctype, reference_name, party_account_currency):
	...
	company_currency = ref_doc.get("company_currency") or erpadda.get_company_currency(ref_doc.company)
...
def get_payment_entry(dt, dn, party_amount=None, bank_account=None, bank_amount=None):
	...
	if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
		...
		if (doc.doctype in ('Sales Invoice', 'Purchase Invoice')
			and vmraid.get_value('Payment Terms Template',
	...
	if party_account and bank:
		...
		if discount_amount:
			pe.set_gain_or_loss(account_details={
				...
				'cost_center': pe.cost_center or vmraid.get_cached_value('Company', pe.company, "cost_center"),
