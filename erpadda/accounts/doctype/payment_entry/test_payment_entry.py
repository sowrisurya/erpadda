...
from erpadda.accounts.doctype.payment_entry.payment_entry import get_payment_entry, InvalidPaymentEntry
from erpadda.accounts.doctype.sales_invoice.test_sales_invoice import create_sales_invoice, create_sales_invoice_against_cost_center
from erpadda.accounts.doctype.purchase_invoice.test_purchase_invoice import make_purchase_invoice, make_purchase_invoice_against_cost_center
...
class TestPaymentEntry(unittest.TestCase):
	...
	def test_payment_against_sales_invoice_to_check_status(self):
		...
		outstanding_amount, status = vmraid.db.get_value("Sales Invoice", si.name, ["outstanding_amount", "status"])
	...
	def test_payment_against_purchase_invoice_to_check_status(self):
		...
		outstanding_amount, status = vmraid.db.get_value("Purchase Invoice", pi.name, ["outstanding_amount", "status"])
		self.assertEqual(flt(outstanding_amount), 0)
		self.assertEqual(status, 'Paid')
	...
	def test_payment_entry_against_ec(self):
		...
		outstanding_amount = flt(vmraid.db.get_value("Expense Claim", ec.name, "total_sanctioned_amount")) - \
	...
	def test_payment_entry_retrieves_last_exchange_rate(self):
		from erpadda.setup.doctype.currency_exchange.test_currency_exchange import test_records, save_new_records
	...
	def get_gle(self, voucher_no):
		return vmraid.db.sql("""select account, debit, credit, against_voucher
