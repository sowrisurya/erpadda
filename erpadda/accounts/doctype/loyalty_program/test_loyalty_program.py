...
import vmraid
import unittest
from vmraid.utils import today, cint, flt, getdate
from erpadda.accounts.doctype.loyalty_program.loyalty_program import get_loyalty_program_details_with_points
...
class TestLoyaltyProgram(unittest.TestCase):
	...
	def test_loyalty_points_earned_single_tier(self):
		vmraid.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Single Loyalty")
		...
		lpe = vmraid.get_doc('Loyalty Point Entry', {'invoice_type': 'Sales Invoice', 'invoice': si_original.name, 'customer': si_original.customer})
		...
		lpe_redeem = vmraid.get_doc('Loyalty Point Entry', {'invoice_type': 'Sales Invoice', 'invoice': si_redeem.name, 'redeem_against': lpe.name})
		lpe_earn = vmraid.get_doc('Loyalty Point Entry', {'invoice_type': 'Sales Invoice', 'invoice': si_redeem.name, 'name': ['!=', lpe_redeem.name]})
	...
	def test_loyalty_points_earned_multiple_tier(self):
		vmraid.db.set_value("Customer", "Test Loyalty Customer", "loyalty_program", "Test Multiple Loyalty")
		# assign multiple tier program to the customer
		customer = vmraid.get_doc('Customer', {"customer_name": "Test Loyalty Customer"})
		customer.loyalty_program = vmraid.get_doc('Loyalty Program', {'loyalty_program_name': 'Test Multiple Loyalty'}).name
		customer.save()
	...
	def test_cancel_sales_invoice(self):
		...
		lpe = vmraid.get_doc('Loyalty Point Entry', {'invoice_type': 'Sales Invoice', 'invoice': si.name, 'customer': si.customer})
	...
	def test_sales_invoice_return(self):
		...
		earned_points = get_points_earned(si_original)
		lpe_original = vmraid.get_doc('Loyalty Point Entry', {'invoice_type': 'Sales Invoice', 'invoice': si_original.name, 'customer': si_original.customer})
		...
		earned_points = get_points_earned(si_original)
		lpe_after_return = vmraid.get_doc('Loyalty Point Entry', {'invoice_type': 'Sales Invoice', 'invoice': si_original.name, 'customer': si_original.customer})
...
def create_records():
	...
	# create item price
	if not vmraid.db.exists("Item Price", {"price_list": "Standard Selling", "item_code": "Loyal Item"}):
		vmraid.get_doc({
			"doctype": "Item Price",
