...
class ExchangeRateRevaluation(Document):
	...
	def check_journal_entry_condition(self):
		total_debit = vmraid.db.get_value("Journal Entry Account", {
	...
	def get_accounts_data(self, account=None):
		...
		precision = get_field_precision(vmraid.get_meta("Exchange Rate Revaluation Account")
	...
	def make_jv_entry(self):
		...
		unrealized_exchange_gain_loss_account = vmraid.get_cached_value('Company',  self.company,
			"unrealized_exchange_gain_loss_account")
		if not unrealized_exchange_gain_loss_account:
			vmraid.throw(_("Please set Unrealized Exchange Gain/Loss Account in Company {0}")
				.format(self.company))
...
def get_account_details(account, company, posting_date, party_type=None, party=None):
	account_currency, account_type = vmraid.db.get_value("Account", account,
