...
from vmraid.utils import cint, now, get_link_to_form
...
class POSProfile(Document):
	...
	def validate_default_profile(self):
		for row in self.applicable_for_users:
			res = vmraid.db.sql("""select pf.name
				from
	...
	def validate_all_link_fields(self):
		...
		for link_dt, dn_list in iteritems(accounts):
			for link_dn in dn_list:
				if link_dn and not vmraid.db.exists({"doctype": link_dt,
	...
	def validate_duplicate_groups(self):
		...
		if len(item_groups) != len(set(item_groups)):
			vmraid.throw(_("Duplicate item group found in the item group table"), title = "Duplicate Item Group")

		if len(customer_groups) != len(set(customer_groups)):
			vmraid.throw(_("Duplicate customer group found in the cutomer group table"), title = "Duplicate Customer Group")
	...
	def set_defaults(self, include_current_pos=True):
		...
		pos_view_users = vmraid.db.sql_list("""select pfu.user
...
def get_item_groups(pos_profile):
	...
	if pos_profile.get('item_groups'):
		...
		for data in pos_profile.get('item_groups'):
			item_groups.extend(["%s" % vmraid.db.escape(d.name) for d in get_child_nodes('Item Group', data.item_group)])

	return list(set(item_groups))

def get_child_nodes(group_type, root):
	lft, rgt = vmraid.db.get_value(group_type, root, ["lft", "rgt"])
	return vmraid.db.sql(""" Select name, lft, rgt from `tab{tab}` where
...
def pos_profile_query(doctype, txt, searchfield, start, page_len, filters):
	...
	pos_profile = vmraid.db.sql("""select pf.name
		from
...
def set_default_profile(pos_profile, company):
	...
	if pos_profile and company:
		vmraid.db.sql(""" update `tabPOS Profile User` pfu, `tabPOS Profile` pf
			set
