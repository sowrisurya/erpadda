...
class ShareTransfer(Document):
	...
	def validate(self):
		...
		if self.transfer_type == 'Issue':
			...
			if ret_val in ('Outside', 'Partial'):
				vmraid.throw(_("The shares don't exist with the {0}")
	...
	def basic_validations(self):
		...
		if self.amount != self.rate * self.no_of_shares:
			vmraid.throw(_('There are inconsistencies between the rate, no of shares and the amount calculated'))
	...
	def get_company_shareholder(self):
		...
		company_shareholder = vmraid.db.get_value('Shareholder',
			{
