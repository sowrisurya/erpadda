...
from erpadda.accounts.doctype.bank_reconciliation_tool.bank_reconciliation_tool import reconcile_vouchers, get_linked_payments
...
class TestBankTransaction(unittest.TestCase):
	...
	def test_linked_payments(self):
		bank_transaction = vmraid.get_doc("Bank Transaction", dict(description="Re 95282925234 FE/000002917 AT171513000281183046 Conrad Electronic"))
		linked_payments = get_linked_payments(bank_transaction.name, ['payment_entry', 'exact_match'])
		self.assertTrue(linked_payments[0][6] == "Conrad Electronic")

	# This test validates a simple reconciliation leading to the clearance of the bank transaction and the payment
	def test_reconcile(self):
		bank_transaction = vmraid.get_doc("Bank Transaction", dict(description="1512567 BG/000003025 OPSKATTUZWXXX AT776000000098709849 Herr G"))
		...
		unallocated_amount = vmraid.db.get_value("Bank Transaction", bank_transaction.name, "unallocated_amount")
	...
	def test_debit_credit_output(self):
		bank_transaction = vmraid.get_doc("Bank Transaction", dict(description="Auszahlung Karte MC/000002916 AUTOMAT 698769 K002 27.10. 14:07"))
		linked_payments = get_linked_payments(bank_transaction.name, ['payment_entry', 'exact_match'])
		self.assertTrue(linked_payments[0][3])

	# Check error if already reconciled
	def test_already_reconciled(self):
		bank_transaction = vmraid.get_doc("Bank Transaction", dict(description="1512567 BG/000002918 OPSKATTUZWXXX AT776000000098709837 Herr G"))
		...
		vouchers = json.dumps([{
			...
			"payment_name":payment.name,
			"amount":bank_transaction.unallocated_amount}])
		self.assertRaises(vmraid.ValidationError, reconcile_vouchers, bank_transaction_name=bank_transaction.name, vouchers=vouchers)

	# Raise an error if debitor transaction vs debitor payment
	def test_clear_sales_invoice(self):
		bank_transaction = vmraid.get_doc("Bank Transaction", dict(description="I2015000011 VD/000002514 ATWWXXX AT4701345000003510057 Bio"))
		...
		self.assertEqual(vmraid.db.get_value("Bank Transaction", bank_transaction.name, "unallocated_amount"), 0)
		self.assertTrue(vmraid.db.get_value("Sales Invoice Payment", dict(parent=payment.name), "clearance_date") is not None)
...
def add_vouchers():
	...
	if not vmraid.db.get_value('Mode of Payment Account', {'company': "_Test Company", 'parent': "Cash"}):
