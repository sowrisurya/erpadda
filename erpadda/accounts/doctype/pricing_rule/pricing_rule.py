	...
	def validate_mandatory(self):
		...
		if self.apply_rule_on_other:
			...
			if not self.get(o_field) and o_field in other_fields:
				vmraid.throw(_("For the 'Apply Rule On Other' condition the field {0} is mandatory")
					.format(vmraid.bold(self.apply_rule_on_other)))
		...
		if self.apply_discount_on_rate:
			if not self.priority:
				throw(_("As the field {0} is enabled, the field {1} is mandatory.")
					.format(vmraid.bold("Apply Discount on Discounted Rate"), vmraid.bold("Priority")))
...
def apply_pricing_rule(args, doc=None):
	...
	set_serial_nos_based_on_fifo = vmraid.db.get_single_value("Stock Settings",
...
def get_pricing_rule_for_item(args, price_list_rate=0, doc=None, for_validate=False):
	from erpadda.accounts.doctype.pricing_rule.utils import (get_pricing_rules,
		...
		for pricing_rule in pricing_rules:
			...
			if pricing_rule.mixed_conditions or pricing_rule.apply_rule_on_other:
				item_details.update({
					...
					'apply_rule_on': (vmraid.scrub(pricing_rule.apply_rule_on_other)
						if pricing_rule.apply_rule_on_other else vmraid.scrub(pricing_rule.get('apply_on')))
				})
...
def update_args_for_pricing_rule(args):
	if not (args.item_group and args.brand):
		try:
			args.item_group, args.brand = vmraid.get_cached_value("Item", args.item_code, ["item_group", "brand"])
		except TypeError:
			# invalid item_code
			return item_details
...
def remove_pricing_rule_for_item(pricing_rules, item_details, item_code=None):
	...
	for d in get_applied_pricing_rules(pricing_rules):
		if not d or not vmraid.db.exists("Pricing Rule", d): continue
		...
		if pricing_rule.get("mixed_conditions") or pricing_rule.get("apply_rule_on_other"):
			...
			item_details.apply_on = (vmraid.scrub(pricing_rule.apply_rule_on_other)
				if pricing_rule.apply_rule_on_other else vmraid.scrub(pricing_rule.get('apply_on')))
...
def get_item_uoms(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.get_all('UOM Conversion Detail', filters={
			'parent': ('in', items),
