...
from vmraid.utils import cint, flt, get_link_to_form, getdate, today, fmt_money

class MultiplePricingRuleConflict(vmraid.ValidationError): pass
...
def _get_pricing_rules(apply_on, args, values):
	...
	pricing_rules = vmraid.db.sql("""select `tabPricing Rule`.*,
...
def _get_tree_conditions(args, parenttype, table, allow_blank=True):
	...
	if args.get(field):
		...
		parent_groups = vmraid.db.sql_list("""select name from `tab%s`
			where lft>=%s and rgt<=%s""" % (parenttype, '%s', '%s'), (lft, rgt))

		if parenttype in ["Customer Group", "Item Group", "Territory"]:
			parent_field = "parent_{0}".format(vmraid.scrub(parenttype))
			root_name = vmraid.db.get_list(parenttype,
		...
		if parent_groups:
			...
			condition = "ifnull({table}.{field}, '') in ({parent_groups})".format(
				table=table,
				field=field,
				parent_groups=", ".join([vmraid.db.escape(d) for d in parent_groups])
			)
...
def filter_pricing_rules(args, pricing_rules, doc=None):
	...
	if len(pricing_rules) > 1 and not args.for_shopping_cart:
		vmraid.throw(_("Multiple Price Rules exists with same criteria, please resolve conflict by assigning priority. Price Rules: {0}")
...
def get_qty_amount_data_for_cumulative(pr_doc, doc, items=[]):
	...
	date_field = 'transaction_date' if vmraid.get_meta(doctype).has_field('transaction_date') else 'posting_date'
	...
	data_set = vmraid.db.sql(""" SELECT `tab{child_doc}`.stock_qty,
...
def apply_pricing_rule_on_transaction(doc):
	...
	pricing_rules = vmraid.db.sql(""" Select `tabPricing Rule`.* from `tabPricing Rule`
			...
			if d.price_or_product_discount == 'Price':
				...
				for field in ['additional_discount_percentage', 'discount_amount']:
					...
					if d.validate_applied_rule and doc.get(field) is not None and doc.get(field) < d.get(pr_field):
						vmraid.msgprint(_("User has not applied rule on the invoice {0}")
							.format(doc.name))
					else:
...
def get_product_discount_rule(pricing_rule, item_details, args=None, doc=None):
	...
	if not free_item:
		vmraid.throw(_("Free item not set in the pricing rule {0}")
	...
	item_data = vmraid.get_cached_value('Item', free_item, ['item_name',
	...
	if coupon:
		if transaction_type=='used':
			if coupon.used<coupon.maximum_use:
				...
				vmraid.throw(_("{0} Coupon used are {1}. Allowed quantity is exhausted").format(coupon.coupon_code,coupon.used))
