...
class IncorrectCustomerGroup(vmraid.ValidationError): pass
class IncorrectSupplierType(vmraid.ValidationError): pass
class ConflictingTaxRule(vmraid.ValidationError): pass
...
class TaxRule(Document):
	...
	def validate_filters(self):
		...
		tax_rule = vmraid.db.sql("select name, priority \
	...
	def validate_use_for_shopping_cart(self):
		...
		if (not self.use_for_shopping_cart
			and cint(vmraid.db.get_single_value('Shopping Cart Settings', 'enabled'))
			and not vmraid.db.get_value('Tax Rule', {'use_for_shopping_cart': 1, 'name': ['!=', self.name]})):

			self.use_for_shopping_cart = 1
			vmraid.msgprint(_("Enabling 'Use for Shopping Cart', as Shopping Cart is enabled and there should be at least one Tax Rule for Shopping Cart"))
...
def get_tax_template(posting_date, args):
	...
	conditions.append("ifnull(tax_category, '') = {0}".format(vmraid.db.escape(cstr(args.get("tax_category")))))
	...
	tax_rule = vmraid.db.sql("""select * from `tabTax Rule`
...
def get_customer_group_condition(customer_group):
	condition = ""
	customer_groups = ["%s"%(vmraid.db.escape(d.name)) for d in get_parent_customer_groups(customer_group)]
	if customer_groups:
