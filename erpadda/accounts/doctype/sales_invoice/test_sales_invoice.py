...
from vmraid.utils import nowdate, flt, getdate, cint, add_days, add_months
from vmraid.model.dynamic_links import get_dynamic_link_map
from erpadda.stock.doctype.stock_entry.test_stock_entry import make_stock_entry, get_qty_after_transaction
from erpadda.accounts.doctype.purchase_invoice.test_purchase_invoice import unlink_payment_on_cancel_of_invoice
...
class TestSalesInvoice(unittest.TestCase):
	...
	def test_discount_amount_gl_entry(self):
		...
		gle = vmraid.db.sql("""select * from `tabGL Entry`
	...
	def pos_gl_entry(self, si, pos, cash_amount):
		...
		sle = vmraid.db.sql("""select * from `tabStock Ledger Entry`
	...
	def _insert_purchase_receipt(self):
		from erpadda.stock.doctype.purchase_receipt.test_purchase_receipt import test_records \
			as pr_test_records
	...
	def _insert_delivery_note(self):
		from erpadda.stock.doctype.delivery_note.test_delivery_note import test_records \
			as dn_test_records
	...
	def test_serialized(self):
		...
		self.assertEqual(vmraid.db.get_value("Serial No", serial_nos[0],
			"delivery_document_no"), si.name)
		self.assertEqual(vmraid.db.get_value("Serial No", serial_nos[0], "sales_invoice"),
			si.name)

		# check if the serial number is already linked with any other Sales Invoice
		_si = vmraid.copy_doc(si.as_dict())
		self.assertRaises(vmraid.ValidationError, _si.insert)
	...
	def test_serialized_cancel(self):
		...
		self.assertEqual(vmraid.db.get_value("Serial No", serial_nos[0], "warehouse"), "_Test Warehouse - _TC")
		self.assertFalse(vmraid.db.get_value("Serial No", serial_nos[0],
	...
	def test_return_sales_invoice(self):
		...
		# outgoing_rate
		outgoing_rate = vmraid.db.get_value("Stock Ledger Entry", {"voucher_type": "Sales Invoice",
		...
		incoming_rate, stock_value_difference = vmraid.db.get_value("Stock Ledger Entry",
		...
		# Check gl entry
		gle_warehouse_amount = vmraid.db.get_value("GL Entry", {"voucher_type": "Sales Invoice",
			"voucher_no": si1.name, "account": stock_in_hand_account}, "debit")

		self.assertEqual(gle_warehouse_amount, stock_value_difference)

		party_credited = vmraid.db.get_value("GL Entry", {"voucher_type": "Sales Invoice",
	...
	def test_company_monthly_sales(self):
		existing_current_month_sales = vmraid.get_cached_value('Company',  "_Test Company",  "total_monthly_sales")
	...
	def test_internal_transfer_gl_entry(self):
		...
		vmraid.db.set_value('Company', '_Test Company with perpetual inventory',
	...
	def test_einvoice_json(self):
		from erpadda.regional.india.e_invoice.utils import make_einvoice
...
def check_gl_entries(doc, voucher_no, expected_gle, posting_date):
	gl_entries = vmraid.db.sql("""select account, debit, credit, posting_date
		from `tabGL Entry`
	...
	def test_item_tax_validity(self):
		item = vmraid.get_doc("Item", "_Test Item 2")
		...
		self.assertRaises(vmraid.ValidationError, sales_invoice.save)
