						...
						vmraid.call({
							...
							callback: function(r) {
								if (!r.exc && r.message.length > 0) {
									...
									vmraid.msgprint(__('No Timesheets found with the selected filters.'))
								}
								d.hide();
							}
						});
...
vmraid.ui.form.on('Sales Invoice Timesheet', {
	time_sheet: function(frm, cdt, cdn){
		...
		if(d.time_sheet) {
			vmraid.call({
				method: "erpadda.projects.doctype.timesheet.timesheet.get_timesheet_data",
				args: {
					'name': d.time_sheet,
				...
				callback: function(r, rt) {
					if(r.message){
						let data = r.message;
						vmraid.model.set_value(cdt, cdn, "billing_hours", data.billing_hours);
						vmraid.model.set_value(cdt, cdn, "billing_amount", data.billing_amount);
						vmraid.model.set_value(cdt, cdn, "timesheet_detail", data.timesheet_detail);
						calculate_total_billing_amount(frm)
					}
				}
			})
...
var get_healthcare_services_to_invoice = function(frm) {
	...
	dialog.fields_dict["patient"].df.onchange = () => {
		...
		if(patient && patient!=selected_patient){
			...
			var method = "erpadda.healthcare.utils.get_healthcare_services_to_invoice";
...
var make_list_row= function(columns, invoice_healthcare_services, result={}) {
	...
	columns.forEach(function(column) {
		contents += `<div class="list-item__content ellipsis">
			${
				head ? `<span class="ellipsis">${__(vmraid.model.unscrub(column))}</span>`
...
var set_primary_action= function(frm, dialog, $results, invoice_healthcare_services) {
	...
	dialog.set_primary_action(__('Add'), function() {
		...
		if(checked_values.length > 0){
			...
			if(invoice_healthcare_services){
				vmraid.msgprint(__("Please select Healthcare Service"));
			}
			else{
				vmraid.msgprint(__("Please select Drug"));
			}
		}
	});
};
...
var get_drugs_to_invoice = function(frm) {
	...
	dialog.fields_dict["encounter"].df.onchange = () => {
		...
		if(encounter && encounter!=selected_encounter){
			...
			var method = "erpadda.healthcare.utils.get_drugs_to_invoice";
...
var add_to_item_line = function(frm, checked_values, invoice_healthcare_services){
	if(invoice_healthcare_services){
		...
		for(let i=0; i<checked_values.length; i++){
			var si_item = vmraid.model.add_child(frm.doc, 'Sales Invoice Item', 'items');
			vmraid.model.set_value(si_item.doctype, si_item.name, 'item_code', checked_values[i]['item']);
			vmraid.model.set_value(si_item.doctype, si_item.name, 'qty', 1);
			if(checked_values[i]['qty'] > 1){
				vmraid.model.set_value(si_item.doctype, si_item.name, 'qty', parseFloat(checked_values[i]['qty']));
			}
		}
		frm.refresh_fields();
	}
