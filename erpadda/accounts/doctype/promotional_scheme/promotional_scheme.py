...
class PromotionalScheme(Document):
	...
	def on_update(self):
		data = vmraid.get_all('Pricing Rule', fields = ["promotional_scheme_id", "name"],
	...
	def on_trash(self):
		for d in vmraid.get_all('Pricing Rule',
			{'promotional_scheme': self.name}):
			vmraid.delete_doc('Pricing Rule', d.name)
...
def _get_pricing_rules(doc, child_doc, discount_fields, rules = {}):
	...
	for d in doc.get(child_doc):
		if d.name in rules:
			pr = vmraid.get_doc('Pricing Rule', rules.get(d.name))
		else:
