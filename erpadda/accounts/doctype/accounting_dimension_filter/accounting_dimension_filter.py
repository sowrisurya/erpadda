	...
	def validate_applicable_accounts(self):
		...
		for account in self.get('accounts'):
			if account.applicable_on_account in account_list:
				vmraid.throw(_("Row {0}: {1} account already applied for Accounting Dimension {2}").format(
					account.idx, vmraid.bold(account.applicable_on_account), vmraid.bold(self.accounting_dimension)))
