...
import vmraid, csv, os
from vmraid import _
from vmraid.utils import cstr, cint
from vmraid.model.document import Document
from vmraid.utils.csvutils import UnicodeWriter
from erpadda.accounts.doctype.account.chart_of_accounts.chart_of_accounts import create_charts, build_tree_from_json
from vmraid.utils.xlsxutils import read_xlsx_file_from_attached_file, read_xls_file_from_attached_file
...
def validate_company(company):
	parent_company, allow_account_creation_against_child_company = vmraid.db.get_value('Company',
...
def get_file(file_name):
	...
	if extension not in ('csv',  'xlsx', 'xls'):
		vmraid.throw("Only CSV and Excel files can be used to for importing data. Please check the file format you are trying to upload")
		...
		for row in data:
			...
			if parent_account == account_name == child:
				...
				if not parent_account_list and parent_account:
					vmraid.throw(_("The parent account {0} does not exists in the uploaded template").format(
...
def unset_existing_data(company):
	linked = vmraid.db.sql('''select fieldname from tabDocField
	...
	for doctype in ["Account", "Party Account", "Mode of Payment Account", "Tax Withholding Account",
		...
		vmraid.db.sql('''delete from `tab{0}` where `company`="%s"''' # nosec
...
def set_default_accounts(company):
	...
	company.update({
		"default_receivable_account": vmraid.db.get_value("Account",
			{"company": company.name, "account_type": "Receivable", "is_group": 0}),
		"default_payable_account": vmraid.db.get_value("Account",
