...
vmraid.listview_settings['Purchase Invoice'] = {
	...
	get_indicator: function(doc) {
		if ((flt(doc.outstanding_amount) <= 0) && doc.docstatus == 1 &&  doc.status == 'Debit Note Issued') {
			...
			} else if (cint(doc.on_hold) && doc.release_date && vmraid.datetime.get_diff(doc.release_date, vmraid.datetime.nowdate()) > 0) {
				return [__("Temporarily on Hold"), "darkgrey"];
			} else if (vmraid.datetime.get_diff(doc.due_date) < 0) {
