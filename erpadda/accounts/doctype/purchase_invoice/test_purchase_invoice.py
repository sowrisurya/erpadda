...
from vmraid.utils import cint, flt, today, nowdate, add_days, getdate
import vmraid.defaults
from erpadda.stock.doctype.purchase_receipt.test_purchase_receipt import make_purchase_receipt, get_taxes
...
class TestPurchaseInvoice(unittest.TestCase):
	...
	def test_gl_entries_without_perpetual_inventory(self):
		...
		gl_entries = vmraid.db.sql("""select account, debit, credit from `tabGL Entry`
	...
	def test_purchase_invoice_for_blocked_supplier_payment(self):
		...
		self.assertRaises(
			vmraid.ValidationError, get_payment_entry, dt='Purchase Invoice', dn=pi.name, bank_account="_Test Bank - _TC")
	...
	def test_purchase_invoice_for_blocked_supplier_payment_today_date(self):
		...
		self.assertRaises(
			vmraid.ValidationError, get_payment_entry, dt='Purchase Invoice', dn=pi.name,
	...
	def check_gle_for_pi(self, pi):
		gl_entries = vmraid.db.sql("""select account, sum(debit) as debit, sum(credit) as credit
	...
	def test_gl_entries_for_non_stock_items_with_perpetual_inventory(self):
		...
		gl_entries = vmraid.db.sql("""select account, debit, credit
			from `tabGL Entry` where voucher_type='Purchase Invoice' and voucher_no=%s
	...
	def test_purchase_invoice_with_advance(self):
		from erpadda.accounts.doctype.journal_entry.test_journal_entry \
		...
		self.assertTrue(vmraid.db.sql("""select name from `tabJournal Entry Account`
			where reference_type='Purchase Invoice'
			and reference_name=%s and debit_in_account_currency=300""", pi.name))

		pi.cancel()

		self.assertFalse(vmraid.db.sql("""select name from `tabJournal Entry Account`
	...
	def test_total_purchase_cost_for_project(self):
		...
		existing_purchase_cost = vmraid.db.sql("""select sum(base_net_amount)
		...
		self.assertEqual(vmraid.db.get_value("Project", project.name, "total_purchase_cost"),
		...
		pi.cancel()
		self.assertEqual(vmraid.db.get_value("Project", project.name, "total_purchase_cost"), existing_purchase_cost)
	...
	def test_multi_currency_gle(self):
		...
		gl_entries = vmraid.db.sql("""select account, account_currency, debit, credit,
		...
		gle = vmraid.db.sql("""select name from `tabGL Entry`
	...
	def test_purchase_invoice_update_stock_gl_entry_with_perpetual_inventory(self):
		...
		pi = make_purchase_invoice(update_stock=1, posting_date=vmraid.utils.nowdate(),
			posting_time=vmraid.utils.nowtime(), cash_bank_account="Cash - TCP1", company="_Test Company with perpetual inventory", supplier_warehouse="Work In Progress - TCP1", warehouse= "Stores - TCP1", cost_center = "Main - TCP1", expense_account ="_Test Account Cost for Goods Sold - TCP1")
	...
	def test_purchase_invoice_for_is_paid_and_update_stock_gl_entry_with_perpetual_inventory(self):
		...
		pi = make_purchase_invoice(update_stock=1, posting_date=vmraid.utils.nowdate(),
			posting_time=vmraid.utils.nowtime(), cash_bank_account="Cash - TCP1", is_paid=1, company="_Test Company with perpetual inventory", supplier_warehouse="Work In Progress - TCP1", warehouse= "Stores - TCP1", cost_center = "Main - TCP1", expense_account ="_Test Account Cost for Goods Sold - TCP1")

		gl_entries = vmraid.db.sql("""select account, account_currency, sum(debit) as debit,
	...
	def test_auto_batch(self):
		item_code = vmraid.db.get_value('Item',
		...
		pi = make_purchase_invoice(update_stock=1, posting_date=vmraid.utils.nowdate(),
			posting_time=vmraid.utils.nowtime(), item_code=item_code)

		self.assertTrue(vmraid.db.get_value('Batch',
	...
	def test_rejected_serial_no(self):
		...
		self.assertEqual(vmraid.db.get_value("Serial No", pi.get("items")[0].serial_no, "warehouse"),
			pi.get("items")[0].warehouse)

		self.assertEqual(vmraid.db.get_value("Serial No", pi.get("items")[0].rejected_serial_no,
	...
	def test_purchase_invoice_with_shipping_rule(self):
		from erpadda.accounts.doctype.shipping_rule.test_shipping_rule \
	...
	def test_purchase_invoice_with_cost_center(self):
		...
		gl_entries = vmraid.db.sql("""select account, cost_center, account_currency, debit, credit,
			debit_in_account_currency, credit_in_account_currency
			from `tabGL Entry` where voucher_type='Purchase Invoice' and voucher_no=%s
			order by account asc""", pi.name, as_dict=1)
	...
	def test_purchase_invoice_with_project_link(self):
		...
		gl_entries = vmraid.db.sql("""select account, cost_center, project, account_currency, debit, credit,
			debit_in_account_currency, credit_in_account_currency
			from `tabGL Entry` where voucher_type='Purchase Invoice' and voucher_no=%s
			order by account asc""", pi.name, as_dict=1)
	...
	def test_deferred_expense_via_journal_entry(self):
		...
		gl_entries = gl_entries = vmraid.db.sql("""select account, debit, credit, posting_date
			from `tabGL Entry`
