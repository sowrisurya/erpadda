...
from vmraid.utils import get_datetime, flt
...
from erpadda.accounts.doctype.pos_invoice_merge_log.pos_invoice_merge_log import consolidate_pos_invoices, unconsolidate_pos_invoices
...
class POSClosingEntry(StatusUpdater):
	...
	def validate_pos_invoices(self):
		...
		for d in self.pos_transactions:
			...
			pos_invoice = vmraid.db.get_values("POS Invoice", d.pos_invoice,
				["consolidated_invoice", "pos_profile", "docstatus", "owner"], as_dict=1)[0]
			if pos_invoice.consolidated_invoice:
				invalid_row.setdefault('msg', []).append(_('POS Invoice is {}').format(vmraid.bold("already consolidated")))
				invalid_rows.append(invalid_row)
				continue
			if pos_invoice.pos_profile != self.pos_profile:
				invalid_row.setdefault('msg', []).append(_("POS Profile doesn't matches {}").format(vmraid.bold(self.pos_profile)))
			if pos_invoice.docstatus != 1:
				invalid_row.setdefault('msg', []).append(_('POS Invoice is not {}').format(vmraid.bold("submitted")))
			if pos_invoice.owner != self.user:
				invalid_row.setdefault('msg', []).append(_("POS Invoice isn't created by user {}").format(vmraid.bold(self.owner)))
	...
	def get_payment_reconciliation_details(self):
		...
		return vmraid.render_template("erpadda/accounts/doctype/pos_closing_entry/closing_voucher_details.html",
...
def make_closing_entry_from_opening(opening_entry):
	...
	for d in invoices:
		pos_transactions.append(vmraid._dict({
			'pos_invoice': d.name,
