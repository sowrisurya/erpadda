...
from vmraid.model.mapper import map_doc, map_child_doc
...
class POSInvoiceMergeLog(Document):
	...
	def validate_customer(self):
		for d in self.pos_invoices:
			if d.customer != self.customer:
				vmraid.throw(_("Row #{}: POS Invoice {} is not against customer {}").format(d.idx, d.pos_invoice, self.customer))
	...
	def validate_pos_invoice_status(self):
		for d in self.pos_invoices:
			...
			if status == "Consolidated":
				vmraid.throw(_("Row #{}: POS Invoice {} has been {}").format(d.idx, bold_pos_invoice, bold_status))
	...
	def on_submit(self):
		pos_invoice_docs = [vmraid.get_doc("POS Invoice", d.pos_invoice) for d in self.pos_invoices]
...
def get_all_unconsolidated_invoices():
	...
	pos_invoices = vmraid.db.get_all('POS Invoice', filters=filters,
...
def create_merge_logs(invoice_by_customer, closing_entry=None):
	try:
		...
		vmraid.db.rollback()
		message_log = vmraid.message_log.pop()
