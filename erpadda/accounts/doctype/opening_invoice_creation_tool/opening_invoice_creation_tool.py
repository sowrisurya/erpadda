...
class OpeningInvoiceCreationTool(Document):
	...
	def get_opening_invoice_summary(self):
		...
		for doctype in ["Sales Invoice", "Purchase Invoice"]:
			invoices = vmraid.get_all(doctype, filters=dict(is_opening="Yes", docstatus=1),
	...
	def validate_mandatory_invoice_fields(self, row):
		if not vmraid.db.exists(row.party_type, row.party):
			if self.create_missing_party:
				...
				vmraid.throw(_("Row #{}: {} {} does not exist.").format(row.idx, vmraid.bold(row.party_type), vmraid.bold(row.party)))
	...
	def get_invoices(self):
		...
		for row in self.invoices:
			...
			company_details = vmraid.get_cached_value('Company', self.company, ["default_currency", "default_letter_head"], as_dict=1) or {}
			if company_details:
				invoice.update({
	...
	def get_invoice_dict(self, row=None):
		def get_item_dict():
			cost_center = row.get('cost_center') or vmraid.get_cached_value('Company', self.company,  "cost_center")
			if not cost_center:
				vmraid.throw(_("Please set the Default Cost Center in {0} company.").format(vmraid.bold(self.company)))
...
def start_import(invoices):
	...
	for idx, d in enumerate(invoices):
		try:
			...
			vmraid.db.commit()
	if errors:
		vmraid.msgprint(_("You had {} errors while creating opening invoices. Check {} for more details")
...
def get_temporary_opening_account(company=None):
	...
	accounts = vmraid.get_all("Account", filters={
		'company': company,
