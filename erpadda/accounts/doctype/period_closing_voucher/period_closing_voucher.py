...
class PeriodClosingVoucher(AccountsController):
	...
	def validate_account_head(self):
		...
		if closing_account_type not in ["Liability", "Equity"]:
			vmraid.throw(_("Closing Account {0} must be of type Liability / Equity")
	...
	def validate_posting_date(self):
		...
		pce = vmraid.db.sql("""select name from `tabPeriod Closing Voucher`
			where posting_date > %s and fiscal_year = %s and docstatus = 1""",
			(self.posting_date, self.fiscal_year))
		if pce and pce[0][0]:
			vmraid.throw(_("Another Period Closing Entry {0} has been made after {1}")
	...
	def make_gl_entries(self):
		...
		if net_pl_balance:
			cost_center = vmraid.db.get_value("Company", self.company, "cost_center")
