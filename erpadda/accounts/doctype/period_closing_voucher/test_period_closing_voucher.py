...
class TestPeriodClosingVoucher(unittest.TestCase):
	def test_closing_entry(self):
		...
		random_expense_account = vmraid.db.sql("""
			select t1.account,
		...
		profit_or_loss = vmraid.db.sql("""select sum(t1.debit) - sum(t1.credit) as balance
			from `tabGL Entry` t1, `tabAccount` t2
		...
		gle_amount_for_closing_account = vmraid.db.sql("""select debit - credit
		...
		if random_expense_account:
			...
			gle_for_random_expense_account = vmraid.db.sql("""
