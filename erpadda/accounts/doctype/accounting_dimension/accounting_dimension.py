...
class AccountingDimension(Document):
	...
	def validate(self):
		...
		exists = vmraid.db.get_value("Accounting Dimension", {'document_type': self.document_type}, ['name'])

		if exists and self.is_new():
			vmraid.throw("Document Type already used as a dimension")
	...
	def on_trash(self):
		if vmraid.flags.in_test:
			...
			vmraid.enqueue(delete_accounting_dimension, doc=self)
...
def get_accounting_dimensions(as_list=True):
	if vmraid.flags.accounting_dimensions is None:
		vmraid.flags.accounting_dimensions = vmraid.get_all("Accounting Dimension",
...
def get_checks_for_pl_and_bs_accounts():
	dimensions = vmraid.db.sql("""SELECT p.label, p.disabled, p.fieldname, c.default_dimension, c.company, c.mandatory_for_pl, c.mandatory_for_bs
...
def get_dimension_with_children(doctype, dimension):
	...
	children = vmraid.get_all(doctype, filters={"lft": [">=", lft], "rgt": ["<=", rgt]}, order_by="lft")
...
def get_dimensions(with_cost_center_and_project=False):
	...
	default_dimensions = vmraid.db.sql("""SELECT p.fieldname, c.company, c.default_dimension
