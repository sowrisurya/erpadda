...
import vmraid, json, erpadda
from vmraid import _
from vmraid.utils import flt, getdate, nowdate, add_days
...
class InvoiceDiscounting(AccountsController):
	...
	def validate_invoices(self):
		discounted_invoices = [record.sales_invoice for record in
			vmraid.get_all("Discounted Invoice",fields=["sales_invoice"], filters={"docstatus":1})]

		for record in self.invoices:
			if record.sales_invoice in discounted_invoices:
				vmraid.throw(_("Row({0}): {1} is already discounted in {2}")
					.format(record.idx, vmraid.bold(record.sales_invoice), vmraid.bold(record.parent)))

			actual_outstanding = vmraid.db.get_value("Sales Invoice", record.sales_invoice,"outstanding_amount")
	...
	def make_gl_entries(self):
		...
		for d in self.invoices:
			...
			if d.outstanding_amount:
				...
				ar_credit_account_currency = vmraid.get_cached_value("Account", self.accounts_receivable_credit, "currency")
	...
	def close_loan(self):
		...
		if getdate(self.loan_end_date) > getdate(nowdate()):
			for d in self.invoices:
				outstanding_amount = vmraid.db.get_value("Sales Invoice", d.sales_invoice, "outstanding_amount")
