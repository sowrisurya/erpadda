...
from erpadda.accounts.utils import (get_outstanding_invoices,
...
class PaymentReconciliation(Document):
	...
	def get_jv_entries(self):
		dr_or_cr = ("credit_in_account_currency" if erpadda.get_party_account_type(self.party_type) == 'Receivable'
	...
	def get_dr_or_cr_notes(self):
		dr_or_cr = ("credit_in_account_currency"
			if erpadda.get_party_account_type(self.party_type) == 'Receivable' else "debit_in_account_currency")
		...
		return vmraid.db.sql(""" SELECT doc.name as reference_name, %(voucher_type)s as reference_type,
	...
	def get_difference_amount(self, child_row):
		...
		child_row = vmraid._dict(child_row)
		...
		doc = vmraid.get_doc(row.voucher_type, row.voucher_no)
	...
	def validate_invoice(self):
		if not self.get("invoices"):
			vmraid.throw(_("No records found in the Invoice table"))

		if not self.get("payments"):
			vmraid.throw(_("No records found in the Payment table"))
		...
		for p in self.get("payments"):
			if p.invoice_type and p.invoice_number and p.allocated_amount:
				...
				if p.invoice_number not in unreconciled_invoices.get(p.invoice_type, {}):
					vmraid.throw(_("{0}: {1} not found in Invoice Details table")
						.format(p.invoice_type, p.invoice_number))

				if flt(p.allocated_amount) > flt(p.amount):
					vmraid.throw(_("Row {0}: Allocated amount {1} must be less than or equals to Payment Entry amount {2}")
				...
				if flt(p.allocated_amount) - invoice_outstanding > 0.009:
					vmraid.throw(_("Row {0}: Allocated amount {1} must be less than or equals to invoice outstanding amount {2}")
						.format(p.idx, p.allocated_amount, invoice_outstanding))

		if not invoices_to_reconcile:
			vmraid.throw(_("Please select Allocated Amount, Invoice Type and Invoice Number in atleast one row"))

	def check_condition(self):
		cond = " and posting_date >= {0}".format(vmraid.db.escape(self.from_date)) if self.from_date else ""
		cond += " and posting_date <= {0}".format(vmraid.db.escape(self.to_date)) if self.to_date else ""
		dr_or_cr = ("debit_in_account_currency" if erpadda.get_party_account_type(self.party_type) == 'Receivable'
