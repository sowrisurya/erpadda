...
vmraid.ui.form.on("Payment Reconciliation Payment", {
	invoice_number: function(frm, cdt, cdn) {
		...
		if(row.invoice_number) {
			...
			vmraid.model.set_value(cdt, cdn, "allocated_amount", Math.min(invoice_amount, row.amount));
	...
	onload() {
		...
		this.frm.set_query('receivable_payable_account', function() {
			...
			return {
				filters: {
					...
					"is_group": 0,
					"account_type": vmraid.boot.party_account_types[me.frm.doc.party_type]
				}
			};
		});
		...
		var check_mandatory = (frm, only_company=false) => {
			...
			if (only_company && !frm.doc.company) {
				vmraid.throw({message: __("Please Select a Company First"), title: title});
			} else if (!frm.doc.company || !frm.doc.party_type) {
				vmraid.throw({message: __("Please Select Both Company and Party Type First"), title: title});
			}
		};
	}
		...
		if (show_dialog && show_dialog.length) {
			...
			const dialog = new vmraid.ui.Dialog({
				...
				primary_action: function() {
					...
					args.forEach(d => {
						vmraid.model.set_value("Payment Reconciliation Payment", d.docname,
