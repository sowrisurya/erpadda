...
import vmraid, erpadda, json
from vmraid.utils import cstr, flt, fmt_money, formatdate, getdate, nowdate, cint, get_link_to_form
from vmraid import msgprint, _, scrub
from erpadda.controllers.accounts_controller import AccountsController
from erpadda.accounts.utils import get_balance_on, get_stock_accounts, get_stock_and_account_balance, \
...
from erpadda.accounts.doctype.invoice_discounting.invoice_discounting \
...
class StockAccountInvalidTransaction(vmraid.ValidationError): pass
...
class JournalEntry(AccountsController):
	...
	def validate_stock_accounts(self):
		...
		for account in stock_accounts:
			...
			if account_bal == stock_bal:
				vmraid.throw(_("Account: {0} can only be updated via Stock Transactions")
					.format(account), StockAccountInvalidTransaction)

	def update_inter_company_jv(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			vmraid.db.set_value("Journal Entry", self.inter_company_journal_entry_reference,\
	...
	def update_invoice_discounting(self):
		def _validate_invoice_discounting_status(inv_disc, id_status, expected_status, row_id):
			...
			if id_status != expected_status:
				vmraid.throw(_("Row #{0}: Status must be {1} for Invoice Discounting {2}").format(d.idx, expected_status, id_link))
	...
	def unlink_inter_company_jv(self):
		if self.voucher_type == "Inter Company Journal Entry" and self.inter_company_journal_entry_reference:
			...
			vmraid.db.set_value("Journal Entry", self.name,\
				"inter_company_journal_entry_reference", "")

	def unlink_asset_adjustment_entry(self):
		vmraid.db.sql(""" update `tabAsset Value Adjustment`
		...
		for d in self.get("accounts"):
			...
			if account_type in ["Receivable", "Payable"]:
				if not (d.party_type and d.party):
					vmraid.throw(_("Row {0}: Party Type and Party is required for Receivable / Payable account {1}").format(d.idx, d.account))
			...
			if d.reference_type not in ("Sales Invoice", "Purchase Invoice", "Journal Entry"):
				if (d.party_type == 'Customer' and flt(d.credit) > 0) or \
					...
					if d.is_advance=="No":
						...
						vmraid.throw(_("Row {0}: Payment against Sales/Purchase Order should always be marked as advance").format(d.idx))
		...
		for d in self.get('accounts'):
			if d.reference_type=="Journal Entry":
				...
				if account_root_type == "Asset" and flt(d.debit) > 0:
					vmraid.throw(_("Row #{0}: For {1}, you can select reference document only if account gets credited")
						.format(d.idx, d.account))
				elif account_root_type == "Liability" and flt(d.credit) > 0:
					vmraid.throw(_("Row #{0}: For {1}, you can select reference document only if account gets debited")
						.format(d.idx, d.account))

				if d.reference_name == self.name:
					vmraid.throw(_("You can not enter current voucher in 'Against Journal Entry' column"))

				against_entries = vmraid.db.sql("""select * from `tabJournal Entry Account`
					where account = %s and docstatus = 1 and parent = %s
					and (reference_type is null or reference_type in ("", "Sales Order", "Purchase Order"))
					""", (d.account, d.reference_name), as_dict=True)

				if not against_entries:
					vmraid.throw(_("Journal Entry {0} does not have account {1} or already matched against other voucher")
					...
					for jvd in against_entries:
						if flt(jvd[dr_or_cr]) > 0:
							valid = True
					if not valid:
						vmraid.throw(_("Against Journal Entry {0} does not have any unmatched {1} entry")
		...
		for d in self.get("accounts"):
			...
			if d.reference_type and d.reference_name and (d.reference_type in list(field_dict)):
				...
				if d.reference_type=="Sales Order" and flt(d.debit) > 0:
					vmraid.throw(_("Row {0}: Debit entry can not be linked with a {1}").format(d.idx, d.reference_type))

				if d.reference_type == "Purchase Order" and flt(d.credit) > 0:
					vmraid.throw(_("Row {0}: Credit entry can not be linked with a {1}").format(d.idx, d.reference_type))
				...
				against_voucher = vmraid.db.get_value(d.reference_type, d.reference_name,
				...
				if d.reference_type in ("Sales Invoice", "Purchase Invoice"):
					...
					if (against_voucher[0] != d.party or party_account != d.account):
						vmraid.throw(_("Row {0}: Party / Account does not match with {1} / {2} in {3} {4}")
				...
				if d.reference_type in ("Sales Order", "Purchase Order"):
					...
					if against_voucher != d.party:
						vmraid.throw(_("Row {0}: {1} {2} does not match with {3}") \
		...
		for reference_name, total in iteritems(self.reference_totals):
			...
			if reference_type in ("Sales Order", "Purchase Order"):
				...
				if flt(voucher_total) < (flt(order.advance_paid) + total):
					vmraid.throw(_("Advance paid against {0} {1} cannot be greater than Grand Total {2}").format(reference_type, reference_name, formatted_voucher_total))
	...
	def validate_invoices(self):
		...
		for reference_name, total in iteritems(self.reference_totals):
			...
			if (reference_type in ("Sales Invoice", "Purchase Invoice") and
				...
				invoice = vmraid.db.get_value(reference_type, reference_name,
				...
				if total and flt(invoice.outstanding_amount) < total:
					vmraid.throw(_("Payment against {0} {1} cannot be greater than Outstanding Amount {2}")
	...
	def validate_total_debit_and_credit(self):
		...
		if self.difference:
			vmraid.throw(_("Total Debit must be equal to Total Credit. The difference is {0}")
	...
	def validate_multi_currency(self):
		alternate_currency = []
		for d in self.get("accounts"):
			account = vmraid.db.get_value("Account", d.account, ["account_currency", "account_type"], as_dict=1)
			if account:
	...
	def create_remarks(self):
		...
		for d in self.get('accounts'):
			...
			if d.reference_type == "Purchase Invoice" and d.debit:
				bill_no = vmraid.db.sql("""select bill_no, bill_date
	...
	def set_print_format_fields(self):
		...
		if party_type and pay_to_recd_from:
			self.pay_to_recd_from = vmraid.db.get_value(party_type, pay_to_recd_from,
	...
	def get_values(self):
		...
		if self.write_off_based_on == 'Accounts Receivable':
			return vmraid.db.sql("""select name, debit_to as account, customer as party, outstanding_amount
				from `tabSales Invoice` where docstatus = 1 and company = %s
				and outstanding_amount > 0 %s""" % ('%s', cond), self.company, as_dict=True)
		elif self.write_off_based_on == 'Accounts Payable':
			return vmraid.db.sql("""select name, credit_to as account, supplier as party, outstanding_amount
	...
	def validate_expense_claim(self):
		for d in self.accounts:
			if d.reference_type=="Expense Claim":
				sanctioned_amount, reimbursed_amount = vmraid.db.get_value("Expense Claim",
					d.reference_name, ("total_sanctioned_amount", "total_amount_reimbursed"))
				pending_amount = flt(sanctioned_amount) - flt(reimbursed_amount)
				if d.debit > pending_amount:
					vmraid.throw(_("Row No {0}: Amount cannot be greater than Pending Amount against Expense Claim {1}. Pending Amount is {2}").format(d.idx, d.reference_name, pending_amount))
	...
	def validate_credit_debit_note(self):
		if self.stock_entry:
			...
			if vmraid.db.exists({"doctype": "Journal Entry", "stock_entry": self.stock_entry, "docstatus":1}):
				vmraid.msgprint(_("Warning: Another {0} # {1} exists against stock entry {2}").format(self.voucher_type, self.name, self.stock_entry))
...
def get_default_bank_cash_account(company, account_type=None, mode_of_payment=None, account=None):
	...
	if not account:
		...
		if account_type=="Bank":
			...
			if not account:
				account_list = vmraid.get_all("Account", filters = {"company": company,
	...
	if account:
		account_details = vmraid.db.get_value("Account", account,
...
def get_payment_entry(ref_doc, args):
	cost_center = ref_doc.get("cost_center") or vmraid.get_cached_value('Company',  ref_doc.company,  "cost_center")
	exchange_rate = 1
...
def get_opening_accounts(company):
	...
	accounts = vmraid.db.sql_list("""select
			name from tabAccount
		where
			is_group=0 and report_type='Balance Sheet' and company={0} and
			name not in (select distinct account from tabWarehouse where
			account is not null and account != '')
		order by name asc""".format(vmraid.db.escape(company)))
...
def get_outstanding(args):
	...
	if args.get("doctype") == "Journal Entry":
		...
		invoice = vmraid.db.get_value(args["doctype"], args["docname"],
...
def get_account_balance_and_party_type(account, date, company, debit=None, credit=None, exchange_rate=None, cost_center=None):
	...
	account_details = vmraid.db.get_value("Account", account, ["account_type", "account_currency"], as_dict=1)
