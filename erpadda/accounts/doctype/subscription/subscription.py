...
from vmraid.utils.data import nowdate, getdate, cstr, cint, add_days, date_diff, get_last_day, add_to_date, flt
...
class Subscription(Document):
	...
	def validate_end_date(self):
		...
		if self.end_date and getdate(self.end_date) <= getdate(end_date):
			vmraid.throw(_('Subscription End Date must be after {0} as per the subscription plan').format(end_date))
	...
	def validate_to_follow_calendar_months(self):
		if self.follow_calendar_months:
			...
			if billing_info[0]['billing_interval'] != 'Month':
				vmraid.throw(_('Billing Interval in Subscription Plan must be Month to follow calendar months'))
	...
	def create_invoice(self, prorate):
		...
		if not company:
			vmraid.throw(_("Company is mandatory was generating invoice. Please set default company in Global Defaults"))
	...
	def has_outstanding_invoice(self):
		...
		outstanding_invoices = vmraid.get_all(doctype, fields=['name'],
