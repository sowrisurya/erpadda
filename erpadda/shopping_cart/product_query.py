...
class ProductQuery:
	...
	def __init__(self):
		self.settings = vmraid.get_doc("Products Settings")
		self.cart_settings = vmraid.get_doc("Shopping Cart Settings")
	...
	def query(self, attributes=None, fields=None, search_term=None, start=0):
		...
		if attributes:
			...
			result = vmraid.get_all("Item", fields=self.fields, filters=self.filters, or_filters=self.or_filters, start=start, limit=self.page_length)
