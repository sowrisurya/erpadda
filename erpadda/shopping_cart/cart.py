...
from vmraid.utils import cint, flt, get_fullname, cstr
from vmraid.contacts.doctype.address.address import get_address_display
from erpadda.shopping_cart.doctype.shopping_cart_settings.shopping_cart_settings import get_shopping_cart_settings
from vmraid.utils.nestedset import get_root_of
from erpadda.accounts.utils import get_account_name
from erpadda.utilities.product import get_qty_in_stock
...
def set_cart_count(quotation=None):
	if cint(vmraid.db.get_singles_value("Shopping Cart Settings", "enabled")):
		if not quotation:
...
def get_cart_quotation(doc=None):
	...
	return {
		...
		"cart_settings": vmraid.get_cached_doc("Shopping Cart Settings")
	}
...
def place_order():
	quotation = _get_cart_quotation()
	cart_settings = vmraid.db.get_value("Shopping Cart Settings", None,
	...
	if not cint(cart_settings.allow_items_not_in_stock):
		for item in sales_order.get("items"):
			item.reserved_warehouse, is_stock_item = vmraid.db.get_value("Item",
...
def update_cart(item_code, qty, additional_notes=None, with_items=False):
	...
	if cint(with_items):
		return {
			"items": vmraid.render_template("templates/includes/cart/cart_items.html",
				context),
			"taxes": vmraid.render_template("templates/includes/order/order_taxes.html",
				context),
		}
	else:
		return {
...
def update_cart_address(address_type, address_name):
	...
	return {
		"taxes": vmraid.render_template("templates/includes/order/order_taxes.html",
			context),
		"address": vmraid.render_template("templates/includes/cart/address_card.html",
			context)
	}
...
def guess_territory():
	...
	return territory or \
		vmraid.db.get_value("Shopping Cart Settings", None, "territory") or \
			get_root_of("Territory")

def decorate_quotation_doc(doc):
	for d in doc.get("items", []):
		d.update(vmraid.db.get_value("Item", d.item_code,
...
def _get_cart_quotation(party=None):
	...
	quotation = vmraid.get_all("Quotation", fields=["name"], filters=
	...
	if quotation:
		...
		company = vmraid.db.get_value("Shopping Cart Settings", None, ["company"])
		qdoc = vmraid.get_doc({
			"doctype": "Quotation",
...
def apply_cart_settings(party=None, quotation=None):
	...
	cart_settings = vmraid.get_doc("Shopping Cart Settings")
...
def get_debtors_account(cart_settings):
	...
	payment_gateway_account_currency = \
		vmraid.get_doc("Payment Gateway Account", cart_settings.payment_gateway_account).currency
...
def get_address_docs(doctype=None, txt=None, filters=None, limit_start=0, limit_page_length=20,
	...
	address_names = vmraid.db.get_all('Dynamic Link', fields=('parent'),
...
def get_shipping_rules(quotation=None, cart_settings=None):
	...
	if quotation.shipping_address_name:
		...
		if country:
			shipping_rules = vmraid.db.sql_list("""select distinct sr.name
...
def get_address_territory(address_name):
	...
	if address_name:
		address_fields = vmraid.db.get_value("Address", address_name,
			["city", "state", "country"])
...
def apply_coupon_code(applied_code, applied_referral_sales_partner):
	...
	if applied_referral_sales_partner:
		sales_partner_list = vmraid.get_all('Sales Partner', filters={'referral_code': applied_referral_sales_partner})
