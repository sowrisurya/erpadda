...
class FindItemBot(BotParser):
	def get_reply(self):
		if self.startswith('where is', 'find item', 'locate'):
			...
			items = vmraid.db.sql('''select name from `tabItem` where item_code like %(txt)s
			...
			if items:
				...
				for item in items:
					...
					for warehouse in warehouses:
						qty = vmraid.db.get_value("Bin", {'item_code': item[0], 'warehouse': warehouse.name}, 'actual_qty')
						if qty:
