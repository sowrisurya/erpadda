...
import vmraid
from vmraid.utils import cint, fmt_money, flt, nowdate, getdate
...
def get_qty_in_stock(item_code, item_warehouse_field, warehouse=None):
	...
	template_item_code, is_stock_item = vmraid.db.get_value("Item", item_code, ["variant_of", "is_stock_item"])

	if not warehouse:
		warehouse = vmraid.db.get_value("Item", item_code, item_warehouse_field)

	if not warehouse and template_item_code and template_item_code != item_code:
		warehouse = vmraid.db.get_value("Item", template_item_code, item_warehouse_field)
	...
	return vmraid._dict({"in_stock": in_stock, "stock_qty": stock_qty, "is_stock_item": is_stock_item})
...
def get_price(item_code, price_list, customer_group, company, qty=1):
	...
	if price_list:
		price = vmraid.get_all("Item Price", fields=["price_list_rate", "currency"],
		...
		if price:
			pricing_rule = get_pricing_rule_for_item(vmraid._dict({
				"item_code": item_code,
				"qty": qty,
				"stock_qty": qty,
			...
			if price_obj:
				...
				price_obj["currency_symbol"] = not cint(vmraid.db.get_default("hide_currency_symbol")) \
					and (vmraid.db.get_value("Currency", price_obj.currency, "symbol", cache=True) or price_obj.currency) \
					or ""

				uom_conversion_factor = vmraid.db.sql("""select	C.conversion_factor
...
def get_non_stock_item_status(item_code, item_warehouse_field):
	...
	if vmraid.db.exists("Product Bundle", item_code):
		...
		bundle_warehouse = vmraid.db.get_value('Item', item_code, item_warehouse_field)
