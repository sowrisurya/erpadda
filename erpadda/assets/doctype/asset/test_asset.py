...
import vmraid
import unittest
from vmraid.utils import cstr, nowdate, getdate, flt, get_last_day, add_days, add_months
from erpadda.assets.doctype.asset.depreciation import post_depreciation_entries, scrap_asset, restore_asset
...
from erpadda.stock.doctype.purchase_receipt.purchase_receipt import make_purchase_invoice as make_invoice
...
class TestAsset(unittest.TestCase):
	...
	def test_purchase_asset(self):
		...
		gle = vmraid.db.sql("""select account, debit, credit from `tabGL Entry`
	...
	def test_schedule_for_straight_line_method_for_existing_asset(self):
		...
		asset = vmraid.get_doc("Asset", {"asset_name": "Macbook Pro 1"})
		asset.calculate_depreciation = 1
	...
	def test_cwip_accounting(self):
		...
		pr_gle = vmraid.db.sql("""select account, debit, credit from `tabGL Entry`
		...
		pi_gle = vmraid.db.sql("""select account, debit, credit from `tabGL Entry`
			where voucher_type='Purchase Invoice' and voucher_no = %s
			order by account""", pi.name)

		self.assertEqual(pi_gle, expected_gle)

		asset = vmraid.db.get_value('Asset',
	...
	def test_asset_cwip_toggling_cases(self):
		...
		name = vmraid.db.get_value("Asset Category Account", filters={"parent": "Computers"}, fieldname=["name"])
		...
		asset_doc.submit()
		gle = vmraid.db.sql("""select name from `tabGL Entry` where voucher_type='Asset' and voucher_no = %s""", asset_doc.name)
