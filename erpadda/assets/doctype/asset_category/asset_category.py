		...
		for d in self.finance_books:
			for field in ("Total Number of Depreciations", "Frequency of Depreciation"):
				if cint(d.get(vmraid.scrub(field)))<1:
					vmraid.throw(_("Row {0}: {1} must be greater than 0").format(d.idx, field), vmraid.MandatoryError)
			...
			for type_of_account in account_types:
				if d.get(type_of_account):
					...
					if account_currency != company_currency:
						invalid_accounts.append(vmraid._dict({ 'type': type_of_account, 'idx': d.idx, 'account': d.get(type_of_account) }))
	
		for d in invalid_accounts:
			vmraid.throw(_("Row #{}: Currency of {} - {} doesn't matches company currency.")
				.format(d.idx, vmraid.bold(vmraid.unscrub(d.type)), vmraid.bold(d.account)),
			...
			for fieldname in account_type_map.keys():
				if d.get(fieldname):
					...
					if selected_key_type != expected_key_type:
						vmraid.throw(_("Row #{}: {} of {} should be {}. Please modify the account or select a different account.")
							.format(d.idx, vmraid.unscrub(key_to_match), vmraid.bold(selected_account), vmraid.bold(expected_key_type)),
							title=_("Invalid Account"))
		...
		if self.enable_cwip_accounting:
			...
			for d in self.accounts:
				if (not d.capital_work_in_progress_account and 
					not vmraid.db.get_value("Company", d.company_name, "capital_work_in_progress_account")):
...
def get_asset_category_account(fieldname, item=None, asset=None, account=None, asset_category = None, company = None):
	...
	account = vmraid.db.get_value("Asset Category Account",
