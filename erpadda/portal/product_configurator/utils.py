...
def get_field_filter_data():
	...
	for f in fields:
		...
		values = [d.name for d in vmraid.get_all(doctype, filters)]
...
def get_attribute_filter_data():
	...
	attribute_docs = [
		vmraid.get_doc('Item Attribute', attribute) for attribute in attributes
	]

	# mark attribute values as checked if they are present in the request url
	if vmraid.form_dict:
		for attr in attribute_docs:
			if attr.name in vmraid.form_dict:
				value = vmraid.form_dict[attr.name]
				if value:
...
def get_products_html_for_website(field_filters=None, attribute_filters=None):
	field_filters = vmraid.parse_json(field_filters)
	attribute_filters = vmraid.parse_json(attribute_filters)

	items = get_products_for_website(field_filters, attribute_filters)
	html = ''.join(get_html_for_items(items))

	if not items:
		html = vmraid.render_template('erpadda/www/all-products/not_found.html', {})
...
def get_item_codes_by_attributes(attribute_filters, template_item_code=None):
	...
	for attribute, values in attribute_filters.items():
		...
		item_codes = set([r[0] for r in vmraid.db.sql(query, query_values)])
...
def get_attributes_and_values(item_code):
	...
	item_attribute_values = vmraid.db.get_all('Item Attribute Value',
...
def get_items_by_fields(field_filters):
	...
	for fieldname, values in field_filters.items():
		...
		if df.fieldtype == 'Table MultiSelect':
			...
			child_meta = vmraid.get_meta(child_doctype)
...
def get_items(filters=None, search=None):
	start = vmraid.form_dict.get('start', 0)
...
def get_item_attributes(item_code):
	attributes = vmraid.db.get_all('Item Variant Attribute',
		fields=['attribute'],
		filters={
			'parenttype': 'Item',
			'parent': item_code
...
def get_html_for_items(items):
	html = []
	for item in items:
		html.append(vmraid.render_template('erpadda/www/all-products/item_row.html', {
			'item': item
		}))
	return html

def get_product_settings():
	doc = vmraid.get_cached_doc('Products Settings')
