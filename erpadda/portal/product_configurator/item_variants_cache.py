...
class ItemVariantsCacheManager:
	...
	def get_ordered_attribute_values(self):
		...
		all_attribute_values = vmraid.db.get_all('Item Attribute Value',
	...
	def build_cache(self):
		...
		attributes = [a.attribute for a in vmraid.db.get_all('Item Variant Attribute',
			{'parent': parent_item_code}, ['attribute'], order_by='idx asc')
		]

		item_variants_data = vmraid.db.get_all('Item Variant Attribute',
			{'variant_of': parent_item_code}, ['parent', 'attribute', 'attribute_value'],
			order_by='name',
			as_list=1
		)

		disabled_items = set([i.name for i in vmraid.db.get_all('Item', {'disabled': 1})])
...
def enqueue_build_cache(item_code):
	if vmraid.cache().hget('item_cache_build_in_progress', item_code):
		return
	vmraid.enqueue(build_cache, item_code=item_code, queue='long')
