...
def get_all_certifications_of_a_member():
	...
	all_certifications = []
	all_certifications = vmraid.db.sql(""" select cc.name,cc.from_date,cc.to_date,ca.amount,ca.currency
		from `tabCertified Consultant` cc
		inner join `tabCertification Application` ca
		on cc.certification_application = ca.name
		where paid = 1 and email = %(user)s order by cc.to_date desc""" ,{'user': vmraid.session.user},as_dict=True)
