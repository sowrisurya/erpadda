...
def execute():
	...
	for company in company_list:
		party_account_list = vmraid.get_all('Party Account', filters={'company': company.name}, fields=['name', 'account', 'debtor_creditor_number'])
		for party_account in party_account_list:
			...
			account_number = vmraid.db.get_value('Account', party_account.account, 'account_number')
			if not account_number:
				continue

			vmraid.db.set_value('Party Account', party_account.name, 'debtor_creditor_number', account_number)
			vmraid.db.set_value('Party Account', party_account.name, 'account', '')
