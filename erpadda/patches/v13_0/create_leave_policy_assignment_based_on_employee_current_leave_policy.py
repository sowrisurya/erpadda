...
def execute():
    if "leave_policy" in vmraid.db.get_table_columns("Employee"):
        employees_with_leave_policy = vmraid.db.sql("SELECT name, leave_policy FROM `tabEmployee` WHERE leave_policy IS NOT NULL and leave_policy != ''", as_dict = 1)
        ...
        for employee in employees_with_leave_policy:
            alloc = vmraid.db.exists("Leave Allocation", {"employee":employee.name, "leave_policy": employee.leave_policy, "docstatus": 1})
            if not alloc:
    ...
    if "default_leave_policy" in vmraid.db.get_table_columns("Employee"):
        employee_grade_with_leave_policy = vmraid.db.sql("SELECT name, default_leave_policy FROM `tabEmployee Grade` WHERE default_leave_policy IS NOT NULL and default_leave_policy!=''", as_dict = 1)
        ...
        for grade in employee_grade_with_leave_policy:
            ...
            for employee in employees:
                ...
                if employee not in employee_with_assignment: #Will ensure no duplicate
                    alloc = vmraid.db.exists("Leave Allocation", {"employee":employee.name, "leave_policy": grade.default_leave_policy, "docstatus": 1})
                    if not alloc:
    ...
    leave_allocations = vmraid.db.sql("SELECT leave_policy, leave_period, employee FROM `tabLeave Allocation` WHERE leave_policy IS NOT NULL and leave_policy != '' and docstatus = 1 ", as_dict = 1)
