...
def execute():
	...
	if vmraid.db.exists('DocType', 'Service Level Agreement'):
		...
		priorities = vmraid.db.get_all('Service Level Priority', fields=['*'], filters={
		...
		for entry in sla_details:
			values = vmraid.db.get_value('Service Level', entry.service_level, ['holiday_list', 'employee_group'])
			if values:
				holiday_list = values[0]
				employee_group = values[1]
				vmraid.db.set_value('Service Level Agreement', entry.name, {
		...
		for priority in priorities:
			if priority.parenttype == 'Service Level Agreement':
				...
				vmraid.db.set_value('Service Level Priority', priority.name, {
		...
		if vmraid.db.exists('DocType', 'Service Level'):
			service_levels = vmraid.db.get_all('Service Level', filters={'service_level': ('not in', sl)}, fields=['*'])
