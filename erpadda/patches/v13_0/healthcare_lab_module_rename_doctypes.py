...
def execute():
	if vmraid.db.exists('DocType', 'Lab Test') and vmraid.db.exists('DocType', 'Lab Test Template'):
		...
		for old_dt, new_dt in doctypes.items():
			if not vmraid.db.table_exists(new_dt) and vmraid.db.table_exists(old_dt):
		...
		for new, old in rename_fields.items():
			if vmraid.db.has_column('Normal Test Result', old):
				vmraid.db.sql("""UPDATE `tabNormal Test Result` SET {} = {}"""
					.format(new, old))
		...
		if vmraid.db.has_column('Descriptive Test Result', 'test_particulars'):
			vmraid.db.sql("""UPDATE `tabDescriptive Test Result` SET lab_test_particulars = test_particulars""")
		...
		for new, old in rename_fields.items():
			if vmraid.db.has_column('Lab Test Group Template', old):
				vmraid.db.sql("""UPDATE `tabLab Test Group Template` SET {} = {}"""
					.format(new, old))
