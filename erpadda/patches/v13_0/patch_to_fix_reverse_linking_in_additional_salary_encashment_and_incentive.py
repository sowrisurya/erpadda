...
def execute():
	...
	additional_salaries = vmraid.get_all("Additional Salary",
		fields = ['name', "salary_slip", "type", "salary_component"],
		filters = {'salary_slip': ['!=', '']},
		group_by = 'salary_slip'
	)
	leave_encashments = vmraid.get_all("Leave Encashment",
		fields = ["name","additional_salary"],
		filters = {'additional_salary': ['!=', '']}
	)
	employee_incentives = vmraid.get_all("Employee Incentive",
		fields= ["name", "additional_salary"],
		filters = {'additional_salary': ['!=', '']}
	)

	for incentive in employee_incentives:
		vmraid.db.sql(""" UPDATE `tabAdditional Salary`
