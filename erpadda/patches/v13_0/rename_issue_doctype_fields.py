...
def execute():
	if vmraid.db.exists('DocType', 'Issue'):
		issues = vmraid.db.get_all('Issue', fields=['name', 'response_by_variance', 'resolution_by_variance', 'mins_to_first_response'],
		...
		for entry in issues:
			...
			vmraid.db.set_value('Issue', entry.name, {
	...
	if vmraid.db.exists('DocType', 'Opportunity'):
		opportunities = vmraid.db.get_all('Opportunity', fields=['name', 'mins_to_first_response'], order_by='creation desc')
		...
		for entry in opportunities:
			mins_to_first_response = convert_to_seconds(entry.mins_to_first_response, 'Minutes')
			vmraid.db.set_value('Opportunity', entry.name, 'first_response_time', mins_to_first_response, update_modified=False)
			# commit after every 100 updates
			count += 1
			if count%100 == 0:
				vmraid.db.commit()
