...
def execute():
	if not (vmraid.db.table_exists("Payroll Period") and vmraid.db.table_exists("Taxable Salary Slab")):
		return
	...
	standard_tax_exemption_amount_exists = vmraid.db.has_column("Payroll Period", "standard_tax_exemption_amount")
	...
	migrated = []
	proofs = vmraid.get_all("Employee Tax Exemption Proof Submission",
		filters = {'docstatus': 1},
	...
	declerations = vmraid.get_all("Employee Tax Exemption Declaration",
		filters = {'docstatus': 1},
		fields =['payroll_period', 'employee', 'company', 'income_from_other_sources']
	)
