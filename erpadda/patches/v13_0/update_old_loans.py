...
from vmraid.utils import nowdate, flt
...
from erpadda.loan_management.doctype.loan.loan import make_repayment_entry
from erpadda.loan_management.doctype.loan_repayment.loan_repayment import get_accrued_interest_entries
...
def execute():
	...
	if vmraid.db.has_column('Repayment Schedule', 'paid'):
		loans_list = vmraid.db.sql("""SELECT distinct parent from `tabRepayment Schedule`
			where paid = 0 and docstatus = 1""", as_dict=1)

		loans_to_close = [d.parent for d in loans_list]

	if loans_to_close:
		vmraid.db.sql("UPDATE `tabLoan` set status = 'Closed' where name not in (%s)" % (', '.join(['%s'] * len(loans_to_close))), tuple(loans_to_close))

	loans = vmraid.get_all('Loan', fields=['name', 'loan_type', 'company', 'status', 'mode_of_payment',
	...
	for loan in loans:
		...
		group_income_account = vmraid.get_value('Account', {'company': loan.company,
		...
		if loan_type_company and loan_type_company != loan.company:
			...
			loan_type_name = vmraid.get_value('Loan Type', {'company': loan.company,
			...
			vmraid.db.sql("UPDATE `tabLoan` set loan_type = %s where name = %s", (loan_type_name,
				loan.name))
		...
		if loan_type in updated_loan_types:
			...
			vmraid.db.set_value('Loan', loan.name, {
				'is_term_loan': 1,
			...
			if vmraid.db.has_column('Repayment Schedule', 'paid'):
				total_principal, total_interest = vmraid.db.get_value('Repayment Schedule', {'paid': 1, 'parent': loan.name},
