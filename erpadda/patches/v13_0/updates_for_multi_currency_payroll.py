...
def execute():
	...
	all_companies = vmraid.db.get_all("Company", fields=["name", "default_currency", "default_payroll_payable_account"])
	for d in all_companies:
		...
		if not default_payroll_payable_account:
			default_payroll_payable_account = vmraid.db.get_value("Account",
		...
		for dt in doctypes_for_currency:
			vmraid.db.sql("""update `tab{doctype}` set currency = %s where company=%s"""
