...
def execute():
	...
	if vmraid.db.exists('DocType', 'Lab Prescription'):
		if vmraid.db.has_column('Lab Prescription', 'parentfield'):
			vmraid.db.sql("""
				update `tabLab Prescription` set parentfield = 'lab_test_prescription'
				where parentfield = 'test_prescription'
			""")

	if vmraid.db.exists('DocType', 'Lab Test Groups'):
		if vmraid.db.has_column('Lab Test Groups', 'parentfield'):
			vmraid.db.sql("""
