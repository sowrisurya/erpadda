...
def set_series(doctype, options, default):
	def _make_property_setter(property_name, value):
		property_setter = vmraid.db.exists('Property Setter',
...
def get_series_to_preserve(doctype):
	series_to_preserve = vmraid.db.sql_list("""select distinct naming_series from `tab{doctype}` where ifnull(naming_series, '') != ''""".format(doctype=doctype))
