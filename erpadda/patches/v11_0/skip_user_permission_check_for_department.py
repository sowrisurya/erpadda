...
def execute():
	...
	for doctype in ['Appraisal', 'Leave Allocation', 'Expense Claim', 'Instructor', 'Salary Slip',
		...
		if vmraid.db.exists('Custom Field', { 'dt': doctype, 'fieldname': 'department'}): continue
		doctypes_to_skip.append(doctype)

	vmraid.reload_doctype('User Permission')

	user_permissions = vmraid.get_all("User Permission",
	...
	if user_permissions_to_delete:
		vmraid.db.sql('DELETE FROM `tabUser Permission` WHERE `name` IN ({})'.format( # nosec
