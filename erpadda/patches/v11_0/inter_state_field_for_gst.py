...
def execute():
	...
	if vmraid.db.has_column("Sales Taxes and Charges Template", "is_inter_state") and\
		vmraid.db.has_column("Purchase Taxes and Charges Template", "is_inter_state"):

		igst_accounts = set(vmraid.db.sql_list('''SELECT igst_account from `tabGST Account` WHERE parent = "GST Settings"'''))
		cgst_accounts = set(vmraid.db.sql_list('''SELECT cgst_account FROM `tabGST Account` WHERE parenttype = "GST Settings"'''))
		...
		if when_then_sales:
			vmraid.db.sql('''update `tabSales Taxes and Charges Template`
				set is_inter_state = Case {when_then} Else 0 End
			'''.format(when_then=" ".join(when_then_sales)))

		if when_then_purchase:
			vmraid.db.sql('''update `tabPurchase Taxes and Charges Template`
				set is_inter_state = Case {when_then} Else 0 End
