...
from erpadda.payroll.doctype.salary_structure_assignment.salary_structure_assignment import DuplicateAssignment
...
def execute():
	...
	for d in all_companies:
		...
		vmraid.db.sql("""update `tabSalary Structure` set currency = %s where company=%s""", (company_currency, company))

	for d in ss_details:
		try:
			joining_date, relieving_date = vmraid.db.get_value("Employee", d.employee,
