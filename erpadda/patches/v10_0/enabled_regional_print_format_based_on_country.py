...
def execute():
	...
	vmraid.db.sql(""" update `tabPrint Format` set disabled = 1 where name
		in ('GST POS Invoice', 'GST Tax Invoice', 'Simplified Tax Invoice', 'Detailed Tax Invoice')""")

	for d in vmraid.get_all('Company', fields = ["country"],
		...
		if print_format_mapper.get(d.country):
			...
			vmraid.db.sql(""" update `tabPrint Format` set disabled = 0
