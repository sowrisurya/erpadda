...
def execute():
	...
	if not vmraid.db.exists('DocType', 'Setup Progress') or not vmraid.db.exists('DocType', 'Setup Progress Action'):
		return

	vmraid.reload_doc("setup", "doctype", "setup_progress")
	vmraid.reload_doc("setup", "doctype", "setup_progress_action")
	...
	actions = [
		...
		{"action_name": "Set Sales Target", "action_doctype": "Company", "min_doc_count": 99,
			"action_document": vmraid.defaults.get_defaults().get("company") or '',
	...
	setup_progress = vmraid.get_doc("Setup Progress", "Setup Progress")
