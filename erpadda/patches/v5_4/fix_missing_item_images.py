...
def fix_files_for_item(files_path, unlinked_files):
	...
	for (file_url, item_code), children in file_item_code.items():
		...
		for row in children:
			...
			vmraid.db.set_value(row.doctype, row.name, "image", new_file_url, update_modified=False)

		# set it as attachment of this item code
		file_data = vmraid.get_doc("File", unlinked_files[file_url]["file"])
		...
		if not vmraid.db.get_value("Item", item_code, "image"):
			vmraid.db.set_value("Item", item_code, "image", new_file_url, update_modified=False)
...
def rename_and_set_content_hash(files_path, unlinked_files, file_url):
	...
	if not vmraid.db.get_value("File", file_data_name, "content_hash"):
		with open(new_filename, "r") as f:
			content_hash = get_content_hash(f.read())
			vmraid.db.set_value("File", file_data_name, "content_hash", content_hash)
...
def get_unlinked_files(files_path):
	...
	for file in files:
		if not vmraid.db.exists("File", {"file_name": file}):
			file_data = vmraid.db.get_value("File", {"name": file},
...
def get_file_item_code(file_urls):
	...
	doctypes = vmraid.db.sql_list("""select name from `tabDocType` dt
		where istable=1
	...
	for doctype in doctypes:
		result = vmraid.db.sql("""select name, image, item_code, '{0}' as doctype from `tab{0}`
