...
def execute():
	...
	vmraid.reload_doctype("Stock Entry Detail")
	vmraid.reload_doctype("Landed Cost Taxes and Charges")

	stock_entry_db_columns = vmraid.db.get_table_columns("Stock Entry")
	...
	vmraid.db.sql("""update `tabStock Entry Detail` sed, `tabStock Entry` se
	...
	stock_entries = vmraid.db.sql_list("""select name from `tabStock Entry`
	...
	for d in stock_entries:
		stock_entry = vmraid.get_doc("Stock Entry", d)
