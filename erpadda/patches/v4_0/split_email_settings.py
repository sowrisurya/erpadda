...
def execute():
	...
	vmraid.reload_doc("support", "doctype", "support_email_settings")
...
def map_outgoing_email_settings(email_settings):
	outgoing_email_settings = vmraid.get_doc("Outgoing Email Settings")
...
def map_support_email_settings(email_settings):
	support_email_settings = vmraid.get_doc("Support Email Settings")
...
def get_email_settings():
	ret = {}
	for field, value in vmraid.db.sql("select field, value from tabSingles where doctype='Email Settings'"):
		ret[field] = value
	return ret
