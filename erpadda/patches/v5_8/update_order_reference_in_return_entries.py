...
def execute():
	...
	vmraid.reload_doctype("Purchase Order Item Supplied")

	# sales return
	return_entries = list(vmraid.db.sql("""
	...
	for d in return_entries:
		...
		if order_details:
			...
			if (d.doctype=="Sales Invoice" and d.update_stock) or d.doctype=="Delivery Note":
				doc = vmraid.get_doc(d.doctype, d.name)
	...
	for d in return_entries:
		...
		if order_details:
			...
			pr = vmraid.get_doc("Purchase Receipt", d.name)
