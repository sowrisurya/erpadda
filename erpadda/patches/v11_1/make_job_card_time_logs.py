...
def execute():
    ...
    if (vmraid.db.table_exists("Job Card")
        and vmraid.get_meta("Job Card").has_field("actual_start_date")):
        time_logs = []
        for d in vmraid.get_all('Job Card',
            ...
            if d.actual_start_date:
                time_logs.append([d.actual_start_date, d.actual_end_date, d.time_in_mins,
                    d.for_quantity, d.name, 'Job Card', 'time_logs', vmraid.generate_hash("", 10)])
        ...
        vmraid.db.sql(""" update `tabJob Card` set total_completed_qty = for_quantity,
