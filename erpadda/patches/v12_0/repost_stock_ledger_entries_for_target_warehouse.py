...
def execute():
	warehouse_perm = vmraid.get_all("User Permission",
	...
	for perm_data in warehouse_perm:
		if perm_data.p_count == 1 or (perm_data.p_count > 1 and vmraid.get_all("User Permission",
	...
	for doctype in ["Sales Invoice", "Delivery Note"]:
		if not vmraid.get_meta(doctype + ' Item').get_field("target_warehouse").hidden: continue

		cond = ""
		if doctype == "Sales Invoice":
			cond = " AND parent_doc.update_stock = 1"

		data = vmraid.db.sql(""" SELECT parent_doc.name as name, child_doc.name as child_name
			FROM
		...
		if data:
			...
			vmraid.db.sql(""" UPDATE `tab{0} Item` set target_warehouse = null
				WHERE name in ({1}) """.format(doctype, ','.join(["%s"] * len(names) )), tuple(names))

			vmraid.db.sql(""" UPDATE `tabPacked Item` set target_warehouse = null
	...
	if vmraid.get_meta('Sales Order Item').get_field("target_warehouse").hidden:
		vmraid.db.sql(""" UPDATE `tabSales Order Item` set target_warehouse = null
