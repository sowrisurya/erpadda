...
def execute():
	...
	if vmraid.db.has_column('Company', 'default_terms'):
		...
		for company in vmraid.get_all("Company", ["name", "default_selling_terms", "default_buying_terms"]):
			if company.default_selling_terms and not company.default_buying_terms:
				vmraid.db.set_value("Company", company.name, "default_buying_terms", company.default_selling_terms)
