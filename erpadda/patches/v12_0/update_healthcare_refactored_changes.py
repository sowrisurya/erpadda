...
def execute():
	...
	if vmraid.db.exists('DocType', 'Healthcare Practitioner'):
		practitioners = vmraid.db.sql("select name from `tabHealthcare Practitioner` where 'active'= 0", as_dict=1)
		...
		if practitioners_lst:
			vmraid.db.sql("update `tabHealthcare Practitioner` set status = 'Disabled' where name IN %(practitioners)s""", {"practitioners": practitioners_lst})
	...
	if vmraid.db.exists('DocType', 'Patient Encounter'):
		...
		encounter_details = vmraid.db.sql("""select symptoms, diagnosis, name from `tabPatient Encounter`""", as_dict=True)
