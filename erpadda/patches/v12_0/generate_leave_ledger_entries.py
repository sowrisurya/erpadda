...
def generate_allocation_ledger_entries():
	...
	for allocation in allocation_list:
		if not vmraid.db.exists("Leave Ledger Entry", {'transaction_type': 'Leave Allocation', 'transaction_name': allocation.name}):
...
def generate_application_leave_ledger_entries():
	...
	for application in leave_applications:
		if not vmraid.db.exists("Leave Ledger Entry", {'transaction_type': 'Leave Application', 'transaction_name': application.name}):
...
def generate_encashment_leave_ledger_entries():
	...
	for encashment in leave_encashments:
		if not vmraid.db.exists("Leave Ledger Entry", {'transaction_type': 'Leave Encashment', 'transaction_name': encashment.name}):
...
def generate_expiry_allocation_ledger_entries():
	...
	for allocation in allocation_list:
		if not vmraid.db.exists("Leave Ledger Entry", {'transaction_type': 'Leave Allocation', 'transaction_name': allocation.name, 'is_expired': 1}):
			allocation_obj = vmraid.get_doc("Leave Allocation", allocation)
			if allocation_obj.to_date <= getdate(today()):
				expire_allocation(allocation_obj)

def get_allocation_records():
	return vmraid.get_all("Leave Allocation", filters={"docstatus": 1},
