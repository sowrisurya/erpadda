...
def execute():
	...
	for doctype, module in doctypes.items():
		...
		vmraid.db.sql(""" UPDATE `tab{child_doctype}` SET pricing_rules = pricing_rule
			WHERE docstatus < 2 and pricing_rule is not null and pricing_rule != ''
		""".format(child_doctype= child_doctype))

		data = vmraid.db.sql(""" SELECT pricing_rule, name, parent,
		...
		for d in data:
			values.append((d.pricing_rule, d.name, d.parent, 'pricing_rules', d.parenttype,
				d.creation, d.modified, d.docstatus, d.modified_by, d.owner, vmraid.generate_hash("", 10)))

		if values:
			vmraid.db.sql(""" INSERT INTO
	...
	for doctype, apply_on in {'Pricing Rule Item Code': 'Item Code',
		...
		data = vmraid.get_all('Pricing Rule', fields=[field, "name", "creation", "modified",
			"owner", "modified_by"], filters= {'apply_on': apply_on})

		values = []
		for d in data:
			values.append((d.get(field), d.name, parentfield.get(field), 'Pricing Rule',
				d.creation, d.modified, d.owner, d.modified_by, vmraid.generate_hash("", 10)))
