...
def execute():
	...
	existing_templates = vmraid.db.sql("""select template.name, details.tax_type, details.tax_rate
	...
	for d in vmraid.db.sql("""select parent as item_code, tax_type, tax_rate from `tabItem Tax`""", as_dict=1):
	...
	for dt in doctypes:
		for d in vmraid.db.sql("""select name, parenttype, parent, item_code, item_tax_rate from `tab{0} Item`
...
def get_item_tax_template(item_tax_templates, item_tax_map, item_code, parenttype=None, parent=None, tax_types=None):
	...
	for tax_type, tax_rate in iteritems(item_tax_map):
		account_details = vmraid.db.get_value("Account", tax_type, ['name', 'account_type'], as_dict=1)
		if account_details:
			...
			if not account_name:
				...
				parent_account = vmraid.get_value("Account", {"account_name": account_name, "company": company}, "parent_account")
				if not parent_account:
					parent_account = vmraid.db.get_value("Account",
				...
				if not tax_type:
					...
					try:
						...
						tax_type = vmraid.db.get_value("Account", {"account_name": account_name, "company": company}, "name")
