...
def update_existing_custom_fields():
	vmraid.db.sql("""update `tabCustom Field` set label = 'HSN/SAC'
		where fieldname='gst_hsn_code' and label='GST HSN Code'
	""")

	vmraid.db.sql("""update `tabCustom Field` set print_hide = 1
		where fieldname in ('customer_gstin', 'supplier_gstin', 'company_gstin')
	""")

	vmraid.db.sql("""update `tabCustom Field` set insert_after = 'address_display'
		where fieldname in ('customer_gstin', 'supplier_gstin')
	""")

	vmraid.db.sql("""update `tabCustom Field` set insert_after = 'company_address_display'
		where fieldname = 'company_gstin'
	""")

	vmraid.db.sql("""update `tabCustom Field` set insert_after = 'description'
