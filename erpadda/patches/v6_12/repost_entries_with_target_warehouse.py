...
To execute this patch run following commands from vmraid-bench directory:
```
	bench --site [your-site-name] execute erpadda.patches.v6_12.repost_entries_with_target_warehouse.check
	bench --site [your-site-name] backup
	bench --site [your-site-name] execute erpadda.patches.v6_12.repost_entries_with_target_warehouse.repost
```
...
def repost_dn(dn_failed_list):
	...
	for dn in dn_list:
		...
		try:
			dn_doc = vmraid.get_doc("Delivery Note", dn.name)
			dn_doc.docstatus = 2
			...
			vmraid.db.sql("update `tabDelivery Note Item` set target_warehouse='' where parent=%s", dn.name)
			dn_doc = vmraid.get_doc("Delivery Note", dn.name)
			dn_doc.docstatus = 1
			dn_doc.on_submit()
			vmraid.db.commit()
			...
			vmraid.local.stockledger_exceptions = None
			print(vmraid.get_traceback())
			vmraid.db.rollback()
		
	vmraid.db.sql("update `tabDelivery Note Item` set target_warehouse='' where docstatus=0")
...
def repost_si(si_failed_list):
	...
	for si in si_list:
		...
		try:
			...
			vmraid.db.sql("update `tabSales Invoice Item` set target_warehouse='' where parent=%s", si.name)
			si_doc = vmraid.get_doc("Sales Invoice", si.name)
			si_doc.docstatus = 1
			si_doc.update_stock_ledger()
	...
	vmraid.db.sql("update `tabSales Invoice Item` set target_warehouse='' where docstatus=0")
	
def repost_so():
	so_list = get_affected_sales_order()
	
	vmraid.db.sql("update `tabSales Order Item` set target_warehouse=''")
...
def get_affected_delivery_notes():
	return vmraid.db.sql("""select distinct dn.name, dn.docstatus
...
def get_affected_sales_invoice():
	return vmraid.db.sql("""select distinct si.name, si.docstatus
...
def get_affected_sales_order():
	return vmraid.db.sql_list("""select distinct parent from `tabSales Order Item` 
