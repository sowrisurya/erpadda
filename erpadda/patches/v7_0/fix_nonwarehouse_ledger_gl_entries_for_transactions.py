...
def execute():
	...
	warehouses = vmraid.db.sql("""select name, company from tabAccount
		where account_type = 'Stock' and is_group = 0
		and (warehouse is null or warehouse = '')""", as_dict=1)
	warehouses = [d.name for d in warehouses if erpadda.is_perpetual_inventory_enabled(d.company)]
	...
	if len(warehouses) > 0:
		...
		stock_vouchers = vmraid.db.sql("""select distinct sle.voucher_type, sle.voucher_no
		...
		for voucher_type, voucher_no in stock_vouchers:
			try:
				...
				voucher = vmraid.get_doc(voucher_type, voucher_no)
...
def set_warehouse_for_stock_account(warehouse_account):
	for account in warehouse_account:
		if vmraid.db.exists('Warehouse', account):
			vmraid.db.set_value("Account", account, "warehouse", account)
		else:
