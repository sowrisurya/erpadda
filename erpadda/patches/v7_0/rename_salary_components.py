...
import vmraid
from vmraid.model.utils.rename_field import update_property_setters

def execute():
	if not vmraid.db.exists("DocType", "Salary Structure Earning"):
		return
	...
	earning_type_exists = True if "earning_type" in vmraid.db.get_table_columns("Salary Slip Earning") else False
	e_type_exists = True if "e_type" in vmraid.db.get_table_columns("Salary Slip Earning") else False


	if e_type_exists and earning_type_exists:
		vmraid.db.sql("""update `tabSalary Slip Earning`
			set e_type = earning_type, e_modified_amount = earning_amount
			where e_type is null and earning_type is not null""")

		vmraid.db.sql("""update `tabSalary Structure Earning` set e_type = earning_type
			where e_type is null and earning_type is not null""")

		vmraid.db.sql("""update `tabSalary Slip Deduction` set
			d_type = deduction_type, d_modified_amount = deduction_amount
			where d_type is null and deduction_type is not null""")

		vmraid.db.sql("""update `tabSalary Structure Deduction` set d_type = deduction_type
			where d_type is null and deduction_type is not null""")
	...
	for doctype, cols in dt_cols.items():
		...
		vmraid.db.sql("""INSERT INTO `tabSalary Detail` ({0}) SELECT {1} FROM `tab{2}`"""
	...
	for doctype, cols in dt_cols_de.items():
		...
		try:
			vmraid.db.sql("""INSERT INTO `tabSalary Component` ({0}) SELECT {1} FROM `tab{2}`"""
				.format(target_cols, source_cols, doctype))
...
def update_property_setters_and_custom_fields(new_dt, dt_cols):
	for doctype, cols in dt_cols.items():
		vmraid.db.sql("update `tabProperty Setter` set doc_type = %s where doc_type=%s", (new_dt, doctype))
		vmraid.db.sql("update `tabCustom Field` set dt = %s where dt=%s", (new_dt, doctype))
