...
def execute():
	...
	if check_is_warehouse_associated_with_company():
		for company in vmraid.get_all("Company", fields=["name", "abbr"]):
			make_warehouse_nestedset(company)
	else:
		sle_against_companies = vmraid.db.sql_list("""select distinct company from `tabStock Ledger Entry`""")

		if len(sle_against_companies) == 1:
			company = vmraid.get_cached_value('Company',  sle_against_companies[0],  
...
def check_is_warehouse_associated_with_company():
	...
	for warehouse in vmraid.get_all("Warehouse", fields=["name", "company"]):
...
def make_warehouse_nestedset(company=None):
	...
	enable_perpetual_inventory = cint(erpadda.is_perpetual_inventory_enabled(company.name)) or 0
	...
	if not vmraid.db.get_value("Warehouse", warehouse_group):
...
def validate_parent_account_for_warehouse(company=None):
	...
	if cint(erpadda.is_perpetual_inventory_enabled(company.name)):
		parent_account = vmraid.db.sql("""select name from tabAccount
			where account_type='Stock' and company=%s and is_group=1
			and (warehouse is null or warehouse = '')""", company.name)

		if not parent_account:
			current_parent_accounts_for_warehouse = vmraid.db.sql("""select parent_account from tabAccount
				where account_type='Warehouse' and (warehouse is not null or warehouse != '') """)

			if current_parent_accounts_for_warehouse:
				vmraid.db.set_value("Account", current_parent_accounts_for_warehouse[0][0], "account_type", "Stock")
...
def set_parent_to_warehouse(warehouse_group, company=None):
	vmraid.db.sql(""" update tabWarehouse set parent_warehouse = %s, is_group = 0
...
def set_parent_to_warehouse_account(company):
	vmraid.db.sql(""" update tabAccount set parent_account = %s
...
def set_company_to_warehouse(company):
	vmraid.db.sql("update tabWahouse set company=%s", company)

def get_stock_account_group(company):
	stock_account_group = vmraid.db.get_all('Account', filters = {'company': company, 'is_group': 1,
