...
def execute():
	...
	if not vmraid.db.table_exists("Time Log"):
		return

	from erpadda.manufacturing.doctype.work_order.work_order \
		import make_timesheet, add_timesheet_detail

	for data in vmraid.db.sql("select * from `tabTime Log`", as_dict=1):
		if data.task:
			company = vmraid.db.get_value("Task", data.task, "company")
		elif data.work_order:
			company = vmraid.db.get_value("Work Order", data.work_order, "company")
		else:
