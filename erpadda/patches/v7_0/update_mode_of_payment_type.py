...
def execute():
	...
	vmraid.db.sql(""" update `tabMode of Payment` set type = 'Cash' where (type is null or type = '') and name = 'Cash'""")

	for data in vmraid.db.sql("""select name from `tabSales Invoice` where is_pos=1 and docstatus<2 and
			...
			if mode_of_payment != set(mode_of_payment):
				for payment_data in si_doc.payments:
					if payment_data.idx != 1 and payment_data.amount == si_doc.grand_total:
						...
						vmraid.db.sql(""" delete from `tabSales Invoice Payment` 
