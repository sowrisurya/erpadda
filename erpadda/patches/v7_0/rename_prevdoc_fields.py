...
import vmraid
import json
from vmraid.model.utils.rename_field import update_reports, rename_field, update_property_setters
...
def execute():
	...
	vmraid.reload_doctype('Purchase Receipt Item')
	update_po_fields()
...
def update_po_fields():
	for data in vmraid.db.sql(""" select prevdoc_docname, prevdoc_detail_docname, name, prevdoc_doctype
		from `tabPurchase Order Item` where prevdoc_doctype is not null""", as_dict=True):
		if data.prevdoc_doctype == 'Material Request':
			vmraid.db.set_value("Purchase Order Item", data.name, "material_request", data.prevdoc_docname, update_modified=False)
			vmraid.db.set_value("Purchase Order Item", data.name, "material_request_item", data.prevdoc_detail_docname, update_modified=False)
		elif data.prevdoc_doctype == 'Sales Order':
			vmraid.db.set_value("Purchase Order Item", data.name, "sales_order", data.prevdoc_docname, update_modified=False)
			vmraid.db.set_value("Purchase Order Item", data.name, "sales_order_item", data.prevdoc_detail_docname, update_modified=False)
...
def update_print_format_for_po(old_fieldname, new_fieldname, doc_type):
	...
	for data in vmraid.db.sql(""" select name, format_data from `tabPrint Format` where
...
def update_print_format_fields(old_fieldname, new_fieldname, args):
	...
	if update:
		...
		vmraid.db.sql("""update `tabPrint Format` set `format_data`=%s where name=%s""", (val, args.name))
		
def set_sales_order_field():
	for data in vmraid.db.sql("""select doc_type, field_name, property, value, property_type 
