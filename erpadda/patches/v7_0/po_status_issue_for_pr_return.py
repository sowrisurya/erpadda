...
def execute():
	...
	for data in vmraid.db.sql("""
		...
		where
			...
			name = vmraid.db.get_value('Purchase Order Item',
				{'item_code': data.item_code, 'parent': data.purchase_order, 'idx': data.idx}, 'name')

			if name:
				vmraid.db.set_value('Purchase Receipt Item', data.name, 'purchase_order_item', name, update_modified=False)
	...
	if len(parent_list) > 0:
		for parent in set(parent_list):
			doc = vmraid.get_doc('Purchase Receipt', parent)
