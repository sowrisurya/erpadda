...
def execute():
	vmraid.reload_doctype("Sales Invoice Advance")
	vmraid.reload_doctype("Purchase Invoice Advance")
	
	je_rows = vmraid.db.sql("""
	...
	for d in je_rows:
		...
		if d.reference_type in ("Sales Invoice", "Purchase Invoice") and d.reference_name:
			is_advance_entry = vmraid.db.sql("""select name from `tab{0}` 
