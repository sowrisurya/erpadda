...
def execute():
	for company in vmraid.db.sql("select name, expenses_included_in_valuation from tabCompany", as_dict=1):
		...
		for pi in pi_list:
			...
			gle_for_expenses_included_in_valuation = vmraid.db.sql("""select name from `tabGL Entry`
			...
			if gle_for_expenses_included_in_valuation:
				...
				purchase_invoice = vmraid.get_doc("Purchase Invoice", pi.name)
...
def get_frozen_date(company, account):
	...
	accounts_frozen_upto = vmraid.db.get_single_value("Accounts Settings", "acc_frozen_upto")

	# Last adjustment entry to correct Expenses Included in Valuation account balance
	last_adjustment_entry = vmraid.db.sql("""select posting_date from `tabGL Entry`
	...
	last_closing_entry = vmraid.db.sql("""select posting_date from `tabGL Entry`
