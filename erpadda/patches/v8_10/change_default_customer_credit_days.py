...
def execute():
	...
	vmraid.reload_doc("setup", "doctype", "supplier_type")
	vmraid.reload_doc("accounts", "doctype", "payment_term")
	vmraid.reload_doc("accounts", "doctype", "payment_terms_template_detail")
	vmraid.reload_doc("accounts", "doctype", "payment_terms_template")

	payment_terms = []
	records = []
	for doctype in ("Customer", "Supplier", "Supplier Type"):
		credit_days = vmraid.db.sql("""
		...
		for days, based_on, party_name in credit_records:
			...
			if not vmraid.db.exists("Payment Terms Template", pyt_template_name):
				payment_term = make_payment_term(days, based_on)
				template = make_template(payment_term)
			else:
				template = vmraid.get_doc("Payment Terms Template", pyt_template_name)

			payment_terms.append('WHEN `name`={0} THEN {1}'.format(vmraid.db.escape(party_name), template.template_name))
			records.append(vmraid.db.escape(party_name))
...
def make_template(payment_term):
	doc = vmraid.new_doc('Payment Terms Template Detail')
	...
	template = vmraid.new_doc('Payment Terms Template')
...
def make_payment_term(days, based_on):
	...
	doc = vmraid.new_doc('Payment Term')
