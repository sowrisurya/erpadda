...
def execute():
	...
	vmraid.db.sql(""" update `tabSales Invoice Item` sii inner join
	...
	items = vmraid.db.sql(""" select  sii.parent, sii.serial_no from  `tabSales Invoice Item` sii
	...
	for item in items:
		...
		serial_nos = ["{}".format(vmraid.db.escape(no)) for no in serial_nos.split("\n")]
		...
		vmraid.db.sql("""
			...
			WHERE
				name in ({serial_nos})
			""".format(
				sales_invoice=vmraid.db.escape(sales_invoice),
