...
def execute():
	...
	fields = ["name"]
	if "source_warehouse" in vmraid.db.get_table_columns("Work Order"):
		fields.append("source_warehouse")
		
	wo_orders = vmraid.get_all("Work Order", filters={"docstatus": ["!=", 2]}, fields=fields)
	
	count = 0
	for p in wo_orders:
		wo_order = vmraid.get_doc("Work Order", p.name)
		count += 1
