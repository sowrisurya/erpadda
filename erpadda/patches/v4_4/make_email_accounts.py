...
def execute():
	...
	# outgoing
	outgoing = dict(vmraid.db.sql("select field, value from tabSingles where doctype='Outgoing Email Settings'"))
	if outgoing and outgoing['mail_server'] and outgoing['mail_login']:
		account = vmraid.new_doc("Email Account")
		mapping = {
	...
	# support
	support = dict(vmraid.db.sql("select field, value from tabSingles where doctype='Support Email Settings'"))
	...
	for doctype in ("Sales Email Settings", "Jobs Email Settings"):
		source = dict(vmraid.db.sql("select field, value from tabSingles where doctype=%s", doctype))
...
def insert_or_update(account):
	try:
		account.insert()
	except vmraid.NameError as e:
		if e.args[0]=="Email Account":
			existing_account = vmraid.get_doc("Email Account", e.args[1])
