...
def execute():
	...
	for dt, script_field, ref_dt_field in (("Client Script", "script", "dt"), ("Print Format", "html", "doc_type")):
		...
		for name, script, ref_dt in vmraid.db.sql("select name, {0} as script, {1} as ref_dt from `tab{2}` where ({3}) {4}".format(script_field, ref_dt_field, dt, cond1, cond2)):
			update_script(dt, name, ref_dt, script_field, script, renamed_fields)

def get_all_renamed_fields():
	from erpadda.patches.v5_0.rename_table_fieldnames import rename_map
...
def update_script(dt, name, ref_dt, script_field, script, renamed_fields):
	...
	vmraid.db.set_value(dt, name, script_field, script)