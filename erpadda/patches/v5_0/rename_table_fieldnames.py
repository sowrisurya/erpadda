...
def execute():
	...
	vmraid.reload_doc("manufacturing", "doctype", "workstation_working_hour")
	vmraid.reload_doc("stock", "doctype", "item_variant")
	vmraid.reload_doc("Payroll", "doctype", "salary_detail")
	vmraid.reload_doc("accounts", "doctype", "party_account")
	vmraid.reload_doc("accounts", "doctype", "fiscal_year_company")

	#rename table fieldnames
	for dn in rename_map:
		if not vmraid.db.exists("DocType", dn):
			continue
		vmraid.reload_doc(get_doctype_module(dn), "doctype", scrub(dn))

	for dt, field_list in rename_map.items():
		if not vmraid.db.exists("DocType", dt):
			continue
	...
	for old, new in [["Bank Voucher", "Bank Entry"], ["Cash Voucher", "Cash Entry"],
		...
		["Write Off Voucher", "Write Off Entry"], ["Excise Voucher", "Excise Entry"]]:
			vmraid.db.sql("update `tabJournal Entry` set voucher_type=%s where voucher_type=%s", (new, old))
