...
def update_values(dt, tax_table):
	...
	vmraid.reload_doctype(tax_table)

	net_total_precision = get_field_precision(vmraid.get_meta(dt).get_field("net_total"))
	for field in ("total", "base_total", "base_net_total"):
		make_property_setter(dt, field, "precision", net_total_precision, "Select")

	rate_field_precision = get_field_precision(vmraid.get_meta(dt + " Item").get_field("rate"))
	...
	tax_amount_precision = get_field_precision(vmraid.get_meta(tax_table).get_field("tax_amount"))
