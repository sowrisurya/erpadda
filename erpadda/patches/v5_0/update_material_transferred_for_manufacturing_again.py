...
def execute():
	wo_order_qty_transferred = vmraid._dict()
	for se in vmraid.db.sql("""select work_order, sum(fg_completed_qty) as transferred_qty 
		from `tabStock Entry`
	...
	for d in vmraid.get_all("Work Order", filters={"docstatus": 1}, fields=["name", "qty"]):
		if d.name in wo_order_qty_transferred:
			...
			vmraid.db.sql("""update `tabWork Order` set material_transferred_for_manufacturing=%s
