...
def execute():
	vmraid.db.sql("update tabItem set has_batch_no = 0 where ifnull(has_batch_no, '') = ''")
	vmraid.db.sql("update tabItem set has_serial_no = 0 where ifnull(has_serial_no, '') = ''")

	item_list = vmraid.db.sql("""select name, has_batch_no, has_serial_no from tabItem
	...
	for d in item_list:
		...
		if d.has_serial_no == 1:
			if d.name not in serialized_items and sle_count.get(d.name) and sle_count.get(d.name) != sle_with_serial.get(d.name):
				vmraid.db.set_value("Item", d.name, "has_serial_no", 0)
		else:
			if d.name in serialized_items or (sle_count.get(d.name) and sle_count.get(d.name) == sle_with_serial.get(d.name)):
				vmraid.db.set_value("Item", d.name, "has_serial_no", 1)


def get_sle_count():
	sle_count = {}
	for d in vmraid.db.sql("""select item_code, count(name) as cnt from `tabStock Ledger Entry` group by item_code""", as_dict=1):
...
def get_sle_with_batch():
	sle_with_batch = {}
	for d in vmraid.db.sql("""select item_code, count(name) as cnt from `tabStock Ledger Entry`
...
def get_items_with_batch():
	return vmraid.db.sql_list("select item from tabBatch")

def get_items_with_serial():
	return vmraid.db.sql_list("select item_code from `tabSerial No`")
