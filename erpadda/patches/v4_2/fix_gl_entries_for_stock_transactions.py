...
def execute():
	...
	warehouse_account = vmraid.db.sql("""select name, master_name from tabAccount
	...
	if warehouse_account:
		...
		for voucher_type, voucher_no in stock_vouchers:
			stock_bal = vmraid.db.sql("""select sum(stock_value_difference) from `tabStock Ledger Entry`
			...
			account_bal = vmraid.db.sql("""select ifnull(sum(ifnull(debit, 0) - ifnull(credit, 0)), 0)
				from `tabGL Entry`
