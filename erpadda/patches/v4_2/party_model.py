...
def create_receivable_payable_account():
	receivable_payable_accounts = vmraid._dict()

	def _create_account(args):
		if args["parent_account"] and vmraid.db.exists("Account", args["parent_account"]):
			account_id = vmraid.db.get_value("Account", 
			...
			vmraid.db.set_value("Company", args["company"], ("default_receivable_account"
...
def get_parent_account(company, master_type):
	...
	if "receivables_group" in vmraid.db.get_table_columns("Company"):
		parent_account = vmraid.get_cached_value('Company',  company, 
			"receivables_group" if master_type=="Customer" else "payables_group")
	if not parent_account:
		parent_account = vmraid.db.get_value("Account", {"company": company,
			"account_name": "Accounts Receivable" if master_type=="Customer" else "Accounts Payable"})

	if not parent_account:
		parent_account = vmraid.db.sql_list("""select parent_account from tabAccount
...
def set_party_in_jv_and_gl_entry(receivable_payable_accounts):
	accounts = vmraid.db.sql("""select name, master_type, master_name, company from `tabAccount`
		where ifnull(master_type, '') in ('Customer', 'Supplier') and ifnull(master_name, '') != ''""", as_dict=1)

	account_map = vmraid._dict()
	for d in accounts:
	...
	for dt in ["Journal Entry Account", "GL Entry"]:
		records = vmraid.db.sql("""select name, account from `tab%s` 
		...
		for i, d in enumerate(records):
			...
			vmraid.db.sql("update `tab{0}` set account=%s, party_type=%s, party=%s where name=%s".format(dt),
...
def delete_individual_party_account():
	vmraid.db.sql("""delete from `tabAccount` 
	...
	accounts_not_deleted = vmraid.db.sql_list("""select tabAccount.name from `tabAccount` 
		where ifnull(master_type, '') in ('Customer', 'Supplier')
		and ifnull(master_name, '') != '' 
