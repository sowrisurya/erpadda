...
def execute():
	for doc in (
		...
		for order in order_list:
			...
			stock_doc_list = vmraid.db.sql("""select distinct parent from `tab{0} Item`
				where docstatus=1 and ifnull({1}, '')=%s"""
				.format(doc["stock_doctype"], doc["stock_doctype_ref_field"]), order.name)

			if stock_doc_list:
				for dn in stock_doc_list:
					vmraid.get_doc(doc["stock_doctype"], dn[0]).update_qty(update_modified=False)

			# Check against Invoice
			invoice_list = vmraid.db.sql("""select distinct parent from `tab{0} Item`
				where docstatus=1 and ifnull({1}, '')=%s"""
				.format(doc["invoice_doctype"], doc["invoice_ref_field"]), order.name)

			if invoice_list:
				for dn in invoice_list:
					vmraid.get_doc(doc["invoice_doctype"], dn[0]).update_qty(update_modified=False)

			vmraid.get_doc(doc["doctype"], order.name).set_status(update=True, update_modified=False)