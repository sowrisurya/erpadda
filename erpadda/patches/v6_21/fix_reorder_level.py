...
import vmraid
from erpadda.stock.doctype.item.item import DuplicateReorderRows

def execute():
	if vmraid.db.has_column("Item", "re_order_level"):
		for item in vmraid.db.sql("""select name, default_warehouse, re_order_level, re_order_qty
			from tabItem
