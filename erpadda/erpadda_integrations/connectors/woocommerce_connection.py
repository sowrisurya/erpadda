...
import vmraid, base64, hashlib, hmac, json
...
def verify_request():
	...
	if vmraid.request.data and \
		vmraid.get_request_header("X-Wc-Webhook-Signature") and \
		not sig == bytes(vmraid.get_request_header("X-Wc-Webhook-Signature").encode()):
...
def link_customer_and_address(raw_billing_data, raw_shipping_data, customer_name):
	...
	if customer_exists:
		...
		for address_type in ("Billing", "Shipping",):
			try:
				address = vmraid.get_doc("Address", {"woocommerce_email": customer_woo_com_email, "address_type": address_type})
...
def set_items_in_sales_order(new_sales_order, woocommerce_settings, order, sys_lang):
	...
	if not vmraid.db.exists("Warehouse", default_warehouse) \
