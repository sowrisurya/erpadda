...
import json
from vmraid.utils import cstr, cint, nowdate, getdate, flt, get_request_session, get_datetime
from erpadda.erpadda_integrations.utils import validate_webhooks_request
from erpadda.selling.doctype.sales_order.sales_order import make_delivery_note, make_sales_invoice
from erpadda.erpadda_integrations.doctype.shopify_settings.sync_product import sync_item_from_shopify
from erpadda.erpadda_integrations.doctype.shopify_settings.sync_customer import create_customer
from erpadda.erpadda_integrations.doctype.shopify_log.shopify_log import make_shopify_log, dump_request_data
from erpadda.erpadda_integrations.doctype.shopify_settings.shopify_settings import get_shopify_url, get_header
...
def store_request_data(order=None, event=None):
	if vmraid.request:
		...
		event = vmraid.request.headers.get('X-Shopify-Topic')
...
def sync_sales_order(order, request_id=None, old_order_sync=False):
	...
	vmraid.flags.request_id = request_id

	if not vmraid.db.get_value("Sales Order", filters={"shopify_order_id": cstr(order['id'])}):
		try:
...
def get_sales_order(shopify_order_id):
	sales_order = vmraid.db.get_value("Sales Order", filters={"shopify_order_id": shopify_order_id})
	if sales_order:
		so = vmraid.get_doc("Sales Order", sales_order)
		return so

def validate_customer(order, shopify_settings):
	customer_id = order.get("customer", {}).get("id")
	if customer_id:
		if not vmraid.db.get_value("Customer", {"shopify_customer_id": customer_id}, "name"):
			create_customer(order.get("customer"), shopify_settings)

def validate_item(order, shopify_settings):
	for item in order.get("line_items"):
		if item.get("product_id") and not vmraid.db.get_value("Item", {"shopify_product_id": item.get("product_id")}, "name"):
...
def create_sales_order(shopify_order, shopify_settings, company=None):
	product_not_exists = []
	customer = vmraid.db.get_value("Customer", {"shopify_customer_id": shopify_order.get("customer", {}).get("id")}, "name")
	so = vmraid.db.get_value("Sales Order", {"shopify_order_id": shopify_order.get("id")}, "name")
	...
	if not so:
		...
		so = vmraid.get_doc("Sales Order", so)

	vmraid.db.commit()
	return so

def create_sales_invoice(shopify_order, shopify_settings, so, old_order_sync=False):
	if not vmraid.db.get_value("Sales Invoice", {"shopify_order_id": shopify_order.get("id")}, "name")\
...
def create_delivery_note(shopify_order, shopify_settings, so):
	...
	for fulfillment in shopify_order.get("fulfillments"):
		if not vmraid.db.get_value("Delivery Note", {"shopify_fulfillment_id": fulfillment.get("id")}, "name")\
			and so.docstatus==1:
...
def get_item_code(shopify_item):
	item_code = vmraid.db.get_value("Item", {"shopify_variant_id": shopify_item.get("variant_id")}, "item_code")
	if not item_code:
		item_code = vmraid.db.get_value("Item", {"shopify_product_id": shopify_item.get("product_id")}, "item_code")
	if not item_code:
		item_code = vmraid.db.get_value("Item", {"item_name": shopify_item.get("title")}, "item_code")
...
def get_tax_account_head(tax):
	...
	tax_account =  vmraid.db.get_value("Shopify Tax Account", \
		{"parent": "Shopify Settings", "shopify_tax": tax_title}, "tax_account")

	if not tax_account:
		vmraid.throw(_("Tax Account not specified for Shopify Tax {0}").format(tax.get("title")))
