...
def make_shopify_log(status="Queued", exception=None, rollback=False):
	...
	if not vmraid.flags.request_id:
		make_new = True
	...
	if make_new:
		log = vmraid.get_doc({"doctype":"Shopify Log"}).insert(ignore_permissions=True)
	else:
		log = log = vmraid.get_doc("Shopify Log", vmraid.flags.request_id)

	log.message = get_message(exception)
	log.traceback = vmraid.get_traceback()
	log.status = status
...
def dump_request_data(data, event="create/order"):
	...
	vmraid.db.commit()
	vmraid.enqueue(method=event_mapper[event], queue='short', timeout=300, is_async=True,
		**{"order": data, "request_id": log.name})

@vmraid.whitelist()
def resync(method, name, request_data):
	vmraid.db.set_value("Shopify Log", name, "status", "Queued", update_modified=False)
	vmraid.enqueue(method=method, queue='short', timeout=300, is_async=True,
