...
class QuickBooksMigrator(Document):
	...
	def on_update(self):
		if self.company:
			...
			company_warehouses = vmraid.get_all('Warehouse', filters={"company": self.company, "is_group": 0})
	...
	def _make_root_accounts(self):
		...
		for root in roots:
			try:
				if not vmraid.db.exists({"doctype": "Account", "name": encode_company_abbr("{} - QB".format(root), self.company), "company": self.company}):
					vmraid.get_doc({
						"doctype": "Account",
	...
	def _create_fiscal_years(self):
		try:
			...
			oldest_fiscal_year = vmraid.get_all("Fiscal Year",
	...
	def _save_account(self, account):
		...
		try:
			if not vmraid.db.exists({"doctype": "Account", "quickbooks_id": account["Id"], "company": self.company}):
	...
	def _save_tax_rate(self, tax_rate):
		try:
			if not vmraid.db.exists({"doctype": "Account", "quickbooks_id": "TaxRate - {}".format(tax_rate["Id"]), "company": self.company}):
				vmraid.get_doc({
					"doctype": "Account",
	...
	def _save_customer(self, customer):
		try:
			if not vmraid.db.exists({"doctype": "Customer", "quickbooks_id": customer["Id"], "company": self.company}):
				try:
					receivable_account = vmraid.get_all("Account", filters={
	...
	def _save_item(self, item):
		try:
			if not vmraid.db.exists({"doctype": "Item", "quickbooks_id": item["Id"], "company": self.company}):
	...
	def _save_vendor(self, vendor):
		try:
			if not vmraid.db.exists({"doctype": "Supplier", "quickbooks_id": vendor["Id"], "company": self.company}):
	...
	def _save_sales_invoice(self, invoice, quickbooks_id, is_return=False, is_pos=False):
		try:
			if not vmraid.db.exists({"doctype": "Sales Invoice", "quickbooks_id": quickbooks_id, "company": self.company}):
				invoice_dict = {
					...
					"customer": vmraid.get_all("Customer",
						filters={
		...
		try:
			...
			for line in self.general_ledger["Invoice"][invoice["Id"]]["lines"]:
				...
				if vmraid.db.get_value("Account", line["account"], "account_type") == "Receivable":
					...
					account_line["party"] = vmraid.get_all("Customer",
	...
	def __save_journal_entry(self, quickbooks_id, accounts, posting_date):
		try:
			if not vmraid.db.exists({"doctype": "Journal Entry", "quickbooks_id": quickbooks_id, "company": self.company}):
				je = vmraid.get_doc({
	...
	def __save_purchase_invoice(self, invoice, quickbooks_id, is_return=False):
		try:
			if not vmraid.db.exists({"doctype": "Purchase Invoice", "quickbooks_id": quickbooks_id, "company": self.company}):
				...
				invoice_dict = {
					...
					"supplier": vmraid.get_all("Supplier",
						filters={
		...
		try:
			...
			for line in payment["Line"]:
				...
				if linked_transaction["TxnType"] == "Invoice":
					...
					if vmraid.db.exists({"doctype": "Sales Invoice", "quickbooks_id": si_quickbooks_id, "company": self.company}):
						sales_invoice = vmraid.get_all("Sales Invoice",
							filters={
					...
					if vmraid.db.exists({"doctype": "Journal Entry", "quickbooks_id": si_quickbooks_id, "company": self.company}):
						journal_entry = vmraid.get_doc("Journal Entry",
							{
								"quickbooks_id": si_quickbooks_id,
								"company": self.company,
							}
		...
		try:
			...
			for line in bill_payment["Line"]:
				...
				if linked_transaction["TxnType"] == "Bill":
					...
					if vmraid.db.exists({"doctype": "Purchase Invoice", "quickbooks_id": pi_quickbooks_id, "company": self.company}):
						purchase_invoice = vmraid.get_all("Purchase Invoice",
							filters={
	...
	def _get_account_name_by_id(self, quickbooks_id):
		return vmraid.get_all("Account", filters={"quickbooks_id": quickbooks_id, "company": self.company})[0]["name"]
	...
	def _get_unique_account_name(self, quickbooks_name, number=0):
		...
		if vmraid.db.exists({"doctype": "Account", "name": company_encoded_account_name, "company": self.company}):
	...
	def _log_error(self, execption, data=""):
		vmraid.log_error(title="QuickBooks Migration Error",
			message="\n".join([
				"Data",
				json.dumps(data,
					sort_keys=True,
