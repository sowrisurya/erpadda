...
from erpadda.erpadda_integrations.utils import get_webhook_address
from erpadda.erpadda_integrations.doctype.shopify_log.shopify_log import make_shopify_log
...
class ShopifySettings(Document):
	...
	def validate_access_credentials(self):
		if not (self.get_password(raise_exception=False) and self.api_key and self.shopify_url):
			vmraid.msgprint(_("Missing value for Password, API Key or Shopify URL"), raise_exception=vmraid.ValidationError)
	...
	def unregister_webhooks(self):
		...
		for d in self.webhooks:
			...
			try:
				...
				vmraid.log_error(message=e, title='Shopify Webhooks Issue')
...
def get_series():
	return {
		"sales_order_series" : vmraid.get_meta("Sales Order").get_options("naming_series") or "SO-Shopify-",
		"sales_invoice_series" : vmraid.get_meta("Sales Invoice").get_options("naming_series")  or "SI-Shopify-",
		"delivery_note_series" : vmraid.get_meta("Delivery Note").get_options("naming_series")  or "DN-Shopify-"
	}
