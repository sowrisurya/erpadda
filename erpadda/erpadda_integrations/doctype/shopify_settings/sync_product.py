...
from vmraid.utils import cstr, cint, get_request_session
...
def create_attribute(shopify_item):
	...
	for attr in shopify_item.get('options'):
		if not vmraid.db.get_value("Item Attribute", attr.get("name"), "name"):
			vmraid.get_doc({
			...
			item_attr = vmraid.get_doc("Item Attribute", attr.get("name"))
...
def create_item(shopify_item, warehouse, has_variant=0, attributes=None,variant_of=None):
	...
	if not is_item_exists(item_dict, attributes, variant_of=variant_of):
		...
		if not item_details:
			new_item = vmraid.get_doc(item_dict)
...
def create_item_variants(shopify_item, warehouse, attributes, shopify_variants_attr_list):
	template_item = vmraid.db.get_value("Item", filters={"shopify_product_id": shopify_item.get("id")},
...
def get_attribute_value(variant_attr_val, attribute):
	attribute_value = vmraid.db.sql("""select attribute_value from `tabItem Attribute Value`
...
def get_item_group(product_type=None):
	import vmraid.utils.nestedset
	parent_item_group = vmraid.utils.nestedset.get_root_of("Item Group")

	if product_type:
		if not vmraid.db.get_value("Item Group", product_type, "name"):
			item_group = vmraid.get_doc({
				"doctype": "Item Group",
...
def add_to_price_list(item, name):
	shopify_settings = vmraid.db.get_value("Shopify Settings", None, ["price_list", "update_price_in_erpadda_price_list"], as_dict=1)
	if not shopify_settings.update_price_in_erpadda_price_list:
		return

	item_price_name = vmraid.db.get_value("Item Price",
	...
	if not item_price_name:
		...
		item_rate = vmraid.get_doc("Item Price", item_price_name)
...
def get_supplier(shopify_item):
	if shopify_item.get("vendor"):
		supplier = vmraid.db.sql("""select name from tabSupplier
...
def get_supplier_group():
	supplier_group = vmraid.db.get_value("Supplier Group", _("Shopify Supplier"))
	if not supplier_group:
		supplier_group = vmraid.get_doc({
...
def get_item_details(shopify_item):
	...
	item_details = vmraid.db.get_value("Item", {"shopify_product_id": shopify_item.get("id")},
	...
	if item_details:
		...
		item_details = vmraid.db.get_value("Item", {"shopify_variant_id": shopify_item.get("id")},
			["name", "stock_uom", "item_name"], as_dict=1)
		return item_details
...
def is_item_exists(shopify_item, attributes=None, variant_of=None):
	if variant_of:
		...
		name = vmraid.db.get_value("Item", {"item_name": shopify_item.get("item_name")})

	if name:
		item = vmraid.get_doc("Item", name)
		...
		if item.shopify_product_id and attributes and attributes[0].get("attribute_value"):
			if not variant_of:
				variant_of = vmraid.db.get_value("Item",
			...
			parent = vmraid.db.sql(""" select * from tabItem it where
			...
			if parent:
				variant = vmraid.get_doc("Item", parent[0][0])
