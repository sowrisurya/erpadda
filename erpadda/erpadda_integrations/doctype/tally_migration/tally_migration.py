...
from erpadda.accounts.doctype.chart_of_accounts_importer.chart_of_accounts_importer import unset_existing_data

from vmraid import _
from vmraid.custom.doctype.custom_field.custom_field import create_custom_field
from vmraid.model.document import Document
from vmraid.model.naming import getseries, revert_series_if_last
...
class TallyMigration(Document):
	...
	def set_account_defaults(self):
		self.default_cost_center, self.default_round_off_account = vmraid.db.get_value("Company", self.erpadda_company, ["cost_center", "round_off_account"])
		self.default_warehouse = vmraid.db.get_value("Stock Settings", "Stock Settings", "default_warehouse")
	...
	def publish(self, title, message, count, total):
		vmraid.publish_realtime("tally_migration_progress_update", {"title": title, "message": message, "count": count, "total": total})
	...
	def _process_day_book_data(self):
		...
		def voucher_to_journal_entry(voucher):
			...
			for entry in ledger_entries:
				account = {"account": encode_company_abbr(entry.LEDGERNAME.string.strip(), self.erpadda_company), "cost_center": self.default_cost_center}
		...
		def get_voucher_items(voucher, doctype):
			...
			for entry in inventory_entries:
				...
				items.append({
					...
					account_field: encode_company_abbr(entry.find_all("ACCOUNTINGALLOCATIONS.LIST")[0].LEDGERNAME.string.strip(), self.erpadda_company),
				})
			return items
	...
	def _import_day_book_data(self):
		def create_fiscal_years(vouchers):
			...
			oldest_year = vmraid.get_all("Fiscal Year", fields=["year_start_date", "year_end_date"], order_by="year_start_date")[0]
		...
		try:
			vmraid.db.set_value("Account", encode_company_abbr(self.tally_creditors_account, self.erpadda_company), "account_type", "Payable")
			vmraid.db.set_value("Account", encode_company_abbr(self.tally_debtors_account, self.erpadda_company), "account_type", "Receivable")
			vmraid.db.set_value("Company", self.erpadda_company, "round_off_account", self.default_round_off_account)
			...
			for index in range(0, total, VOUCHER_CHUNK_SIZE):
				if index + VOUCHER_CHUNK_SIZE >= total:
					is_last = True
				vmraid.enqueue_doc(self.doctype, self.name, "_import_vouchers", queue="long", timeout=3600, start=index+1, total=total, is_last=is_last)
