...
from vmraid.utils import get_url, call_hook_method, flt, cint
...
class GoCardlessSettings(Document):
	...
	def on_payment_request_submission(self, data):
		if data.reference_doctype != "Fees":
			customer_data = vmraid.db.get_value(data.reference_doctype, data.reference_name, ["company", "customer_name"], as_dict=1)
	...
	def check_mandate_validity(self, data):
		...
		if vmraid.db.exists("GoCardless Mandate", dict(customer=data.get('payer_name'), disabled=0)):
			registered_mandate = vmraid.db.get_value("GoCardless Mandate", dict(customer=data.get('payer_name'), disabled=0), 'mandate')
			self.initialize_client()
	...
	def create_payment_request(self, data):
		...
		try:
			...
			return{
				"redirect_to": vmraid.redirect_to_message(_('Server Error'), _("There seems to be an issue with the server's GoCardless configuration. Don't worry, in case of failure, the amount will get refunded to your account.")),
				"status": 401
			}
	...
	def create_charge_on_gocardless(self):
		...
		reference_doc = vmraid.get_doc(self.data.get('reference_doctype'), self.data.get('reference_docname'))
		self.initialize_client()
		...
		try:
			...
			if payment.status=="pending_submission" or payment.status=="pending_customer_approval" or payment.status=="submitted":
				...
				vmraid.log_error(_("Payment Cancelled. Please check your GoCardless Account for more details"), "GoCardless Payment Error")
				self.integration_request.db_set('error', payment.status, update_modified=False)
			else:
				self.integration_request.db_set('status', 'Failed', update_modified=False)
				vmraid.log_error(_("Payment Failed. Please check your GoCardless Account for more details"), "GoCardless Payment Error")
				self.integration_request.db_set('error', payment.status, update_modified=False)
		...
		if self.flags.status_changed_to == "Completed":
			...
			if 'reference_doctype' in self.data and 'reference_docname' in self.data:
				...
				try:
					custom_redirect_to = vmraid.get_doc(self.data.get('reference_doctype'),
