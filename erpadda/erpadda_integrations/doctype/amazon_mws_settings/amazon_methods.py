...
import vmraid, time, dateutil, math, csv
from six import StringIO
import erpadda.erpadda_integrations.doctype.amazon_mws_settings.amazon_mws_api as mws
...
def get_products_details():
	...
	mws_settings = vmraid.get_doc("Amazon MWS Settings")
	...
	for marketplace in market_place_list:
		...
		if report_id:
			...
			#Get ASIN Codes
			string_io = StringIO(vmraid.safe_decode(listings_response.original))
...
def call_mws_method(mws_method, *args, **kwargs):
	...
	for x in range(0, max_retries):
		try:
			...
			vmraid.log_error(message=e, title=f'Method "{mws_method.__name__}" failed')
			time.sleep(delay)
			continue
	...
	vmraid.throw(_("Sync has been temporarily disabled because maximum retries have been exceeded"))

def create_item_code(amazon_item_json, sku):
	if vmraid.db.get_value("Item", sku):
		return
	...
	item_group = vmraid.db.get_value("Item Group",filters={"item_group_name": temp_item_group})

	if not item_group:
		igroup = vmraid.new_doc("Item Group")
...
def create_manufacturer(amazon_item_json):
	...
	existing_manufacturer = vmraid.db.get_value("Manufacturer",
		filters={"short_name":amazon_item_json.Product.AttributeSets.ItemAttributes.Manufacturer})

	if not existing_manufacturer:
		manufacturer = vmraid.new_doc("Manufacturer")
...
def create_item_price(amazon_item_json, item_code):
	...
	item_price.price_list = vmraid.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "price_list")
...
def get_orders(after_date):
	try:
		...
		vmraid.log_error(title="get_orders", message=e)
...
def create_sales_order(order_json,after_date):
	...
	so = vmraid.db.get_value("Sales Order",
			filters={"amazon_order_id": market_place_order_id},
			fieldname="name")

	taxes_and_charges = vmraid.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "taxes_charges")
	...
	if not so:
		...
		so = vmraid.get_doc({
				...
				"items": items,
				"company": vmraid.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "company")
			})
		...
		try:
			...
			import traceback
			vmraid.log_error(message=traceback.format_exc(), title="Create Sales Order")
...
def create_customer(order_json):
	...
	existing_customer_name = vmraid.db.get_value("Customer",
	...
	if existing_customer_name:
		...
		existing_contacts = vmraid.get_list("Contact", filters)
		...
		mws_customer_settings = vmraid.get_doc("Amazon MWS Settings")
		new_customer = vmraid.new_doc("Customer")
...
def create_address(amazon_order_item_json, customer_name):
	...
	if not("ShippingAddress" in amazon_order_item_json):
		...
		make_address = vmraid.new_doc("Address")
...
def get_order_items(market_place_order_id):
	...
	warehouse = vmraid.db.get_value("Amazon MWS Settings", "Amazon MWS Settings", "warehouse")
...
def get_item_code(order_item):
	...
	item_code = vmraid.db.get_value("Item", {"item_code": sku}, "item_code")
	if item_code:
		return item_code
...
def get_account(name):
	existing_account = vmraid.db.get_value("Account", {"account_name": "Amazon {0}".format(name)})
	account_name = existing_account
	mws_settings = vmraid.get_doc("Amazon MWS Settings")

	if not existing_account:
		try:
			new_account = vmraid.new_doc("Account")
			...
			vmraid.log_error(message=e, title="Create Account")
