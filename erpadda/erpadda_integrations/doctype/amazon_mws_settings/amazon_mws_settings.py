...
from erpadda.erpadda_integrations.doctype.amazon_mws_settings.amazon_methods import get_orders
...
class AmazonMWSSettings(Document):
	...
	def get_products_details(self):
		if self.enable_amazon == 1:
			vmraid.enqueue('erpadda.erpadda_integrations.doctype.amazon_mws_settings.amazon_methods.get_products_details')

	@vmraid.whitelist()
	def get_order_details(self):
		if self.enable_amazon == 1:
			after_date = dateutil.parser.parse(self.after_date).strftime("%Y-%m-%d")
			vmraid.enqueue('erpadda.erpadda_integrations.doctype.amazon_mws_settings.amazon_methods.get_orders', after_date=after_date)
