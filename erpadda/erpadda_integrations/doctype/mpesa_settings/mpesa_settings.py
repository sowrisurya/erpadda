...
from vmraid.utils import call_hook_method, fmt_money
...
from erpadda.erpadda_integrations.doctype.mpesa_settings.mpesa_custom_fields import create_custom_pos_fields
...
class MpesaSettings(Document):
	...
	def validate_transaction_currency(self, currency):
		if currency not in self.supported_currencies:
			vmraid.throw(_("Please select another payment method. Mpesa does not support transactions in currency '{0}'").format(currency))
	...
	def request_for_payment(self, **kwargs):
		...
		for i, amount in enumerate(request_amounts):
			...
			if vmraid.flags.in_test:
				from erpadda.erpadda_integrations.doctype.mpesa_settings.test_mpesa_settings import get_payment_request_response_payload
	...
	def split_request_amount_according_to_transaction_limit(self, args):
		...
		if request_amount > self.transaction_limit:
			...
			request_amounts = []
			requests_to_be_made = vmraid.utils.ceil(request_amount / self.transaction_limit) # 480/150 = ceil(3.2) = 4
	...
	def get_account_balance_info(self):
		...
		if vmraid.flags.in_test:
			from erpadda.erpadda_integrations.doctype.mpesa_settings.test_mpesa_settings import get_test_account_balance_response
...
def generate_stk_push(**kwargs):
	...
	args = vmraid._dict(kwargs)
	try:
		callback_url = get_request_site_address(True) + "/api/method/erpadda.erpadda_integrations.doctype.mpesa_settings.mpesa_settings.verify_transaction"
		...
		vmraid.throw(_("Issue detected with Mpesa configuration, check the error logs for more details"), title=_("Mpesa Express Error"))
	...
	if transaction_response['ResultCode'] == 0:
		if integration_request.reference_doctype and integration_request.reference_docname:
			try:
				...
				pr = vmraid.get_doc(integration_request.reference_doctype, integration_request.reference_docname)
...
def get_completed_integration_requests_info(reference_doctype, reference_docname, checkout_id):
	output_of_other_completed_requests = vmraid.get_all("Integration Request", filters={
		'name': ['!=', checkout_id],
...
def get_account_balance(request_payload):
	...
	try:
		...
		callback_url = get_request_site_address(True) + "/api/method/erpadda.erpadda_integrations.doctype.mpesa_settings.mpesa_settings.process_balance_info"
...
def process_balance_info(**kwargs):
	...
	if account_balance_response["ResultCode"] == 0:
		try:
			...
			vmraid.publish_realtime("refresh_mpesa_dashboard", doctype="Mpesa Settings",
				docname=transaction_data.reference_docname, user=transaction_data.owner)
		except Exception:
			request.handle_failure(account_balance_response)
			vmraid.log_error(title=_("Mpesa Account Balance Processing Error"), message=account_balance_response)
	else:
		request.handle_failure(account_balance_response)
