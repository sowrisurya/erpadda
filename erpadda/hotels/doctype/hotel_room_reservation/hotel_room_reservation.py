...
from vmraid.utils import date_diff, add_days, flt

class HotelRoomUnavailableError(vmraid.ValidationError): pass
class HotelRoomPricingNotSetError(vmraid.ValidationError): pass
	...
	def validate_availability(self):
		for i in range(date_diff(self.to_date, self.from_date)):
			...
			for d in self.items:
				...
				room_type = vmraid.db.get_value("Hotel Room Package",
				...
				if total_rooms < rooms_booked:
					vmraid.throw(_("Hotel Rooms of type {0} are unavailable on {1}").format(d.item,
						vmraid.format(day, dict(fieldtype="Date"))), exc=HotelRoomUnavailableError)

				self.rooms_booked[d.item] += rooms_booked

	def get_total_rooms(self, item):
		if not item in self.total_rooms:
			self.total_rooms[item] = vmraid.db.sql("""
				select count(*)
				from
				...
				if day_rate:
					...
					vmraid.throw(
						_("Please set Hotel Room Rate on {}").format(
							vmraid.format(day, dict(fieldtype="Date"))), exc=HotelRoomPricingNotSetError)
			d.rate = net_rate
...
def get_room_rate(hotel_room_reservation):
	...
	doc = vmraid.get_doc(json.loads(hotel_room_reservation))
	doc.set_rates()
	return doc.as_dict()
...
def get_rooms_booked(room_type, day, exclude_reservation=None):
	...
	if exclude_reservation:
		exclude_condition = 'and reservation.name != {0}'.format(vmraid.db.escape(exclude_reservation))
