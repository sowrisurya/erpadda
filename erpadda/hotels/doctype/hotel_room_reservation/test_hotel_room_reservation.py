...
import vmraid
import unittest
from erpadda.hotels.doctype.hotel_room_reservation.hotel_room_reservation import HotelRoomPricingNotSetError, HotelRoomUnavailableError
test_dependencies = ["Hotel Room Package", "Hotel Room Pricing", "Hotel Room"]

class TestHotelRoomReservation(unittest.TestCase):
	def setUp(self):
		vmraid.db.sql("delete from `tabHotel Room Reservation`")
		vmraid.db.sql("delete from `tabHotel Room Reservation Item`")
