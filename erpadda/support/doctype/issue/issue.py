...
from vmraid import utils
from vmraid.model.document import Document
from vmraid.utils import cint, now_datetime, getdate, get_weekdays, add_to_date, get_time, get_datetime, time_diff_in_seconds
from datetime import datetime, timedelta
from vmraid.model.mapper import get_mapped_doc
from vmraid.utils.user import is_website_user
from erpadda.support.doctype.service_level_agreement.service_level_agreement import get_active_service_level_agreement_for
...
class Issue(Document):
	...
	def set_lead_contact(self, email_id):
		...
		if email_id:
			...
			if not self.company:
				self.company = vmraid.db.get_value("Lead", self.lead, "company") or \
					vmraid.db.get_default("Company")

	def update_status(self):
		status = vmraid.db.get_value("Issue", self.name, "status")
		if self.status != "Open" and status == "Open" and not self.first_responded_on:
			self.first_responded_on = vmraid.flags.current_time or now_datetime()

		if self.status in ["Closed", "Resolved"] and status not in ["Resolved", "Closed"]:
			self.resolution_date = vmraid.flags.current_time or now_datetime()
			if vmraid.db.get_value("Issue", self.name, "agreement_status") == "Ongoing":
	...
	def handle_hold_time(self, status):
		if self.service_level_agreement:
			...
			pause_sla_on = vmraid.db.get_all("Pause SLA On Status", fields=["status"],
			...
			if hold_statuses:
				if self.status in hold_statuses and status not in hold_statuses:
					update_values['on_hold_since'] = vmraid.flags.current_time or now_datetime()
				...
				if self.status not in hold_statuses and status in hold_statuses:
					...
					now_time = vmraid.flags.current_time or now_datetime()
					last_hold_time = 0
	...
	def update_agreement_status(self):
		if self.service_level_agreement and self.agreement_status == "Ongoing":
			if cint(vmraid.db.get_value("Issue", self.name, "response_by_variance")) < 0 or \
				cint(vmraid.db.get_value("Issue", self.name, "resolution_by_variance")) < 0:
	...
	def split_issue(self, subject, communication_id):
		...
		vmraid.get_doc({
			...
			"content": " - Split the Issue from <a href='/app/Form/Issue/{0}'>{1}</a>".format(self.name, vmraid.bold(self.name)),
		}).insert(ignore_permissions=True)

		return replicated_issue.name

	def before_insert(self):
		if vmraid.db.get_single_value("Support Settings", "track_service_level_agreement"):
	...
	def set_response_and_resolution_time(self, priority=None, service_level_agreement=None):
		...
		if not service_level_agreement:
			if vmraid.db.get_value("Issue", self.name, "service_level_agreement"):
				vmraid.throw(_("Couldn't Set Service Level Agreement {0}.").format(self.service_level_agreement))
			return

		if (service_level_agreement.customer and self.customer) and not (service_level_agreement.customer == self.customer):
			vmraid.throw(_("This Service Level Agreement is specific to Customer {0}").format(service_level_agreement.customer))
	...
	def change_service_level_agreement_and_priority(self):
		if self.service_level_agreement and vmraid.db.exists("Issue", self.name) and \
			vmraid.db.get_single_value("Support Settings", "track_service_level_agreement"):
			...
			if not self.service_level_agreement == vmraid.db.get_value("Issue", self.name, "service_level_agreement"):
				self.set_response_and_resolution_time(priority=self.priority, service_level_agreement=self.service_level_agreement)
				vmraid.msgprint(_("Service Level Agreement has been changed to {0}.").format(self.service_level_agreement))
...
def get_priority(issue):
	service_level_agreement = vmraid.get_doc("Service Level Agreement", issue.service_level_agreement)
...
def set_service_level_agreement_variance(issue=None):
	current_time = vmraid.flags.current_time or now_datetime()

	filters = {"status": "Open", "agreement_status": "Ongoing"}
	if issue:
		filters = {"name": issue}

	for issue in vmraid.get_list("Issue", filters=filters):
		doc = vmraid.get_doc("Issue", issue.name)
		...
		if not doc.first_responded_on: # first_responded_on set when first reply is sent to customer
			...
			vmraid.db.set_value(dt="Issue", dn=doc.name, field="response_by_variance", val=variance, update_modified=False)
			if variance < 0:
				vmraid.db.set_value(dt="Issue", dn=doc.name, field="agreement_status", val="Failed", update_modified=False)
		...
		if not doc.resolution_date: # resolution_date set when issue has been closed
			...
			vmraid.db.set_value(dt="Issue", dn=doc.name, field="resolution_by_variance", val=variance, update_modified=False)
			if variance < 0:
				vmraid.db.set_value(dt="Issue", dn=doc.name, field="agreement_status", val="Failed", update_modified=False)
...
def set_status(name, status):
	st = vmraid.get_doc("Issue", name)
	st.status = status
	st.save()

def auto_close_tickets():
	"""Auto-close replied support tickets after 7 days"""
	auto_close_after_days = vmraid.db.get_value("Support Settings", "Support Settings", "close_issue_after_days") or 7

	issues = vmraid.db.sql(""" select name from tabIssue where status='Replied' and
