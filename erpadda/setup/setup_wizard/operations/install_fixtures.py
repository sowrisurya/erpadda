...
def install(country=None):
	...
	response = vmraid.read_file(os.path.join(base_path, "leave_application/leave_application_email_template.html"))

	records += [{'doctype': 'Email Template', 'name': _("Leave Approval Notification"), 'response': response,\
		'subject': _("Leave Approval Notification"), 'owner': vmraid.session.user}]

	records += [{'doctype': 'Email Template', 'name': _("Leave Status Notification"), 'response': response,\
		'subject': _("Leave Status Notification"), 'owner': vmraid.session.user}]

	base_path = vmraid.get_app_path("erpadda", "stock", "doctype")
	response = vmraid.read_file(os.path.join(base_path, "delivery_trip/dispatch_notification_template.html"))

	records += [{'doctype': 'Email Template', 'name': _("Dispatch Notification"), 'response': response,\
		'subject': _("Your order is out for delivery!"), 'owner': vmraid.session.user}]
...
def add_uom_data():
	# add UOMs
	uoms = json.loads(open(vmraid.get_app_path("erpadda", "setup", "setup_wizard", "data", "uom_data.json")).read())
	for d in uoms:
	...
	uom_conversions = json.loads(open(vmraid.get_app_path("erpadda", "setup", "setup_wizard", "data", "uom_conversion_data.json")).read())
	for d in uom_conversions:
		...
		if not vmraid.db.exists("UOM Conversion Factor", {"from_uom": _(d.get("from_uom")), "to_uom": _(d.get("to_uom"))}):
...
def update_stock_settings():
	...
	stock_settings.default_warehouse = vmraid.db.get_value('Warehouse', {'warehouse_name': _('Stores')})
...
def create_bank_account(args):
	...
	company_name = args.company_name
	bank_account_group =  vmraid.db.get_value("Account",
	...
	if bank_account_group:
		...
		try:
			...
			vmraid.db.set_value("Company", args.company_name, "default_bank_account", bank_account.name, update_modified=False)

		except RootNotEditable:
			vmraid.throw(_("Bank account cannot be named as {0}").format(args.bank_account))
		except vmraid.DuplicateEntryError:
			# bank account same as a CoA entry
			pass

def update_shopping_cart_settings(args):
	shopping_cart = vmraid.get_doc("Shopping Cart Settings")
