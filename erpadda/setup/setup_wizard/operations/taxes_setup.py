...
def make_taxes_and_charges_template(company_name, doctype, template):
	...
	return vmraid.get_doc(template).insert(ignore_permissions=True)
...
def get_or_create_account(company_name, account):
	...
	existing_accounts = vmraid.get_list('Account',
		filters={
	...
	return vmraid.get_doc(account).insert(ignore_permissions=True, ignore_mandatory=True)


def get_or_create_tax_group(company_name, root_type):
	# Look for a group account of type 'Tax'
	tax_group_name = vmraid.db.get_value('Account', {
		'is_group': 1,
		'root_type': root_type,
		'account_type': 'Tax',
		'company': company_name
	...
	root_account = vmraid.get_list('Account', {
		'is_group': 1,
		'root_type': root_type,
		'company': company_name,
