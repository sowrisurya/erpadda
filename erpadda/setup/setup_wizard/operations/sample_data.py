...
import vmraid
from vmraid.utils.make_random import add_random_children
import vmraid.utils
...
def make_sample_data(domains, make_dependent = False):
	...
	if make_dependent:
		items = vmraid.get_all("Item", {'is_sales_item': 1})
		customers = vmraid.get_all("Customer")
		...
		if items and warehouses:
			make_material_request(vmraid.get_all("Item"))
...
def make_material_request(items):
	for i in items:
		mr = vmraid.get_doc({
			...
			"schedule_date": vmraid.utils.add_days(vmraid.utils.nowdate(), 7),
			"items": [{
				"schedule_date": vmraid.utils.add_days(vmraid.utils.nowdate(), 7),
				"item_code": i.name,
				"qty": 10
...
def make_projects(domains):
	current_date = vmraid.utils.nowdate()
	project = vmraid.get_doc({
		"doctype": "Project",
		"project_name": "ERPAdda Implementation",
	})

	tasks = [
		{
			"title": "Explore ERPAdda",
	...
	if 'Education' in domains:
		tasks += [
			{
				"title": _("Setup your Institute in ERPAdda"),
				"start_date": current_date,
				"end_date": vmraid.utils.add_days(current_date, 1),
		...
		tasks += [
			...
			},
			{
				...
				"start_date": current_date,
				"end_date": vmraid.utils.add_days(current_date, 2),
				"file": "sales.md"
			},
			{
				"title": "Start Managing Purchases",
				"start_date": current_date,
				"end_date": vmraid.utils.add_days(current_date, 3),
				"file": "purchase.md"
			},
			{
				"title": "Import Data",
				"start_date": current_date,
				"end_date": vmraid.utils.add_days(current_date, 4),
				"file": "import_data.md"
			},
			{
				"title": "Go Live!",
				"start_date": current_date,
				"end_date": vmraid.utils.add_days(current_date, 5),
				"file": "go_live.md"
			}]

	for t in tasks:
		with open (os.path.join(os.path.dirname(__file__), "tasks", t['file'])) as f:
			t['description'] = vmraid.utils.md_to_html(f.read())
			del t['file']
...
def import_notification():
	...
	with open (os.path.join(os.path.dirname(__file__), "tasks/task_alert.json")) as f:
		notification = vmraid.get_doc(json.loads(f.read())[0])
		notification.insert()

	# trigger the first message!
	from vmraid.email.doctype.notification.notification import trigger_daily_alerts
	trigger_daily_alerts()

def test_sample():
	vmraid.db.sql('delete from `tabNotification`')
