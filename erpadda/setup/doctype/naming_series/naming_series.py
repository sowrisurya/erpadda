...
class NamingSeriesNotSetError(vmraid.ValidationError): pass

class NamingSeries(Document):
	@vmraid.whitelist()
	def get_transactions(self, arg=None):
		doctypes = list(set(vmraid.db.sql_list("""select parent
				from `tabDocField` df where fieldname='naming_series'""")
			+ vmraid.db.sql_list("""select dt from `tabCustom Field`
		...
		custom_prefixes = vmraid.get_all('DocType', fields=["autoname"],
	...
	def set_series_for(self, doctype, ol):
		...
		for prop in prop_dict:
			ps_exists = vmraid.db.get_value("Property Setter",
	...
	def check_duplicate(self):
		parent = list(set(
			vmraid.db.sql_list("""select dt.name
	...
	def get_current(self, arg=None):
		...
		if self.prefix:
			...
			self.current_value = vmraid.db.get_value("Series",
	...
	def update_series_start(self):
		if self.prefix:
			...
			vmraid.db.sql("update `tabSeries` set current = %s where name = %s",
...
def set_by_naming_series(doctype, fieldname, naming_series, hide_name_field=True):
	...
	if naming_series:
		...
		try:
			vmraid.db.sql("""update `tab{doctype}` set naming_series={s} where
		...
		if hide_name_field:
			...
			# set values for mandatory
			vmraid.db.sql("""update `tab{doctype}` set `{fieldname}`=`name` where
...
def get_default_naming_series(doctype):
	...
	if not out:
		vmraid.throw(_("Please set Naming Series for {0} via Setup > Settings > Naming Series").format(doctype),
