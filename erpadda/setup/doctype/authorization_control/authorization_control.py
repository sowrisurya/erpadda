...
import vmraid
from vmraid.utils import cstr, flt, has_common, comma_or
from vmraid import session, _
...
class AuthorizationControl(TransactionBase):
	def get_appr_user_role(self, det, doctype_name, total, based_on, condition, item, company):
		...
		if det:
			...
			app_dtl = vmraid.db.sql("""select approving_user, approving_role from `tabAuthorization Rule`
			...
			if not has_common(appr_roles, vmraid.get_roles()) and not has_common(appr_users, [session['user']]):
	...
	def validate_auth_rule(self, doctype_name, total, based_on, cond, company, item = ''):
		...
		if based_on == 'Itemwise Discount':
			...
			itemwise_exists = vmraid.db.sql("""select value from `tabAuthorization Rule`
		...
		if chk == 1:
			...
			appr = vmraid.db.sql("""select value from `tabAuthorization Rule`
				where transaction = %s and value <= %s and based_on = %s
	...
	def bifurcate_based_on_type(self, doctype_name, total, av_dis, based_on, doc_obj, val, company):
		...
		if val == 1: add_cond += " and system_user = {}".format(vmraid.db.escape(session['user']))
		elif val == 2: add_cond += " and system_role IN %s" % ("('"+"','".join(vmraid.get_roles())+"')")
	...
	def validate_approving_authority(self, doctype_name,company, total, doc_obj = ''):
		...
		based_on = [x[0] for x in vmraid.db.sql("""select distinct based_on from `tabAuthorization Rule`
		...
		based_on = [x[0] for x in vmraid.db.sql("""select based_on
			from `tabAuthorization Rule`
			where transaction = %s and system_role IN (%s) and based_on IN (%s)
			and (company = %s or ifnull(company,'')='')
			and docstatus != 2
		""" % ('%s', "'"+"','".join(vmraid.get_roles())+"'", "'"+"','".join(final_based_on)+"'", '%s'), (doctype_name, company))]
	...
	def get_value_based_rule(self,doctype_name,employee,total_claimed_amount,company):
		val_lst =[]
		val = vmraid.db.sql("""select value from `tabAuthorization Rule`
		...
		max_val = max(val_lst)
		rule = vmraid.db.sql("""select name, to_emp, to_designation, approving_role, approving_user
			from `tabAuthorization Rule`
				...
				for m in rule:
					if m['to_emp'] or m['to_designation']:
						if m['approving_user']:
							...
							user_lst = [z[0] for z in vmraid.db.sql("""select distinct t1.name
