...
class AuthorizationRule(Document):
	def check_duplicate_entry(self):
		exists = vmraid.db.sql("""select name, docstatus from `tabAuthorization Rule`
	...
	def validate_rule(self):
		if self.transaction != 'Appraisal':
			if not self.approving_role and not self.approving_user:
				...
				vmraid.throw(_("Cannot set authorization on basis of Discount for {0}").format(self.transaction))
