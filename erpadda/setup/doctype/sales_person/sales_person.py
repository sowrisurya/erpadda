...
class SalesPerson(NestedSet):
	...
	def load_dashboard_info(self):
		...
		allocated_amount = vmraid.db.sql("""
	...
	def validate_employee_id(self):
		if self.employee:
			...
			if sales_person and sales_person != self.name:
				vmraid.throw(_("Another Sales Person {0} exists with the same Employee id").format(sales_person))
...
def get_timeline_data(doctype, name):
	...
	out.update(dict(vmraid.db.sql('''select
	...
	sales_invoice = dict(vmraid.db.sql('''select
	...
	delivery_note = dict(vmraid.db.sql('''select
			unix_timestamp(dt.posting_date), count(st.parenttype)
		from
