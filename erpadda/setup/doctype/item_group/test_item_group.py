...
import unittest
import vmraid
from vmraid.utils.nestedset import NestedSetRecursionError, NestedSetMultipleRootsError, \
...
class TestItem(unittest.TestCase):
	def test_basic_tree(self, records=None):
		...
		for item_group in records:
			lft, rgt, parent_item_group = vmraid.db.get_value("Item Group", item_group["item_group_name"],
				["lft", "rgt", "parent_item_group"])

			if parent_item_group:
				parent_lft, parent_rgt = vmraid.db.get_value("Item Group", parent_item_group,
					["lft", "rgt"])
			else:
				# root
	...
	def get_no_of_children(self, item_group):
		def get_no_of_children(item_groups, no_of_children):
			...
			for ig in item_groups:
				children += vmraid.db.sql_list("""select name from `tabItem Group`
	...
	def print_tree(self):
		import json
		print(json.dumps(vmraid.db.sql("select name, lft, rgt from `tabItem Group` order by lft"), indent=1))
	...
	def test_delete_leaf(self):
		# for checking later
		parent_item_group = vmraid.db.get_value("Item Group", "_Test Item Group B - 3", "parent_item_group")
		rgt = vmraid.db.get_value("Item Group", parent_item_group, "rgt")

		ancestors = get_ancestors_of("Item Group", "_Test Item Group B - 3")
		ancestors = vmraid.db.sql("""select name, rgt from `tabItem Group`
	...
	def test_delete_group(self):
		...
		self.assertRaises(NestedSetChildExistsError, vmraid.delete_doc, "Item Group", "_Test Item Group B")
	...
	def test_merge_groups(self):
		...
		for name in vmraid.db.sql_list("""select name from `tabItem Group`
	...
	def test_merge_leaf_into_group(self):
		self.assertRaises(NestedSetInvalidMergeError, vmraid.rename_doc, "Item Group", "_Test Item Group B - 3",
			"_Test Item Group B", merge=True)

	def test_merge_group_into_leaf(self):
		self.assertRaises(NestedSetInvalidMergeError, vmraid.rename_doc, "Item Group", "_Test Item Group B",
