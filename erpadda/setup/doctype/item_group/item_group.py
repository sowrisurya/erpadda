...
from vmraid.utils import nowdate, cint, cstr
...
from erpadda.shopping_cart.product_info import set_product_info_for_website
...
class ItemGroup(NestedSet, WebsiteGenerator):
	...
	def validate_name_with_item(self):
		if vmraid.db.exists("Item", self.name):
			vmraid.throw(vmraid._("An item exists with same name ({0}), please change the item group name or rename the item").format(self.name), vmraid.NameError)

	def get_context(self, context):
		context.show_search=True
		context.page_length = cint(vmraid.db.get_single_value('Products Settings', 'products_per_page')) or 6
...
def get_product_list_for_group(product_group=None, start=0, limit=10, search=None):
	if product_group:
		item_group = vmraid.get_cached_doc('Item Group', product_group)
	...
	child_groups = ", ".join([vmraid.db.escape(i[0]) for i in get_child_groups(product_group)])
	...
	data = vmraid.db.sql(query, {"product_group": product_group,"search": search, "today": nowdate()}, as_dict=1)
	data = adjust_qty_for_expired_items(data)

	if cint(vmraid.db.get_single_value("Shopping Cart Settings", "enabled")):
		for item in data:
...
def get_child_groups_for_list_in_html(item_group, start, limit, search):
	...
	data = vmraid.db.get_all('Item Group',
...
def get_child_groups(item_group_name):
	item_group = vmraid.get_doc("Item Group", item_group_name)
	return vmraid.db.sql("""select name
		from `tabItem Group` where lft>=%(lft)s and rgt<=%(rgt)s
			and show_in_website = 1""", {"lft": item_group.lft, "rgt": item_group.rgt})

def get_child_item_groups(item_group_name):
	item_group = vmraid.get_cached_value("Item Group",
		item_group_name, ["lft", "rgt"], as_dict=1)

	child_item_groups = [d.name for d in vmraid.get_all('Item Group',
...
def get_item_for_list_in_html(context):
	...
	context["show_availability_status"] = cint(vmraid.db.get_single_value('Products Settings',
...
def get_group_item_count(item_group):
	...
	return vmraid.db.sql("""select count(*) from `tabItem`
...
def get_parent_item_groups(item_group_name):
	base_parents = [
		{"name": vmraid._("Home"), "route":"/"},
		{"name": vmraid._("All Products"), "route":"/all-products"},
	]
	if not item_group_name:
		return base_parents

	item_group = vmraid.get_doc("Item Group", item_group_name)
	parent_groups = vmraid.db.sql("""select name, route from `tabItem Group`
