...
from vmraid.utils import (fmt_money, formatdate, format_time, now_datetime,
...
class EmailDigest(Document):
	...
	def send(self):
		...
		valid_users = [p[0] for p in vmraid.db.sql("""select name from `tabUser`
			where enabled=1""")]
		recipients = list(filter(lambda r: r in valid_users,
			self.recipient_list.split("\n")))

		original_user = vmraid.session.user

		if recipients:
			for user_id in recipients:
				vmraid.set_user(user_id)
				vmraid.set_user_lang(user_id)
		...
		vmraid.set_user(original_user)
		vmraid.set_user_lang(original_user)
	...
	def get_msg_html(self):
		...
		# style
		return vmraid.render_template("erpadda/setup/doctype/email_digest/templates/default.html",
	...
	def get_todo_list(self, user_id=None):
		...
		todo_list = vmraid.db.sql("""select *
	...
	def get_todo_count(self, user_id=None):
		...
		return vmraid.db.sql("""select count(*) from `tabToDo`
	...
	def get_issue_list(self, user_id=None):
		...
		issue_list = vmraid.db.sql("""select *
	...
	def get_issue_count(self):
		"""Get count of Issue"""
		return vmraid.db.sql("""select count(*) from `tabIssue`
	...
	def get_project_list(self, user_id=None):
		...
		project_list = vmraid.db.sql("""select *
	...
	def get_project_count(self):
		...
		return vmraid.db.sql("""select count(*) from `tabProject`
	...
	def get_income(self):
		...
		income_account = vmraid.db.get_all('Account',
			fields=["name"],
			filters={
				"root_type":"Income",
				"parent_account":'',
	...
	def get_expenses_booked(self):
		...
		expense_account = vmraid.db.get_all('Account',
			fields=["name"],
			filters={
				"root_type": "Expense",
				"parent_account": '',
	...
	def get_sales_orders_to_bill(self):
		...
		value, count = vmraid.db.sql("""select ifnull((sum(grand_total)) - (sum(grand_total*per_billed/100)),0),
	...
	def get_sales_orders_to_deliver(self):
		...
		value, count = vmraid.db.sql("""select ifnull((sum(grand_total)) - (sum(grand_total*per_delivered/100)),0),
					count(*) from `tabSales Order`
	...
	def get_purchase_orders_to_receive(self):
		...
		value, count = vmraid.db.sql("""select ifnull((sum(grand_total))-(sum(grand_total*per_received/100)),0),
	...
	def get_type_balance(self, fieldname, account_type, root_type=None):
		...
		if root_type:
			accounts = [d.name for d in \
				vmraid.db.get_all("Account", filters={"account_type": account_type,
	...
	def get_roots(self, root_type):
		return [d.name for d in vmraid.db.get_all("Account",
	...
	def get_root_type_accounts(self, root_type):
		if not root_type in self._accounts:
			self._accounts[root_type] = [d.name for d in \
				vmraid.db.get_all("Account", filters={"root_type": root_type.title(),
					"company": self.company, "is_group": 0})]
	...
	def get_summary_of_pending(self, doc_type, fieldname, getfield):
		...
		value, count, billed_value, delivered_value = vmraid.db.sql("""select ifnull(sum(grand_total),0), count(*),
	...
	def get_summary_of_pending_quotations(self, fieldname):
		...
		value, count = vmraid.db.sql("""select ifnull(sum(grand_total),0), count(*) from `tabQuotation`
			where (transaction_date <= %(to_date)s)
			and company = %(company)s
			and status not in ('Ordered','Cancelled', 'Lost') """,{"to_date": self.future_to_date, "company": self.company})[0]

		last_value = vmraid.db.sql("""select ifnull(sum(grand_total),0) from `tabQuotation`
			where (transaction_date <= %(to_date)s)
			and company = %(company)s
	...
	def get_total_on(self, doc_type, from_date, to_date):
		...
		return vmraid.get_all(doc_type,
			filters={
...
def send():
	...
	for ed in vmraid.db.sql("""select name from `tabEmail Digest`
