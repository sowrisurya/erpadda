...
from erpadda.accounts.doctype.monthly_distribution.monthly_distribution import get_periodwise_distribution_data
...
def get_data_column(filters, partner_doctype):
	...
	for key, value in rows.items():
		value.update({
			vmraid.scrub(partner_doctype): key[0],
			'item_group': key[1]
		})
...
def get_actual_data(filters, item_groups, sales_users_or_territory_data, date_field, sales_field):
	...
	return vmraid.db.sql(""" SELECT `tab{child_doc}`.item_group,
...
def get_parents_data(filters, partner_doctype):
	...
	return vmraid.get_all('Target Detail',
