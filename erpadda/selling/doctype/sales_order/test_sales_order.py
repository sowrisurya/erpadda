...
from vmraid.utils import flt, add_days, nowdate
from vmraid.core.doctype.user_permission.test_user_permission import create_user
from erpadda.selling.doctype.sales_order.sales_order \
	import make_material_request, make_delivery_note, make_sales_invoice, WarehouseRequired
from erpadda.stock.doctype.stock_entry.stock_entry_utils import make_stock_entry
from erpadda.selling.doctype.sales_order.sales_order import make_work_orders
from erpadda.controllers.accounts_controller import update_child_qty_rate
from erpadda.selling.doctype.sales_order.sales_order import make_raw_material_request
...
class TestSalesOrder(unittest.TestCase):
	...
	@classmethod
	def setUpClass(cls):
		cls.unlink_setting = int(vmraid.db.get_value("Accounts Settings", "Accounts Settings",
			"unlink_advance_payment_on_cancelation_of_order"))

	@classmethod
	def tearDownClass(cls) -> None:
		# reset config to previous state
		vmraid.db.set_value("Accounts Settings", "Accounts Settings",
	...
	def test_update_child_removing_item(self):
		...
		self.assertRaises(vmraid.ValidationError, update_child_qty_rate, 'Sales Order', trans_item, so.name)
	...
	def test_update_child_with_tax_template(self):
		...
		if not vmraid.db.exists("Item Tax", {"parent":item, "item_tax_template":tax_template}):
			...
			# update valid from
			vmraid.db.sql("""UPDATE `tabItem Tax` set valid_from = CURDATE()
		...
		# teardown
		vmraid.db.sql("""UPDATE `tabItem Tax` set valid_from = NULL
	...
	def test_warehouse_user(self):
		...
		vmraid.permissions.remove_user_permission("Warehouse", "_Test Warehouse 2 - _TC1", test_user_2.name)
	...
	def test_auto_insert_price(self):
		...
		item_price = vmraid.db.get_value("Item Price", {"price_list": "_Test Price List",
		...
		self.assertEqual(vmraid.db.get_value("Item Price",
	...
	def test_drop_shipping(self):
		from erpadda.selling.doctype.sales_order.sales_order import make_purchase_order_for_default_supplier, \
		...
		bin_po_item = vmraid.get_all("Bin", filters={"item_code": po_item.item_code, "warehouse": "_Test Warehouse - _TC"},
	...
	def test_reserved_qty_for_closing_so(self):
		bin = vmraid.get_all("Bin", filters={"item_code": "_Test Item", "warehouse": "_Test Warehouse - _TC"},
			fields=["reserved_qty"])
	...
	def test_make_work_order(self):
		...
		for item in so_item_name:
			wo_qty = vmraid.db.sql("select sum(qty) from `tabWork Order` where sales_order=%s and sales_order_item=%s", (so.name, item))
	...
	def test_serial_no_based_delivery(self):
		...
		so.submit()
		from erpadda.manufacturing.doctype.work_order.test_work_order import \
	...
	def test_request_for_raw_materials(self):
		...
		mr = vmraid.db.sql("""select name from `tabMaterial Request` ORDER BY creation DESC LIMIT 1""", as_dict=1)[0]
...
def get_reserved_qty(item_code="_Test Item", warehouse="_Test Warehouse - _TC"):
	return flt(vmraid.db.get_value("Bin", {"item_code": item_code, "warehouse": warehouse},
		"reserved_qty"))
...
def make_sales_order_workflow():
	...
	vmraid.get_doc(dict(doctype='Role', role_name='Test Junior Approver')).insert(ignore_if_duplicate=True)
