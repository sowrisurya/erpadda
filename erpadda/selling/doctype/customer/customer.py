...
from vmraid.utils import flt, cint, cstr, today, get_formatted_email
...
class Customer(TransactionBase):
	...
	def get_customer_name(self):
		...
		if vmraid.db.get_value("Customer", self.customer_name) and not vmraid.flags.in_import:
			count = vmraid.db.sql("""select ifnull(MAX(CAST(SUBSTRING_INDEX(name, ' ', -1) AS UNSIGNED)), 0) from tabCustomer
	...
	def validate_default_bank_account(self):
		if self.default_bank_account:
			is_company_account = vmraid.db.get_value('Bank Account', self.default_bank_account, 'is_company_account')
			if not is_company_account:
				vmraid.throw(_("{0} is not a company bank account").format(vmraid.bold(self.default_bank_account)))

	def validate_internal_customer(self):
		internal_customer = vmraid.db.get_value("Customer",
			{"is_internal_customer": 1, "represents_company": self.represents_company, "name": ("!=", self.name)}, "name")

		if internal_customer:
			vmraid.throw(_("Internal Customer for company {0} already exists").format(
	...
	def update_lead_status(self):
		...
		if self.lead_name:
			lead = vmraid.get_doc('Lead', self.lead_name)
	...
	def create_lead_address_contact(self):
		if self.lead_name:
			...
			address_names = vmraid.get_all('Dynamic Link', filters={
								"parenttype":"Address",
								"link_doctype":"Lead",
			...
			for address_name in address_names:
				address = vmraid.get_doc('Address', address_name.get('name'))
				if not address.has_link('Customer', self.name):
					address.append('links', dict(link_doctype='Customer', link_name=self.name))
					address.save(ignore_permissions=self.flags.ignore_permissions)

			lead = vmraid.db.get_value("Lead", self.lead_name, ["organization_lead", "lead_name", "email_id", "phone", "mobile_no", "gender", "salutation"], as_dict=True)

			if not lead.lead_name:
				vmraid.throw(_("Please mention the Lead Name in Lead {0}").format(self.lead_name))

			if lead.organization_lead:
				contact_names = vmraid.get_all('Dynamic Link', filters={
									"parenttype":"Contact",
									"link_doctype":"Lead",
									"link_name":self.lead_name
								}, fields=["parent as name"])
				...
				for contact_name in contact_names:
					contact = vmraid.get_doc('Contact', contact_name.get('name'))
				...
				contact.autoname()
				if not vmraid.db.exists("Contact", contact.name):
					contact.insert()

	def validate_name_with_customer_group(self):
		if vmraid.db.exists("Customer Group", self.name):
			vmraid.throw(_("A Customer Group exists with same name please change the Customer name or rename the Customer Group"), vmraid.NameError)
	...
	def validate_credit_limit_on_change(self):
		...
		past_credit_limits = [d.credit_limit
			for d in vmraid.db.get_all("Customer Credit Limit", filters={'parent': self.name}, fields=["credit_limit"], order_by="company")]
		...
		for limit in self.credit_limits:
			if limit.company in company_record:
				vmraid.throw(_("Credit limit is already defined for the Company {0}").format(limit.company, self.name))
			else:
				company_record.append(limit.company)

			outstanding_amt = get_customer_outstanding(self.name, limit.company)
			if flt(limit.credit_limit) < outstanding_amt:
				vmraid.throw(_("""New credit limit is less than current outstanding amount for the customer. Credit limit has to be atleast {0}""").format(outstanding_amt))

	def on_trash(self):
		if self.customer_primary_contact:
			vmraid.db.sql("""update `tabCustomer`
	...
	def set_loyalty_program(self):
		...
		if len(loyalty_program) == 1:
			...
			vmraid.msgprint(
				...
				.format(vmraid.bold(self.customer_name))
			)
...
def make_quotation(source_name, target_doc=None):
	...
	price_list, currency = vmraid.db.get_value("Customer", {'name': source_name}, ['default_price_list', 'default_currency'])
	if price_list:
...
def _set_missing_values(source, target):
	address = vmraid.get_all('Dynamic Link', {
			'link_doctype': source.doctype,
			'link_name': source.name,
			'parenttype': 'Address',
		}, ['parent'], limit=1)

	contact = vmraid.get_all('Dynamic Link', {
			'link_doctype': source.doctype,
			'link_name': source.name,
			'parenttype': 'Contact',
		}, ['parent'], limit=1)
...
def get_loyalty_programs(doc):
	...
	lp_details = []
	loyalty_programs = vmraid.get_all("Loyalty Program",
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		if not credit_controller_role or credit_controller_role not in vmraid.get_roles():
			...
			if not credit_controller_users_formatted:
				vmraid.throw(_("Please contact your administrator to extend the credit limits for {0}.").format(customer))
...
def send_emails(args):
	...
	vmraid.sendmail(recipients=args.get('credit_controller_users_list'), subject=subject, message=message)
...
def get_customer_outstanding(customer, company, ignore_outstanding_sales_order=False, cost_center=None):
	...
	cond = ""
	if cost_center:
		lft, rgt = vmraid.get_cached_value("Cost Center",
			cost_center, ['lft', 'rgt'])
	...
	unmarked_delivery_note_items = vmraid.db.sql("""select
...
def get_credit_limit(customer, company):
	...
	if customer:
		credit_limit = vmraid.db.get_value("Customer Credit Limit",
...
def make_address(args, is_primary_address=1):
	...
	if reqd_fields:
		...
		vmraid.throw("{0} <br><br> <ul>{1}</ul>".format(msg, '\n'.join(reqd_fields)),
