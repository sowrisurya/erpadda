...
from erpadda.exceptions import PartyFrozen, PartyDisabled
...
class TestCustomer(unittest.TestCase):
	...
	def test_party_details(self):
		...
		vmraid.db.set_value("Contact", "_Test Contact for _Test Customer-_Test Customer",
	...
	def test_party_details_tax_category(self):
		...
		billing_address = vmraid.get_doc(dict(
			doctype='Address',
		...
		shipping_address = vmraid.get_doc(dict(
			doctype='Address',
			address_title='_Test Address With Tax Category',
	...
	def test_rename(self):
		...
		for name in ("_Test Customer 1", new_name):
			vmraid.db.sql("""delete from `tabComment`
				where reference_doctype=%s and reference_name=%s""",
				("Customer", name))

		# add comments
		comment = vmraid.get_doc("Customer", "_Test Customer 1").add_comment("Comment", "Test Comment for Rename")
		...
		self.assertEqual(vmraid.db.get_value("Comment", {
	...
	def test_duplicate_customer(self):
		...
		if not vmraid.db.get_value("Customer", "_Test Customer 1"):
			test_customer_1 = vmraid.get_doc(
		...
		duplicate_customer = vmraid.get_doc(
			get_customer_dict('_Test Customer 1')).insert(ignore_permissions=True)
