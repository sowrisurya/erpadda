...
def get_funnel_data(from_date, to_date, company):
	...
	active_leads = vmraid.db.sql("""select count(*) from `tabLead`
		where (date(`creation`) between %s and %s)
		and company=%s""", (from_date, to_date, company))[0][0]

	opportunities = vmraid.db.sql("""select count(*) from `tabOpportunity`
		where (date(`creation`) between %s and %s)
		and opportunity_from='Lead' and company=%s""", (from_date, to_date, company))[0][0]

	quotations = vmraid.db.sql("""select count(*) from `tabQuotation`
		where docstatus = 1 and (date(`creation`) between %s and %s)
		and (opportunity!="" or quotation_to="Lead") and company=%s""", (from_date, to_date, company))[0][0]

	converted = vmraid.db.sql("""select count(*) from `tabCustomer`
...
def get_opp_by_lead_source(from_date, to_date, company):
	...
	opportunities = vmraid.get_all("Opportunity", filters=[['status', 'in', ['Open', 'Quotation', 'Replied']], ['company', '=', company], ['transaction_date', 'Between', [from_date, to_date]]], fields=['currency', 'sales_stage', 'opportunity_amount', 'probability', 'source'])
...
def get_pipeline_data(from_date, to_date, company):
	...
	opportunities = vmraid.get_all("Opportunity", filters=[['status', 'in', ['Open', 'Quotation', 'Replied']], ['company', '=', company], ['transaction_date', 'Between', [from_date, to_date]]], fields=['currency', 'sales_stage', 'opportunity_amount', 'probability'])
